...
- [ ] **【絶対項目】ナビゲーション整合性**: 当該 IT で新規／実画面化する画面が、`ui_design.md` のナビゲーション構成表に表示ロール付きで定義されている。かつ計画の設計節に「navbar（`_Layout.cshtml`）とダッシュボード（`Home/Index.cshtml`）の両方へロール条件付きで反映する」ことがタスク／注記として明示されている（画面は作ったがナビ／ダッシュボードから辿れない状態を防ぐ）
- [ ] **【絶対項目】ナビゲーション検証テスト**: 当該 IT のロールについて、ナビ／ダッシュボード表示を検証する自動テスト（`WalkingSkeletonTest` のロール別アサート等）がタスクに含まれている
...
- **新規画面の受入テストはあるが、navbar／ダッシュボードへの反映とその検証テストが計画・実装から抜けている（個別画面の整合性は確認したがナビゲーション整合性を確認していない）**
