 ...
 * ずれていても「どちらの時刻か」は読める。港を足すときはここへ足す。</p>
 *
 * <p><b>足し忘れを拾う正典は無い。</b> 港の一覧を持つ設計ドキュメントが無く、
 * 港は利用者が UN/LOCODE を打ち込んで増える（航海登録・予約登録）ので、
 * 名簿と突き合わせる検査は書けない——<b>書けない約束をコメントに書かない</b>
 * （IT12 レビュー 高。ここには実在しない `portTimeZone.canon.test.ts` が
 * 拾うと書いてあった）。代わりに `portTimeZone.test.ts` が (1) 表の値が
 * すべて実在するタイムゾーンであること、(2) 知らない港が業務タイムゾーンに
 * 倒れること、(3) 併記の形——を固定する。<b>足りないことは「JST として
 * 出す」側に倒れる</b>ので、黙って壊れるのではなく併記がずれる形で出る。</p>
 */
