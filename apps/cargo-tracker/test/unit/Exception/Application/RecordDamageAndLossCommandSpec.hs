...
spec = do
  describe "RecordDamageExceptionCommand.execute (US20)" $ do
    it "正常系: DamageException 集約が構築され永続化される" $ do
      ...
      result <- Damage.execute repo (\_ -> pure (Right ())) validDamageInput
      case result of
        Right r -> do
    ...
    it "負の損害額は InvalidCost、永続化されない" $ do
      (repo, getSaved) <- makeRepo
      result <- Damage.execute repo (\_ -> pure (Right ())) (validDamageInput {Damage.inputAmountValue = -1})
    ...
    it "空 description は InvalidExceptionReason \"empty\"、永続化されない" $ do
      (repo, getSaved) <- makeRepo
      result <- Damage.execute repo (\_ -> pure (Right ())) (validDamageInput {Damage.inputDescription = ""})
    ...
    it "小文字通貨は InvalidCurrency" $ do
      (repo, _) <- makeRepo
      result <- Damage.execute repo (\_ -> pure (Right ())) (validDamageInput {Damage.inputAmountCurrency = "jpy"})
  ...
  describe "RecordLossExceptionCommand.execute (US20)" $ do
    it "正常系: 5 文字 lastSeenAt を持つ LossException が永続化される" $ do
      (repo, getSaved) <- makeRepo
      result <- Loss.execute repo (\_ -> pure (Right ())) validLossInput
      case result of
        Right r -> do
    ...
    it "Nothing lastSeenAt (不明) も受理される" $ do
      ...
      result <- Loss.execute repo (\_ -> pure (Right ())) input
      case result of
    ...
    it "空 reporter role は InvalidReporter" $ do
      (repo, _) <- makeRepo
      result <- Loss.execute repo (\_ -> pure (Right ())) (validLossInput {Loss.inputReporterRole = ""})
