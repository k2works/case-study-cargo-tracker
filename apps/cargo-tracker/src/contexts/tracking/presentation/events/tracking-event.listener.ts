 ...
 * - customs.held: 通関留置で CUSTOMS_HOLD 例外を自動登録（ADR-010）
...
export class TrackingEventListener {
  ...
  @OnEvent('customs.held')
  async onCustomsHeld(payload: CustomsHeldPayload): Promise<void> {
    if (payload.trackingNumber === null) {
      this.logger.warn(`通関留置の CUSTOMS_HOLD 登録をスキップ（追跡番号未発行・申告 ${payload.declarationNumber}）`);
      return;
    }
    try {
      // システム起点の自動登録のため isTracker=true（CUSTOMS_HOLD はロール制限外）。
      // 未解決の同種例外があれば addException が冪等スキップする。
      await this.registerException.register(
        payload.trackingNumber,
        {
          exceptionType: ExceptionType.CUSTOMS_HOLD,
          location: payload.location,
          occurredAt: new Date(),
      ...
      this.logger.error(`通関留置の CUSTOMS_HOLD 登録に失敗（${payload.trackingNumber}）: ${String(error)}`);
    }
  }
}
