...
module Billing
  module Domain
    ...
    class Invoice
      ...
      attr_reader :invoice_number, :booking_id, :shipper_id, :base_amount, :discount_rate,
                  :tax_amount, :total_amount, :payment_status, :issued_at, :due_date, :paid_at

      # 請求書を発行する（PENDING で採番）。支払期限は発行日 + 30 日。
      def self.generate(invoice_number:, booking_id:, shipper_id:, base_amount:, discount_rate:,
                        tax_amount:, total_amount:, issued_at:)
        new(invoice_number: invoice_number, booking_id: booking_id, shipper_id: shipper_id,
            base_amount: base_amount, discount_rate: discount_rate, tax_amount: tax_amount,
            total_amount: total_amount, payment_status: PaymentStatus.initial,
            issued_at: issued_at, paid_at: nil)
      end
      ...
      def initialize(invoice_number:, booking_id:, shipper_id:, base_amount:, discount_rate:,
                     tax_amount:, total_amount:, payment_status:, issued_at:, paid_at: nil)
