...
module Billing
  module Public
    ...
    class BillingService
      LineItemView = Data.define(:description, :amount, :adjustment_type)
      ...
      def adjust(invoice_number, description:, amount:, adjustment_type:)
        Application::AdjustFreight.new(repository: @repository).call(
          invoice_number: invoice_number, description: description, amount: amount, adjustment_type: adjustment_type
        )
      end
      ...
      def to_view(invoice)
        ...
        View.new(
          ...
          line_items: invoice.line_items.map do |li|
            LineItemView.new(description: li.description, amount: li.amount.amount.to_i,
                             adjustment_type: li.adjustment_type)
          end
        )
      end
    end
