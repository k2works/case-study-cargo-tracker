...
module Routing
  module Domain
    ...
    RouteCandidate = Data.define(:legs, :transit_days, :cost, :voyage_numbers, :fallback,
                                 :arrival_date, :carrier_names) do
      # legs: [{ from:, to:, voyage_number: }] の配列
      def initialize(legs:, transit_days:, cost:, voyage_numbers:, fallback: false,
                     arrival_date: nil, carrier_names: [])
        ...
        super(legs: legs, transit_days: transit_days.to_i, cost: cost,
              ...
              arrival_date: arrival_date, carrier_names: Array(carrier_names))
      end
