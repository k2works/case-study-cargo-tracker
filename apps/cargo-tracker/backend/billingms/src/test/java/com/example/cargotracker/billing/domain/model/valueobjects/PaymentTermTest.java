...
class PaymentTermTest {
    ...
    private static final LocalDate ISSUED_ON = LocalDate.of(2026, Month.OCTOBER, 12);
    private static final LocalDate DUE_ON = LocalDate.of(2026, Month.NOVEMBER, 11);
    ...
    @Test
    @DisplayName("期限超過の判定: 請求済で期限の翌日から超過になる")
    ...
    @Test
    @DisplayName("期限超過の判定: 期限当日は超過ではない（当日中の入金はふつうにある）")
    ...
    @Test
    @DisplayName("期限超過の判定: 請求済でなければ超過は無い（未発行・入金済・取消）")
    ...
    @Test
    @DisplayName("期限超過の判定: 期限も「今日」も欠けていれば超過にしない（落ちない）")
    ...
    @Test
    @DisplayName("状態の述語: 発行できるのは算出済だけ")
    ...
    @Test
    @DisplayName("状態の述語: 入金を記録できるのは請求済だけ")
    ...
    @Test
    @DisplayName("状態の述語: 取り消せるのは算出済と請求済（入金済は決着している）")
    ...
    @Test
    @DisplayName("状態の述語: 決着したのは入金済と取消（一覧の既定から外す）")
