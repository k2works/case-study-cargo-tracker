 ...
 */
/** 貨物を選び直したときに戻る種別。作業は受領から始まる。 */
const DEFAULT_TYPE = "RECEIVE";
...
export function HandlingPage() {
  ...
  function submit(event: React.SyntheticEvent<HTMLFormElement>) {
    ...
    // **引取は取り消せない。** 記録後も種別が残るため、次の貨物に「引取」のまま
    // 記録する事故が起きうる。どの貨物を引き渡すのかを、送る前にもう一度見せる。
    // 他の種別では出さない——1 日数十件を打つ画面で、毎回の確認は手数にしかならない
    if (type === "CLAIM" && !confirmingClaim) {
    ...
    <div className="space-y-6">
          ...
          <div>
            ...
            <input
              ...
              onChange={(event) => {
                ...
                // **貨物が変われば、種別も選び直す**（IT8 返済枠 0.9）。
                // 記録後に種別が残るため、次の貨物に「引取」のまま記録する事故が起きる。
                // 確認を足す前に、事故の源を断つ
                setType(DEFAULT_TYPE);
                setConfirmingClaim(false);
              }}
        ...
        {historyError !== null && historyError !== undefined && (
          <p role="alert" className="text-sm text-red-800">
            {historyError instanceof ApiError
              ? ((historyError.body as { message?: string } | undefined)
                  ?.message ?? "履歴を取得できませんでした")
              : "履歴を取得できませんでした"}
          </p>
        )}
      </section>
      ...
      {isHandler && (
        ...
        >
            ...
            {selectedType?.requiresConsigneeConfirmation === true && (
              <div className="md:col-span-2">
                ...
                <input
                  ...
                  onChange={(event) =>
                    setConsigneeConfirmation(event.target.value)
                  }
                  className="mt-1 w-full rounded border border-gray-300 px-3 py-2"
                />
                {/* **荷受人の確認は残る**（[ADR-025] 決定 9）。これは US16 の受入基準
                  そのものであり、通関ガードの代用として入れたのではない。
                  代用を兼ねていただけである。

                  **外したのは「通関を仕組みで確かめていない」という但し書きだけ。**
                  ガードが入った以上、その文は誤りになる（[ADR-023] 決定 4 が
                  「消し忘れるほうを踏むな」と予告していた） */}
                <p className="mt-1 text-sm text-gray-600">
                  {/* 改行を空白と読ませない（日本語は語間を空けない） */}
                  誰に引き渡したかの記録です。署名または確認コードを入れてください。
                </p>
              </div>
            )}
          </div>
          ...
          {register.error !== null && register.error !== undefined && (
            ...
            >
              {register.error instanceof ApiError
                ? ((register.error.body as { message?: string } | undefined)
                    ?.message ?? "記録できませんでした")
                : "記録できませんでした"}
            </p>
          )}
          ...
          {/* **引取は取り消せない。**どの貨物を引き渡すのかを、送る前にもう一度見せる
            （IT8 返済枠 0.9）。他の種別では出さない——1 日数十件を打つ画面で、
            毎回の確認は手数にしかならない */}
          {confirmingClaim && (
            <div
              role="alert"
              className="space-y-2 rounded bg-amber-50 p-3 text-sm text-amber-900"
            >
              <p>
                <strong>{trackingNumber.trim()}</strong>
                {/* 改行を空白と読ませない（日本語は語間を空けない） */}
                の貨物を<strong>荷受人へ引き渡した</strong>
                {"ものとして記録します。"}
              </p>
              <p>引取の記録は取り消せません。追跡番号を確かめてください。</p>
              <button
                type="button"
          ...
          >
            {confirmingClaim ? "この貨物の引取を記録する" : "記録する"}
          </button>
        </form>
      )}
