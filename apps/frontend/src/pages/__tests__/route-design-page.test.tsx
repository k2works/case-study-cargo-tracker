  ...
  describe('誤配のあとの組み直し（US28-4・ADR-026 決定 4）', () => {
    ...
    it('現在地を出発地として候補を探す', async () => {
      ...
      expect(asked!.searchParams.get('origin'), '元の出発地で候補を探している')
        .toBe('SGSIN')
      // **期限で弾かないことを伝える**（[ADR-026] 決定 4）。サーバは既定で期限を
      // 超える候補を刈るため、伝えないと**候補が 1 本も出ない**——誤配した貨物は
      // 遅れているのが普通で、元の期限に間に合う便はまず残っていない
      expect(asked!.searchParams.get('reroute'), '期限を超える候補が刈られる')
        .toBe('true')
