...
public class LoginUseCase {
    ...
    public Optional<LoginResult> login(String username, String rawPassword) {
        ...
        if (found.isEmpty()) {
            ...
            auditLogger.record(username, AuthEventType.LOGIN_FAILURE, "利用者が存在しない");
        ...
        if (!user.canAttemptLoginAt(now)) {
            ...
            // 正規の利用者がいつまでも入れなくなる
            auditLogger.record(username, AuthEventType.LOGIN_FAILURE, "ロック中の試行");
            return Optional.empty();
        }
        ...
        auditLogger.record(username, AuthEventType.LOGIN_SUCCESS, null);
