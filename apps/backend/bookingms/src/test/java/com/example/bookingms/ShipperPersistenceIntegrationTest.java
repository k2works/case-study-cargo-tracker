...
class ShipperPersistenceIntegrationTest {
    ...
    void assignsShipperCode() {
        RegistrationOutcome outcome = useCase.register(command("採番太郎", "saiban@example.com"), false);
    ...
    void assignsDistinctCodes() {
        Shipper first = ((RegistrationOutcome.Registered)
                useCase.register(command("連番一郎", "renban1@example.com"), false)).shipper();
        Shipper second = ((RegistrationOutcome.Registered)
                useCase.register(command("連番二郎", "renban2@example.com"), false)).shipper();
    ...
    void detectsDuplicate() {
        useCase.register(command("重複太郎", "duplicate@example.com"), false);

        RegistrationOutcome outcome =
                useCase.register(command("重複太郎（別入力）", "duplicate@example.com"), false);
    ...
    void registersAnyway() {
        useCase.register(command("あえて太郎", "anyway@example.com"), false);

        useCase.register(command("あえて太郎（本社）", "anyway@example.com"), true);
    ...
    void searchesByNameOrEmail() {
        useCase.search("");
        useCase.register(command("検索花子", "kensaku@example.com"), false);
