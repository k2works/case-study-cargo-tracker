...
import com.example.bookingms.config.BookingConfig;
 ...
 * 突き合わせる。
 */
@DisplayName("交換機の宣言が契約と一致する（bookingms）")
class EventExchangeDeclarationTest {

    private final BookingConfig config = new BookingConfig();
    ...
    void namesTheUnroutableExchangeAsAgreed() {
        assertThat(com.example.bookingms.infrastructure.messaging.CargoEventChannels.UNROUTABLE_EXCHANGE).isEqualTo(EventExchangeContract.UNROUTABLE_EXCHANGE);
        assertThat(com.example.bookingms.infrastructure.messaging.CargoEventChannels.UNROUTABLE_QUEUE).isEqualTo(EventExchangeContract.UNROUTABLE_QUEUE);
    }

    private List<TopicExchange> businessExchanges() {
        return List.of(config.cargoEventExchange());
    }
}
