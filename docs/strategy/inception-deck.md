...
国際貨物輸送会社では、**貨物追跡・予約管理の情報システムが存在しない**（ケイパビリティ評価：新規）ため、以下の業務課題が発生している。
...
> **「荷主が出発地・目的地・期限を指定するだけで最適ルートが提案され、いつでもどこでも貨物をリアルタイムで追跡できる、透明性の高い国際貨物輸送サービスを実現する」**
...
| 4 | **最適ルート提案** | 出発地・目的地・期限・貨物種別から最適ルートを自動算出し、経路設計工数を削減する |
...
| 2 | **経路設計** | 最適ルート算出・提案、航海スケジュール管理、追跡番号発行 |
...
| 2 | **IT システムの新規構築リスク**：貨物追跡・予約管理システムがゼロからの構築 | 高 | MVP 優先、段階的リリースで早期フィードバックを得る |
...
| 6 | **危険物・冷凍貨物の規制対応**：貨物種別ごとの法的要件を見落とす可能性 | 中 | 要件定義段階で規制要件をヒアリング、専門家レビューを実施 |
...
| **フェーズ 1（MVP）** | 予約管理、経路設計、基本的な貨物追跡、追跡番号発行 | **MVP リリース** |
