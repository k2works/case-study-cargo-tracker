...
| ドキュメント | 概要 | 状況 |
| :--- | :--- | :--- |
| [アプリケーション開発環境セットアップ手順書](アプリケーション開発環境セットアップ手順書.md) | ローカルアプリケーション開発環境の構築手順（Java 25 / Gradle / Docker / Testcontainers） | 作成済・構築済 |
| [開発環境セットアップ手順書](開発環境セットアップ手順書.md) | Heroku Container Registry へのデプロイ手順（dev プロファイル / H2） | 作成済・デプロイ済 |
| [荷主紐付け設定手順書](荷主紐付け設定手順書.md) | 利用者アカウント（ROLE_SHIPPER）を荷主に紐付ける運用手順（ADR-013 の代償を埋める） | 作成済 |
...
- `docs/manual/` は IT1 で作成済みです（ログイン・荷主管理）。画面キャプチャは `npm run screenshots:manual` で生成します。**手作業で PNG を置かないこと**（次回の再生成で上書きされ、UI 変更のたびに撮り漏れが出ます）
- ソースが無い状態で実行した場合は、エラーにせず「生成しなかった」ことを表示して停止します（他の成果物のデプロイを止めないため）
