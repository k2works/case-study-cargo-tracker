...
| [開発環境セットアップ手順書](開発環境セットアップ手順書.md) | Heroku Container Registry へのデプロイ手順（dev プロファイル / H2） | 作成済（デプロイは未実施） |
...
| 2 | 開発環境（Heroku Container Registry） | 作成済 | **一部**（Dockerfile・dev プロファイル・Gulp タスクは整備済み。Heroku へのデプロイは未実施） |
