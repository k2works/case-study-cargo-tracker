...
> **現時点でルート URL は `401` を返します。** 認証機能（US26）が未実装のため、Spring Security の既定設定（Basic 認証）がすべてのリクエストを保護している状態です。**ログイン画面はまだ存在しません。** US26 の実装後にログイン画面の表示を確認項目に加えます。
...
| ヘルスチェック | `{"status":"UP"}` |
| ルート URL | `401`（認証機能 US26 が未実装のため Spring Security の既定設定で保護されている） |
...
**認証機能（US26）が未実装のため、Spring Security の既定設定で動いています。**

- ルート URL を含むすべてのエンドポイントが Basic 認証で保護されます（`/actuator/health` を除く）
- 起動のたびに**ランダムなパスワードが生成され、ログに出力されます**（`Using generated security password: ...`）
- **この状態を「認証が実装されている」と誤解しないでください。** US26 / US31 の実装で `SecurityConfig` を追加し、RBAC 8 ロールによる認可（`non_functional.md` §4.1）に置き換えます

現時点の開発環境は、**コンテナが起動し DB マイグレーションが通ることの確認**が目的です。画面の確認は US26 実装後になります。
