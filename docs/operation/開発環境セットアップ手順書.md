...
Gulp タスク（`--platform` と `--build-arg` を自動で付与します）:
...
| `deploy:dev:build` / `deploy:dev:push` / `deploy:dev:release` | ビルド / push / リリース |
...
| **Heroku へのデプロイ（push / release）** | ❌ **未実施** | 外部サービスへの公開操作のため未実行 |

### ローカルでの検証結果

Heroku へのデプロイは行っていませんが、**イメージが実際に動作することは確認済み**です。
...
1. Heroku アプリの作成とデプロイ（`npx gulp deploy:dev:setup`）
2. ステージング環境（AWS）のセットアップ（段階 3）
