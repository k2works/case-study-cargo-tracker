...
- Flyway のマイグレーションが適用される（`Successfully applied N migration(s)`）
- `/actuator/health` が `{"status":"UP"}` を返す

- ルート URL にアクセスするとログイン画面（`/login`）へリダイレクトされる
- ログイン画面に**動作確認用の利用者が事前入力**されており、「ログイン」を押すだけでダッシュボードが表示される
- 荷主一覧に動作確認用の荷主（`SHP-000001 山田商事`）が 1 件表示される
- ログイン画面に**ロール別の利用者一覧**が表示され、利用者 ID のクリックで切り替えられる（パスワードは全員共通）

> **開発環境ではログイン情報を事前入力しています**（`app.demo-login`、`application-dev.yml`）。画面を触るたびに ID とパスワードを打ち直す手間をなくすためです。
>
> **既定は無効です。** 有効化を明示した環境（`local` / `dev`）でのみ効きます。本番でうっかり有効になる経路を作らないため、安全側を既定にして opt-in にしています。有効時は画面に「開発環境です」と必ず表示し、事前入力していることを隠しません。既定で入らないことは `DemoLoginPrefillTest` が固定しています。
...
| ヘルスチェック | `{"status":"UP"}` |
| ルート URL | `302` → `/login`（未認証はログイン画面へ誘導される） |
| ログイン | 動作確認用の利用者を事前入力済み。「ログイン」だけでダッシュボードに入れる |
...
| 項目 | 内容 |
| :--- | :--- |
| アプリ名 | `cargo-tracker-take-6-docs`（container stack） |
| URL | `https://cargo-tracker-take-6-docs-340bd48cb9d2.herokuapp.com/` |
| 配信内容 | ポータル / MkDocs 110 ページ / JIG 11 ページ / ER 図 3 種 / マニュアル（未作成のプレースホルダ） |
| イメージサイズ | 127 MB |
| 検証 | ポータルのリンク先を含む 10 パスすべて 200 |

### 次に実施すること

1. ステージング環境（AWS）のセットアップ（段階 3）
2. Backend CI からの自動デプロイ（`operating-cicd` スキル）
3. UI 実装後にユーザーマニュアルを執筆（`creating-manual` スキル）

---

## 11. ドキュメントサイトのデプロイ

設計ドキュメントと**コードから生成した成果物**を、アプリとは別の Heroku アプリで公開します。

| パス | 内容 | 生成元 |
| :--- | :--- | :--- |
| `/` | **ドキュメントポータル**（入口） | `apps/www/`（手書き。リポジトリで管理） |
| `/docs/` | MkDocs（設計・要件・運用ドキュメント） | `docs/` の Markdown |
| `/jig/` | JIG（ドメインモデル・パッケージ関連・用語集） | バイトコード |
| `/jig-erd/` | ER 図（概要 / サマリー / 詳細） | Flyway が構築した実スキーマ |
| `/manual/` | ユーザーマニュアル | `docs/manual/` の Markdown |
| `/healthz` | ヘルスチェック（`{"status":"UP"}`） | nginx |

**アプリ本体とは別アプリにしています。** 配信するものも更新頻度も異なるためです。

### 11.0 ドキュメントポータル（`apps/www/`）

`/` に置く入口ページです。**生成物ではなく手書きであり、リポジトリで管理します。**

- 「**設計ドキュメント**（人が書いたもの）」と「**コードから生成したドキュメント**」を節として分けて並べています。両者を突き合わせられることがこのサイトの目的だからです
- 生成物のカードには**突き合わせ先の設計ドキュメント**を併記しています
- 稼働中のアプリケーションへのリンクも置いています。IT1 でログイン画面が入ったため、注記も「動作確認用の利用者が事前入力済み」に更新しました。**リンク先の状態が変わったら注記も同時に直します**（古い注記はリンク先が壊れていると誤解させます）
- 生成物側のページ（ER 図・マニュアル）と MkDocs のトップからも**ポータルへ戻れます**。片方向の導線は行き止まりになります

### 11.1 構成

`nginx:1.29-alpine` の静的配信イメージです。`$PORT` は nginx 公式イメージの envsubst 機構（`/etc/nginx/templates/*.template`）で展開します。

> **このイメージは配信するだけで、成果物の生成は行いません。** JIG は JDK、jig-erd は Docker と Graphviz を必要とするため、生成はホスト側で行い成果物だけをイメージに載せます。

### 11.2 手順

```bash
# 初回のみ: Heroku アプリを作成
npx gulp deploy:docs:app:create

# 一括実行（成果物生成 → build-push → release → verify）
npx gulp deploy:docs

# ブラウザで開く
npx gulp deploy:docs:open
```

`.env` に設定します。

```dotenv
DOCS_HEROKU_APP_NAME=<app-name>-docs
```
...
| タスク | 内容 |
| :--- | :--- |
| `deploy:docs:artifacts` | 4 種類の成果物を生成（`mkdocs:build` → `app:jig` → `app:jig-erd` → `manual:build`） |
| `deploy:docs:build-push` | サイトイメージをビルドして push |
| `deploy:docs:release` | リリース |
| `deploy:docs:verify` | **4 種類すべてが 200 を返すことを確認** |

> **`deploy:docs:verify` はポータルのリンク先を含む 10 パスを確認します。** ポータルが表示されただけでは、リンク先が見えているとは限りません。

### 11.4 マニュアルが未作成の場合

`docs/manual/` が存在しない場合、`manual:build` は**エラーにせず「生成しなかった」ことを表示して終了**します。他の成果物のデプロイを止めないためです。`/manual/` にはプレースホルダのページが配信され、**未作成であることが利用者に分かるようにしています**（生成されたふりをしません）。

### 11.5 生成物はコミットしない

`site/` `build/jig/` `build/jig-erd/` `apps/manual/` はいずれも Git 管理外です。**生成物をコミットすると「コードを変えたのに図が古い」状態がリポジトリに固定されます。** デプロイのたびに生成します。
...
**開発環境は公開されており、動作確認用の利用者が事前入力されています。**

- シードされている利用者（`db/seed/V800__seed_users.sql`）は**動作確認用であり、実在の利用者ではありません**。パスワードはリポジトリに記録されており、ログイン画面にも表示されます
- 一覧の内容は `app.demo-login.accounts` で定義しています。**シードの SQL とずれると画面の説明が嘘になる**ため、両者の一致は `DemoAccountListTest` が固定しています（片方だけ変更するとビルドが落ちます）
- **業務データを入れないでください。** H2 インメモリのため dyno 再起動で消えるうえ、公開環境です
- ロックの閾値・時間などセキュリティの挙動は本番と同じです。ここで確認できます
- 荷主のシード（`db/demo/V900__demo_shipper.sql`）は**ユーザーマニュアルのキャプチャと同じ内容**です。マニュアルの図と開発環境の画面が食い違わないようにしています。`local` / `dev` プロファイルの `spring.flyway.locations` にのみ含まれ、**本番には適用されません**（`DemoDataTest` が固定）

事前入力を止めたい場合は Config Vars で無効にします。

```bash
# ハイフンは環境変数名から除く（Spring の relaxed binding）。動作を確認済み
heroku config:set APP_DEMOLOGIN_ENABLED=false -a cargo-tracker-take-6
```
