...
| 配信内容 | MkDocs 110 ページ / JIG 11 ページ / ER 図 3 種 / マニュアル（未作成のプレースホルダ） |
| イメージサイズ | 127 MB |
| 検証 | `/` `/jig/` `/jig-erd/` `/manual/` `/healthz` すべて 200 |
...
| パス | 内容 | 生成元 |
| :--- | :--- | :--- |
| `/` | MkDocs（設計・要件・運用ドキュメント） | `docs/` の Markdown |
...
> **`deploy:docs:verify` は 4 パスすべてを確認します。** トップが表示されただけでは、JIG や ER 図が見えているとは限りません。
