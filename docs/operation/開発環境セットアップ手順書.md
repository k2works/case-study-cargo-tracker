...
- 起動コマンドは**シェル形式の `CMD`** を使用する（exec 形式 ENTRYPOINT + `sh -c` は Heroku のコマンドラップと干渉し即時終了する）
...
- Dockerfile の `CMD` が `ASPNETCORE_URLS=http://+:${PORT:-8080}` を設定していることを確認する
- `heroku logs --tail` で bind エラーを確認する

### `Process exited with status 0` で即クラッシュする

原因:

- exec 形式 `ENTRYPOINT ["sh", "-c", "..."]` が Heroku のコマンドラップと干渉している

対処:

- シェル形式の `CMD ASPNETCORE_URLS=http://+:${PORT:-8080} dotnet CargoTracker.Web.dll` に変更する

### push 時に `error from registry: unsupported` が出る

原因:

- Docker Desktop の containerd イメージストアが OCI マニフェストを push し、Heroku Registry が拒否している

対処:

- `deploy:dev:push`（buildx の `--output type=image,oci-mediatypes=false,push=true` + `--provenance=false`）を使用する
