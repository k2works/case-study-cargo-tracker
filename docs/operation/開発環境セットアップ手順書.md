...
> **Note**: `heroku container:push` は Dockerfile を検出してビルドから push まで実行します。
> 事前に `./gradlew bootJar` で JAR をビルドしておく必要があります（`deploy:dev:build` が実施します）。
> Spring Boot プラグイン適用プロジェクトでは plain jar の生成を無効化しているため、`COPY build/libs/*.jar` は 1 つの JAR だけを拾います。
...
```bash
# 1. バックエンド JAR をビルド
cd apps/backend && ./gradlew bootJar -x test && cd ../..
...
タスクランナーでは `.env` の `DEV_DOCKER_PLATFORM`（既定 `linux/amd64`）が自動適用されます。
...
| API Gateway ヘルスチェック | `https://{prefix}-gatewayms.herokuapp.com/actuator/health` |
| 公開追跡照会（認証不要） | `https://{prefix}-gatewayms.herokuapp.com/api/v1/tracking/{追跡番号}` |
