...
- **kind の統合環境に対して E2E を流すときは、開発サーバー（:3000）を経由しない。**
  `npm run test:e2e:real` は、クラスタが配信しているフロントエンド（`http://localhost`）を
  そのまま使う。Gateway の CORS は Ingress の出所（`http://localhost`）だけを
  許すため、:3000 から呼ぶと 403 になる。これは設定の誤りではなく、許していない出所からの
  呼び出しを断っている正しい動きである
- ローカルに Gateway（:8080）・authms・bookingms を直接起動して確認する場合だけ、
  `E2E_START_DEV_SERVER=true E2E_BASE_URL=http://localhost:3000 npm run test:e2e:real` で
  MSW 無効の開発サーバーを起動する
...
> **手で並べた一覧を置かない。** 以前ここにはアプリ名を書き写した一覧があったが、
> **サービスを足しても追随しなかった**——simulationms（IT14 で追加）が一覧にも
> 配布先にも無く、`deploy:dev` が 401 で止まった。Heroku は**存在しないアプリへの
> push に 404 ではなく 401 を返す**ため、認証の問題に見える。
>
> 対象は `ops/scripts/deploy.js` の `ALL_SERVICES` が正であり、上のタスクが
> そこから作る。
...
| サービス | 役割 | 主なイベント |
| :--- | :--- | :--- |
| bookingms | パブリッシャー | `TrackingNumberIssuedEvent`（実装済み・IT6） / `CargoRoutedEvent` / `CargoCancelledEvent` |
