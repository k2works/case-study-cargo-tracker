...
description: "国際貨物輸送管理システム（Cargo Tracker）のドキュメント環境をセットアップする手順。MkDocs によるドキュメントサイト、ユーザーマニュアルの HTML 変換、それらをまとめて配るドキュメントポータル（apps/www）と kind クラスタへの配置。"
...
| 生成ドキュメント | ユーザーマニュアル | 毎回作り直す。**コミットしない** |
...
| ツール | バージョン | 確認コマンド | 用途 |
| :--- | :--- | :--- | :--- |
| Docker | 最新 | `docker -v` | MkDocs のビルド・ポータルのイメージ作成 |
...
| `apps/www/index.html` `style.css` | ポータルのトップページ | 管理する |
| `apps/www/Dockerfile` `default.conf` | ポータルの配信設定 | 管理する |
| `apps/www/docs/` | MkDocs の出力（`site/` の写し） | **無視する** |
| `apps/www/manual/` | マニュアルの HTML 出力 | **無視する** |
...
`docs/manual/*.md` を HTML に変換し、`apps/www/manual/` へ出力します。
...
次の 3 つを順に実行します（所要 1〜2 分。大半は MkDocs のビルドです）。

| 順 | タスク | 出力 |
| :--- | :--- | :--- |
| 1 | `mkdocs:build` | `site/` |
| 2 | `portal:docs` | `site/` → `apps/www/docs/` |
| 3 | `manual:build` | `apps/www/manual/` |

確認します。

```bash
ls apps/www/docs/index.html apps/www/manual/index.html
```
...
```bash
docker build -t cargo-tracker/www:latest apps/www
```
...
| ポータルは開くが「ドキュメントサイト」が 404 | `apps/www/docs/` が空のままイメージを作った | 手順 5 → 手順 8 |
