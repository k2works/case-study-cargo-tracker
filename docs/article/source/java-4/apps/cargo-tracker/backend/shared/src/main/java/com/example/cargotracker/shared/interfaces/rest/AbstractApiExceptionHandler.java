 ...
 *
 * <p><b>4 サービスに同じものを写していた</b>（IT9 レビュー H.2。139 行 × 4）。
 * 対応表は architecture_backend.md が 1 つ持つ決まりなのに、実装が 4 つあると
 * 片方だけ直る——実際に文言と説明が少しずつ食い違っていた。</p>
 *
 * <p><b>{@code @RestControllerAdvice} はここに付けない。</b> 各サービスが
 * 自分の {@code interfaces.rest} で継承して付ける——サービス固有の断り方
 * （bookingms の荷主メール重複など）を足せる場所を残すためである。</p>
 */
