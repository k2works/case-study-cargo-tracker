...
public record CargoSpecification(
    ...
    public CargoSpecification {
        ...
        if (dimensions == null) {
            throw new BusinessRuleViolation("寸法は必須です");
        }
        if (quantity <= 0) {
            throw new BusinessRuleViolation("数量は 1 以上です: " + quantity);
        }
        if (productName == null || productName.isBlank()) {
            throw new BusinessRuleViolation("品名は必須です");
        }
        ...
        if (cargoType == CargoType.REFRIGERATED && temperatureRequirement == null) {
            throw new BusinessRuleViolation("冷凍・冷蔵貨物には温度管理条件が必要です");
        }
        // 種別と付帯情報の食い違いも断る。一般貨物に危険物申告が付いていると、
        // 経路設計が「危険物対応の航海だけ」に絞るべきか判断できない。
        if (cargoType != CargoType.HAZARDOUS && hazardousDeclaration != null) {
            throw new BusinessRuleViolation("危険物以外に危険物申告は付けられません");
        }
        if (cargoType != CargoType.REFRIGERATED && temperatureRequirement != null) {
            throw new BusinessRuleViolation("冷凍・冷蔵以外に温度管理条件は付けられません");
        }
    }
}
