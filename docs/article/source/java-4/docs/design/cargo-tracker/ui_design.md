  ...
  {* Cargo Tracker | 管理 花子 (システム管理者) | [ログアウト] }
  {
    ...
    } |
    {
      ...
      {#
        ...
        一般貨物の標準輸送 | 実行中 | 8 / 14 工程 | 09-14 10:00 | — | admin01 | [工程を見る]
...
**「どこまで進んだか」を一覧に出します。** 標準シナリオは 14 工程を連鎖の追いつきを待ちながら進むので、開かないと分からない形にすると、走っているあいだ何も判断できません。
...
**予定の工程を先に並べます**（IT16 のレビュー N4）。記録済みだけを出すと、連鎖待ちの 30 秒のあいだ画面が何も変わらず「進んでいるのか固まったのか」が読めません。**所要時間と連鎖の待ちは別の列**です（同 N8）——足し合わせると「14 工程が数ミリ秒ずつ」と読めてしまい、実際に時間を使っている場所が見えません。**止まった工程からは次に取れる行動へ送ります**（同 N5。連鎖が追いつかない → S91、経路が組めない → S32）。分からない失敗にリンクは出しません。**同じシナリオをもう一度流せます**（同 N6）——切り分けは「直す → 流す」を何度も回す作業です。
