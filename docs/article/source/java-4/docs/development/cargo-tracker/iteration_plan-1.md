...
| 1.2 | kind + Kustomize（`ops/k8s/base/axonserver/`・`postgres/`・各サービス）。Axon Server は `AXONIQ_AXONSERVER_STANDALONE_DCB=true` | 6h | [x] マニフェストと gulp タスク。適用はサーバ側 dry-run で検証（実クラスタでの通し運転は IT2） |
...
| # | タスク | 見積 | 状態 |
| :--- | :--- | :--: | :--: |
| 7.1 | マニュアルの構成・執筆テンプレート・キャプチャ方針を決める（`creating-manual`） | 2h | [x] |
| 7.2 | 「ログインする」「ログアウトする」「荷主を登録する」「要確認一覧を確認する」の 4 節を執筆 | 4h | [x] |
| 7.3 | 画面キャプチャを Playwright（2.7 の基盤）で自動生成する仕組み | 3h | [x] |
| | 小計 | 9h | |
...
| # | タスク | 見積 | 状態 |
| :--- | :--- | :--: | :--: |
| 4.1 | ロール名の確定（`ROLE_SALES` ほか 7 種）と `shared/domain/auth`（`AuthenticatedUser` / `Role`） | 2h | [x] |
...
| 6.7 | 鍵の破棄 → リプレイ → 個人情報が消えることの統合テストと `gulp shipper:shred` の雛形 | 3h | [x] デモ項目 6 を受け入れテストで固定（鍵破棄 → リプレイ → 個人情報のみ null） |
| | 小計 | 28h | |
...
- [x] デモ項目 7 件の受け入れテストがすべて緑（**7 件目は当初 E2E 2 本と記録したが誤り**。機能は `routes.test.tsx` の全ロール × 全ナビの往復検査で固定。E2E の 1 本は名前だけがデモ項目 7 を謳っていたため、名前を実態に合わせた）
