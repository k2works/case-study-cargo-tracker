...
- [x] ADR-0001 決定 2 の発動条件を判定した（**発動せず**。実績 9 SP = 100%。[ふりかえり](retrospective-2.md)）
...
- [~] US31・US03 は全項目を満たす。**US04 は §3（希望引渡日）と §6（見積との整合）が未実装**。前者は設計に置き場が無く、後者は US01（IT14）が前提。数を合わせずに報告書へ記録した
- [x] デモ項目 8 件の受け入れテストがすべて緑。**対応はテスト名でなく本文のアサーションで確かめた**
- [x] 画面は**ナビゲーションと到達性テストに対で**足した。**レビューで 1 件の漏れを見つけた**（予約詳細はナビに載せないため到達性の検査から外れていた）
- [x] IT1 の持ち越し 9 件のうち **8 件を返済**。契約の往復のみ IT3 へ（理由と代替を明記）
- [x] 本 IT で足した検査を壊して赤を見た（Q.1）
- [x] `./gradlew build` が緑
- [x] `TZ=UTC ./gradlew cleanTest test` が緑
- [x] フロントの `npm run test`・`npx tsc -b`・`npm run build` が緑（20 ファイル・130 テスト）
- [x] UI 設計・navbar・ダッシュボード・到達性テストの 4 点が一致している
- [x] 追加した各画面を、**そのロールで実際に 1 回開いた**（T7。kind クラスタに対する E2E で実施）
- [x] **kind クラスタで動く**（Axon Server は評価版なので毎回取り直す。`operation.md` 9.1）
- [x] **クラスタに対して E2E が緑**（13 件）
- [x] `npx gulp okf:check` が ERROR 0
- [x] SonarQube の Quality Gate がバックエンド・フロントエンドとも PASS、`mkdocs build` が成功する
- [x] ユーザーマニュアルの該当章が更新され、画面キャプチャが再生成されている（**レビューで 1 件の食い違いを見つけて直した**）
- [~] レビューは**逐次のフォールバックで 5 視点をたどった**。並列は 2 IT 連続で結果を返さず、催促にも応答が無かった。届くのを待つのではなく**並列を使わない**ことにする（ふりかえり T1）
- [x] ADR-0001 決定 2 の発動条件を判定した（**発動せず**。実績 9 SP = 100%。判定は [ふりかえり](retrospective-2.md) に記録し、ADR には発動しなかったため追記していない）
- [x] ふりかえり（[retrospective-2.md](retrospective-2.md)）と完了報告書（[iteration_report-2.md](iteration_report-2.md)）を作成した
...
| 2026-09-04 | IT2 クローズ。実績でタスクの状態・成功基準・DoD を更新。未達（US04 §3・§6・契約の往復）を明記 | claude-code/claude-opus-5 |
