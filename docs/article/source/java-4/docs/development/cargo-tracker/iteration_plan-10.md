...
- [x] **レビュー依頼の文面で出力の形を指定した**（Try T7）。**5 通とも指定どおりの形で返り、切れたものは無かった**（IT9 は 5 通中 4 通が途中で切れた）
...
- [x] **並列レビューをクローズの最初に起動した**（全通 2 分以内に返着）
- [x] **全体ビルドで落ちたテストは、単独で再実行して切り分けたうえで記録した**（Testcontainers の資源不足・並列実行時のタイミング・build ディレクトリ破損を、いずれも単独実行で切り分けた）
- [x] **クローズで計画を更新するとき、文書内の `- [ ]` を全部数えてから埋めた**
