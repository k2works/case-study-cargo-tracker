...
- [x] SonarQube の Quality Gate が**両プロジェクト PASS**（**2 度赤にして直した**——`BookingQueryHandler` の依存が 25／否定の分岐）
...
- [x] CI が緑（**1 度赤にして直した**）
- [x] SonarQube の Quality Gate が**両プロジェクト PASS**
...
| 2026-09-16 | **クローズで実績を反映**。成功基準 13 件のうち **11 件達成・2 件未達**（壊して赤を見るが 15/20・待ち条件の確認）。SonarQube は 2 度赤にして直し、**両プロジェクト PASS**。フルビルドと CI が**除外の副作用を 2 件**見つけた——`EXISTS ... simulated = FALSE` が写しの届く前の本物まで消す（通関の投影 12 件が赤）、除外を足した承認待ち一覧を連鎖の待ちに使いシミュレーション自身の申請が見えない（輸送中キャンセルが必ず時間切れ）。レビューの高 14 件はすべてクローズ前に修正した | claude-code/claude-opus-5 |
