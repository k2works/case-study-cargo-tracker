...
verified:
  - { by: human:k2works, at: 2026-09-10T21:11:46Z }
---
...
**新しい集約を 1 つ作ります**（`CustomsDeclaration`）。既存の集約に混ぜられないためです——通関は税関という別の当事者との手続きで、荷役（`HandlingActivity`）の一種ではありません（`HandlingType` の要素表が「税関は荷役ではない」と明記しています）。
...
- [x] **レビューは 5 通そろうまでクローズしない。返らない視点には再送した**（**Try T7**。IT11 は user 視点が 1 時間後に返り、**高 3 件**を持っていた） — **一部未達**（48 分無応答で逐次に切り替えたが、**その後 5 通すべて届き、統合した**。遅着分に実欠陥 5 件。待ち方の判断は誤ったが、返着を捨てなかった。[レビュー](../../review/cargo-tracker/IT12実装_review_20260910.md)に経緯を記録）
...
| T10 | 受け入れテスト（デモ項目）・**マニュアル 16 章「通関を通す」新設と 13・14 章への追記**・全体のクラスタ E2E | — | 8h |
| **合計** | | | **73h** |
...
| 2026-09-10 | **並列レビュー 5 通が遅れて全通届き、統合した。** 遅着分に実欠陥が 5 件——再留置が誰にも見えない／不可でも税関保留を解決していた／申告日時がブラウザの時間帯で送られる／決着済でも更新フォームが出る／宣言した安全網が実在しない 2 件。**高 8・中 11 をクローズ前に修正**し、低 9 件を IT13 へ送った。あわせて**引取待ちに通関状態の列**を出した（現場の入口だけが US29 の成果から取り残されていた）。クラスタ通し 21/21・フロント 431 件緑 | claude-code/claude-opus-5 |
| 2026-09-11 | 「終盤で新設するのはこれが唯一」を削除。**IT13 でも `Invoice` を新設する**ので事実でなくなった（IT13 開始準備の横断検証 C6。開発戦略の終盤の目的文に例外を明記した） | claude-code/claude-opus-5 |
