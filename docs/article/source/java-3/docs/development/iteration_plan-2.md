...
| 2.7 | `CargoBookingController`（`POST/GET /api/v1/bookings`）と MockMvc テスト。**登録も参照も ROLE_SALES のみ**（ADR-008。利用者と荷主を結ぶキーが無く「自分の予約だけ」に絞れないため、IT2 では荷主に開かない。US18 で広げ直す）。**荷主が参照も登録もできないこと**を、認可を外すと赤になる形で検証 | 4h | [x] |
...
- [x] kind 統合環境で Gateway 経由の動作確認済み
  - Ingress → Gateway → bookingms → PostgreSQL を通し、法人荷主の登録・冷凍貨物の予約
    （`BKG-2026000001` を採番）・荷主ロールの 403・未認証の 401 を確認
  - **起動しなかった原因は資源ではなくプローブの設定**だった。`timeoutSeconds` が既定の
    1 秒で、負荷のかかった作業機では JVM が応答しきれず、正常に起動しているコンテナが
    「起動できていない」と判定されていた。5 秒に緩めて解消（IT1 の「ヘルスチェックを
