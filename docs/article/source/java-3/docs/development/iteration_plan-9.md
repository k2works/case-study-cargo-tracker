...
> **進捗（2026-08-24）**: **14 件すべて完了**。想定と違ったのは 3 点。
>
> - **0.1 の検査が 0.2 の対象を作り直した。** 引き継ぎは「11 件」と数えていたが、検査が指したのは
>   `data-model.md` の 13 件（うち 9 件は引き継ぎに無かったもの）。**列挙して数えた件数のほうが
>   間違っていた。**
> - **0.3 でモックを本物に合わせたら、画面が赤になった。** 甘いモックが隠していたのは
>   モック自身の欠陥ではなく、**画面が本物の規則を満たしていないこと**だった
>   （遅延の解決に到着予定日を送っていなかった）。
> - **0.1 の検査そのものが沈黙する形だった。** `docs/` が Gradle のタスク入力に宣言されておらず、
>   設計だけを変えたときに検査が走らなかった。**壊して赤を確認しなければ気づかなかった。**
>
...
| # | リスク | 影響 | 対応 |
| :--- | :--- | :--- | :--- |
| 1 | **bookingms が初めてイベントを受け取る側になる**（[ADR-025](../adr/025-customs-declaration-and-cancellation-approval.md) 決定 1） | 交換機の引数が 3 サービスで食い違うと `PRECONDITION_FAILED` で落ち、**後続のキュー宣言まで止まる**。Testcontainers では出ず kind で出る | 宣言を handlingms・trackingms と**同じ引数**で揃え、`CargoHandlingExchangeArgumentsTest` で固定する。Phase 6 で kind に通す |
...
- [x] 対象ストーリーの受入基準を満たす。**果たせないもの（US29-4・US30-6・US30-7 の通知は代替、US30-9 のキャンセル料は US21）を [ADR-025] と完了報告書に記録する**
- [x] 成功基準 1〜9 を満たす
- [x] 全テストが緑。**品質ゲートは CI と同じコマンドで回す**——バックエンドは `./gradlew build`（`test` ではない。Try 9）
- [x] **CI が緑**（run 32797800380 以降）（GitHub Actions の run 番号を完了報告書に記録）
- [x] **`TZ=UTC` でも緑**（`--rerun-tasks` で強制再実行）
- [x] **ドメイン層のカバレッジが 90% 以上**（91.9〜100%）（6 サービス + shared）
- [x] SonarQube Quality Gate が **両プロジェクトで PASS**（Bug 0・Vulnerability 0）
- [x] **ADR-025 の決定ごとに検査があり、1 件ずつ壊して赤を確認した記録がある**（Try 1・成功基準 5）
- [x] **この IT で書いた「〜しない」「〜まで確かめる」の数だけ検査があり、壊して赤を確認した**（Try 2・成功基準 6）
- [x] **新しく足した値が層をまたいで生き延びる**（Try 4・成功基準 7）。値ごとに 1 本
- [x] **設計への反映が検査で担保されている**（Try 3・成功基準 8）。**注の表の消化ではなく、検査が緑になったことで判定する**
- [x] **統合テストが土台を継承している**（Try 6）。handlingms に土台を置いた
- [x] **ナビゲーションの 4 点が一致している**。**ロール別・状態別の到達性**を確かめた
- [x] **共有画面のリンクをロールで出し分けた**（荷役作業員に状態更新を見せて 403 にしない）
- [x] **件数から対象一覧へ辿れる**（[横断規約](release_plan.md)）
- [x] **サービス間の呼び出しを実際に 1 往復させた**（kind で通関 → 留置 → 公開追跡）
- [x] **実環境で確かめる前にイメージを作り直した**
- [x] **デモ項目 15 件をこの順で通した**
- [x] **IT7・IT8 時点のコメントが残っていない**（返済枠 0.11。実装した IT で消す）
- [x] **JIG / jig-erd の出力を再生成した**
- [x] ユーザーマニュアル 10 章を新設し、**キャプチャを再生成して目視した**。**索引・ナビ・キャプチャの有無が検査に入っている**（Try 5）
- [x] **利用者と荷主の紐付けストーリーを起票した**（タスク 8.1。US33 / [Issue #555](https://github.com/k2works/case-study-cargo-tracker/issues/555)）
...
| 区分 | 状態 |
| :--- | :--- |
| 返済枠（0.1〜0.14） | 完了（14 件・42h） |
| Phase 1 受け入れテストと ADR | 完了（[ADR-025](../adr/025-customs-declaration-and-cancellation-approval.md) の 10 決定すべて「壊して赤を確認」済み） |
| Phase 2 画面と導線 | 完了 |
| Phase 3 API と結合 | 完了 |
| Phase 4 ドメイン | 完了 |
| Phase 5 通知の代替 | 完了 |
| Phase 6 実環境 | 完了（kind で成功基準 1〜4 を通した。**実環境でしか出ない欠陥を 3 件捕まえた**） |
| ユーザーマニュアル | 完了（10 章・11 章を新設） |
| 計画への差し戻し | 完了 |
...
| 日付 | 内容 | 担当 |
| :--- | :--- | :--- |
| 2026-08-24 | **返済枠 14 件（42h）を全て完了**（Day 1-3 相当）。0.1 の検査が置いた直後に 13 件の実ドリフトを指し、0.2 の対象を機械的に示した。**0.3 でモックを本物に合わせたところ、画面が本物の規則を満たしていないことが出た**（遅延の解決に到着予定日が要る）。0.9 は営業への開放にあたりサーバ・ルート・ナビ・ダッシュボード・`ui_design.md` の 5 点を揃えた。0.12 では `manual-coverage` の検査が新しいメニューの記載漏れを捕まえた。**各項目とも壊して赤を確認済み** | 開発 |
| 2026-08-25 | **Phase 6 完了。** kind で成功基準 1〜4 を 1 本ずつ通し、`e2e/real-backend.spec.ts` に固定した。**実環境でしか出ない欠陥を 3 件捕まえた**——`/api/v1/cancellations` が Gateway のどこにも振られておらず（画面からは 404、サービス側は無実）、本番プロファイルでは公開追跡とロック解除も同じ状態だった。同型を機械で見つける `GatewayRouteCoverageTest` を置き、プロファイルごとに検査する（まとめて見ると壊しても赤にならなかった）。IT7 の「荷受人の確認」の検査は、通関ガードが先に立つことで一度も踏まれなくなっていたため、通関済にしてから確かめる形へ直した | 開発 |
