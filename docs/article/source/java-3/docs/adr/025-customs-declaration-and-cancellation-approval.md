...
| 決定 | 検査 | 壊して赤を確認 |
| :--- | :--- | :--- |
| 1. 荷役のイベントで輸送中を知る | `HandlingActivityRegisteredRoundTripTest#advancesTheBookingWhenTheEventArrives`（実 RabbitMQ の往復。**契約の値で送る**——こちらの定数で送ると綴りのずれを一度も確かめられない）。巻き戻さないことは同テストの `#neverRegressesTheBookingStatus`、冪等と受領で動かないことは `AdvanceBookingUseCaseTest#isIdempotent` / `#doesNotAdvanceOnReceive`。**ACL を引いていない**ことと**経路の状態を動かしていない**ことは `HandlingEventSubscriptionScopeTest`。**交換機の引数が 3 サービスで一致している**ことは `CargoHandlingExchangeArgumentsTest`——引数を 1 つ変えると赤 | **済**（ルーティングキーの綴り／巻き戻さない守り／交換機の引数 3 通り、いずれも赤） |
| 2. 緊急は載せ続け、言葉を変え、届くことを固定する | `tracking-lookup-page.test.tsx#紛失のときだけ、次の行動を案内する`——**集約 → 応答 → モック → 画面のどこで潰しても赤になる 1 本**（緊急の定義／公開応答の項目／画面の表示のいずれを壊しても赤）。**急かす言葉を使わない**ことは同テストが文言の不在で見る。遅延で案内を出さないことは `#遅延では、連絡をすすめる案内を出さない`。`CUSTOMS_HOLD` が緊急でないことは `ExceptionTypeTest` | **済**（緊急の定義を落とす／遅延も緊急にする／公開応答が落とす／画面が出さない／急かす言葉に戻す、の 5 つとも赤） |
| 1 の付随: `SETTLED` へ進めない | `BookingStatusTest#hasNoTransitionIntoSettled`——値の不在と、**遷移の呼び出し箇所**を数える（コメントは除く。説明を書くことを罰しない） | **済**（`SETTLED` を足して遷移を書くと赤） |
| 3. キャンセルは trackingms だけが購読する | **理由を載せていない**ことは `CargoEventPublishingTest#doesNotCarryInternalJudgement`——契約の「載せる項目」と**「載せてはいけない項目」を対で見る**（載せる一覧だけだと足したときに気づけない）。**billingms へ発行していない**ことは同テストの `#publishesOnlyTwoKindsOfEvents` / `#sendsOnlyToTheAgreedRoutingKeys`——発行の呼び出し箇所を数える。**承認したら知らせ、却下では知らせない**ことは `CancellationUseCaseTest#announcesTheCancellation` / `#doesNotAnnounceOnRejection`。購読側は `NoteCancellationUseCaseTest`——冪等・社内の手がかりを書かない・**`TrackingStatus` に値を足さない** | **済**（冪等を外す／社内の手がかりを書く／イベントに理由を載せる、の 3 つとも赤） |
| 4. 陸揚げ地の候補は bookingms が作る | `DischargeCandidatesTest`——候補外の港を断る（`#rejectsAPortOutsideTheCandidates`）、現在地が先頭に来る、なぜ候補かを添える、同じ港を二重に出さない。**ACL を引いていない**ことは `HandlingEventSubscriptionScopeTest#doesNotCallBackIntoHandling`。**列を足したこと**は返済枠 0.1 の `SchemaDesignConsistencyTest` | **済**（候補の絞り込みを外す／現在地を含めない／重複を許す／理由を空にする、の 4 つとも赤） |
| 5. 手配は陸揚げ地の記録まで | 陸揚げ地が行に残ることは `CancellationRequestPersistenceIntegrationTest#persistsTheDischargeLocation`。**却下では陸揚げ地が付かない**ことは `CancellationRequestTest#doesNotAttachADischargeLocationOnRejection`——付くと「荷降しの手配が要る」と読まれる。画面に出ることは `cancellations-page.test.tsx`。**運用条件は [リリース計画](../development/release_plan.md) に明記した**（承認した追跡管理者から荷役の担当者へ連絡する） | **済**（承認を INSERT にする／陸揚げ地を書かない、の 2 つとも赤。運用条件は文書に記載） |
...
| 9 の前提: CLAIM ガード（US29-3） | `CustomsGuardTest#allowsClaimWhenCleared` / `#rejectsClaimWhileHeld` / `#rejectsClaimWhilePending` / `#rejectsClaimWhenRejected` / `#rejectsClaimWithoutAnyDeclaration` / `#doesNotGuardOtherHandlingTypes`。**画面から踏むテストと対にする**——`handling-page.test.tsx#通関が留置のままなら、引取は断られ、いまの状態が画面に出る` と `#通関申告が無い貨物は、引取が断られる` | **済**（ガードを外す／申告が無い貨物を素通りさせる／引取以外も止める／現在の状態を伝えない、の 4 つとも赤） |
| 9. 荷受人確認は残し、但し書きは外す | 荷受人の確認が残ることは `HandlingActivityTest`（引取で必須）と `handling-page.test.tsx#通関を仕組みで確かめていない、とは書かない`（欄そのものの存在）。**但し書きが消えたこと**は同テストが**文言の不在**で見る。**さかのぼらない**ことは、ガードが `requireCustomsCleared` の中だけにあり復元（`CustomsDeclaration#restore` / `HandlingActivity` の読み出し）に無いことによる——過去の記録を読む経路に検査が入っていない | **済**（但し書きを戻す／モックのガードを外す、の 2 つとも赤） |
