...
1. **育つ負債が 0 になっている**。D5（ADR-024 の代償）は契約どおりの検査で止め、D1（`Cargo` 500 行）は切り出す —— **達成**
2. **「あとで判断する」と書いたまま残っている決定が無くなっている**（返金・多通貨・`invoice_line_item`・見積の `EXPIRED`）—— **達成**
3. **見積一覧が毎朝の確認に使える**（既定・0 件の理由・件数と並び順・一覧からの作り直し）—— **達成**（**クローズ前レビューで、既定が新しく作った 0 件を 1 件見つけて塞いだ**）
4. **記録の誤りが直っている**（下記「着手前に分かったこと」）—— **達成**

### 成功基準

- [x] **ADR-024 の契約（誰がどのメソッドを呼んでよいか）が検査になっている**（ArchUnit。**許可外から呼ぶと赤になることを確かめる**）
  - **クローズ前レビューで穴が 3 つ見つかり、すべて塞いだ** —— メソッド参照の素通り（実測）・契約表の実在未検証・破壊検証の証跡なし
- [x] **`Cargo` が 500 行を下回り、余裕がある**（500 → **485 行**。猶予 15 行）
- [x] **`release_scope.md` の「スコープ外」表が実態と一致している**（5 行すべて。多通貨は ADR-025 で確定）
- [x] **落としたものを名前で記録している**（「本 IT では扱わないもの」表 6 件 ＋ レビューのスコープ外 9 件）
...
>
> **【IT20 の実測】上表 6 か所のうち、直す必要があったのは 4 か所でした。**
>
> | 場所 | 計画の記載 | 実測 |
> | :--- | :--- | :--- |
> | `release_plan.md:62` | 「開発期間: **19 イテレーション**」 | **既に 20 イテレーション**（開始準備で更新済み） |
> | `release_plan.md` 進捗表 | 「IT20 の行なし」 | **既に IT20 の行がある** |
>
> **記録と実測のずれが 10 度目・11 度目です。** 本 IT の Try
> 「数を書くときは数えた方法も書く」は、**この計画自身にも適用されていませんでした** ——
> 注 1 の 6 か所は、数えた方法を書かずに列挙していました。
>
...
| # | タスク | 見積 | 状態 |
| ---: | :--- | ---: | :--- |
| 1 | **D5: `entities` の生成・変更を集約ルートに限る**（**育つ**） | 10h | **完了**（`EntityEncapsulationTest`。4 メソッドすべて破壊検証済み） |
| 2 | **D1: `Cargo` の状態遷移を寄せる**（**育つ**） | 10h | **完了**（500 → 485 行。猶予 15 行） |
| 3 | U4 / U5: 見積一覧の既定と、0 件の理由 | 8h | **完了**（(c)(d) も実装した） |
| 4 | U6 / U8: 件数・並び順と、一覧からの作り直し | 6h | **完了** |
| 5 | J1〜J3: 正典に残った未判断を決着させる | 6h | **完了**（ADR-025 を追加） |
...
| 8 | 途中レビュー | 3h | **未実施**（利用者の指示により中断。クローズ時の正式レビューで代替した） |
...
## 計画外に受けたもの（**Try T5: その場で落とす順序を見直して記録する**）

### 計画外 1: 真夜中に落ちるテスト 20 件（2026-08-13 00:09 に発覚）

**品質ゲート（タスク 9）の CI 実行で 20 件が落ちた。** 切り分けたところ、
**IT20 の着手前のコミット（`71149ef24`）でも同じ 11 件が落ちる**ため、
本 IT の変更が原因ではない。

**原因**: テストが作業日時を `LocalDateTime.now(clock).minusHours(1)` で作っており、
業務のタイムゾーンで 00:00〜00:59 に走らせると**前日に落ちる**。アプリは
「作業日時が追跡番号の発行日（今日）より前です」と**正しく拒否**する。
**赤いのはテストの作り方であって実装ではない。**

**同じ書き方が 6 か所**にあった（`CustomsScreenTest` 2・`MisrouteScenarioTest` 2・
`ManualStatusUpdateTest` 1・`ExceptionTestBase` 1）。共有ヘルパ
（`RecentBusinessTime`）へ寄せた —— **同じ間違いを 6 回書けるなら、7 回目も書ける。**

**時計を固定した単体テストを付けた**（`RecentBusinessTimeTest`）。
**実時刻で走らせるだけのテストは、1 日のうち 1 時間しか赤くならない。**

**落とす順序は見直していない。** 本件は品質ゲートの一部であり、
これを落とすと**「CI が緑である」と言えなくなる**。所要 2h は余白（18h）で吸収した。
