...
> **本報告書は `CHANGELOG.md` と git タグが無い状態で作成し、その後にバージョンを確定しました。** 数値はすべて `release_plan.md`・イテレーション完了報告書・`git log`・テスト実行結果の**実測値**です。
>
> | 項目 | 状態 |
> | :--- | :--- |
> | バージョン | **1.1.0**（1.0.0 → 1.1.0。`release:minor`） |
> | git タグ | **`java/take-6/v1.1.0`**（push 済み） |
> | `CHANGELOG.md` | **生成済み**（296 エントリ・8 カテゴリ） |
> | リリースコミット | `8f9ff9638` |
>
> **タグに `java/take-6/` の接頭辞を付けました。** 本リポジトリは同一題材の複数実装が同居しており、**素の `v1.1.0` ではどの実装のリリースか判別できない**ためです（既存の `v1.0.0` は別実装の 2026-05 のタグです）。
...
### リリースの確定（2026-08-10 追記）

| 項目 | 状態 |
| :--- | :--- |
| 品質ゲート（preflight 5 項目） | **全通過**（clean / lint / test / build / E2E 14 本） |
| バージョン | 1.0.0 → **1.1.0** |
| `CHANGELOG.md` | **生成済み** |
| git タグ | **`java/take-6/v1.1.0`**（push 済み） |

**リリースワークフロー自体が動かない状態だったため、あわせて直しました。**

- `ops/scripts/release.js` が gulpfile に登録されておらず、`release:*` タスクが 1 つも使えなかった
- `release:deploy:*` が存在しない `deploy:prd` を無条件に参照し、**`gulp --tasks` すら実行できなかった**
- 品質ゲートが参照する npm scripts（lint / test / build / test:e2e）が未定義だった
- サマリーが接頭辞なしのタグ名を表示していた（**その名前で push すると失敗する**）

**品質ゲートの `lint` に `spotbugsTest` を含め、`test` を `TZ=UTC` で走らせるようにしました。** どちらも本リリースの開発中に CI だけが赤くなった原因です（本体の静的解析しか見ていなかった／時刻の表記が JVM のタイムゾーンで変わる）。**リリースの門で同じ取りこぼしを繰り返さない**ためです。
