...
- [x] **T1: 「実装しなかった守り」を数える。** マニュアル 11 章の「〜できません」4 件を数え上げ、**3 件にその道を実行したテストが無かった**（算出できない理由が画面に届くか／理由の無い調整／請求額を超える減額。いずれも単体では拒んでいたが画面からの道が未検証だった）。テストを足した
...
| # | 装置 | 数え上げ元 | 壊し方 | 結果 |
| :--- | :--- | :--- | :--- | :--- |
| 1 | 金額の切り捨て（荷主に不利な方向へ丸めない） | 実装後の数え上げ | `RoundingMode.DOWN` を `HALF_UP` に | **赤**（6 件） |
| 2 | 個人荷主に割引を適用しない | 同上 | 種別の判定を通す | **赤**（2 件） |
| 3 | 確定後は調整できない | 同上 | `requireDraft` を素通りさせる | **赤**（1 件） |
| 4 | 訂正申請中は請求できない | 同上 | 判定を通す | **赤**（1 件） |
| 5 | 二重請求を拒む | 同上 | 判定を通す | **赤**（2 件） |
| 6 | 確定済みは UPDATE できない（SQL 側） | 同上 | `WHERE charge_status = 'DRAFT'` を外す | **赤**（1 件） |
| 7 | 税率を保存して読み戻す | 同上 | 復元時に既定値（0.1200）を当てる | **赤**（1 件） |
| 8 | 請求書は経理担当者だけ | 同上 | 認可を `authenticated()` に緩める | **赤**（1 件） |

**8 件すべてが赤になった。空振りは 0 件。** IT12 に続き、**作った直後に壊す**を各コミットの中で行った。

> **判定そのものを 2 回間違えた。** 1 回目は `BUILD FAILED` をテストの赤と読んだが、
> コンパイルエラーでも同じ表示になる（**壊し方が下手なだけで「守れている」と読める**）。
> 2 回目は失敗の記録が stderr に出ることを見落とした。
> <strong>破壊検証は「赤になったか」ではなく「テストが赤になったか」を見る。</strong>
