...
| 計画 SP | 7 |
| 完了 SP | **7**（実装完了。クローズは `closing-iteration` で行う） |
...
| 8 | **経路状態の日本語ラベルが正典に無く、呼び名も割れている。** `ui_design.md` の付録は「全 enum の日本語ラベルの正典」と宣言しているのに `RoutingStatus` の節が無い。さらに US09 は「経路状態が**確定**になる」、US11 は「経路状態が**割り当て済**になる」と、同じ値を別の言葉で呼んでいる | **タスク 5-1 で付録に追加する。** ラベルは **`NOT_ROUTED` = 未割り当て / `ROUTED` = 割り当て済 / `MISROUTED` = 誤配**とする。「割り当て済」を採るのは、作業入口が「経路割り当て待ち」であり、画面のことばと揃うためである。**US09 の「確定」は操作の名前**（`[この経路で確定]`）として残し、状態の名前とは分ける |
...
> **0-0 と 0-1 は完了。** 依存は更新済みで Trivy 0 件。乗船地の選び方は
> **経由が少なくなる乗り方**を選ぶようにし、最初に見つけた出発だけを見る形に戻すと
> 2 件落ちることを実測した。経由の上限が 0（直行のみ）のときに候補ごと消えていた
> 欠陥も、あわせて塞がった。

#### タスク 0-2 の結論: 経路候補を自動算出しない（C3）

**画面を開いた時点では算出しない。** 押して算出する現在の形を維持する。

| 理由 | 内容 |
| :--- | :--- |
| 候補が黙って入れ替わる | 再算出は候補を丸ごと入れ替える（ビジネスルール 5）。開くたびに算出すると、**さっき見て検討していた候補が理由も告げずに消える** |
| 算出回数が意味を失う | `calculation_count` は「何回試したか」であり、条件を緩めるかどうかの判断材料になる。画面を開いた回数が混ざると読めない |
| US10 と一貫しない | 条件を変えた再算出（US10）は明示的な操作である。初回だけ暗黙に走るのは筋が通らない |

**ただし「まだ算出していない」ことは画面で明示する**（IT4 で実装済み）。
押さなければ何も出ないことが分からない画面にはしない。

#### タスク 0-3 の結論: 並行操作の検証方法（C5）

**リポジトリのテストで固定し、画面レベルでは「衝突を受け取ったときの振る舞い」だけを検証する。**

| 層 | 何を確かめるか | 方法 |
| :--- | :--- | :--- |
| リポジトリ | 後の保存が拒否される | 2 つの集約を読んでから順に保存する（IT4 で実装済み） |
| 画面 | 衝突が 500 にならず、次にどうすればよいか伝わる | **衝突するテスト用のリポジトリに差し替え**、フラッシュメッセージを検証する |

MockMvc で並行リクエストを再現しようとしない。**再現しにくいものを再現しようとするより、
「衝突が起きたら何が起こるか」を確実に固定するほうが守れる。**
...
| # | タスク | 状態 |
| :--- | :--- | :--- |
| 0-0 | 依存の更新（Trivy 0 件） | 完了 |
| 0-1 | 乗船地の選び方（C2） | 完了 |
| 0-2・0-3 | 自動算出の是非・並行操作の検証方法（C3・C5） | 完了（結論を上記に記録） |
| 1-1 | 管理者の seed | 完了 |
| 2-1〜2-5 | ドメイン（旅程・経路状態・選択・空き容量・解除） | 完了 |
| 3-1〜3-5 | 永続化・ACL・H2 方言スモーク | 完了 |
| 4-1〜4-4 | 管理画面・確定ボタン・予約詳細の経路・認可 | 完了 |
| 5-1・5-2 | 設計ドキュメント・マニュアル | 完了 |
...
**うち 2 件は、最初は装置を外しても緑のままだった。** 乗降の区間の絞り込み
（航海の全区間が乗降区間と一致する便しか使っていなかった）と、ロック切れの除外
（期限切れのロックを持つ利用者を作っていなかった）である。**どちらも
テストを判別する形に差し替えてから採用した。**

### 計画外に見つけたもの

| 内容 | 対応 |
| :--- | :--- |
| **候補の算出で割当済み重量を渡していなかった。** 空き容量が常に「あり」になり、満船の便が確定できていた | 渡すよう修正。渡さない形に戻すと落ちることを実測 |
| **V9 で NOT NULL にした直後、動作確認用データの投入が H2 方言スモークで落ちた** | デモデータにも容量を入れた。**制約を足すことは入力を足すこととセット**（IT4 の外部キーと同型） |
| **積載可能重量が航海の属性として無かった** | 集約・V9・登録画面に追加。容量が分からない便を作れると、空き容量の判定が形骸化する |
| **BC をまたぐ受け入れシナリオのテストの置き場が無かった** | `scenario` パッケージを新設し、ArchUnit のルール 4・5 に理由つきで追加 |
| SonarQube の指摘 8 件 | 宙に浮いた Javadoc・重複リテラル・入れ子のみのテスト・テスト内のシステム時計。**埋め草ではなく必要な検証を足して解消** |
...
| Backend CI | 緑 |
| SonarQube Quality Gate | **PASS**（新規カバレッジ 89.6% / 重複 0.55% / 指摘 0 件） |
| Trivy HIGH / CRITICAL | 0 件（**開始時にも確認済み**。T4） |
