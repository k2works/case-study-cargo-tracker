...
| 0-1 | 航海詳細画面 | 完了 |
| 0-2 | 区間追加の htmx 化 | 完了 |
| 0-3 | `release_plan.md` の再配分 | 完了（開始準備で実施） |
| 0-4 | ADR-008 の起票 | 完了 |
| 1-1 | `cargo` の港マスタ外部キー（V7） | 完了 |
| 1-2 | 動作確認用データの拡充（V903） | 完了 |
| 2-1〜2-5 | ドメイン（探索・候補・概算費用・集約） | 完了 |
| 3-1〜3-4 | 永続化・ACL・H2 方言スモーク | 完了（V8 を追加） |
| 4-1〜4-4 | アプリケーションと画面・認可 | 完了 |
...
**逆走チェックのテストは、最初は装置を外しても緑のままだった。** 出発地が
そもそも航路上に無いだけで、順序を見ているかを判別していなかった。
乗る港と降りる港がどちらも航路上にある形に差し替えて、判別することを確かめた。

### 計画外に見つけたもの

| 内容 | 対応 |
| :--- | :--- |
| **港マスタの外部キーを張ると、形式は正しいがマスタに無い港での予約が 500 になる** | Booking にも `KnownPorts` を置き、どの港が登録されていないかを画面に返す |
| **`booking_route_proposal` に貨物種別と重量の列が無く、探索条件を復元できない** | V8 で追加（設計への反映が必要 7 件目） |
| **読み戻しで貨物種別を落としており、危険物の予約に扱えない便が「選択可」と並んでいた** | 探索条件から渡すよう修正。テストで赤にしてから直した |
| **ACL ポートの戻り値に Routing の値オブジェクトを置き、ArchUnit ルール 4 が落ちた** | 境界は素の値にし、翻訳は Routing 側で行う |
| CI の Trivy が micrometer-core に HIGH を 2 件検出 | 1.16.6 に固定 |
| SonarQube の S2187（入れ子クラスのみのテスト） | 親クラスに**必要な検証**（条件の無い探索を拒否する）を追加。埋め草を置かない |
...
| Backend CI | 緑（**一度赤**。Trivy の HIGH 2 件） |
| SonarQube Quality Gate | **PASS**（新規カバレッジ 90.9% / 重複 0.68% / 指摘 0 件） |
