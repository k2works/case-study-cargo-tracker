...
- [x] Heroku 開発環境にデプロイして動作を確認する
      - ヘルスチェック UP。営業担当者で予約一覧・登録・荷主一覧に到達を確認
      - **権限の無いロール（荷役作業員）では 403 になり、ダッシュボードにも導線が出ない**ことを
        デプロイ後の環境で確認した（レビュー H1 の修正が実環境で働いている）
- [x] 品質ゲート: `check`（テスト・Checkstyle・SpotBugs・JaCoCo・ArchUnit・依存検証）が緑
- [x] CI が緑（Backend CI run 31105856285。ローカル緑と CI 緑を混同しない）
- [x] **SonarQube Quality Gate: PASS**（カバレッジ 89.7% / 重複 0.0% / 指摘 0 件 / セキュリティホットスポット 0 件）
      - 新規セキュリティホットスポット 2 件は**メールアドレス正規表現の ReDoS**（java:S5852）だった。
        `SONAR_TOKEN` に権限が無く API から特定できなかったため、ローカルコンテナの DB を
        読み取りのみで参照して箇所を割り出した
      - 抑止せず実際に直した。ドメイン部のラベルからドットを除いて区切りを一意にし、
        **長さの検査を形式の検査より先に**行うようにした（逆だと数万文字の入力が照合にかかる）
      - この修正が S5998（上限の無い繰り返し）と S2629（ログレベルに関わらず整形が走る）を
        新たに生んだため、続けて対応した。**1 つ直すと別の指摘が出るのは普通であり、
        1 回のスキャンで終わりにしない**
