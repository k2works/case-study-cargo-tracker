...
| 8 | **`ui_design.md` の荷役作業一覧のロール ／ `SecurityConfig.handlingRules`** | **正典が食い違っている（実装中に判明）。** 下記 |

> **反映 #8: 荷役作業一覧を追跡管理者が開けるのか、正典が 2 つに割れている。**
>
> | 出典 | 内容 |
> | :--- | :--- |
> | `ui_design.md` の画面一覧 | 荷役作業一覧の表示ロールは **`ROLE_HANDLER, ROLE_TRACKER`** |
> | `SecurityConfig.handlingRules` | `/handling` は **`ROLE_HANDLER` のみ**。「現場が使う唯一の画面である」と理由つき |
>
> **どちらかが古い。** C1 のテストで追跡管理者の到達性を主張したところ 403 で落ちて判明した。
>
> **本 IT では認可を広げない。** 認可の変更は判断が要るものであり、
> **テストで既成事実にしてよいものではない**（CLAUDE.md の「確認必須」）。
> US30 の受入基準が求めるのは**荷降しを行う荷役作業員に届くこと**であり、それは満たしている。
>
> **決着のさせ方**: 追跡管理者は訂正・取り消しの承認（US36）とキャンセルの承認（US30）を
> 行う立場であり、`/handling/corrections` と `/handling/customs` には GET で入れる。
> **荷役作業一覧だけが閉じているのが意図なのか、書き漏れなのかを決めてから**、
> 決めた側を検査に落とす（ADR-009 で学んだ「規則と検査を同じ変更で入れる」）。
...
> **タスク 3 とタスク 5 の順序を入れ替えた（着手後の判明）。** タスク 3 は
> 一覧クエリサービス **7 件**に新しい統合テストを足す作業であり、**それぞれが
> 貨物のフィクスチャを要する**。先に書くと C2 の重複が **7 → 最大 14 クラス**に増える。
>
> **T5 が言っているのはまさにこれである** — 落とす順序を見積もりの大きさでなく
> 「次に対象が増えるか」で決める。**同じ判断は、タスクの順序にも効く。**
> よって **タスク 5（C2 のヘルパー統合）を先に行い、タスク 3 はその支援クラスを使う。**

### 3. 一覧の問い合わせ回数を計測する（T3。**タスク 5 の後に行う**）— 6h
...
### 途中レビュー（T6）で次イテレーションへ送るもの（**記名して送る**）

**記名のない引き継ぎは返済されない**（IT7 の C1・C2、IT16 の C10 で 2 度確かめた）。

| # | 内容 | 出典 |
| :--- | :--- | :--- |
| R1 | **荷降し手配に期限の手がかりが無い**（出ているのは承認日時だけで、いつまでに降ろすかが読めない） | IT16 レビュー M2 |
| R2 | **所有表が `data-model.md` と検査の 2 か所にある**（突き合わせる検査が無い） | IT16 レビュー L2 |
| R3 | **荷降し手配の合成が infrastructure の実装クラスにある**（クエリサービスの実装が「読み取り」以上をする最初の例） | IT16 レビュー L3 |
...
>
> **6 件はいずれも T3 が定まる前に書かれたものである。** T3 そのもの — 「一覧を書いたら
> その場で計測する」— は本 IT から構造で守られる。
> **落とした返済枠が固定化する形（ADR-008 が 3 IT 繰り越した形）にしないため、
> 表を縮める検査（解消済み・実在しない名前を残さない）を同時に入れた。**
...
| 次 IT へ記名して送るもの | R1〜R10 |

> **「落としたもの」が 0 なのは IT10・IT11 以来である。** ただし
> **見積もりは大きく外した** — C2 は 7 → 39 クラス、C3 は 2 → 21 件だった。
> 落とさずに済んだのは、**実測が判明するたびに順序を組み替えた**ためである
> （タスク 3 と 5 の入れ替え、C1 の前倒し）。
...
| 2026-08-11 | **実装中の判明を反映**: タスク 3（T3）とタスク 5（C2）の順序を入れ替えた。T3 は 7 件の統合テストを新設する作業であり、**先に書くと C2 の重複が 7 → 最大 14 クラスに増える**。T5 の判断基準（次に対象が増えるか）はタスクの順序にも効く |
