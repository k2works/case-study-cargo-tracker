...
- [x] US01 の受入基準 6 件をすべて満たす（正典: `../requirements/user_story.md`）
- [x] **見積の数字が Routing の探索と概算から来ている**（固定値でない。破壊検証で確認）
- [x] **育つ負債（C1・C2）に着手している**（C2 は完了、C1 は部分返済＋ラチェット）
...
- [x] 正典: `../requirements/user_story.md` の US01 受入基準 6 件
- [x] デモ項目 9 件がすべて動作する
- [x] **落としたものは名前で記録する**（C1 の残り 210 個・C3〜C6。下記）

#### 局面の完了条件

- [x] 正典: `development_strategy.md` の「見積（IT18）」の節

#### 品質

- [x] `./gradlew check` が緑
- [x] **`TZ=UTC ./gradlew check --rerun-tasks` が緑**
- [x] E2E が緑（21 件）
- [x] **CI が緑**（`gh run`）
- [x] SonarQube Quality Gate が PASS

#### 到達性（ロール別・状態別・**操作可否**）

- [x] **営業担当者がダッシュボード・navbar の両方から見積一覧へ到達できる**（破壊検証で判別を確認）
- [x] **営業担当者以外に見積の入口が出ない**
- [x] **T1: 押せないボタンが出ていない**（見えているのに 403 になる操作を作らない）
- [x] **`EXPIRED` の見積から予約へ進めない**（状態別の到達性）

#### 主張とテスト

- [x] **着手時に返済枠を数え直した**（C1 は 34 → **249**、C2 は 39 件中 **29** 件）
- [x] **破壊検証で空振り 0**。ただし**作る過程で空振りを 2 件作り、その場で直した**（画面全体を見る判定・部分文字列の重なり）
- [x] **T2: 「検査で固定した」と書く前に、失敗の出力を確認した**
...
| 安全装置 | 何を止めるか | 破壊検証 |
| :--- | :--- | :--- |
| `EstimateRouteCandidateTest` | 見積の数字が固定値になる | スタブに戻すと 3 件が赤 |
| `EstimateScreenTest` | 見積へ到達できない／他ロールに入口が出る | 認可・カードのどちらを外しても赤 |
| `EstimateToBookingTest` | 予約へ内容が引き継がれない | `applyTo` を外すと赤 |
| `CheckMessageOriginTest` | 検査の失敗メッセージから理由に辿れない | 由来を 1 件消すと赤 |
| `DelegatingAccessorRatchetTest` | 委譲アクセサが増える | 1 個足すと赤（**実際に自分で踏んだ**） |
| `H2DialectSmokeTest`（見積の経路） | ローカル起動だけ見積一覧が 500 になる | — |
| `ListQueryEfficiencyTest`（見積の経路） | 一覧の問い合わせが件数に比例する | — |

**やらなかったもの**: C1 の残り 210 個（上限で固定）・C3〜C6。

#### ドキュメント

- [x] `domain-model.md` の 3 件の改訂（ルート候補の出所・`EXPIRED` の契機・`EstimationCargoType`）＋ ACL ポート一覧に `RouteCandidateSource`
- [x] ADR-023 を起票
- [x] **マニュアルに「見積管理」の章を追加**（キャプチャは生成 spec 経由。**候補 0 件の画面も撮った**）
- [x] `mkdocs build` の新規警告 0
...
| 2026-08-12 | タスク 0〜10 を完了。**US01 の受入基準 6 件をすべて満たした**。返済枠は C2 完了・C1 部分返済（実測 249 個のうち 39 個を返し、残りは上限で固定）。**マイグレーションは計画の 0 本から 2 本**（候補 0 件の理由・危険物の申告）。品質: `check` / `TZ=UTC check` / E2E 21 件 / CI / SonarQube すべて緑 |
