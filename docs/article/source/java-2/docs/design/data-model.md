...
> **本表と検査の名簿の一致は、検査自身が確かめる**（IT17 の R2）。`MapperTableOwnershipTest.所有表は正典と一致する` が本表を読んで名簿と突き合わせるため、**片方だけを直せば赤くなる**。IT16 までは名簿が本表の書き写しで、「ずれたら正典を読み直して直す」という人の注意力だけが同期の担保だった。ビルドは本表をテストの入力に含めているため、**本表だけを直したときもテストが再実行される**。
