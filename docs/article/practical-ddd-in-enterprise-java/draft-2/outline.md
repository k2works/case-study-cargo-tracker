...
| 参照元実装 | `docs/article/source/java-2/apps/cargo-tracker/`（`java/take-6`。単一 Gradle モジュール）。**第 4 章のみ** `docs/article/source/java-3/apps/backend/`（`java/take-7`。8 サービス + 共有ライブラリ）。**第 5 章のみ** `docs/article/source/java-4/apps/cargo-tracker/backend/`（`java/take-8`。同じ分割 + Axon Framework 5） |
| 参照元一次資料 | `docs/article/source/java-2/docs/`（`design/` 11 本・`adr/` 25 本ほか） |
| 題材 | 国際貨物輸送管理システム（Cargo Tracker） |
| **今回の章数** | **6**（第 1〜6 章。**完結**） |
| 保留 | **無し。** 第 6 章の成立でシリーズは完結（§5） |
...
| ~~Axon Framework / Event Sourcing~~ | **対象に入った。** `source/java-4/` の収録で第 5 章が成立（§5）。java-2・java-3 に無いことは変わらないので、第 5 章だけが java-4 を引く |
...
| 第 5 章 | `05-spring-cqrs-es-axon.md` | イベントを正典にする — CQRS / Event Sourcing 版の Cargo Tracker | M1・M2・M3・M4 |
| 第 6 章 | `06-conclusion.md` | 3 つの実装を同じ尺度で比べる | M1・M2・M3 |

**ファイル名は draft-1 から変えません。** 章の中身は書き直しますが、扱う範囲は同じであり、稿の対比を追いやすくするためです。

### 6 章で完結します

**します。** 第 6 章がシリーズを閉じます。ただし閉じ方は「どれが良いか」ではなく、**何が違うか**と**どの順序で問うか**までです。**この比較が確かめていないこと**（制御実験ではない・本番運用が無い・書籍の 4 アプローチのうち 2 つを扱っていない）を同じ章に置き、書けないものを書きません。読者に対しては「実装アプローチの比較は続編で扱う」と明示し、現時点で書けないことを書けるふりをしません（P1 の再発防止）。
...
保留の理由は「コマンドとクエリの分離は両実装に実在するが、**イベントストアも投影テーブルも無い**」ことでした。着手条件は「Event Sourcing 実装が `docs/article/source/` に収録されること」です。

`java/take-8`（Axon Framework 5 による CQRS / Event Sourcing 版）を [`source/java-4/`](../../source/java-4) に収録して条件が満たされたため、第 5 章を起こしました。

| 項目 | 値 |
| :--- | :--- |
| ファイル | `05-spring-cqrs-es-axon.md`（draft-1 から変えない） |
| 主題 | イベントを正典にする — CQRS / Event Sourcing 版の Cargo Tracker |
| 参照元 | `source/java-4/`（8 サービス + 共有ライブラリ + テスト専用 2・Axon Server・Database per Service） |
| 中心の一次資料 | ADR-0001「CQRS / Event Sourcing を Axon Framework 5 でマイクロサービスとして実装する」（決定 6・スパイク結果 12 項）、ADR-0010「調整役を Reaction Handler に一本化する」 |

**サービスの分割を第 4 章と揃えた実装である**ことを、章の冒頭で明示します。これは偶然ではなく ADR-0001 決定 1 の意図であり、**差分が永続化と読み書きの分離だけになる**ようにしてあります。第 4 章と同じく「続きではなく別実装」ですが、**比較可能であるように作られた別実装**である点だけが前章と違います。

**この章は draft-1 で構想だったものです（P1 の当事者）。** 実装から書き直して何が変わったかを、章の末尾（まとめの最終項）で明示します。構想版が書けたのは構成要素の一覧までで、本章の中心（タグの付け忘れ・単体テストの限界・復元演習の落とし穴・検査の書き方）はどれも出てきませんでした。**P1 の再発防止が何を守ったのかを、読者が確かめられる形にします。**

### 第 6 章 — 着手条件が満たされました

保留の理由は参照元ではなく**書き方**にありました。着手条件は「比較の尺度を先に定めること（配置・境界を直す費用・検知の手段の 3 軸を候補とする）」です。

**候補の 3 軸をそのまま採用**して章を起こしました。加えて、**軸にしなかったもの**（性能・チーム規模・運用実績・学習コスト）を理由つきで章に明示します。3 本とも測っていないものを軸にすると、比較ではなく一般論になるためです。

| 項目 | 値 |
| :--- | :--- |
| ファイル | `06-conclusion.md`（draft-1 から変えない） |
| 主題 | 3 つの実装を同じ尺度で比べる |
| 参照元 | 3 本すべて（`source/java-2/`・`source/java-3/`・`source/java-4/`） |
| 尺度 | 軸 1 何を分けたか／軸 2 境界を直す費用／軸 3 守りが壊れたとき何が赤くなるか |

**軸ごとに段差の位置が違う**ことが、この章で分かった主な事実です。軸 1 の段差は「プロセスを分けたか」の 1 か所だけで、イベントソーシングはこの軸にほとんど現れません。軸 2 と軸 3 は 3 段階あります。**1 つの表に全項目を潰すとこの違いが消える**ので、軸ごとに表を分けています。

**「どれが良いか」は書きません。** 書けるのは「何が違うか」と「どの順序で問うか」までです。同じ業務を 3 通りに作れたという事実自体が、単一の正解が無いことを示しています。

### この比較の限界を同じ章に置きます

3 本は**制御実験ではありません**。別の時期に同じ作り手が作っており、方式の差と学習の差を分離できません。本番運用の記録も 3 本ともゼロです。**§10 の完了条件（参照元に存在しないものを実在するように書かない）を、比較の章では「測っていないものを比較の結論にしない」として適用します。**

ただし軸 3 の中心（`@EventTag` の付け忘れ・集約の単体テストで判別できないこと）は Axon のスパイク結果として ADR に記録されており、学習の差では説明できません。**分離できる部分とできない部分を、章の中で分けて書きます。**
...
**この章が扱わないこと**：Kafka との比較（実装に無い）、Saga のオーケストレーション（実装に無い）、CQRS の投影（第 5 章が扱う）。

### 第 5 章：イベントを正典にする — CQRS / Event Sourcing 版の Cargo Tracker

**この章のゴール**：集約の保存をイベント列に替えたとき**何が判断の材料でなくなるか**を説明できること。**守りが丸ごと外れてなおテストが緑になる**種類の欠陥を挙げ、それを静的な検査に落とす方法を辿れること。

**参照元は `source/java-4`（`java/take-8`）です。** 第 1〜4 章と違うことを冒頭で明示します。**第 4 章とサービス分割が揃っているのは意図**（ADR-0001 決定 1）であることも同時に書きます。

| 節 | 内容 | 主な引用元 |
| :--- | :--- | :--- |
| 参照元がまた変わります | 3 実装の対照表。**比較可能であるように作られた別実装**であること。draft-1 で構想だった章を実装から書き直すこと | ADR-0001 決定 1 |
| サービスの分割は前章とほぼ同じ | `settings.gradle.kts`。業務サービスの顔ぶれは第 4 章と同じで、増えたのはテスト専用の 2 つ。**数え方までコメントに書いてある**こと。版が `connector` の公開状況で決まっていること | `settings.gradle.kts`、`gradle/libs.versions.toml` |
| 何が入れ替わったか | 集約は状態を保存しない（`@EventSourced`・`@EventSourcingHandler`）。作成系も static にしない理由。イベントが永続化フォーマットになること（素の型・追記専用・パッケージ移動が無料でない）。読み取りが別モデルになること | `Cargo.java`、`CargoBookedEvent.java`、`CargoProjection.java`、`architecture_backend.md` |
| 緑のまま守られていなかったもの | 3 件。`@EventTag` の付け忘れ・集約の単体テストでは判別できないこと・復元演習が件数一致のまま失敗すること。**第 4 章の 3 件（検査が本番と違う条件で回っていた）とは種類が違う**——ここでは検査する手段が原理的に無かった | ADR-0001 決定 5 の第 7・8・11 項 |
| 守りを検査に落とす | `EventTagAccompaniesEventSourcedTest`（**正しい形のものだけを探さない**・**空振りしていないことを確かめる**）、`EventSourcedServicesHaveTheSameShapeTest`、`AdrHasChecksTest`（決定の数だけ検査を持つ・許可リストは減る方向にしか動かさない）、`ReplayCheckAccompaniesReactionTest`（**ADR の記述が実態と一致していることまで赤にする**） | 各テストと `application.yml` |
| 設計文書が実装より古かった箇所 | 3 件。`@Saga` を前提にした設計（**コンパイラが止める**種類）、「ArchUnit で固定する」と書いた検査が実在しなかった（**止まらない**種類・本数は 4 本ずれていた）、補償の宛先が実装側で直された（**設計が誤りで実装が正しい**向き） | ADR-0010、`ContractEventRosterTest`、`BookingReactionHandler` |
| Saga が無いということ | `SagaIsStillAbsentTest` が**再評価の発動条件を赤にする**こと。Reaction Handler + `process_state` への置き換え表。**代替品としてではなく、上回らない限り移らないものとして**書かれていること | ADR-0001 決定 6、ADR-0010 決定 1 |
| 投影とリプレイ | 投影はコマンドを送らない。**ArchUnit では代われない**（動的な保証は別物）こと。`@SequencingPolicy` の切り方が退避の被害範囲を決めること | `ReplayCheckAccompaniesReactionTest`、`CargoProjection` |
| Axon Server が引き受けないもの | 繋がらなくても起動が成功すること。`TokenStore` が自動設定されないこと。**判定をログの検出で行わない**理由 | `AxonServerStartupCheck`、`AxonJdbcConfiguration` |
| 同期の越境は Query Bus になる | ACL の責務は変わらず、**落ちているときの見え方**が変わること（空リストを返さない／相手が居ないことが明示的に返る） | `QueryBusRouteCandidateFinder` |
| この実装にまだ無いもの | Saga・Deadline（**フレームワークの側に無い**）、Event Store の汎用復元（成立しない）、authms は ES にしない判断、GA の connector が未公開（**採用したフレームワークが未完成**） | ADR-0001 決定 2・5 |
| 前章までとの対比 | 7 項目の対照表。**下 2 行（守りが壊れたことの検知・ArchUnit で足りるか）が新しく現れたもの** | 3 実装 |

**トレードオフ節で扱うこと**：得たもの（経緯が残る・読み取りモデルを何枚でも作れる・配送経路が 1 種類）と、払った代金（イベントが契約になる・結果整合が業務判断に入り込む・**検知の手段を自分で作る必要がある**）。**採用しなかった選択肢**として、`Quotation` と `Voyage` を ES にしたのが記事の比較のためであること、そして**降りる条件が数値で置かれている**ことを扱う。

**この章が扱わないこと**：Axon 4 の API での書き方（5 系に存在しない）、Kafka との比較（実装に無い）、3 実装の比較（第 6 章）。

### 第 6 章：3 つの実装を同じ尺度で比べる

**この章のゴール**：3 つの方式の差を**印象ではなく実在する成果物**（ビルド構成・契約・検査）で言えること。方式を選ぶとき**どの順序で何を問えばよいか**を説明できること。この比較が**何を確かめていないか**を言えること。

**参照元は 3 本すべてです。** 各章が個別の言葉で書いた代金を、同じ尺度に載せ直すのがこの章の仕事です。

| 節 | 内容 | 主な引用元 |
| :--- | :--- | :--- |
| 尺度を先に決めます | 3 軸とその決め方。**軸にしなかったもの**（性能・チーム規模・運用実績・学習コスト）を理由つきで置く。**3 本とも測っていない**ものは軸にしない（P1 と同じ規律） | — |
| 軸 1：何を分けたか | `settings.gradle` の対比（java-2 は 1 行・java-4 は 11 include）。6 項目の対照表。**段差は「プロセスを分けたか」の 1 か所だけ**で、イベントソーシングはこの軸にほとんど現れないこと | `java-2/.../settings.gradle`、`java-4/.../settings.gradle.kts` |
| 軸 2：境界を直す費用 | 3 段階。コンパイラが止める → **受け手が読んだあと取り消せない**（ADR-022 決定 3）→ **過去の自分も読み手になる**（Upcaster・型名がメタデータに載る）。**間違った境界の寿命が方式で決まる**こと | `java-3/docs/adr/022`、`java-4/.../architecture_backend.md` |
| 軸 3：守りが壊れたとき何が赤くなるか | 3 段階。静的な依存で足りる → **送り手はエラーにならないまま届かない** → **集約の単体テストでは判別できない**。「検査が増える」だけでなく**いちばん基本的な検査が判別力を失う**こと。ArchUnit が届かない範囲が名指しされていること | `PackageStructureTest`、`EventRoundTripTestBase`、`EventTagAccompaniesEventSourcedTest`、`ReplayCheckAccompaniesReactionTest` |
| 3 本に共通して現れたもの | 方式に依存しない 4 つの規律（空振りする検査を作らない・規則が赤を出せることを確かめる・決定を文章のまま残さない・無いものを無いと書く）。**同じ規律が 3 本に別々の言葉で現れている**こと。M3 は方式に関係なく起き、**向きは両方ある**こと（設計が先／実装が先）。いちばん見つけにくいのは**「検査で守る」と書いてその検査が無かった**形 | 3 本 |
| 選ぶときの順序 | 4 段の問い。境界が安定しているか → 分ける理由があるか → 経緯を業務が読むか → **軸 3 の検査を引き受けられるか**。**4 を先に問わない**こと | — |
| この比較が確かめていないこと | 5 項目（同時期でない・完成度が違う・同じ人が作っている・本番運用が無い・書籍の 4 アプローチのうち 2 つを扱っていない）。**方式の差と学習の差を分離できない**こと、ただし軸 3 の中心はスパイク結果として記録されており分離できること | `source/README.md`、ADR-0001 |

**トレードオフ節で扱うこと**：軸を 3 つに絞って失ったもの（性能・チーム規模・学習コストは実務では選択を左右する）。**「どれが良いか」を書かなかった**こと。**採用しなかった書き方**として、1 つの表に全項目を並べる案を退けた理由（軸ごとに段差の位置が違う）。

**この章が扱わないこと**：どの方式を推奨するか、性能比較、Jakarta EE / MicroProfile 版（参照元に無い）。
...
| 第 5 章 | **使わない。** java-4 の設計図は節ごとに分かれており、章の主題（検査と食い違い）に対応する 1 枚が無い。対比は表で示す |
| 第 6 章 | **使わない。** 3 本にまたがる図は参照元のどこにも無い。**記事のために描き起こさない**という §7 の方針に従い、軸ごとの表で示す |
...
| 第 4 章 | `source/java-3/apps/backend/`、`source/java-3/docs/adr/022` ほか |
| 第 5 章 | `source/java-4/apps/cargo-tracker/backend/{bookingms,shared}/`、`source/java-4/docs/adr/cargo-tracker/{0001,0010}`、`source/java-4/docs/design/cargo-tracker/architecture_backend.md` |
| 第 6 章 | 3 本すべて。`java-2/.../{settings.gradle,PackageStructureTest}`、`java-3/.../{EventRoundTripTestBase,docs/adr/022}`、`java-4/.../{settings.gradle.kts,EventTagAccompaniesEventSourcedTest,ReplayCheckAccompaniesReactionTest,architecture_backend.md}` |

**参照元は 3 本とも収録済みです。** `source/java-2/` は既存 4 シリーズのために、`source/java-3/` は第 4 章のために、`source/java-4/` は第 5 章のために収録しました。**追加収録は不要**です。
...
| 5 | 第 4 章 | `source/java-3` の収録後。第 3 章と 1 行も重ねない |
| 6 | 第 5 章 | `source/java-4` の収録後。第 4 章との差分が永続化と読み書きの分離だけになる |
| 7 | 第 6 章 | 3 本が揃ってから。**尺度を先に決めてから書く**（決めずに書くと印象の比較になる） |
| 8 | `docs/article/index.md`・`mkdocs.yml`・`source/README.md` への登録 | 章が揃ってから一括（P10） |

**第 3 章から書きます。** draft-1 の失敗は概念先行で章立てを決めたことに起因するため、順序を反転させます。

## 10. 完了条件

各章について、次をすべて満たしたときに完了とします。

- [ ] 引用したコードが参照元（第 1〜3 章は `source/java-2/`、第 4 章は `source/java-3/`、第 5 章は `source/java-4/`）の実ファイルと**一字一句一致**している
- [ ] `{ ... }` による本体省略が 0 件
...
| 参照元実装 `source/java-4/` | **完了**（第 5 章のために収録） |
| **第 6 章** `06-conclusion.md` | **完了。** §10 の完了条件を確認済み（コードブロック 6 箇所を実ファイルと機械照合・引用 2 箇所とも無改変・相対リンク 11 本の解決・`{ ... }` 省略 0 件・Markdown Lint は MD013 のみ） |
| **第 5 章** `05-spring-cqrs-es-axon.md` | **完了。** §10 の完了条件を確認済み（コードブロック 21 箇所を実ファイルと機械照合・引用 17 箇所のうち 16 は無改変／1 箇所の括弧落ちを修正・相対リンク 21 本の解決・`{ ... }` 省略 0 件・Markdown Lint は MD013 のみ） |
...
| Q2 | 章数 | **初回は 3 章まで**。第 4 章以降は参照元ソースの追加待ちとして §5 に保留を明記する。**書けないものを構想で埋めない**（P1 の再発防止）。→ **2026-09-02 に第 4 章が成立**、**2026-09-16 に第 5 章が成立**（§5）。`java/take-7` を `source/java-3/`、`java/take-8` を `source/java-4/` に収録し、それぞれ着手条件を満たした |
| Q12 | 比較の尺度をどう決めるか | **配置・境界を直す費用・検知の手段の 3 軸。** 性能・チーム規模・運用実績・学習コストは軸にしない——**3 本とも測っておらず、参照元から読めない**。一般論で補うと、3 本を読んだ結果ではなくなる（P1 と同じ規律）。**軸ごとに表を分ける**（1 つの表に潰すと、軸 1 の段差が 1 か所で軸 2・3 が 3 段階であることが消える） |
| Q13 | 比較の章で推奨を書くか | **書かない。** 書けるのは「何が違うか」と「どの順序で問うか」まで。3 本は制御実験ではなく（別の時期・同じ作り手・本番運用なし）、**方式の差と学習の差を分離できない**。限界は隠さず同じ章に置く |
...
| Q5 | Axon / Event Sourcing | **扱わない。** 参照元に `org.axonframework` は存在しない。draft-1 第 5 章の構想記述は draft-2 に引き継がない。→ **2026-09-16 に第 5 章が成立**（§5）。`java/take-8` を `source/java-4/` に収録して着手条件を満たした。**構想記述を引き継がない判断は変えていない**——draft-1 の記述は 1 行も使わず、実装から書き直した |
