...
| **今回の章数** | **5**（第 1〜5 章） |
| 保留 | 第 6 章（総括）。**第 5 章の成立で着手条件を満たした**（§5） |
...
**しません。** 第 5 章の末尾も総括ではなく、**次章で何を比べるか**で閉じます。読者に対しては「実装アプローチの比較は続編で扱う」と明示し、現時点で書けないことを書けるふりをしません（P1 の再発防止）。

## 5. 第 4 章・第 5 章（成立）と第 6 章（保留）
...
| 章 | 主題 | 保留の理由 | 着手条件 |
| :--- | :--- | :--- | :--- |
| 第 6 章 | 実装アプローチの比較と選択指針 | **比較軸は 3 つ揃った**（モジュラーモノリス／マイクロサービス／CQRS・ES）。保留の理由は参照元ではなく**書き方**に移った。3 実装を並べる章は、各章が個別に述べた代金を**同じ尺度に載せ直す**必要があり、尺度そのものを先に決めないと「印象の比較」になる | 比較の尺度を先に定めること（配置・境界を直す費用・検知の手段の 3 軸を候補とする） |

**参照元の追加は不要になりました。** [`source/java-4/`](../../source/java-4) の収録で、書籍が並べる 4 アプローチのうち本シリーズが扱う 3 つが揃っています。
...
| 7 | `docs/article/index.md`・`mkdocs.yml`・`source/README.md` への登録 | 章が揃ってから一括（P10） |
