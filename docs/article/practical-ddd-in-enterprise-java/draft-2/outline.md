...
| 参照元実装 | `docs/article/source/java-2/apps/cargo-tracker/`（`java/take-6`。単一 Gradle モジュール） |
| 参照元一次資料 | `docs/article/source/java-2/docs/`（`design/` 11 本・`adr/` 25 本ほか） |
| 題材 | 国際貨物輸送管理システム（Cargo Tracker） |
| **今回の章数** | **3**（第 1〜3 章） |
| 保留 | 第 4 章以降（EDA・CQRS/ES・総括）。**参照元ソースの追加待ち**（§5） |
...
| Axon Framework / Event Sourcing | **参照元実装に存在しない**（§2 の P1） |
...
**しません。** 第 3 章の末尾は総括ではなく、**次に何が要るか**で閉じます。読者に対しては「実装アプローチの比較は続編で扱う」と明示し、現時点で書けないことを書けるふりをしません（P1 の再発防止）。
...
| 章 | 主題 | 保留の理由 | 着手条件 |
| :--- | :--- | :--- | :--- |
| 第 4 章 | EDA としての Cargo Tracker | 参照元の BC 間連携は**同一 JVM 内の `ApplicationEventPublisher`** であり、プロセスを跨ぐイベント基盤が無い（`kafka` / `rabbit` / `jms` / `activemq` / `cloud.stream` はいずれも 0 件）。draft-1 は第 3 章と同じコードを別の見出しで再説明することで章を成立させていた（P2） | メッセージングを使う実装が `docs/article/source/` に収録されること |
| 第 5 章 | CQRS / Event Sourcing | コマンドとクエリの分離は実在するが（`commandservices` / `queryservices`・ADR-022）、**イベントストアも投影テーブルも無い**（`CREATE VIEW` 0 件・`outbox` 0 件・`@Async` 0 件）。集約は MyBatis で現在状態を直接 UPDATE する | Event Sourcing 実装が `docs/article/source/` に収録されること |
| 第 6 章 | 実装アプローチの比較と選択指針 | 比較対象が 1 つしか無い状態では、比較表は**書けることではなく書きたいこと**になる | 第 4 章・第 5 章のいずれかが成立すること |
...
| Q2 | 章数 | **今回は 3 章まで**。第 4 章以降は参照元ソースの追加待ちとして §5 に保留を明記する。**書けないものを構想で埋めない**（P1 の再発防止） |
...
| Q6 | EDA を独立章にするか | **しない。** 参照元の BC 間連携は同一 JVM 内の `ApplicationEventPublisher` であり、モジュラーモノリスの章で扱いきれる。独立章にすると draft-1 の P2（同じコードの二度説明）を再現する |
