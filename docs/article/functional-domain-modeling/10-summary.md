...
同じ 8 つの関心事に対して、「型」「テスト」「運用・レビュー」がそれぞれどれだけ効いたかを重ねます。

```mermaid
radar-beta
  axis v["値の妥当性"], c["条件付き必須"], s["状態と付随データ"], t["状態遷移の網羅"]
  axis e["エラー処理の記述漏れ"], p["永続化の往復"], x["外部システムとの契約"], w["起動主体の配線"]
  curve types["型が守った"]{5, 5, 5, 4, 5, 1, 2, 0}
  curve tests["テストが守った"]{2, 1, 1, 3, 1, 4, 5, 3}
  curve ops["運用・レビューが守った"]{0, 0, 0, 0, 0, 3, 3, 5}
  max 5
  min 0
```
...
| 状態と付随データ | 5 | 1 | 0 | `BookingState`・`PaymentState`・`ExceptionResolution` が値を内包 |
| 状態遷移の網羅 | 4 | 3 | 0 | 網羅性検査が漏れを検出。ただし「許可された遷移が正しいか」はテストが要る |
| エラー処理の記述漏れ | 5 | 1 | 0 | `Result` を無視すると型が合わない |
| 永続化の往復 | 1 | 4 | 3 | キャンセル理由が復元で失われる。統合テストとコメントで補った |
| 外部システムとの契約 | 2 | 5 | 3 | ポート型は書けるが、契約は成功・4xx・5xx の 3 系統テストで固定（ADR-0014） |
| 起動主体の配線 | 0 | 3 | 5 | 期限超過チェックを誰が呼ぶか。IT7 で受入残として記録し IT8 で消化 |
