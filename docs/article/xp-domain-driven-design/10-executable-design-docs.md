...
ADR を起票し、Javadoc に意図を書き、遷移表を作る — そのどれもが「守られている」ことを意味しません。決定的な実例が第 8 章で触れた ADR-009 です。

> **「ADR に書いた」と「守られている」の間に何も無かった。**
>
> — `retrospective-15.md` P1

7 イテレーションのあいだ、ADR-009 は半分だけ守られながら違反を増やし続けていました。**指摘は 1 本、検査を書いたら 6 本。**

## どう実践したか

### ADR に「何がどこで守るか」を持たせる

対処は ADR のフォーマット変更でした。すべての ADR に節を置き、**規則ごとに 3 列**で書きます。
...
**「中身が正しいかは見ない」と書くことが、この形式の価値です。** 検査の存在が「完全に守られている」という誤解を生まないようにしています。
