...
JIG の設定は `modelPattern = '.+'` で全パッケージを対象にしています（生成物はビルド成果物のため記事には収録していません。[ドキュメントポータル](index.md#稼働中のポータル)で閲覧できます）。理由は「BC ごとの構造を見たい」からで、**BC 分割が実装に現れているかを確認する道具**としても使われています。
