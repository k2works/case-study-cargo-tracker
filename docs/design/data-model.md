...
ドメインモデル分析で識別した 7 つの境界付けられたコンテキスト（Booking / Shipper / Routing / Tracking / Handling / Billing / Estimation）と共有カーネル（Shared Domain）に対応する 16 テーブルを設計する。
これに加え、ドメインの境界付けられたコンテキストではなく横断的な認証基盤（Security）として位置づける `users` / `user_roles` の 2 テーブルを別掲する（合計 18 テーブル）。

### 設計方針

- **DB**: PostgreSQL 16.x（本番・開発・テストとも共通。テストは Testcontainers で起動）
- **クエリビルダー / データアクセス**: Kysely（型安全 SQL ビルダー）。DB スキーマから型定義を生成し、コンパイル時に SQL の型を検証する
- **マイグレーション**: node-pg-migrate（`NNN_init.js` / `.sql` 形式のバージョン付きマイグレーション）
- **ID 戦略**: サロゲートキー（`BIGSERIAL` / `GENERATED BY DEFAULT AS IDENTITY`）+ 業務キー（`VARCHAR` / `UUID`）の併用
- **命名規則**: スネークケース（PostgreSQL 慣習）。Kysely 側は `CamelCasePlugin` でキャメルケースに変換する
...
package "Security（認証基盤）" #whitesmoke {
  entity "users\n（ユーザー）" as users {
    * id : BIGINT <<PK>>
    --
...
leg }o--|| location : "積込場所"
leg }o--|| location : "荷降場所"
cargo }o--|| location : "出発地"
cargo }o--|| location : "仕向地"
...
貨物の予約・旅程情報を管理する。`cargo` が集約ルートで、`leg` が旅程の各区間を表す。荷主情報は `shipper` テーブルに正規化し、FK 参照とする。経路通知・確定・キャンセル・追跡番号発行の通知送信記録は `notification_record` テーブルに保持する（`booking_id` により `cargo` を参照。IT4 で追加）。
...
note bottom of cargo
  tracking_number は nullable + 部分 UNIQUE（発行済のみ一意、IT4 追加）。
  transport_status / routing_status / booking_amount_* /
  consignee_* 等はテーブル定義の「将来追加予定カラム（IT4+）」を参照
end note
...
note bottom of leg
  voyage_number はコンテキスト間参照のため DB FK を張らない
  （設計判断 5 と整合）
end note

entity "notification_record\n（通知記録）" as notification_record {
  * id : BIGINT <<PK, GENERATED BY DEFAULT AS IDENTITY>>
  --
...
cargo }o--|| shipper : "荷主"
cargo ||--o{ leg : "旅程を持つ"
cargo ||..o{ notification_record : "通知記録（booking_id 参照）"
...
請求書・明細・支払記録を管理する。参考実装には存在しない新規コンテキスト。
...
認証ガード（NestJS の `AuthGuard` / Passport 戦略）が参照するユーザー認証・認可テーブル。
...
>
> **実装状況**: `cargo` は 002 マイグレーションで作成。荷受人（`consignee_*`）は IT2、追跡番号（`tracking_number`）は IT4（004 マイグレーション）、経路状態（`routing_status`。`NOT_ROUTED` デフォルト・経路紐付けで `ROUTED`・LOAD/UNLOAD の場所不一致で `MISROUTED`）は IT5（006 マイグレーション）で追加した。
> 将来フェーズで追加予定のカラム（`transport_status`・`routing_status`・`booking_amount_*` 等）は下表の「将来追加予定」節に記載する。

| カラム名 | データ型 | 制約 | 説明 |
| :--- | :--- | :--- | :--- |
| `id` | `BIGINT` | `PK, NOT NULL` | サロゲートキー（`BIGINT GENERATED BY DEFAULT AS IDENTITY`） |
| `booking_id` | `UUID` | `UK, NOT NULL` | 予約 ID（業務キー） |
| `shipper_id` | `BIGINT` | `FK → shipper.id, NOT NULL` | 荷主 ID（参照先 `shipper.id` は `BIGSERIAL` のため `BIGINT` で一致させる） |
...
| `tracking_number` | `VARCHAR(20)` | | 追跡番号（発行後に設定。nullable + 部分 UNIQUE インデックス `cargo_tracking_number_unique`（`WHERE tracking_number IS NOT NULL`）で発行済のみ一意。IT4 で追加） |
| `created_at` | `TIMESTAMP WITH TIME ZONE` | `NOT NULL, DEFAULT NOW()` | レコード作成日時 |
| `updated_at` | `TIMESTAMP WITH TIME ZONE` | `NOT NULL, DEFAULT NOW()` | レコード更新日時 |
...
| `voyage_number` | `VARCHAR(20)` | `NOT NULL` | 航海番号（コンテキスト間参照のため DB FK は張らず、アプリケーション側で整合を保証。設計判断 5 と整合） |
...
| `load_time` | `TIMESTAMP` | `NOT NULL` | 積込予定日時（IT5 の 005 マイグレーションで NOT NULL 化） |
| `unload_time` | `TIMESTAMP` | `NOT NULL` | 荷降予定日時（IT5 の 005 マイグレーションで NOT NULL 化） |
...
- `leg_different_locations`: 積込場所と荷降場所は異なる
- `leg_unique_sequence`: 同一貨物内の `seq_number` は一意

> **IT4 実装状況（2026-07 実装）**: 004 マイグレーションで `leg` を作成。`cargo_id` は `ON DELETE CASCADE`。`voyage_number` はコンテキスト間参照のため DB FK を張らない（設計判断 5 と整合）。

---

### `notification_record`（通知記録）

経路通知・確定・キャンセル・追跡番号発行の通知送信記録（US12/US13/US14）。`booking_id` により `cargo` を参照するが、参照整合性は書き込み側で保証する（DB FK は張らない）。
...
> **IT5 実装状況（2026-07 実装）**: 007 マイグレーションで `tracking_activity`・`tracking_handling_event` を作成。`tracking_exception_event` は IT6 で作成する。

| カラム名 | データ型 | 制約 | 説明 |
| :--- | :--- | :--- | :--- |
| `id` | `BIGINT` | `PK, NOT NULL` | サロゲートキー（BIGSERIAL） |
| `tracking_number` | `VARCHAR(20)` | `UK, NOT NULL` | 追跡番号（業務キー） |
| `booking_id` | `UUID` | `NOT NULL` | 予約 ID（参照整合性は書き込み側で保証） |
...
| `status_before_exception` | `VARCHAR(30)` | `NOT NULL` | 例外発生前の追跡状態（解決時の復帰先。状態の再導出禁止のため永続化） |
| `resolved_at` | `TIMESTAMP WITH TIME ZONE` | | 解決日時（NULL = 未解決） |
| `resolution_notes` | `TEXT` | | 解決内容メモ |
| `reported_at` | `TIMESTAMP WITH TIME ZONE` | | 対応報告送信日時（NULL = 未報告） |
| `new_estimated_arrival` | `TIMESTAMP WITH TIME ZONE` | | 対応報告での新到着予定日（遅延時・任意） |
| `report_notes` | `TEXT` | | 対応方針・補償方針 |
| `created_at` | `TIMESTAMP WITH TIME ZONE` | `NOT NULL, DEFAULT NOW()` | レコード作成日時 |
| `updated_at` | `TIMESTAMP WITH TIME ZONE` | `NOT NULL, DEFAULT NOW()` | レコード更新日時 |
...
> **IT5 実装状況（2026-07 実装）**: 006 マイグレーションで `handling_activity`・`customs_declaration`・`cargo.routing_status` を作成・追加。
> **IT6 実装状況（2026-07 実装）**: 008 マイグレーションで `consignee_confirmation` を追加（CLAIM 引き渡し証明の永続化・IT5 レビュー tester M3）。
...
| `consignee_confirmation` | `VARCHAR(200)` | | 荷受人確認（署名または確認コード）。CLAIM（引取）時の引き渡し証明として記録（IT6・008 マイグレーション） |
| `created_at` | `TIMESTAMP` | `NOT NULL, DEFAULT NOW()` | レコード作成日時 |
| `updated_at` | `TIMESTAMP` | `NOT NULL, DEFAULT NOW()` | レコード更新日時 |
...
### `invoice_line_item`（請求明細）

| カラム名 | データ型 | 制約 | 説明 |
| :--- | :--- | :--- | :--- |
| `id` | `BIGINT` | `PK, NOT NULL` | サロゲートキー（BIGSERIAL） |
| `invoice_id` | `BIGINT` | `FK → invoice.id, NOT NULL` | 親請求書 ID |
...
認証ガード（NestJS の Passport 戦略）が参照するユーザー認証テーブル。
...
| `role` | `VARCHAR(50)` | `PK, NOT NULL` | ロール名（`ROLE_SHIPPER` / `ROLE_SALES` / `ROLE_ROUTE_DESIGNER` / `ROLE_TRACKER` / `ROLE_HANDLER` / `ROLE_BILLING` の 6 種） |
...
**根拠**: 浮動小数点演算による精度誤差を排除するため、円・セントなど最小通貨単位で整数管理する。複数通貨対応のため通貨コードを常に付随させる。これはドメインモデルの `Money` 値オブジェクトに対応する。アプリケーション側で金額計算を行う場合は `decimal.js` の `Decimal` 型を用い、最終的に最小通貨単位の整数へ変換して永続化する。
...
**根拠**: PostgreSQL `ENUM` 型は値の追加・変更にスキーマ ALTER が必要でマイグレーション時のリスクが高い。`VARCHAR` ならば node-pg-migrate のマイグレーションで CHECK 制約を追加・変更するだけで済む。アプリケーション側では TypeScript のユニオン型（`as const`）で列挙値を表現し、Kysely の型定義と対応させる。
...
**根拠**: DDD の境界付けられたコンテキスト間はイベント連携（NestJS EventEmitter）を前提とする疎結合設計であり、DB 外部キーによる強結合は将来のサービス分割を妨げる。整合性はアプリケーション層で保証する。
...
**根拠**: ドメインモデル分析で識別した `SETTLED`（BookingStatus）と `Invoice` エンティティを実現するために必要。経理担当者のユースケース（請求書生成・支払確認）を支える永続化構造として設計した。
...
## マイグレーション方針（node-pg-migrate）

### ファイル命名規則

```
migrations/
  001_init.sql                # 初期スキーマ全テーブル作成
  002_seed_locations.sql      # 初期 UN/LOCODE マスタデータ
  003_add_xxx.sql             # 機能追加に伴うスキーマ変更
```

node-pg-migrate は `migrations/` 配下のタイムスタンプまたは連番プレフィックス付きファイルを昇順に適用する。SQL マイグレーション（`.sql`）と JavaScript マイグレーション（`.js`、`up` / `down` 関数を export）の両方が利用できる。

#### 実装済みマイグレーション（イテレーション対応）

| ファイル | 対応 IT | 内容 |
| :--- | :--- | :--- |
| `001_init_security_and_shipper.sql` | IT1 | Security Context（`users`・`user_roles`）・`shipper` |
| `002_location_estimation_booking.sql` | IT2 | `location`・`estimate`・`route_candidate`・`cargo`（`consignee_*` 含む） |
| `003_routing_voyage.sql` | IT3 | Routing Context（`voyage`・`carrier_movement`） |
| `004_booking_itinerary_tracking.sql` | IT4 | `leg`・`cargo.tracking_number`・`notification_record` |
| `005_leg_times_not_null.sql` | IT5 | `leg.load_time` / `unload_time` の NOT NULL 化（Try T3） |
| `006_handling_customs_routing_status.sql` | IT5 | `handling_activity`・`customs_declaration`・`cargo.routing_status` |
...
- バージョン番号（連番プレフィックス）は連番とし、番号の欠番を作らない
- 適用済みマイグレーションファイルの編集は禁止（`pgmigrations` テーブルで適用履歴を管理）
- ロールバックは各マイグレーションの `down`（JS）または対の Undo SQL で対応する
- 本番・開発・テストとも PostgreSQL を使用するため（テストは Testcontainers）、PostgreSQL 固有の構文（`BIGSERIAL`・`GENERATED BY DEFAULT AS IDENTITY` 等）をそのまま記述できる
- スキーマ変更後は Kysely の型定義（`kysely-codegen` 等で生成）を再生成し、アプリケーション側の型と同期する

### `001_init.sql` の構成イメージ
...
-- Estimation Context (008_add_estimate.sql)
