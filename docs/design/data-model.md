...
>
> **実装状況**: `cargo` は 002 マイグレーションで作成。荷受人（`consignee_*`）は IT2、追跡番号（`tracking_number`）は IT4（004 マイグレーション）で追加した。
...
| `notification_type` | `VARCHAR(30)` | `NOT NULL` | 通知種別（経路通知・確定・キャンセル・追跡番号発行など） |
