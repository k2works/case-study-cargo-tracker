...
| 有効化時期 | **TS04 で `Components.form` を実装した後**。それまでは検査しても違反 0 件 |
...
> **規約番号について**: テスト戦略 3.3 では規約 6 が「SQL 文字列連結」、規約 7 が `RawUnsafe`、規約 8 が `<form>` である。
> 本ドキュメントでは規約 5 の再定義に伴い規約 6 を新設したため、以降が 1 つずつずれている。
> **本ドキュメントを実装の正典とし、テスト戦略側の番号は IT2 で本ドキュメントへの参照に置き換える。**
