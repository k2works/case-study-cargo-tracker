...
> `retrospective-4.md` の追跡表が「規約 11」としてその規約を指している。
> 番号は本規約（ADR-0011）が先に取った。
> **M1 の規約は規約 12 として IT11 で起票済み**である（下記）。

### 規約 12: BC の SQL に現れるテーブルは自 BC 所有か ACL 配下

**IT4 レビュー M1 由来。7 イテレーション未着手だった**（IT11 で起票）。

規約 4（BC 間の直接参照）は `use` 宣言を見るため、**SQL に降りた結合を
検出できない**（既知の穴 1）。`JdbcCargoRepo` が `shipper` を JOIN しても
モジュール参照は増えず、規約 4 は通ってしまう。
**境界はモジュール名だけでなくデータの触り方にも現れる。**

| 項目 | 内容 |
| :--- | :--- |
| 検出方法 | `FROM` / `JOIN` / `INTO` / `UPDATE` に続く識別子を、BC ごとのテーブル所有表と突き合わせる |
| 所有表の正典 | [データモデル設計](data-model.md)。検査器が持つのは**写し**であり、増減は正典を先に直す |
| 共有テーブル | `location`（UN/LOCODE マスタ）・`users` / `user_roles` / `sessions`（認証基盤）。どの BC からも引いてよい |
| 例外 1 | `**/infrastructure/acl/`: ACL アダプタは相手のテーブルを引くのが役目（ADR-0009）。**そこに置くこと自体が越境の宣言**になる |
| 例外 2 | `shared/infrastructure/db/`: マイグレーションと開発用シード。どの BC のものでもない |
| CI | `npm run arch:lint`（`build / test / arch-lint` ジョブ）から呼ばれる |

**大文字のキーワードに続く識別子だけを見る**。Flix の関数名（`from`・`join`）まで
拾うと誤検出になり、**検査器そのものが信用されなくなる**（IT2 の教訓）。
所有表に載っていない語も無視する——SQL 以外の文字列を拾わないためである。

フィクスチャは**実コードの形の対**にした（IT9 レビュー M2）。
`JdbcCargoRepo` が荷主名で絞り込むために `shipper` を JOIN する形（負例）と、
`location` だけを JOIN する形（正例）で、**違いは 1 行**である。

> **穴を塞ぐのではなく、穴に名前を付けて数えられるようにする**——規約 11 と同じ
> 考え方である。現時点の越境は 2 件（`booking → shipper`・`routing → cargo/leg`）で、
> どちらも `infrastructure/acl/` 配下にあり、ADR-0009 が明示的に許容したものである。

> **既知の限界**（IT11 レビュー L3・IT12 で明記）。次の 2 つは検出できない。
> 塞ぐのではなく**名前を付けて数えられるようにする**のが本規約の考え方であり、
> 名前の無い穴を残さないためにここへ書く。
>
> 1. **キーワードが行末で折り返した SQL**（`"... FROM " + tableName`）。
>    行単位で走査しているため、次の連結行にある表名は見えない
> 2. **大文字表記の表名**（`FROM CARGO`）。小文字の識別子だけを見ている——
>    大文字を含めると Flix の型名（`FROM Cargo` のような散文）を拾い、
>    誤検出で検査そのものが信用されなくなる
>
> どちらも現時点の実コードには該当が無い。SQL の書き方を変えるときは
> ここを思い出すこと。

### 規約 13: 単一 BC へ向かう書き込み ACL アダプタは 3 本まで

[ADR-0014](../adr/ADR-0014-customs-clearance-is-a-handling-record.md) 決定 4 を
**機械が数える形にした**（IT12・IT11 レビュー M15）。

ADR-0011・ADR-0012 は「`composition/acl/` の総数」を再検討条件にしていたが、
**総数は 2 つの異なる事実を潰す**——「連携が増えた」と
「単一の集約がハブ化している」である。IT10 では条件に到達しても気付けなかった。

ADR-0014 で条件を置き換えたが、**散文にしか存在せず、やはり誰も数えていなかった**。
**条件を測れる形にした以上、測る道具まで含めて 1 セットである。**

| 項目 | 内容 |
| :--- | :--- |
| 検出方法 | `composition/acl/` の各アダプタが宣言する `pub def aclTarget(): String = "<BC>:<read\|write>"` を集計し、同一 BC への `write` が 3 本を超えたら違反 |
| 宣言が無い場合 | **違反とする**。宣言が無ければ数えられない |
| 読みを数えない理由 | 相手の不変条件を壊さないため危険の質が違う |
| 推測しない理由 | ファイル名から向きを推測すると**命名を変えるだけで回避できる**（規約 11 が IT9 で同じ理由で書き直された） |
| CI | `npm run arch:lint` |

現時点の値は **Booking へ 3 本**（旅程の割り当て・追跡番号の記録・経路逸脱の報告）、
Tracking へ 1 本、読み 2 本である。**条件ちょうどで、超えていない。**

4 本目を足すと**実際に赤くなることを実測した**（IT12）。
...
| 2026-08-05 | IT12: **規約 13 を追加**（ADR-0014 決定 4 を機械が数える形にした）。規約 12 の既知の限界を明記 |
| 2026-08-05 | IT11: **規約 12 を追加**（IT4 レビュー M1 由来。7 イテレーション未着手だった）。既知の穴 1（SQL に降りた結合）を数えられる形にした |
