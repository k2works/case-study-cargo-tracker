...
| `customs_declaration` | `CustomsDeclarationRegisteredEvent`, `CustomsStatusUpdatedEvent` | `INDEX(tracking_number, status)`, `INDEX(status)`, **`UNIQUE(tracking_number) WHERE status IN ('PENDING','HELD')`** | 現在状態のみ。**`held_business_days` は持たない**（IT13 の V008 で落とした。IT12 レビューで「列は常に 0」と判明——確定値を持つのは契約イベント `CustomsStatusChangedEvent` だが、投影は内部イベントだけを読むので写す相手がいない）。**営業日数は留置中も決着後も読むときに数える**（`CustomsQueryHandler`。留置中は今日まで、決着後は `last_status_changed_at` まで）。並べ替えも読むときに行うので、留置日数の索引も置かない。**不変条件 3（未決着は貨物ごとに高々 1 件）は部分ユニークインデックスでも守る**——登録は投影を読んでからコマンドを送るので、画面側の確認だけでは同時の 2 件が通る（IT12 レビュー #L15）。決着した申告（`CLEARED` / `REJECTED`）は数えないので、不可のあとに出し直せる。投影の `INSERT` は**衝突先を書かない** `ON CONFLICT DO NOTHING`——衝突先を主キーに絞ると、リプレイのたびに部分ユニーク側で例外になって退避される。見送った事実は投影が読み直して警告に出す |
...
| テーブル | 元になるイベント | 制約・インデックス | 備考 |
| :--- | :--- | :--- | :--- |
| `invoice` | `InvoiceCalculatedEvent`, `InvoiceAdjustedEvent`, `InvoiceIssuedEvent`, `PaymentRecordedEvent`, `InvoiceVoidedEvent`, `CancellationFeeAppliedEvent` | `UNIQUE(booking_id, void_marker)`, `INDEX(shipper_id)`、**`INDEX(billing_status, due_on)` は IT14 で足す**（`due_on` の書き手が US23 まで居ない） | `void_marker` は有効中 `''`、取り消し時に `invoice_id` を入れる。有効な請求書は予約ごとに 1 通。**`overdue` 列は持たず**、一覧の SQL が `due_on < :today AND billing_status = 'INVOICED'` で判定する。`quoted_amount` / `quoted_currency` は見積時の概算（任意。見積を経ない予約は `NULL`）で、S61 が「見積時の概算 → 請求 → 差額」を出す。`INDEX(shipper_id)` は荷主向け請求書（`FindShipperInvoiceQuery`）の索引を兼ねる |
