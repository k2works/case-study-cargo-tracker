...
| `gulp projection:dead-letters` | 投影が書けずに退避されたイベントの一覧（原因つき）。**`projection:status` でトークンは進んでいるのに反映されない、という形の止まり方はここに出ます**（[ADR-0014](../../adr/cargo-tracker/0014-poison-events-are-parked-not-blocking.md)）。直したら退避先から処理し直します——Event Store 全体のリプレイは要りません |
