...
| `RespondToConditionReviewCommand` | 営業担当者 | `ConditionReviewRespondedEvent` | — | UC08 / US10 §4 の対（IT8） |
...
キャンセル料 = 基本料金 × 状態別料率（+ 輸送中は陸揚げ実費）
  仮受付            0%    （まだ何も手配していない）
  経路提案中        5%    （経路設計者が動いている）
  経路通知済        5%
  予約確定         10%    （荷主の承認を得て手配に入る）
  追跡番号発行済   20%    （船腹を押さえている）
  輸送中           50%    （+ 陸揚げ実費。実費は自動では出せないので経理が調整行で入れる）
