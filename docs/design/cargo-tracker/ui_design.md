...
{+
  ...
  {#
    算出 | 請求書 | 予約 | 荷主 | 状態 | 合計 |
    09-28 10:12 | INV-2026-0928-011 | B-2026-0902-004 | 山田商事（法人） | 算出済 | ¥ 983,500 | [開く]
    09-27 16:40 | INV-2026-0927-004 | B-2026-0901-011 | 佐藤 太郎（個人） | 算出済 | ¥ 250,000 | [開く]
  }
}
@endsalt
```
...
| 並び | **既定は算出日時の新しい順**。一覧規約は「支払期限が近い順」と書いているが、**期限が決まるのは発行のとき**（不変条件 3）で、US23（IT14）まで書き手がいない。入っていない列で並べても順序は決まらないので、期限が入る IT14 で並びを切り替える |
| 期限超過 | `due_on < 今日`（業務タイムゾーン）で判定し、当日は超過ではない。**IT14 で入る**（本 IT では期限そのものが無い） |
