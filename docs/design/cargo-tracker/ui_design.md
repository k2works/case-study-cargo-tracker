...
**条件を調整すると、経路設計状態は「設計済」から「設計依頼済み」へ戻ります**（US10 / ADR-0009 決定 3）。条件が変われば、確定済みの経路はその条件で組んだものではなくなるためです。**確定済みの旅程は S22 に残ります**（再設計で入れ替わるまで消しません。消すと「何を組み直すのか」が読めなくなります）。S31 では、戻ったことで「この経路で確定」がもう一度押せるようになります。
