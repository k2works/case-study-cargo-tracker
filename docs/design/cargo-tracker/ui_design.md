...
| S54 | 引取待ち（港起点） | `/handling/awaiting-claim` | 荷役 | UC13（US16） | 目的港で荷降しが済み、引取がまだの貨物。**航海起点（S50）では辿り着けない**——引取は船から降りたあとの作業で、どの航海の仕事でもない。モバイル幅では下部タブ、デスクトップ幅では S02 から開く。行の `[引取を記録]` は `/handling/claim?unLocode=&trackingNumber=`（航海番号を取らない S50）を開く。**通関済・未済の区別と滞留日数は IT10 では出さない**（通関は US29・IT12。画面には「通関の状態はこの画面では分かりません」と明示する） |
...
{+
  <b>通関申告一覧
  {
    追跡番号 | "TRK-AB12CD3456" | 通関状態 | ^すべて^ | [絞り込む]
    [X] 督促の対象だけ表示 | [ ] 通関済も表示
  }
  ---
  {#
    申告番号 | 追跡番号 | 状態 | 申告日時 | 留置営業日数 | 最後の理由 | 操作
    D-2026-0912-77 | TRK-AB12CD3456 | <color:#991B1B><b>留置</b></color> | 09-12 10:00 | <color:#991B1B>4 営業日</color> | 原産地証明の不備 | [開く]
    D-2026-0914-03 | TRK-EF45GH6789 | 審査中 | 09-14 09:00 | — | — | [開く]
  }
