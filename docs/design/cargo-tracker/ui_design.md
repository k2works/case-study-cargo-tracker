...
| S92 | シナリオ一覧・実行 | `/admin/simulations` | 管理者 | UC23（US33・US34） | 業務が端から端まで成立していることを確かめる入口。シナリオを選んで実行すると、予約から精算までを**実際の API で順に流す**（[ADR-0020] 決定 2。専用の書き込み経路は作らない）。各行に「どこまで進んだか」（成功した工程数 / 予定の工程数）を出す——開かないと分からない形にすると、走っているあいだ何も判断できない。**無効な環境では断り**（US33 §4）、**同じシナリオの二重実行も断る**（§5。断りに**実行中の識別子を添える**——「二重に実行できません」だけでは、いまの結果へ行けない） |
