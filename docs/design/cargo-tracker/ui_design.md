  ...
  {* Cargo Tracker | 管理 花子 (システム管理者) | [ログアウト] }
  {
    ...
    } |
    {
      ...
      {#
        ...
        経路候補が見つからない輸送 | 失敗 | 3 / 4 工程 | 09-14 09:40 | 09-14 09:41 | admin01 | [工程を見る]
      }
...
{+
  {* Cargo Tracker | 管理 花子 (システム管理者) | [ログアウト] }
  {
    ...
    } |
    {
      [← 業務シミュレーション]
      実行結果
      シナリオ: 経路候補が見つからない輸送 | 状態: 失敗 | 開始: 09-14 09:40 | 終了: 09-14 09:41
      ..
      {#
...
**この画面はまだ実装していません。** US18（IT8）で「作らない」と判断し、その判断は今も有効です——US18 の受入基準 5 項目はすべて「追跡番号での照会」で、荷主は S44（公開追跡）と S40（追跡一覧）で自社の貨物を追えます。**設計だけ先にあり、実装が追いついていない画面**であることをここに明記します（行だけあって節が無いより、節に「未実装」と書いてあるほうが読む人に届きます）。

**作るときの規約**——荷主には**自社のぶんだけ**を出し、絞るのはサーバです（S40 と同じ）。**金額は出しません**（金額を出す荷主向けの画面は S62 だけ）。既定では精算済・キャンセルを外し（一覧の既定条件）、並びは到着期限が近い順です。行から S46（自社予約の進み具合）へ進みます。
