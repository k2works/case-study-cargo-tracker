...
**1 ファイル 400 行（空行とコメントを除く）を超えたら割る。** `oxlint` の `max-lines` で機械的に止めます。

数値で置くのは、**基準を決めていないと触った順で結果が変わる**ためです。IT6 では同じイテレーションの中で `route-design-page.tsx`（558 行）を割り、`booking-detail-page.tsx` は 610 行まで伸ばしました。

割り方は**責務**です。行数で切ると関係のあるものが分かれます。この画面群で使っている境目は次の 2 つです。

- **手番** — 誰の操作か（営業／経路設計者）。認可も、失敗の意味も、変わる理由も違う
- **入力の単位** — 「区間を足したり消したりする」入力は、属性の入力とは変わる理由が違う

コメントを行数に数えないのは、**コメントは資産**だからです。長いから削るのは本末転倒です。テスト・モックは対象外にしています（確かめる場面の数だけ伸びるものであり、割る基準は対象の分割に追随させます）。
