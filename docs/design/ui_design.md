...
| 荷役作業一覧 | `/handling` | 荷役履歴一覧・検索（追跡番号・貨物 ID の両方で検索可）。**引取の行から訂正・取り消しを申請できる**（US36。取り消し済みは申請できない）。**先頭に荷降し手配**（承認済みキャンセルの陸揚げ地。行から荷役登録へ追跡番号つきで進める。**承認からの待ち日数を出し、7 日を超えたら「滞留」と示す**。US30） | ROLE_HANDLER, ROLE_TRACKER（**追跡管理者は GET のみ**） | US15, US16, US36, US30 |
