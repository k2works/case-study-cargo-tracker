---
title: UI 設計 - 国際貨物輸送管理システム（Go 版）
description: OOUX に基づく画面設計。画面一覧・画面遷移図・ワイヤーフレーム・インタラクション設計を定義する。
published: true
date: 2026-07-10T00:00:00.000Z
tags: design, ui, ux, wireframe, go, chi, html-template, htmx
---
...
本ドキュメントでは、国際貨物輸送管理システム（Go 版）の UI 設計を定義する。
...
| 技術 | 役割 |
| :--- | :--- |
| Go + chi | HTTP ルーティングとハンドラで SSR（サーバーサイドレンダリング）を実現 |
| html/template | Go 標準テンプレートエンジンでフル HTML を生成（`templates/` 配下、フラグメントは `define` ブロック） |
...
各画面は chi ルートに対応するハンドラが `templates/` 配下のテンプレートをレンダリングする。

| 画面名 | URL パス | テンプレート | 説明 | 主要アクター | 対応 US |
| :--- | :--- | :--- | :--- | :--- | :--- |
...
| 荷役作業一覧 | `/handling` | `templates/handling/list.html` | 荷役履歴一覧・検索 | 荷役作業員、追跡管理者 | US12 |
...
全画面共通のナビゲーションバー（Bootstrap 5 `navbar`）を上部に配置する。ロールに応じてメニュー項目を表示制御する。共通レイアウトは `templates/layout.html` の `define "layout"` ブロック、ナビゲーションバーは `define "navbar"` フラグメントとして実装する。
...
### Go html/template のレイアウト構成

Thymeleaf のレイアウト・フラグメント機構に相当する構成を、`define` / `template` / `block` で実現する。
...
各画面テンプレートは `define "content"` で本文を定義し、ハンドラ側で `layout` と組み合わせてレンダリングする。
...
- セッションベース認証（chi ミドルウェア + セキュアクッキー）のログインフォーム。`POST /login` をハンドラで処理する
...
- htmx: ページ初期ロード時に `/api/dashboard/summary` を `hx-get` で取得（chi ハンドラが HTML フラグメントを返す）
...
- **htmx**: 検索フォームに `hx-get="/bookings" hx-target="#booking-list"` で部分更新（ハンドラは `HX-Request` ヘッダを判定し、テーブル部分の `define "booking-list"` フラグメントのみを返す）
...
- **認証**: 不要（認証ミドルウェアの適用外パスとして `/public/*` を chi ルータで公開する）
...
- **反映タイムラグ**: イベント登録後の非同期反映により、ステータス反映に最大 30 秒かかる旨を画面下部に注記する
...
<div id="status-timeline"
     ...
     hx-swap="innerHTML">
  <!-- Go html/template でサーバーレンダリングされた初期コンテンツ -->
</div>
<p id="last-updated">最終更新: <span>{{ .LastUpdated }}</span></p>
```

**サーバー側レスポンス**: `GET /tracking/{trackingNumber}/status` の chi ハンドラは `define "status-timeline"` フラグメントをレンダリングした HTML を返す（`Content-Type: text/html`）。
...
<div id="booking-list">
  <!-- 検索結果テーブル（define "booking-list" フラグメント） -->
</div>
```

`hx-push-url="true"` により、検索条件が URL に反映されブラウザ履歴に残る。ハンドラは `HX-Request` ヘッダの有無で「フラグメントのみ」か「フルページ」かを切り替える。
...
- **Go html/template**: ハンドラがバリデーション結果を `Errors map[string]string` としてテンプレートに渡し、フィールドごとに表示する
...
<div class="mb-3">
  ...
  <input type="text" id="origin" name="origin" value="{{ .Form.Origin }}"
         class="form-control{{ if .Errors.Origin }} is-invalid{{ end }}">
  {{ if .Errors.Origin }}
...
PRG パターンのリダイレクト後に、操作結果をセッション格納のフラッシュメッセージ（alexedwards/scs の Pop 操作による Flash 機構）でフィードバックする。
...
フラッシュメッセージは共通フラグメント `templates/fragments/alerts.html`（`define "alerts"` ブロック）で一元管理する。

#### エラーページ

| HTTP ステータス | テンプレート | 内容 |
| :--- | :--- | :--- |
| 400 Bad Request | `templates/errors/400.html` | 不正なリクエスト。入力を確認してください |
| 403 Forbidden | `templates/errors/403.html` | アクセス権限がありません |
| 404 Not Found | `templates/errors/404.html` | 指定されたページまたはリソースが見つかりません |
| 500 Internal Server Error | `templates/errors/500.html` | サーバーエラーが発生しました。管理者に連絡してください |

chi の `NotFound` / `MethodNotAllowed` ハンドラおよび recover ミドルウェアから上記テンプレートをレンダリングする。各エラーページはナビゲーションバーを表示し、ダッシュボードへ戻るリンクを提供する。
