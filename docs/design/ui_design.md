...
| 貨物予約一覧 | `/booking` | 予約済み貨物の一覧・検索 | 営業担当者（ADR-008 により荷主には開かない。US18 で広げ直す） | US04, US05 |
...
> **take-3 からの引き継ぎ方**: 見積・航海スケジュール・精算の各画面は take-3 の設計を踏襲する
...
- 予約金額は IT2 では表示しない（US18・IT11 まで算出できない。0 円と表示すると未算出と無料の区別がつかなくなる。[ADR-009](../adr/009-cargo-status-columns-from-the-start.md)）
