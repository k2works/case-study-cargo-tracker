...
| 技術 | 役割 |
| :--- | :--- |
| ASP.NET Core MVC + Razor ビュー | SSR（サーバーサイドレンダリング）でフル HTML を生成 |
...
| 予約詳細 | `/bookings/{bookingId}` | 予約情報・経路・荷役履歴 | 荷主、営業担当者 | US06, US12, US13 |
| 経路割り当て | `/bookings/{bookingId}/route` | 利用可能な航路から経路を選択 | 営業担当者 | US07, US08, US09, US11 |
...
| 経路設計依頼一覧 | `/routing/requests` | 経路設計待ちの予約一覧 | 経路設計者 | US07, US11 |
...
{+
  ...
  <b>デモアカウント（開発環境）</b>
  {
    営業担当者（sales）      | 荷主管理・見積・貨物予約
    経路設計者（router）     | 航路管理・経路設計
    追跡管理者（tracker）    | 貨物追跡・荷役管理
    荷役作業員（handler）    | 荷役管理
    経理担当者（billing）    | 請求管理
    管理者（admin）          | 管理設定
  }
  <i>行をクリックするとユーザー名がプリフィルされる（本番では非表示）</i>
}
@endsalt
```
...
- ASP.NET Core の Cookie 認証（ASP.NET Core Identity）のログイン画面をカスタマイズ
- ログイン失敗時: 「ユーザー名またはパスワードが正しくありません」を赤色で表示
- ログイン成功後: ロールに応じてダッシュボードへリダイレクト
- セッションタイムアウト後は自動的に `/login?timeout` へリダイレクト
- **デモアカウント提示（開発環境のみ）**: ロールごとに利用できる機能が異なり、単一ユーザーでは一部機能しか見えないため、開発環境ではログイン画面に全ロールのデモアカウント（ロール名・ユーザー名・利用可能機能）を一覧提示する。行をクリックするとユーザー名をプリフィルし、ワンクリックでロールを切り替えられる。本番環境では提示しない（セキュリティ）。
...
- **バリデーション**: htmx で `hx-post` 送信前にクライアントサイドチェック、サーバー側は DataAnnotations によるモデルバリデーション
...
{+
  ...
  ==
  {#
    **予約 ID** | **出発地** | **目的地** | **希望期限** | **貨物種別**   | **引き渡し日** | **操作**
    BK-1236     | JPOSA      | USLAX      | 2026-04-25   | GENERAL_CARGO  | 2026-03-30     | [経路設計]
    BK-1237     | JPYOK      | DEHAM      | 2026-05-01   | REFRIGERATED   | 2026-03-31     | [経路設計]
    BK-1238     | JPKIX      | SGSIN      | 2026-04-20   | HAZARDOUS      | 2026-04-01     | [経路設計]
  }
  ==
  < 前へ | 1 / 2 | 次へ >
}
...
- **一覧対象**: 営業担当者から経路設計者に引き渡された予約（ROUTE_PROPOSED 待ち、`assign-routing` 済み）を引き渡し日昇順で表示する
...
- **確定・紐付け**: `[この経路で確定し予約に紐付ける]` で `POST /routing/requests/{bookingId}/confirm` を送信。経路状態が「確定」となり予約に紐付き、BookingStatus が ROUTE_PROPOSED に更新される。PRG パターンで `/routing/requests` へリダイレクト（US11）
- **経路割り当て画面との役割分担**: 営業担当者向けの経路割り当て画面（`/bookings/{bookingId}/route`）は絞り込み済み候補からの単純割り当てを担い、本画面は経路設計者が候補算出・比較・条件調整を行う実務を担う
- **アクセス制御**: ROLE_ROUTE_DESIGNER のみアクセス可能
...
- **認証**: 不要（ASP.NET Core の認可設定で `/public/**` を `[AllowAnonymous]` として許可）
...
```html
<!-- 追跡詳細画面のステータスタイムライン部分（Views/Tracking/Detail.cshtml） -->
<div id="status-timeline"
     hx-get="/tracking/TRK-20260328-1234/status"
     hx-trigger="every 30s"
     hx-swap="innerHTML">
  <!-- Razor でサーバーレンダリングされた初期コンテンツ -->
</div>
<p id="last-updated">最終更新: <span>@Model.LastUpdated</span></p>
```

**サーバー側レスポンス**: `/tracking/{trackingNumber}/status` は HTML フラグメント（パーシャルビュー）を返す（`Content-Type: text/html`）。
...
- **Razor**: `asp-validation-for` タグヘルパーでサーバーバリデーションエラーを表示

```html
<div class="mb-3">
  <label asp-for="Origin" class="form-label">出発地 <span class="text-danger">*</span></label>
  <input asp-for="Origin" class="form-control" />
  <span asp-validation-for="Origin" class="invalid-feedback d-block">出発地は必須です</span>
</div>
```

#### フラッシュメッセージ

PRG パターンのリダイレクト後に、操作結果を TempData でフィードバックします。
...
フラッシュメッセージは共通パーシャルビュー `Views/Shared/_Alerts.cshtml` で一元管理します。
