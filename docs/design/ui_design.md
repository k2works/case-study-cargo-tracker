...
{+
  ...
  --
  {
    ...
    作成日（まで） | "2026-08-31" | 状態 | ^有効^ | [絞り込む] | [条件をクリア]
  }
  ==
  該当 3 件（作成日の新しい順）
  {#
...
- **既定は「有効」で絞る**（U4。IT20）。**期限切れが混ざったままだと、毎朝の確認でどれがまだ使えるのか分からず一覧全体が信用されない**
  - **「すべて」は `ALL` を明示して選ぶ。** 空欄と同じ意味にすると既定の意味が消え、一覧を開くたびに期限切れが混ざる
  - **`[条件をクリア]` はすべてを出す**（既定に戻すのではない）。期限切れを探しに来た人がクリアで見失わないため
- **表示列**: 見積 ID・出発地・目的地・希望期限・貨物種別・重量・最安候補の費用・状態・作成日・操作
- **件数と並び順を出す**（U6。IT20）。「該当 N 件（作成日の新しい順）」。月末に「今月何件出したか」を聞かれる。**書いていない順序は、読む人が毎回推測することになる**
...
- **0 件のときは理由を 4 つに分ける**（U5。IT20）。「条件に一致しません」としか出さないと、**港コードを打ち間違えた人は条件を何度変えても 0 件のまま**になる
  ...
  | 港コードが実在しない | 指定された港コードは港マスタにありません。UN/LOCODE の綴りをご確認ください | `[条件をクリア]` |
  | 作成日の範囲が逆 | 作成日の「から」が「まで」より後になっています | `[条件をクリア]` |
  | 条件に一致しない | 条件に一致する見積はありません。条件を変えてお試しください | `[条件をクリア]` |

  - **「まだ 1 件も無い」が最優先である。** 1 件も無い人に「港コードが実在しません」と言っても直しようがない
