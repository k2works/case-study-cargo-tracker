...
正典 US01〜US27 と上表の差集合。いずれも画面設計が未着手であり、Release スコープ確定後に追加する。

| US | タイトル | 優先度 | 影響 |
| :--- | :--- | :--- | :--- |
| US02 | 荷主を登録する | 高 | US04 の受入基準「荷主 ID を入力して既存荷主を選択できる」を満たせない。**予約フローのブロッカー** |
| US03 | 法人荷主を登録する | 高 | 同上。加えて US22 の契約割引率の入力手段が無い |
| US12 | 確定経路を荷主に通知する | 高 | 通知の送信操作と送信記録が無く「送ったつもり」を検知できない |
| US16 | 引取作業を記録する | 高 | 荷役作業登録フォームに荷受人確認（署名 or 確認コード）のフィールドが無い |
| US17 | 貨物状態を手動更新する | 高 | 出港・入港は荷役作業員が触らないため ROLE_TRACKER が手動更新する画面が必要 |
| US24 | 航海スケジュールを新規登録する | 高 | `/voyages` は閲覧専用。US07・US08 の入力データを作る手段が無い |
| US25 | 既存航海スケジュールを更新する | 高 | 船会社のスケジュール変更を反映できず初日からデータが古くなる |

※ US10（経路条件を調整して再算出）は経路割り当て画面に対応 US として記載があるが、**候補ゼロ時に条件を緩めて再検索する導線**が仕様に無い。画面追加ではなく既存画面の仕様追加として扱う。
