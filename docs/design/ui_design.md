...
| 予約詳細 | `/bookings/{bookingId}` | 予約情報・経路・追跡番号・**引取確認コード**（US35。確定時に採番。**追跡番号とは別の値**で公開追跡には出さない）・荷役履歴・**この貨物の例外**（読み取り専用。C31）・**特別な取り扱い**（危険物申告 / 温度管理条件）。**ROLE_SHIPPER は自社の予約のみ**（他社は 404） | ROLE_SHIPPER, ROLE_SALES, ROLE_ROUTER（GET のみ）, ROLE_TRACKER（GET のみ） | US06, US12, US13, US14, US19, US28, US30, US35 |
...
| 荷役作業一覧 | `/handling` | 荷役履歴一覧・検索（追跡番号・貨物 ID の両方で検索可）。**引取の行から訂正・取り消しを申請できる**（US36。取り消し済みは申請できない）。**先頭に荷降し手配**（承認済みキャンセルの陸揚げ地。行から荷役登録へ追跡番号つきで進める。**承認からの待ち日数を出し、3 日を超えたら警告色・7 日を超えたら「滞留」と示す**。US30） | ROLE_HANDLER, ROLE_TRACKER（**追跡管理者は GET のみ**） | US15, US16, US36, US30 |
...
| US12 | 確定経路を荷主に通知する | 荷主への経路通知（プレビュー）／予約詳細（通知履歴） |
| US13 | 予約を確定する | 予約詳細 |
...
| US25 | 既存航海スケジュールを更新する | 航海スケジュール編集 / 更新確認 ／ **予約詳細（日程が変わった区間の印と、遅延例外の登録への導線）** |
...
**列が 7 つ以上ある一覧は `table-responsive` で包む**（IT17 の R7）。

Bootstrap の表は幅が足りないと列を潰して折り返す。9 列の請求書一覧では**金額が縦に割れ、日付が 2 行になる**。読めるつもりで読めていない状態は空欄よりたちが悪い — **読み違えたことに気づけない**。

**列は隠さない。** 狭い画面で列を落とすと、その場面に当たった人だけが仕事を進められなくなる。どの列が要るかは場面で変わる（経理担当者が請求書を突き合わせるとき、必要な列は毎回同じではない）。横に送ることで、情報を落とさずに読めるようにする。

この規則は `WideTableReadabilityTest` が検査する（対象範囲: `templates/` 配下の `<thead>` の列数。**列を隠す実装は検出しない**）。
...
state "荷役フロー" as handling_flow {
  ...
  荷役作業登録 --> 予定ルート外の確認 : 予定ルート外の積込・荷降し
  予定ルート外の確認 --> 荷役作業一覧 : [承認して登録する]（PRG）
  予定ルート外の確認 --> 荷役作業登録 : [入力に戻る]（入力を保つ）
}
...
state "見積フロー" as estimation_flow {
  ...
  見積一覧 --> 見積詳細 : 行クリック
  見積一覧 --> 見積一覧 : 絞り込み（出発地・目的地・作成日・状態。GET）
  見積作成 --> 見積詳細 : 作成成功（PRG）
  見積作成 --> 見積作成 : バリデーションエラー
  見積作成 --> 見積作成 : 貨物種別の変更（GET。入力を道連れにしない）
  見積詳細 --> 見積作成 : [同じ条件で再見積]（GET。期限切れのとき）
}

ダッシュボード --> 見積一覧 : [見積管理] クリック（US01）
見積詳細 --> 貨物予約登録 : [この見積で予約する]（GET。希望到着期限内のみ）
...
- 送信後は**通知履歴**を予約詳細に常時表示する。項目は 送信日時 / 送信者 / 送信先 / 種別（経路確定・スケジュール変更・例外発生・例外対応・状態更新・通関完了）/ 結果（成功・失敗）
- **各行に「通知内容」を折りたたみで添え、送った文面そのものを読めるようにする**（IT11 / C20）。日時と結果だけでは、荷主と「何と聞いていたか」が食い違ったときに確かめられない
- **表示ロールは ROLE_SALES / ROLE_TRACKER / ROLE_SHIPPER** とする。通知を作るのは営業（経路確定）と追跡管理者（例外）であり、受け取るのは荷主である。**通知は ADR-006 により記録で満たしているため、書いた本人と受け取る当人が読めなければ「通知した」ことにならない**。荷主は自社の予約しか開けない（他社は 404）
- **「送ったつもり」を検知できることが本機能の目的である。** 送信操作だけを実装して履歴を残さないと、荷主から「聞いていない」と言われたときに確認する手段が無い
- 送信失敗時は履歴に失敗として記録し、`[再送]` を表示する。**成功した通知に再送を出さない**（二重に送ったのか確認したのか、履歴から読めなくなる）
- **ADR-006 により外部へは送らない。** 通知の実体は記録であり、`booking_notification` に残す（IT8 で実装）
- **経路が確定していない予約には通知のボタンを出さない。** 送るべき中身が無い通知を「送信済み」として残すと、履歴そのものが信用できなくなる
- **料金概算は載せない。** 概算式（ADR-008）は候補の並べ替え用であり、荷主に見せた瞬間に請求額として読まれる（料金は US21 / Release 2.0）

#### この貨物の例外（US19 / IT12 の C31）

予約詳細に、その貨物に起きた例外を**読み取り専用**で表示する。

- 荷主から「どうなっているのか」と問われるのは営業担当者である。例外が追跡管理者の画面にしか無いと、**確かめに行くまで答えられない**
- 項目は 発生日時 / 種別 / 状況 / 対応（**対応済**は対応内容を併記、未解決はバッジのみ）。並び順は**発生の新しい順**
- **対応済と未解決を見分けられるようにする。** 荷主に説明するとき、片づいた話と現在進行中の話は別である
- **例外が無い貨物には節ごと出さない。** 何も起きていない貨物に見出しが出続けると、見出しそのものが読み飛ばされる
- **操作は置かない。** 解決の登録は追跡管理者の仕事（`/tracking/exceptions/{id}/resolve`）であり、権限の分かれ目を画面ごとにばらけさせない
...
**探索も現在地から行う**（表示だけを差し替えない）。誤配より前に保存した探索条件は、貨物がまだ出発地にあった前提で組み立てられているため土台にしない。

各候補には、**当初の希望期限を何日超えるか**を「希望期限より N 日超過」として出す（US28）。「期限を過ぎます」だけでは 1 日なのか 2 週間なのかが分からず、荷主への説明も代替を探すかの判断もできない。**比べるのは予約の期限であり、延ばして探した期限ではない**。同じ差分は荷主への通知本文にも載せる。
...
**出港・入港は荷役作業員が登録しない。** 追跡管理者（ROLE_TRACKER）が手で入れるための入口。

> **独立した画面ではない**（IT8 で実装）。**追跡詳細（`/tracking/{trackingNumber}`）の中の
> ROLE_TRACKER 専用パネル**であり、`/status` は POST のみを受ける。
> 更新する人は現在の状態を見ながら判断するため、同じ画面に置く。
>
> **入口は「追跡中の貨物」一覧（`/tracking/queue` の下段）である。** 発行待ち一覧は
> 追跡番号を発行した時点でその予約が消えるため、発行後の貨物へ行く道が無かった。
> **追跡番号を覚えている追跡管理者はいない。**
>
> 選べるのは**荷役作業ではない種別**だけである（出港・入港・引取待ち）。
> 受領・積込・荷降し・引取は現場の作業であり、机上で入れてよいものではない。
> **逆行は拒否する** — 戻す必要が生じるのは誤登録の訂正であり、承認を伴う取り消し
> （US36 / IT12）で扱う。
...
- **フィルタ**: 1 つのプルダウンで**2 つの軸**を選ぶ（ADR-017）。料金の状態（`DRAFT` 下書き / `CONFIRMED` 確定）と支払いの状態（`PENDING` 未入金 / `OVERDUE` 支払期限超過）。**`CONFIRMED` は料金の軸として読む** — 「確定」は US21 から料金の意味で使ってきた語である
- **督促対象一覧は本画面の絞り込みである**（`?status=OVERDUE`。IT14）。新しい画面を作らない。ダッシュボードの「支払期限超過」カードの行き先がここである
- **件数と合計**（IT13 レビュー C2）: 表の上に「N 件／合計 X 円」を出す。**いま絞り込んでいる結果の合計**であり、全件の合計ではない。総勘定元帳との突き合わせに使う
- **発行日・支払期限**（IT13 レビュー C1 / US23）: 列として出す。**未発行の請求書は「—」**と出す（空欄だと読み込めていないのか未発行なのか分からない）
- **ステータスバッジ**: 料金の状態と支払いの状態を**別のバッジ**で出す。正典は `ChargeStatus` / `PaymentStatus` であり、画面で色を決め直さない。**未発行の請求書に支払いのバッジは出さない**（まだ支払いの話が始まっていない）
- **アクセス制御**: ROLE_BILLING のみアクセス可能
- **請求書の発行は経理担当者の操作である**（US23。IT14 で実装）。**確定済みの請求書だけが発行の入口を持つ** — 承認前の金額で請求書を出すと、荷主に伝えた後で金額が変わる。旧版は「配送完了で自動発行」と書いていたが、US21 が定める「算出 → 確認 → 確定」の後に置く形に改めた
- **空状態**: 0 件のとき「該当する請求書はありません。請求対象から料金を算出してください」と表示し、`/billing/pending` へのリンクを添える
...
- **金額内訳**: 基本料金 → 料金調整（減額・補償費用）→ 割引 → 消費税 → 請求総額の順に出す。**上から順に足し引きすると請求総額に着地する**（`domain-model.md`「金額の丸め規則」）
- **この貨物の例外**（IT13 レビュー C3）: 種別・発生日時・状況・対応状況を読み取り専用で出す。**減額の金額はここに出ている事実をもとに決める**。例外が 1 件も無ければ**見出しごと出さない**（空の枠は「読み込み中」に見える）
- **[精算書を発行]**: `POST /billing/invoices/{invoiceNumber}/issuance`。PRG。**確定済みかつ未発行のときだけ出す**。発行すると支払期限が決まり、荷主への通知が記録される（ADR-006 により外部へは送らない）
- **[入金を確認]**: `POST /billing/invoices/{invoiceNumber}/payment`（入金額・支払方法・取引の参照番号）。PRG。**発行済みかつ未入金のときだけ出す**。**期限超過でも出す** — 遅れても入金は入金である。**請求額どおりの入金だけを受け付ける**（ADR-018）
- **確認済み**: PaymentStatus が `CONFIRMED` の場合は入金フォームを出さず、「入金確認済・予約は精算済」と表示する
- **確認ダイアログ**: 発行・入金確認・料金の確定に付ける（取り消せない操作である）
...
@startsalt
{+
  {/ <b>CargoTracker</b> | 貨物予約 | <b>見積管理</b> | 貨物追跡 | [ログアウト] }
  ==
  <b>見積一覧</b> | [+ 新規見積作成]
  --
  {
    出発地 | "JPOSA" | 目的地 | "USLAX" | 作成日（から） | "2026-08-01"
    作成日（まで） | "2026-08-31" | 状態 | ^有効^ | [絞り込む] | [条件をクリア（すべて表示）]
  }
  ==
  該当 3 件（作成日の新しい順）
  {#
    **見積番号** | **出発地** | **目的地** | **希望期限** | **貨物種別** | **重量** | **最安候補** | **状態** | **作成日** | **操作**
    EST-0003 | JPOSA | USLAX | 2026-10-01 | 一般貨物 | 1,000 kg | 120,000 円 | 有効     | 2026-08-12 |
    EST-0002 | JPYOK | DEHAM | 2026-09-20 | 冷凍貨物 |   800 kg | 210,000 円 | 有効     | 2026-08-11 |
    EST-0001 | JPOSA | NLRTM | 2026-08-01 | 一般貨物 | 1,500 kg | —          | 期限切れ | 2026-08-01 | [同じ条件で再見積]
  }
}
@endsalt
```
...
- **検索**: 出発地・目的地・作成日範囲・状態（`CREATED` / `EXPIRED` / `ALL`）で絞り込む
- **既定は「有効」で絞る**（U4。IT20）。**期限切れが混ざったままだと、毎朝の確認でどれがまだ使えるのか分からず一覧全体が信用されない**
  - **「すべて」は `ALL` を明示して選ぶ。** 空欄と同じ意味にすると既定の意味が消え、一覧を開くたびに期限切れが混ざる。**`ALL` は絞り込みの規則に明示的な分岐を持つ**（「知らない値では絞らない」フォールバックに任せると、`?status=FOO` と区別が付かない）
  - **選択肢は「有効・期限切れ・すべて」の順**。既定が先頭にないと、開いた直後の選択位置と一覧の並びが噛み合わない
  - **`[条件をクリア]` はすべてを出す**（既定に戻すのではない）。期限切れを探しに来た人がクリアで見失わないため
- **表示列**: 見積 ID・出発地・目的地・希望期限・貨物種別・重量・最安候補の費用・状態・作成日・操作
- **件数と並び順を出す**（U6。IT20）。「該当 N 件（**有効のみ**・作成日の新しい順）」 —— **何で絞った結果かまで書く**。書かないと、月末に今月分を数える人が**今月出して期限切れになった見積を数え落とす**。月末に「今月何件出したか」を聞かれる。**書いていない順序は、読む人が毎回推測することになる**
- **期限切れの行から直接作り直せる**（U8。IT20）。`[同じ条件で再見積]` は**期限切れの行にだけ出す**。1 件ずつ詳細を開いて戻ると 10 件で 20 回の遷移になる
  - **危険物の申告は運ばない。** 一覧は申告の中身を持っていない。作成フォームが必須で拒むため、黙って落ちることはない
- `[+ 新規見積作成]` から見積作成へ遷移する
- **0 件のときは理由を 4 つに分ける**（U5。IT20）。「条件に一致しません」としか出さないと、**港コードを打ち間違えた人は条件を何度変えても 0 件のまま**になる

  | 理由 | 文言 | 次の行動 |
  | :--- | :--- | :--- |
  | まだ 1 件も無い | 見積がまだありません。輸送条件から概算を作成できます | `[新規見積作成へ]` |
  | 港コードが実在しない | **`ZZZZZ` は**港マスタにありません（**どれが誤りかを名指しする**）。UN/LOCODE の綴りをご確認のうえ、もう一度［絞り込む］を押してください | 綴りを直して再絞り込み |
  | 作成日の範囲が逆 | 作成日の「から」が「まで」より後になっています。範囲を入れ替えて、もう一度［絞り込む］を押してください | 範囲を入れ替えて再絞り込み |
  | **既定の「有効」で隠れているだけ** | 有効な見積はありません（期限切れは既定で表示しません） | `[期限切れも含めて表示]` |
  | 条件に一致しない | 条件に一致する見積はありません。条件を変えてお試しください | `[条件をクリア（すべて表示）]` |

  - **「まだ 1 件も無い」が最優先で、次が「既定で隠れているだけ」である。** 1 件も無い人に「港コードが実在しません」と言っても直しようがない。**条件を触っていない人に「条件を変えてください」と言わない**
  - **打ち間違い・範囲の逆転では「条件をクリア」へ飛ばさない。** 直したいのは 1 つの条件であり、作成日の範囲まで捨てられると入れ直しになる
  - **どれを先に伝えるかは規則であり application 層に置く**（ADR-022）。港の実在は問い合わせであり、**0 件のときしか引かない**
...
{+
  ...
  ==
  <b>ルート候補</b>
  概算です。実際の経路と費用は予約時に確定します。
  {#
    **航海番号** | **経由港** | **所要日数** | **概算費用**
    V0001 | 直行       | 18 日 | 120,000 JPY
    V0007 | JPYOK 経由 | 22 日 | 105,000 JPY
  }
  ==
  [この見積で予約する] | [一覧へ戻る]
}
...
> **期限切れのときは `[この見積で予約する]` の代わりに
> 「この見積は希望到着期限を過ぎています」＋`[同じ条件で再見積]` を出す。**
...
- 状態が `EXPIRED` の場合は `[この見積で予約する]` を無効化し、「この見積は希望到着期限を過ぎています」と理由を表示したうえで `[同じ条件で再見積]` を提示する
