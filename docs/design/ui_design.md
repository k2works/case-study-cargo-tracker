...
| 経路設計 | `/routing/design/:bookingId` | 経路候補の一覧（**IT4**）→ 選択・割り当て（US09 / IT5。誤配時は現在地起点） | 経路設計者 | US07-US11, US28 |
...
> 紐付けを設計する US18（IT8）まで `ROLE_SHIPPER` には開かない。US18 で紐付けと同時に広げ直す。
...
> **費用は<strong>概算</strong>である**（[ADR-018](../adr/018-route-search-rules.md)）。請求金額ではないことを画面に明記する。実料金は US18 で入る。
...
- **CLAIM の通関ガード（US29・IT9）**: `CustomsDeclaration` は IT9 のため、**IT7 では働かない**。代わりに**荷受人の確認を必須**とし、通関の確認が仕組みでは行われないことを引取の操作のそばに書く（決定 4）。IT9 で本物のガードに置き換える
...
- **荷主への通知（US15-5）**: **IT7 では行わない**。通知基盤は US19（IT8）。行われないことを画面に書く
...
| 追跡番号発行 | 「追跡番号 TRK-20260822-0001 を発行しました（荷主への通知は US18 まで行いません）」 | success |
