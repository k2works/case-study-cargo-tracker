...
4. CUSTOMS_HOLD は**画面から登録できない**。旧版は「税関システムから自動登録される」と書いていたが、**ADR-006（外部システムとは連携しない）と矛盾していた**。IT11 で決着した — **担当者が通関状態を「留置」に更新したとき**、`CustomsStatusChangedEvent` を Tracking が購読して自動起票する（US29）。選べる形にすると「選べるのに正しく使えない」項目が画面に残る
...
5-1. **未解決の例外は同時に何件でも持てる**（IT11。旧版は 1 件に限っていた）。遅延の対応中に破損が判明することは実務では珍しくなく、1 件に限ると**破損を登録するために遅延を「解決」する必要が生まれ、その瞬間に荷主へ事実でない対応報告が飛ぶ**。復帰先は**最初に起票された未解決の例外の発生前の状態に固定する**。2 件目以降が「例外発生」を発生前の状態として持つことはない。**1 件でも未解決が残るうちは輸送状態を戻さない**（戻すと例外一覧から消えて誰も見なくなる）
...
調整後料金 = 基本料金 − 減額 + 補償費用
  - 料金調整が無ければ 調整後料金 = 基本料金
  - 減額は基本料金を超えられない（負の請求書を作らない）
...
**割引は調整後の額に掛かる**（IT13 レビュー C5・H1）。順序を書かないと
「基本料金 × 割引 − 減額」と読む実装が生まれ、**画面の内訳が足し算として
成立しなくなる**（経理担当者が電卓で検算する場面で最初に見つかる）。
例外による減額は**その荷主に対する値引き**ではなく請求対象そのものの
目減りであり、契約割引はその後の額に対する取引条件である。
...
| 適用順序 | 基本料金を丸める → **料金調整を適用して丸める** → 割引を適用して丸める → 消費税を計算して丸める → 加算して総額とする |
...
**料金調整を含む計算例**（基本料金 100,000 円、減額 10,000 円、補償費用 3,000 円、
割引率 15%、税率 10%）:

```text
基本料金        : 100,000（丸め済み）
調整後料金      : 100,000 − 10,000 + 3,000 = 93,000
割引後料金      : 93,000 × 0.85 = 79,050    → 切り捨て → 79,050
消費税額        : 79,050 × 0.10 = 7,905     → 切り捨て → 7,905
請求総額        : 79,050 + 7,905 = 86,955
```

**画面の内訳は必ずこの順で並べる。** 「基本料金 − 割引額」を割引後料金として
表示すると、調整のある請求書で内訳の足し算が総額と合わない（IT13 レビュー H1 の
実際の症状であり、経理担当者が検算した瞬間に請求書全体が信用されなくなる）。
...
> `ApprovedCancellations` が運ぶ `DischargeOrder`（荷降し手配）は
> **「ここで降ろせ」という指示**であり、まだ降ろしていない。
>
> **同じ言葉が記録と指示の両方を指すと、一覧を見た作業員が「もう降ろしたのか」と読む。**
> IT15 で「キャンセル」と「取り消し」を分けたのと同じ扱いとする。
