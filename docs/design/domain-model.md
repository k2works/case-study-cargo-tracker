...
description: DDD 戦術的設計。7 つの境界付けられたコンテキストと共有カーネルのエンティティ・値オブジェクト・集約・ドメインサービスを定義する。
published: true
...
本ドキュメントは、国際貨物輸送管理システムの DDD（ドメイン駆動設計）戦術的設計を定義する。システムは以下の 7 つの境界付けられたコンテキスト（Bounded Context）と共有カーネル（Shared Kernel）で構成される。ユーザー認証・認可（Security）はドメインの境界付けられたコンテキストではなく、横断的な認証基盤として扱う。
...
各コンテキストは自律的に変更可能な集約を持ち、コンテキスト間の連携はドメインイベント（NestJS EventEmitter）および ACL（Anti-Corruption Layer）ポートを通じて行う。
...
| Invoice | 請求書 | Billing Context | 貨物輸送 1 件に対して発行される請求書 |
| DiscountRate | 割引率 | Billing Context | 法人契約に基づく割引率（0〜30%） |
...
## 値オブジェクト・不変性の実装方針（TypeScript）

TypeScript には Java の `record` や値型がないため、本設計では以下の方針で不変性と型安全性を確保する。

- **値オブジェクト**: `readonly` プロパティを持つ不変 `class` として実装する。生成はコンストラクタまたは静的ファクトリメソッド（`create` / `generate` 等）で行い、コンストラクタ内で不変条件を検証する。値の等価性はコンストラクタで正規化した値の比較（`equals` メソッド）で判定する。
- **単純な識別子・コード**: 単一プリミティブをラップする値オブジェクトは branded type（`type BookingId = string & { readonly __brand: 'BookingId' }`）でも表現できるが、業務ルール（バリデーション・生成規則）を持つものは不変 `class` を優先する。
- **列挙型**: TypeScript の `enum`、あるいは `as const` によるユニオン型（`type CargoType = 'GENERAL' | 'HAZARDOUS' | 'REFRIGERATED'`）で表現する。表示名（日本語）を伴う場合は `enum` + マッピング、または定数オブジェクトを用いる。
- **金額（BigDecimal 相当）**: `decimal.js` の `Decimal` 型で内部保持するか、最小通貨単位の整数（`number` / `bigint`）+ 通貨コード文字列で保持する。永続化層（PostgreSQL）では `NUMERIC` または `INTEGER` にマッピングする。
- **日付・時刻**: `LocalDate` 相当は `Date`（日付部分のみ利用）または将来的に `Temporal.PlainDate`、`Instant` 相当は `Date` または `Temporal.Instant` で表現する。永続化層では `DATE` / `TIMESTAMP WITH TIME ZONE` にマッピングする。
- **集約・エンティティ**: 同一性（identity）を持つため `class` で実装し、状態遷移メソッドを通じてのみ内部状態を変更する。永続化は Kysely（型安全 SQL ビルダー）のリポジトリアダプターが担う。

以降のクラス図では型注釈を TypeScript の型（`string` / `number` / `boolean` / `Decimal` / `Date` 等）で表記する。
...
interface ShipperExistenceChecker <<ACL Port>> {
  +exists(shipperId: ShipperId): Promise<boolean>
}
...
// ACL ポート: interface として定義（インフラ層のアダプターが実装）
...
2. Email はシステム全体で一意（`EmailAlreadyRegisteredError` で重複検出）
...
package "Read Models（読取専用モデル）" {
  class HandlingActivityHistory <<read model>> {
    -cargoBookingId: CargoBookingId
    -handlingEvents: HandlingActivitySummary[]
...
| 値オブジェクト（不変 class） | RouteCandidate | ルート候補 | 航海番号・経由港・輸送日数・見積コストを保持。Estimate に複数紐づく |
| 列挙型 | CargoType | 貨物種別 | GENERAL / HAZARDOUS / REFRIGERATED |
...
コンテキスト間のイベント連携は NestJS の `EventEmitter`（`@nestjs/event-emitter` の `EventEmitter2`）を用いて実現する。発行側はドメインイベントを `emit` し、受信側は `@OnEvent` デコレーターでハンドラを登録する。
...
Invoice を集約ルートとし、割引は荷主の法人契約割引率（DiscountRate）を Invoice に適用する設計とした。

**根拠**：請求書 1 件の整合性（基本料金・割引率・最終金額の一貫性）は Invoice 集約内で保証される。割引は法人契約割引率（0〜30%）の適用のみを対象とし、割引率の適用ロジックは Invoice の `applyDiscount()` 内で完結するため、外部ドメインサービスやポリシー種別の管理機構は設けない。支払い状態（PaymentStatus）の遷移も Invoice 集約が責任を持つ。

### Estimation Context：Estimate 集約

Estimate を集約ルートとし、RouteCandidate（ルート候補）のリストを集約内に保持する設計とした。

**根拠**：見積とルート候補は 1 対多の関係にあり、ルート候補は見積の文脈でのみ意味を持つ。`replaceCandidates()` でルート候補の一括入替を行うため、トランザクション整合性の観点から単一集約に含める。RouteCandidate は `readonly` プロパティを持つ不変 class で実装し、不変性を保証する。現在のルート候補生成はスタブ実装（重量ベースの固定コスト計算）であり、将来の外部ルーティングサービス連携時にアダプターを差し替える設計とした。
