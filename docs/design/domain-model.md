...
基本料金 = 基準運賃 × 区間係数 × 重量係数 × 貨物種別係数
  ...
  区間係数   = **区間ごとの地域係数の合計**（[ADR-027] 決定 1 と IT12 の追補）。
               **距離の代わり**である（距離は保持していない）。
               地域係数 = 国内 1.0 / 近海 2.5 / 遠洋 6.0。
               両端の区分が違う区間は**重いほう**を採る（片端が国内でも
               太平洋を渡れば遠洋の費用がかかる）
  ...
  消費税は 10%。ただし**出発地と目的地の国が異なれば輸出免税（0%）**
  （[ADR-027] 決定 8 と IT12 の追補）。
...
| CargoCancelledEvent | bookingms | trackingms | **済**（IT9） | キャンセル確定を追跡へ知らせ、お知らせに記録する。**billingms へは発行しない**——キャンセル料の算定は US23・IT11 であり、購読側が無いイベントを配ると誰も読まない配線だけが残る（[ADR-025](../adr/025-customs-declaration-and-cancellation-approval.md) 決定 3） |
...
| CustomsStatusChangedEvent | handlingms | trackingms | **済**（IT9） | 通関状態変更。HELD なら例外「税関保留」を自動起票、CLEARED なら通関完了通知（UC21） |
...
### Simulation Context：SimulationRun 集約（IT14 追加）

`SimulationRun` を集約ルートとし、`StepResult`（工程の結果）を値オブジェクトの列として持つ。
`Scenario` と `ScenarioStep` は値オブジェクトで、**工程はその工程を踏むロールと、生む識別子の
名前を自分で持つ**。

**根拠**: 実行の状態（実行中・完了・失敗）は**工程の結果から導く**。状態を別に持つと、
片方だけ更新された行が生まれる。工程の結果は**足すだけ**で、消したり書き換えたりしない
（[ADR-030](../adr/030-business-simulation-execution.md) 決定 5）——巻き戻すと、
どこまで通ったかという US35 が見たいものが、失敗したときだけ残らない。
