...
tags: design, tech-stack, versions, lts, axon-5, spring-boot, react, kafka, aiven, heroku
---
...
| Axon Kafka Extension | 4.x | <https://github.com/AxonFramework/extension-kafka> | Axon Framework 5 対応版、Spring Boot 4 互換 |
| Aiven Managed Kafka | 3.x (Kafka) | <https://aiven.io/kafka> | SSL 接続設定、Heroku からの接続確認 |
...
| PostgreSQL | 16.x | <https://www.postgresql.org/support/versioning/>, <https://endoflife.date/postgresql> | EOL 2028-11、Heroku Postgres 対応 |
...
> | Axon Kafka Extension | Axon Kafka Extension が未対応の場合 Axon Server 2024.x を採用 |
...
| メッセージング基盤 | **Axon Kafka Extension** | **4.x** | Event Bus（Kafka 経由のイベント配信） | コミュニティ | Kafka を Axon のイベントバスとして使用。Aiven マネージドサービスにより運用負荷削減 |
| マネージド Kafka | **Aiven Kafka** | **3.x** | イベントバス（本番・ステージング） | コミュニティ | Heroku アドオンとして利用可能。SSL 接続対応のフルマネージドサービス |
...
| Event Store | `JpaEventStorageEngine` | Axon 5 同梱 | PostgreSQL ベースの Event Store | コミュニティ | Axon Server 不要でイベント永続化が可能 |
| データベース | **PostgreSQL** | **16.x** | Read Model / Auth DB / **Event Store（JpaEventStorageEngine）** | **2028-11** | Read Model の関係型ストア。インデックス・JSON 型・FTS 等の機能が豊富 |
...
| 統合テスト | Testcontainers | 1.20.x | PostgreSQL / Kafka のコンテナ起動 | コミュニティ | 実 DB・実 Kafka で結合検証 |
...
| 本番オーケストレーション | **Heroku** | - | Eco/Basic dyno でアプリを運用 | - | 低コスト・運用負荷低・デプロイが容易 |
| コンテナレジストリ | **GitHub Container Registry (GHCR)** | - | Docker イメージ管理（`ghcr.io/<owner>/...`） | コミュニティ | リポジトリと統合、CI で `GITHUB_TOKEN` を直接利用可（ADR-0003） |

### クラウドサービス（Heroku + Aiven）

| カテゴリ | 技術 | バージョン | 用途 | サポート期限 | 選定理由 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| クラウドプロバイダ | **Heroku** | - | 本番・ステージング | - | 低コスト、シンプルなデプロイ、dyno スケール |
| 計算 | Heroku Eco Dyno | - | アプリ層コンテナ | - | $5/月、運用負荷低 |
| データベース | Heroku Postgres | 16.x | Read Model / Auth DB / Event Store | 2028-11 | マネージドで運用負荷低 |
| メッセージング | **Aiven Kafka** | 3.x | イベントバス（本番・ステージング） | コミュニティ | SSL 接続対応のフルマネージド Kafka |
| 証明書 | Heroku 自動管理 | - | TLS 証明書 | - | 無料・自動更新 |
| シークレット管理 | Heroku Config Vars | - | DB 認証情報・JWT 鍵・Kafka 接続情報 | - | 環境変数で管理 |

### IaC・CI/CD

| カテゴリ | 技術 | バージョン | 用途 | サポート期限 | 選定理由 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| IaC | **Terraform** | 1.10+ | Heroku / Aiven インフラ定義 | コミュニティ | マルチクラウド可、エコシステム最大 |
...
| Heroku デプロイ認証 | Heroku API Key | - | Heroku へのデプロイ | - | GitHub Actions から Heroku CLI を使用 |
| シークレット管理（CI） | GitHub Actions Secrets | - | CI/CD のシークレット | - | リポジトリ単位の安全管理 |

### 監視・ロギング・トレーシング

| カテゴリ | 技術 | バージョン | 用途 | サポート期限 | 選定理由 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| メトリクス | Heroku Metrics | - | インフラ・アプリメトリクス | - | Heroku 統合 |
| ログ | Heroku Logs / Papertrail | - | アプリケーションログ集約 | - | ログ保持・検索 |
| アラート | Heroku Alerting | - | 閾値超過時の通知 | - | メール通知 |
| メトリクス公開 | Micrometer | 1.13+ | Spring Boot のメトリクス出力 | コミュニティ | Heroku / Prometheus 両対応 |
| 分散トレーシング | OpenTelemetry | 1.x | サービス間トレース | コミュニティ | Axon Framework と統合 |
| エラートラッキング（FE） | Sentry | 8.x | ブラウザ例外・パフォーマンス | 商用無料枠 | フロントエンドの可視化 |
| Kafka 監視 | Aiven Console | - | トピック・コンシューマーラグ監視 | - | Aiven 標準 |

## データベース・メッセージング（再掲）

| カテゴリ | 技術 | バージョン | 用途 | サポート期限 |
| :--- | :--- | :--- | :--- | :--- |
| メッセージング | Axon Kafka Extension | 4.x | Event Bus（Kafka 経由のイベント配信） | コミュニティ |
| マネージド Kafka | Aiven Kafka | 3.x | イベントバス（本番・ステージング） | コミュニティ |
| RDBMS | PostgreSQL | 16.x | Read Model / Auth DB / Event Store | 2028-11 |
...
Axon Kafka Extension のライセンス（Apache 2.0）は OSS であり、商用利用可能。Aiven Kafka はマネージドサービスとして利用するため、ライセンス問題は発生しない。

## 環境別バージョン差異

| 技術 | 開発（ローカル） | 開発環境（Heroku） | ステージング | 本番 |
| :--- | :--- | :--- | :--- | :--- |
| Java JRE | Temurin 25 | Temurin 25 | Temurin 25 | Temurin 25 |
| Node | 22 LTS | 22 LTS | 22 LTS | 22 LTS |
| PostgreSQL | 16（Docker） | Heroku Postgres 16 | Heroku Postgres 16 | Heroku Postgres 16 |
| Kafka | Confluent 7.6（Docker） | Aiven Kafka 3.x | Aiven Kafka 3.x | Aiven Kafka 3.x |
