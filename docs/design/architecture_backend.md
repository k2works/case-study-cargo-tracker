...
booking ..> routing : via RouteRelaxations (ACL・問い合わせ)
...
estimation ..> routing : via RouteCandidateSource (ACL・問い合わせ)
...
> **読み取り側の「規則」は application 層に置く**（ADR-022）。業務の言葉で説明できる判断（どれを出すか・どうまとめるか）があり、かつ**確かめるのに DB の起動が要る**ときは、判断だけを application 層の純粋な関数へ出す。何回・どの SQL で引くかは infrastructure に残る。
>
> **JOIN した結果を DTO に詰め替えるだけの一覧は分けない。** そこに判断は無く、分けると往復が増えるだけである。最初の適用は `DischargeOrderSelection`（荷降し手配。US30）である。

## イベント駆動設計

> **BC 間の状態伝播はドメインイベントで行う**（ADR-009）。荷役の登録（US15）は
> 荷役作業だけを書き、`HandlingActivityRegisteredEvent` を発行する。追跡の輸送状態も
> 予約の誤配・輸送開始も、購読した各 BC が**自分のトランザクション**で更新する。
>
> **すべての境界越えがイベントではない。** 分けるのは次の基準である。
>
> | 種別 | 扱い |
> | :--- | :--- |
> | **状態の伝播**（起きた事実を他 BC が反映する） | ドメインイベント |
> | **問い合わせ**（読むだけ） | 同期（ACL ポート） |
> | **コマンド**（可否をその場で返す必要がある） | 同期（ACL ポート） |
>
> コマンドをイベントにしないのは、**拒否の理由を利用者に返せなくなる**ためである。

```plantuml
@startuml
title ドメインイベント - Spring ApplicationEventPublisher（ADR-009）
...
| `estimation/` | 実装済み（Estimate 集約・EstimateSpecification / RouteCandidate / EstimationCargoType / EstimateStatus・`RouteCandidateSource` ポート・見積一覧 / 作成 / 詳細の 3 画面。IT18。**ルート候補は Routing の探索から作る** — ADR-023） | **Release 2.1**（US01 は IT18） |
