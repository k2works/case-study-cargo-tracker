---
title: バックエンドアーキテクチャ
description: 国際貨物輸送管理システムのバックエンドアーキテクチャ設計。DDD・ヘキサゴナル・CQRS・Event Sourcing・Saga パターンをマイクロサービスとして Axon Framework 5 + Spring Boot で実装する。
published: true
date: 2026-05-12T00:00:00.000Z
tags: architecture, backend, ddd, hexagonal, cqrs, event-sourcing, saga, microservices, axon, axon-5, spring-boot
---
...
| 分散メッセージング | Axon Server により同一インフラ上でコマンド・イベント・クエリのルーティングが可能 |
...
> **参考**: 採用判断の詳細は [ADR-0001 メッセージング基盤として Axon Framework を採用する](../adr/0001-axon-framework-adoption.md) を参照。
...
booking_cmd --> AS : Command (CommandGateway)
booking_agg --> AS : Events (apply)
AS --> booking_events : Event Subscription (@EventHandler)
booking_events --> BDB : Projection 書き込み
booking_qry --> AS : Query (QueryGateway)
...
note bottom of booking
  マイクロサービス間通信:
  非同期 = Axon Event Bus（Axon Server 経由）
...
[Cargo @Aggregate\n(model/)] --> [EventStore (Axon Server)] : apply()
...
> Axon Server とフロントエンドコンテナを含む Docker Compose（`apps/docker-compose.yml`）は `apps/` 直下に配置する。
...
```plantuml
@startuml
title ドメインイベント - Axon Server 経由のマイクロサービス間連携

participant "Cargo @Aggregate\n(bookingms)" as cargo
participant "Axon Server\n(Event Store / Event Bus)" as axon
participant "CargoProjections\nEventHandler\n(bookingms)" as bproj
participant "CargoRoutedEvent\nHandler\n(trackingms)" as treh
participant "TrackingActivity\n@Aggregate\n(trackingms)" as tracking

cargo -> axon : apply(CargoBookedEvent)
axon -> bproj : @EventHandler (local)
axon -> treh : @EventHandler (cross-service)
...
```yaml
# application.yml（各マイクロサービス共通の抜粋）
axon:
  axonserver:
...
> **設計注意**:
>
> - **Aggregate からの `apply()`** は同期的に Event Store に書き込まれ、トランザクションコミット後に購読者へ配信される
...
| 非同期（イベント） | bookingms | bookingms / trackingms | Axon Event Bus | 予約・経路・追跡 |
| 非同期（イベント） | handlingms | trackingms / bookingms | Axon Event Bus | 荷役記録 |
| 非同期（イベント） | trackingms | billingms | Axon Event Bus | 配送完了 |
...
各マイクロサービスは **専用の Read Model 用 DB（Projection）** を持つ。コマンド側のイベントは Axon Server の Event Store に統合的に保存される。

| サービス | DB 用途 | データベース名 | 主要 Projection / テーブル | RDBMS |
| :--- | :--- | :--- | :--- | :--- |
| authms | 状態 DB（CRUD） | auth_db | users, roles, user_roles | PostgreSQL 16.x |
| bookingms | Read Model | booking_read_db | cargo_summary_projection | PostgreSQL 16.x |
| routingms | Read Model | routing_read_db | voyage_projection, schedule_projection | PostgreSQL 16.x |
| trackingms | Read Model | tracking_read_db | tracking_projection, handling_event_projection | PostgreSQL 16.x |
| handlingms | Read Model | handling_read_db | handling_activity_projection | PostgreSQL 16.x |
| billingms | Read Model | billing_read_db | invoice_projection | PostgreSQL 16.x |
| Axon Server | Event Store | axonserver_data（専用ボリューム） | （Event Stream） | Axon Server 2024.x（Standard Edition） |
