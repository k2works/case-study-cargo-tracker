...
コンテナ化された ASP.NET Core（.NET 10）アプリケーションを **AWS ECS Fargate** 上で稼働させ、
...
[Application Load Balancer\n(ALB)] --> [ECS Fargate\ncargo-tracker-app\n(ASP.NET Core)] : HTTP (8080)
[ECS Fargate\ncargo-tracker-app\n(ASP.NET Core)] --> [RDS PostgreSQL 16\n(Multi-AZ)\nPrimary] : 5432
[ECS Fargate\ncargo-tracker-app\n(ASP.NET Core)] --> [Secrets Manager\n(DB 接続情報)]
[ECS Fargate\ncargo-tracker-app\n(ASP.NET Core)] --> [CloudWatch Logs\n(アプリログ)]
[RDS PostgreSQL 16\n(Multi-AZ)\nPrimary] --> [RDS PostgreSQL 16\n(Multi-AZ)\nStandby] : 同期レプリケーション
[NAT Gateway] --> [ECR\n(コンテナイメージ)] : イメージ Pull
[ECS Fargate\ncargo-tracker-app\n(ASP.NET Core)] --> [NAT Gateway] : 外部通信
...
| 原則 | 内容 |
| :--- | :--- |
| **マルチステージビルド** | ビルド依存関係（SDK）を本番イメージに含めない |
| **非 root ユーザー** | `adduser` でアプリケーション専用ユーザー（または組み込みの `app` ユーザー）を作成して実行 |
| **レイヤーキャッシュの最適化** | 依存関係（`*.csproj`）を先にコピーして `dotnet restore` し、ソースコードのコピーを後にする |
...
| タスク定義 | `cargo-tracker-app` | ASP.NET Core コンテナ定義 |
...
| ワークフロー | トリガー | 内容 |
| :--- | :--- | :--- |
| `ci.yml` | PR / push | Lint・テスト・ビルド・カバレッジ（`dotnet restore` → `dotnet build` → `dotnet test`） |
...
[Visual Studio / Rider] --> [ASP.NET Core\n(localhost:8080)]
