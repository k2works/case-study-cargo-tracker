---
title: インフラアーキテクチャ（Flix 版）
description: 国際貨物輸送管理システムのインフラアーキテクチャ設計。Docker/AWS ECS/RDS 構成、CI/CD パイプライン、監視設計を定義する。
published: true
date: 2026-07-31T10:00:00.000Z
tags: architecture, infrastructure, flix, aws, ecs, docker, terraform, github-actions
---

# インフラアーキテクチャ - 国際貨物輸送管理システム（Flix 版）

## 概要

本ドキュメントでは、国際貨物輸送管理システムのインフラアーキテクチャを定義する。
コンテナ化された Flix アプリケーション（JVM 上の実行可能 JAR）を **AWS ECS Fargate** 上で稼働させ、
**RDS PostgreSQL** でデータを永続化する。
**Terraform** によるインフラの IaC 管理と **GitHub Actions** による CI/CD パイプラインを整備する。

実行成果物が単一の実行可能 JAR である点を除けば、構成は言語非依存である。Flix 固有の考慮事項は次の 4 点に集約される。
...
[Application Load Balancer\n(ALB)] --> [ECS Fargate\ncargo-tracker-app\n(Flix / JVM)] : HTTP (8080)
[ECS Fargate\ncargo-tracker-app\n(Flix / JVM)] --> [RDS PostgreSQL 16\n(Multi-AZ)\nPrimary] : 5432
[ECS Fargate\ncargo-tracker-app\n(Flix / JVM)] --> [Secrets Manager\n(DB 接続情報)]
[ECS Fargate\ncargo-tracker-app\n(Flix / JVM)] --> [CloudWatch Logs\n(アプリログ)]
[RDS PostgreSQL 16\n(Multi-AZ)\nPrimary] --> [RDS PostgreSQL 16\n(Multi-AZ)\nStandby] : 同期レプリケーション
[NAT Gateway] --> [ECR\n(コンテナイメージ)] : イメージ Pull
[ECS Fargate\ncargo-tracker-app\n(Flix / JVM)] --> [NAT Gateway] : 外部通信
...
| **レイヤーキャッシュの最適化** | `flix.toml` と `flix.lock` を先にコピーして依存を解決し、ソースのコピーを後にする。Flix コンパイルは時間がかかるためキャッシュ効果が大きい |
| **ヘルスチェック** | `HEALTHCHECK` で自作エンドポイント `/health`（DB 疎通を含む）を使用 |
...
| タスク定義 | `cargo-tracker-app` | Flix アプリ（JAR）コンテナ定義 |
| CPU | 512 (0.5 vCPU) | 初期設定。負荷に応じてスケール |
| メモリ | 1024 MB | JVM ヒープ設定（`-Xmx768m`）に合わせて調整 |
...
[Flix アプリ\n(構造化ログ / /health / /metrics)] --> [CloudWatch Logs\n(/ecs/cargo-tracker)] : awslogs ドライバー
...
| 実行プロファイル（`APP_ENV`） | `local` | `staging` | `production` |
