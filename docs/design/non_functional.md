...
| ROLE_TRACKER | 追跡管理者 | 追跡管理・例外処理（登録と解決）・**通関状態の確認と更新**（US29。引き渡しを止める仕組みであり、追跡の判断に要る）・**輸送中の予約キャンセルの承認**（US30。陸揚げ地を決める判断は貨物がいまどこにあるかを知る者にしかできない） |
| ROLE_BILLING | 経理担当者 | **輸送料金の算出・料金調整の入力・料金の確定**（US21 / US22。IT13）、**精算書の発行・入金の確認・督促対象の確認**（US23。IT14）、請求書管理。**請求書は金額であり、他ロールには開かせない**。**予約は読めるが操作できない**（料金調整の判断材料として読む） |
