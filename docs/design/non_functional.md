...
## 9. ビジネスルール定数の運用 (L-08 反映)

業務領域に存在する **数値定数の業務根拠と変更可否** を運用要件として明示する。
コード上は値オブジェクト (`DiscountRate` 等) に閉じ込めるが、変更時の影響範囲を運用側で把握しておく必要がある。

### 9.1 法人割引率の上限 30%

| 項目 | 内容 |
| :--- | :--- |
| **業務根拠** | 現行の法人契約テンプレートで設定されている割引率の最大値が 30%。営業部門と経理部門のヒアリングに基づく |
| **想定する例外** | 大口顧客 (年間取扱量 1,000 件以上) との特別契約で 30% 超のケースは現状なし |
| **変更可否** | 営業部門 + 経理部門の合議で変更可能。決定後はマイグレーション + デプロイ |
| **変更時の影響範囲** | `DiscountRate` の `mkDiscountRate` 検証ロジック、`shipper.discount_rate` の CHECK 制約、関連テスト |
| **変更不可な状況** | リリース 1.0 GA 後の本番運用中。中間決算期 (3 月・9 月) の変更は避ける |

### 9.2 パスワード強度・履歴・有効期限

| 項目 | 現行値 | 業務根拠 |
| :--- | :--- | :--- |
| 最低文字数 | 12 文字 | 国際輸送業界の情報セキュリティガイドライン |
| パスワード履歴 | 過去 5 世代再利用禁止 | 同上 |
| 有効期限 | 90 日 | 国際輸送業界の情報セキュリティガイドライン |
| アカウントロック | 5 回失敗で 30 分 | 内部規程 (ブルートフォース対策) |

### 9.3 セッションタイムアウト

| 項目 | 現行値 | 業務根拠 |
| :--- | :--- | :--- |
| 無操作タイムアウト | 30 分 | オフィス勤務者の標準休憩時間想定 |
| 絶対上限 | 8 時間 | 1 勤務日内に再認証を強制 |
| 同時セッション数 | 1 | 不正アクセス防止 |
...
| 項目 | 現行値 | 業務根拠 |
| :--- | :--- | :--- |
| 業務操作ログ | 7 年 | 国際輸送・電子帳簿保存法の保管義務 |
| 認証ログ | 1 年 | セキュリティインシデント追跡 |

これらの定数は **コード変更のみで本番影響が出る** ため、変更時は本ドキュメントの該当行も同時更新し、PR レビューでチェックする。
