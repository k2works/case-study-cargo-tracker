...
- バックエンド: NestJS 11 / Node.js 24.18 LTS / TypeScript 5.x（DDD + ヘキサゴナル + CQRS）
- フロントエンド: サーバーサイドレンダリング + htmx（30 秒ポーリング）
...
> **計測方法**: 本番相当の負荷試験環境にて k6 または Artillery で計測する。外部 API 呼び出しを含む操作は nock を用いた内部計測と外部込みの E2E 計測を両方実施する。
...
> **RTO / RPO の根拠**: RDS フェイルオーバー・AZ 障害は Multi-AZ 同期レプリケーションで自動回復するため RPO は 0 である。一方、リージョン障害はクロスリージョンの継続レプリケーションを持たず、日次スナップショット（毎日 02:00 JST 取得・7 日保持）からの手動リストアに依存するため RPO は最大 24 時間となる。本表は運用要件定義（`operation.md`）1.3 節と同一の値に統一している。
...
export class HealthController {
  constructor(
    ...
    private readonly db: DatabaseHealthIndicator,     // RDS 接続確認（Kysely で SELECT 1 を実行するカスタムインジケータ）
    private readonly disk: DiskHealthIndicator,       // ディスク空き容量確認
    private readonly memory: MemoryHealthIndicator,   // ヒープ使用量確認
  ) {}
...
**認証方式**: フォームベース認証（Passport `passport-local` + `express-session` によるセッション管理）

**RBAC ロール定義**:

| ロール | 説明 | 主要画面 |
|---|---|---|
| ROLE_SHIPPER | 荷主 | 自社予約・追跡 |
| ROLE_SALES | 営業担当者 | 予約・荷主管理・見積 |
| ROLE_ROUTE_DESIGNER | 経路設計者 | 経路割り当て・航路管理 |
...
- セッションタイムアウト: ロール別に設定（最終アクセスから）
  ...
  - その他全ロール: **30 分**
- CSRF 保護: `csurf` 相当のミドルウェアで CSRF トークンを有効化（フォーム送信・htmx リクエスト両対応）
- セッション固定攻撃対策: 認証成功後にセッション ID を再生成（`req.session.regenerate`）
...
| ECS〜RDS | 暗号化接続 | SSL/TLS（RDS ssl-mode=require、pg クライアントの `ssl` オプション有効） |
| ECS〜外部 API | HTTPS 強制 | TLS 1.2 以上、証明書検証有効 |

**HTTP セキュリティヘッダー（helmet で設定）**:
...
| 業務操作ログ | 予約作成/変更、荷役登録、請求書発行・変更 | 7 年（電子帳簿保存法準拠） |
| 管理操作ログ | ユーザー管理、権限変更、システム設定変更 | 7 年（電子帳簿保存法準拠） |
| エラーログ | アプリケーションエラー・スタックトレース | 90 日 |

**監査ログの実装方針**:

- NestJS インターセプター（`NestInterceptor`）を使用してコマンドハンドラーへの入力を自動記録
...
| 脅威 | 対応方針 |
|---|---|
| A01: 認証の不備 | Passport フォーム認証・bcrypt・アカウントロック |
| A02: 暗号化の失敗 | TLS 強制・KMS 暗号化・Secrets Manager |
| A03: インジェクション | Kysely（パラメータバインディング）使用・入力値バリデーション（class-validator） |
| A04: 安全でない設計 | セキュリティレビューをリリース前チェックリストに含める |
| A05: セキュリティ設定ミス | helmet によるセキュアヘッダー・NestJS の ValidationPipe を全体適用 |
| A06: 脆弱なコンポーネント | GitHub Dependabot + `npm audit` による自動脆弱性スキャン |
| A07: 認証・セッション管理の不備 | セッション固定対策・CSRF 保護・タイムアウト設定 |
| A08: ソフトウェアの整合性 | CI/CD パイプラインでの署名検証・`package-lock.json` による依存関係ロック |
...
- `npm audit` + ESLint セキュリティプラグイン（`eslint-plugin-security`）: CI ビルドで静的解析
- OWASP Dependency-Check（Node.js 依存関係を対象）: 月次実行・CVSS スコア 7.0 以上は即時対応
...
**ログフォーマット（pino による JSON 構造化ログ）**:

```json
{
  "timestamp": "2025-01-15T10:30:00.123Z",
  "level": "info",
  "logger": "BookingCommandHandler",
  "message": "予約を登録しました",
...
| 監査ログ | CloudWatch Logs（専用ロググループ） | 1 年 | S3（7 年・電子帳簿保存法準拠） |
| RDS スロークエリログ | CloudWatch Logs | 14 日 | - |

**コンテキスト情報の自動付与（pino の child logger / AsyncLocalStorage）**:

- `requestId`: NestJS ミドルウェアで UUID を生成し付与
- `userId`: 認証ガードでログインユーザー ID を付与
...
- Node.js イベントループ遅延（`perf_hooks` の monitorEventLoopDelay で計測）

### 5.3 コード品質目標

| 指標 | 目標値 | 計測ツール |
|---|---|---|
| テストカバレッジ（ドメイン層） | 85% 以上 | Vitest（v8 provider） |
| テストカバレッジ（アプリケーション層） | 80% 以上 | Vitest（v8 provider） |
| SonarQube Reliability | A（バグゼロ） | SonarQube |
| SonarQube Security | A（脆弱性ゼロ） | SonarQube |
| コード重複率 | 3% 以下 | SonarQube |
| 技術的負債比率 | 5% 以下 | SonarQube |
| ESLint 違反数 | 0 件（error レベル） | ESLint + typescript-eslint |
| コードフォーマット | 差分ゼロ | Prettier |

**CI 統合**:

- Pull Request 時に Vitest カバレッジレポート（lcov）をコメントで自動投稿
- SonarQube Quality Gate 失敗時はマージをブロック
- ESLint + Prettier の違反はビルドエラーとして扱う（`eslint --max-warnings 0` / `prettier --check`）
...
| スケールアウトトリガー | CPU 70% または メモリ 80% | ターゲット追跡スケーリング |
| スケールアウト速度 | 60 秒で追加インスタンス起動 | 新タスクのウォームアップを含む |
| スケールイン冷却期間 | 300 秒 | 急激なスケールイン防止 |

> **Node.js のプロセスモデルと水平スケーリング**: Node.js はシングルスレッドのイベントループで動作するため、1 コンテナ = 1 プロセスを基本とし、CPU コアの活用は Node.js の `cluster` モジュールによる垂直分割ではなく、ECS タスクの水平スケールを優先する。タスクあたりのヒープ上限は `--max-old-space-size` で明示的に設定し（下表参照）、OOM Killer による突然の停止を防ぐ。

**タスクリソース設定**:

| 環境 | CPU | メモリ | `--max-old-space-size` | 備考 |
|---|---|---|---|---|
| 本番（通常） | 1 vCPU | 2 GB | 1536（MB） | 2 タスク × 2 AZ |
| 本番（ピーク） | 2 vCPU | 4 GB | 3072（MB） | Auto Scaling 後の最大値 |
| ステージング | 0.5 vCPU | 1 GB | 768（MB） | コスト最適化 |

> ヒープ上限はコンテナメモリの約 75% を目安に設定し、残りをネイティブヒープ・バッファ・スタックに残す。

### 6.2 DB スケーリング

**接続プール設定（Kysely / node-postgres の pool）**:
...
export const db = new Kysely<Database>({
  dialect: new PostgresDialect({
    pool: new Pool({
      // ... 接続情報は Secrets Manager から注入
      max: 20,                       // タスクあたり最大接続数
      min: 5,                        // 最小アイドル接続数
      connectionTimeoutMillis: 3000, // 接続タイムアウト（ms）
      idleTimeoutMillis: 600000,     // アイドル接続の解放時間（10 分）
    }),
  }),
});
```
...
| シナリオ | 想定時期 | 対応方針 |
|---|---|---|
| 荷主セルフサービスポータル拡充 | 1 年後 | 既存 ROLE_SHIPPER の権限拡張、Passport 認証戦略拡張（OAuth2 / OIDC 対応） |
| 多言語対応（日本語・英語・中国語） | 2 年後 | `nestjs-i18n` によるメッセージ国際化、URL パスにロケール追加（`/en/`, `/zh/`） |
...
- [ ] `--max-old-space-size` がコンテナメモリに合わせて設定されていることを確認
...
- [ ] すべての HTTP セキュリティヘッダー（helmet）が設定されていることを確認
...
- [ ] pino による JSON 構造化ログが CloudWatch Logs に出力されていることを確認
...
- [ ] Kysely / pg 接続プール設定が本番環境に適用されていることを確認
