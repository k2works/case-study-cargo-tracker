...
tags: design, non-functional, sla, security, performance, go
---
...
- バックエンド: Go 1.24（DDD + ヘキサゴナル + CQRS、単一の軽量バイナリとしてデプロイ）
- フロントエンド: html/template SSR + htmx（30 秒ポーリング）
...
業務シナリオに基づき、画面・操作別のレスポンスタイム目標を定義する。計測は ALB アクセスログおよび CloudWatch メトリクスを基準とする。Go ランタイムは JIT ウォームアップが不要なため、起動直後から安定したレイテンシを提供できる。
...
| 請求書生成 | 500ms | 1,500ms | 3,000ms | 非同期処理候補（goroutine による非同期化） |
...
- Go の `net/http` はリクエストごとに goroutine を割り当てるため、高い同時接続数を低メモリで処理できる
...
|---|---|---|---|
| ECS タスク障害 | 5 分以内 | 0（ステートレス） | ECS ヘルスチェック自動再起動（Go バイナリは起動 1 秒未満のため実質再起動即時復帰） |
...
3. ECS が新しいタスクを別の AZ で起動（Go バイナリの起動時間は 1 秒未満）
4. 新タスクのヘルスチェック成功後、ALB に再登録
5. 旧タスクを停止（既存接続のドレイン: 30 秒、`http.Server.Shutdown` によるグレースフルシャットダウン）

**自前ヘルスチェックハンドラー構成**:

```go
// internal/adapter/handler/health.go
// GET /healthz で以下を確認して JSON を返す
//   - db:   *sql.DB の PingContext による RDS 接続確認
//   - disk: 一時ディレクトリの空き容量確認
//   - app:  アプリ起動確認（常に ok）
...
**認証方式**: フォームベース認証 + scs（alexedwards/scs）によるサーバーサイドセッション管理（PostgreSQL ストア）

**RBAC ロール定義**（RBAC ミドルウェアでルート単位に強制する）:
...
- ハッシュアルゴリズム: bcrypt（`golang.org/x/crypto/bcrypt`、コスト係数 12）
- 最低文字数: 8 文字以上
...
- セッションタイムアウト: ロール別に scs の `IdleTimeout` を設定（最終アクセスから）
  - `ROLE_HANDLER`（荷役作業員）: **2 時間**（港湾・倉庫現場での連続作業・バーコードスキャン業務を考慮）
  - その他全ロール: **30 分**
- CSRF 保護: nosurf ミドルウェアによる CSRF トークン検証（フォーム送信・htmx リクエスト両対応、htmx は `X-CSRF-Token` ヘッダーで送信）
- セッション固定攻撃対策: 認証成功後に scs の `RenewToken` でセッショントークンを再生成
...
| ECS〜RDS | 暗号化接続 | SSL/TLS（RDS sslmode=require、pgx 接続文字列で指定） |
| ECS〜外部 API | HTTPS 強制 | TLS 1.2 以上、証明書検証有効（`http.Client` デフォルト検証を無効化しない） |

**HTTP セキュリティヘッダー**（セキュリティヘッダーミドルウェアで全レスポンスに付与）:
...
**外部 API 接続タイムアウト設定**（`http.Client` / `http.Transport` で設定）:

| 設定項目 | 値 | 備考 |
|---|---|---|
| 接続タイムアウト | 3 秒 | `net.Dialer.Timeout`（DNS 解決 + TCP 接続） |
| 読み取りタイムアウト | 10 秒 | `http.Client.Timeout`（レスポンスボディ受信含む） |
| 最大リトライ回数 | 3 回 | Exponential Backoff（初回 1 秒、倍増） |
| サーキットブレーカー閾値 | 5 回連続失敗 | 60 秒間オープン状態を維持（sony/gobreaker 等） |
...
- コマンドハンドラーをラップするデコレーター（ミドルウェア関数）で入力を自動記録
...
| 脅威 | 対応方針 |
|---|---|
| A01: 認証の不備 | フォーム認証・bcrypt・アカウントロック・RBAC ミドルウェア |
| A02: 暗号化の失敗 | TLS 強制・KMS 暗号化・Secrets Manager |
| A03: インジェクション | pgx プレースホルダー（PreparedStatement）使用・入力値バリデーション（go-playground/validator） |
| A04: 安全でない設計 | セキュリティレビューをリリース前チェックリストに含める |
| A05: セキュリティ設定ミス | セキュリティヘッダーミドルウェア・nosurf・html/template の自動エスケープを標準有効化 |
| A06: 脆弱なコンポーネント | GitHub Dependabot + govulncheck による自動脆弱性スキャン |
| A07: 認証・セッション管理の不備 | セッション固定対策（RenewToken）・CSRF 保護（nosurf）・タイムアウト設定 |
| A08: ソフトウェアの整合性 | CI/CD パイプラインでの署名検証・go.sum による依存関係ロック |
...
- golangci-lint（gosec リンター含む）: CI ビルドで静的解析
- govulncheck: CI で毎ビルド実行・月次で全依存の棚卸し・CVSS スコア 7.0 以上は即時対応
...
| ログレベル | 出力条件 | 例 |
|---|---|---|
| ERROR | システムエラー・外部 API 接続失敗 | DB 接続不可、決済 API エラー、未捕捉 panic |
...
**ログフォーマット（slog JSON 構造化ログ）**:

```json
{
  "time": "2025-01-15T10:30:00.123Z",
  "level": "INFO",
  "source": "internal/application/booking/command_handler.go:42",
  "msg": "予約を登録しました",
...
**context + slog による共通属性の自動付与**:

- `requestId`: HTTP ミドルウェアで UUID を生成し `context.Context` 経由で slog ハンドラーに付与
...
**カスタムメトリクス（prometheus/client_golang による `/metrics` エンドポイント公開）**:

- 予約登録件数（業務 KPI、Counter）
- 追跡番号検索件数（API 使用量、Counter）
- 外部 API 呼び出しレイテンシ（連携先別、Histogram）
- htmx ポーリングリクエスト数（負荷監視、Counter）
- Go ランタイムメトリクス（goroutine 数・GC 停止時間・ヒープ使用量、collectors.NewGoCollector）

> `/metrics` は VPC 内部からのみアクセス可能とし、CloudWatch へは Prometheus 互換収集（CloudWatch Agent / ADOT Collector）で転送する。

### 5.3 コード品質目標

| 指標 | 目標値 | 計測ツール |
|---|---|---|
| テストカバレッジ（ドメイン層） | 85% 以上 | go test -cover |
| テストカバレッジ（アプリケーション層） | 80% 以上 | go test -cover |
| SonarQube Reliability | A（バグゼロ） | SonarQube |
| SonarQube Security | A（脆弱性ゼロ） | SonarQube |
| コード重複率 | 3% 以下 | SonarQube |
| 技術的負債比率 | 5% 以下 | SonarQube |
| golangci-lint 違反数 | 0 件（有効リンター全件） | golangci-lint |
| govulncheck 検出脆弱性 | 0 件（到達可能なもの） | govulncheck |

**CI 統合**:

- Pull Request 時に go test のカバレッジレポートをコメントで自動投稿
- SonarQube Quality Gate 失敗時はマージをブロック
- golangci-lint + govulncheck の違反はビルドエラーとして扱う
- `gofmt` / `go vet` の違反はコミット前フックと CI の両方で検出する
...
| スケールアウト速度 | 60 秒で追加インスタンス起動 | Go バイナリはウォームアップ不要のため起動後即時サービスイン |
| スケールイン冷却期間 | 300 秒 | 急激なスケールイン防止 |

**タスクリソース設定**（Go の低メモリフットプリントにより JVM 構成より小さいサイズで同等スループットを想定）:

| 環境 | CPU | メモリ | 備考 |
|---|---|---|---|
| 本番（通常） | 0.5 vCPU | 1 GB | 2 タスク × 2 AZ |
| 本番（ピーク） | 1 vCPU | 2 GB | Auto Scaling 後の最大値 |
| ステージング | 0.25 vCPU | 512 MB | コスト最適化 |

> コンテナイメージは distroless / scratch ベースの静的リンクバイナリ（数十 MB）とし、イメージプル時間の短縮によりスケールアウトをさらに高速化する。
...
config.MaxConns = 20                       // タスクあたり最大接続数
config.MinConns = 5                        // 最小アイドル接続数
config.ConnConfig.ConnectTimeout = 3 * time.Second // 接続タイムアウト
config.MaxConnIdleTime = 10 * time.Minute  // アイドル接続の解放時間
config.MaxConnLifetime = 30 * time.Minute  // 接続の最大生存時間
```
...
| シナリオ | 想定時期 | 対応方針 |
|---|---|---|
| 荷主セルフサービスポータル（Phase 2） | 1 年後 | ROLE_SHIPPER ロール追加、認証ミドルウェアの拡張（OAuth2 / OIDC 対応、coreos/go-oidc 等） |
| 多言語対応（日本語・英語・中国語） | 2 年後 | golang.org/x/text/message によるメッセージカタログ国際化、URL パスにロケール追加（`/en/`, `/zh/`） |
...
- セッション延長ボタン: クリックで `/keep-alive` エンドポイントを呼び出し scs の IdleTimeout をリセット
...
- [ ] DB 接続プール（pgxpool）の設定が本番環境に適用されていることを確認

**可用性**:

- [ ] ECS ヘルスチェック（/healthz）が正しく設定されていることを確認
- [ ] RDS Multi-AZ フェイルオーバーを実際にテストし、RTO 2 分以内を確認
- [ ] グレースフルシャットダウン（http.Server.Shutdown）が機能していることを確認
...
- [ ] CSRF 保護（nosurf）が有効であることを確認（フォームおよび htmx リクエスト）
- [ ] govulncheck で到達可能な脆弱性がゼロであることを確認
- [ ] OWASP Top 10 対応のレビューが完了していることを確認

**保守性**:

- [ ] slog による JSON 構造化ログが CloudWatch Logs に出力されていることを確認
- [ ] /metrics エンドポイントが VPC 内部限定で公開されていることを確認
...
- [ ] pgxpool の接続プール設定が本番環境に適用されていることを確認
