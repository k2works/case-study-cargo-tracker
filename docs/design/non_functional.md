...
date: 2026-07-06
tags: design, non-functional, sla, security, performance, fsharp
---
...
- バックエンド: F# 9 / .NET 10（LTS）+ Giraffe（DDD + ヘキサゴナル + CQRS）
- フロントエンド: Giraffe.ViewEngine による SSR + htmx（30 秒ポーリング）
- データアクセス: Donald + DbUp（開発 SQLite / 本番 PostgreSQL 16）
...
| 精算書生成 | 500ms | 1,500ms | 3,000ms | 非同期処理候補 |
| htmx 部分更新（追跡ステータス） | 100ms | 200ms | 500ms | 30 秒ポーリング |

> **前提条件**: 上表の目標値はすべて**外部システム 5 件（外部経路システム・税関システム・決済機関・港湾管理システム・通知システム）が正常応答している状態**を前提とします。外部システム障害時（リトライ発生・サーキットブレーカーオープン）は下記の縮退動作目標を適用し、上表の SLO 違反としてはカウントしません（3.1 の稼働率計測除外と同一の扱い）。
>
> **計測方法**: 本番相当の負荷試験環境にて k6 で計測します（シナリオ・データ量・実行環境の詳細はテスト戦略ドキュメントの「パフォーマンステスト」節を参照）。外部 API 呼び出しを含む操作はスタブを用いた内部計測と外部込みの E2E 計測を両方実施します。

**外部システム障害時の縮退動作目標**:

| 操作 | 障害時の縮退動作 | 縮退時目標（p95） |
|---|---|---|
| 最適ルート検索 | サーキットブレーカーオープン時は外部呼び出しをスキップし、過去実績データ（フォールバック候補）を即時返却 | 1,000ms（フォールバック応答） |
| 荷役作業登録 | 通知システム障害時は通知をキュー退避（ログ記録）し、登録自体は成功として応答 | 500ms（正常時と同一） |
| 精算処理 | 決済機関障害時はエラーを即時応答し、Invoice を未精算のまま保持（後続リトライは運用側で実施） | 1,000ms（エラー即時応答） |
| 通関申請連携 | 税関システム障害時は CUSTOMS_PENDING のまま保留し、エラーを即時応答 | 1,000ms（エラー即時応答） |
...
- 追跡 API は内部ユーザーと荷受人（外部公開）が分離されるため、個別にレートリミットを設定します（荷受人: 100 RPS/IP）
...
| invoices（精算書） | 50,000 件 | 4 KB | 200 MB |
...
**ASP.NET Core Health Checks 構成（Giraffe ホスト上）**:
...
let configureServices (services: IServiceCollection) =
    services
        ...
        .AddDiskStorageHealthCheck(fun options ->     // ローカルディスク確認
            options.AddDrive("/", minimumFreeMegabytes = 1024L) |> ignore)
        .AddCheck("self", fun () -> HealthCheckResult.Healthy()) // アプリ起動確認
    |> ignore

let configureApp (app: IApplicationBuilder) =
    app
        .UseHealthChecks(PathString "/health") // 認可済みのみ詳細表示
        .UseGiraffe webApp
```
...
**認証方式**: ASP.NET Core Cookie 認証（フォームベース・セッション管理。Giraffe の `requiresAuthentication` / `requiresRole` HttpHandler で保護）
...
- セッションタイムアウト: ロール別に設定（最終アクセスから）
  ...
  - その他全ロール: **30 分**
- CSRF 保護: ASP.NET Core Antiforgery トークン有効（Giraffe.ViewEngine のフォーム・htmx リクエスト両対応）
...
| 最大リトライ回数 | 3 回 | Exponential Backoff（初回 1 秒、倍増）。**同期応答系は除く（下記）** |
| トータルタイムアウト | 15 秒 | リトライを含む 1 呼び出しの総時間上限（Polly TotalRequestTimeout） |
| サーキットブレーカー閾値 | 5 回連続失敗 | 60 秒間オープン状態を維持 |

> リトライ・サーキットブレーカーは `Microsoft.Extensions.Http.Resilience`（Polly）で `HttpClient` に統合して実装します（F# からも同一 API を利用可能）。

**同期応答系操作のタイムアウト調整（性能目標との整合）**:

ユーザーが応答を同期的に待つ操作では、リトライ 3 回のワーストケース（接続 3 秒 + 読取 10 秒 + バックオフを 4 試行分 ≈ 最大 50 秒超）が性能目標（2.1）と矛盾するため、以下のとおり操作別に上書きします。

| 操作 | リトライ回数 | トータルタイムアウト | 根拠 |
|---|---|---|---|
| 最適ルート検索（同期） | **1 回** | **15 秒** | p99 10 秒 + フォールバック応答 1 秒に収める。超過時は過去実績データへ即時フォールバック |
| 精算・通関申請（同期） | 0 回（リトライなし） | 13 秒 | 二重決済・二重申告防止。失敗はエラー即時応答（縮退動作目標参照） |
| 通知送信（非同期・非クリティカル） | 3 回 | 制限なし（バックグラウンド） | ユーザー応答をブロックしないため既定値を適用 |
...
| 業務操作ログ | 予約作成/変更、荷役登録、精算書発行・変更 | 5 年 |
| 管理操作ログ | ユーザー管理、権限変更、システム設定変更 | 5 年 |
| エラーログ | アプリケーションエラー・スタックトレース | 90 日 |

**監査ログの実装方針**:

- コマンドハンドラー関数を監査ログ記録関数で合成するデコレーター（高階関数）、または ASP.NET Core ミドルウェアを使用してコマンドへの入力を自動記録
...
| A03: インジェクション | Donald（パラメーター化クエリ）使用・入力値バリデーション（スマートコンストラクタ + `Result` 型による検証） |
...
- FSharpLint + .NET アナライザー: CI ビルドで静的解析
...
| コードスタイル違反数 | 0 件（Error レベル） | .editorconfig + Fantomas（`dotnet fantomas --check`） |
| FSharpLint 警告数 | 0 件（High レベル） | FSharpLint（`dotnet fsharplint lint`） |

**CI 統合**:

- Pull Request 時に Coverlet カバレッジレポートをコメントで自動投稿
- SonarQube Quality Gate 失敗時はマージをブロック
- Fantomas フォーマットチェック + FSharpLint の違反はビルドエラーとして扱う（コンパイラ警告は `TreatWarningsAsErrors`）
...
```fsharp
// appsettings.json の接続文字列（Donald は Npgsql の接続プールをそのまま利用）
...
> **注意**: ピーク時（10 タスク × 20 接続 = 200）は RDS 接続数上限に達するため、必要に応じて RDS Proxy を導入し接続プーリングを委譲します。
...
- スクリーンリーダー対応: セマンティック HTML + ARIA ランドマーク（Giraffe.ViewEngine のセマンティック要素で構築）
...
- 長時間処理（精算書生成等）: プログレスバーまたは「処理中...（通常 1〜3 秒）」のメッセージを表示
...
| 消費税表示 | 消費税法 | 精算書に税率（10%）・税額を明示、軽減税率対象品目は区分を記載 |
| 商業帳票保存 | 電子帳簿保存法 | 精算書・精算記録を電子的に 7 年間保存（改ざん防止のため監査ログで変更履歴を記録） |
...
- [ ] Fantomas / FSharpLint のチェックが CI でパスしていることを確認
