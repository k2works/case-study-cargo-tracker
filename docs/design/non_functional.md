...
> **実装状況（IT7）**: アプリケーション側の同時実行上限・バックプレッシャ・タイムアウトは
> 実装済みである（TS12。[バックエンドアーキテクチャ](architecture_backend.md) の HTTP ランタイム設計）。
> 既定は同時実行 200・タイムアウト 30 秒で、いずれも環境変数で調整できる。

#### 実測値（IT8・TS12b）

**測定した**（[ふりかえり Try T7](../development/retrospective-7.md)「数値を持つ防御は、
その数値が守られているか測ってから恒久化する」）。手順は
[`ops/scripts/load-test/index.js`](../../ops/scripts/load-test/index.js)。

| 測定環境 | 内容 |
| :--- | :--- |
| 実行 | 開発機（macOS・Apple Silicon）。アプリと負荷生成が**同一マシン** |
| DB | H2（ファイル）。**PostgreSQL ではない** |
| 対象 | `GET /public/tracking/{trackingNumber}`（追跡 API・認証なし・DB 参照 1 回） |
| 条件 | 各 8 秒・ウォームアップ 3 秒。同時実行数を変えて RPS と応答時間を測る |

| 同時実行 | RPS | p50 | p95 | p99 | 503 | 504 | 5xx |
| ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 10 | 1,328 | 6 ms | 19 ms | 31 ms | 0 | 0 | 0 |
| 50 | 1,915 | 23 ms | 48 ms | 70 ms | 0 | 0 | 0 |
| 100 | **2,306** | 27 ms | 154 ms | 240 ms | 0 | 0 | 0 |
| 200 | 1,468 | 84 ms | 394 ms | 557 ms | 0 | 0 | 0 |
| 400 | 2,246 | 184 ms | 365 ms | 529 ms | 0 | 0 | 0 |

**目標値はいずれも達成している**。最大 2,306 RPS は追跡 API の目標
1,000 RPS の 2.3 倍であり、平常時 50・ピーク 200 は余裕をもって満たす。

ただし**この数値をそのまま本番の見込みとして扱わない**。

- 負荷生成とアプリが同一マシンであり、**CPU を奪い合っている**（本番より不利にも有利にも働く）
- DB が H2 のファイルであり、**PostgreSQL のネットワーク往復が無い**。
  DB 参照を伴う経路の実測としては**楽観的すぎる**
- 同時実行 200 で RPS が落ちて 400 で戻るのは、測定のばらつきの範囲である。
  **1 回の測定で傾向を語らない**

#### 同時実行の上限は、この経路では到達しない

**既定の 200 では 503 が 1 件も出なかった**。同時実行 400 の測定では
リトルの法則から系内に約 410 件（2,246 RPS × 0.184 秒）あった計算になるが、
それでも拒否は起きていない。

これは**セマフォが数えているのが「ハンドラを実行中の数」だから**である。
接続の受理待ち・ディスパッチ待ちはセマフォの外側で積み上がる。
追跡 API は 1 件あたり数十ミリ秒で終わるため、200 の枠は
**この経路では実質的に効かない**。

観測手段そのものを疑い（[Try T6](../development/retrospective-7.md)）、
`APP_MAX_CONCURRENT_REQUESTS=5` にして再測定した。
...
**防御は正しく働いている**。既定値で 503 が出ないのは、防御が壊れているのではなく
**枠に達していない**からである。

> **上限 200 を下げない**。この経路で効かないことは、
> **遅い経路（DB を多く引く画面・帳票）で効かないことを意味しない**。
> 枠は最も重い経路のために存在する。軽い経路の実測だけを根拠に下げると、
> 重い経路で守るものが無くなる。
...
Actuator 相当は存在しないため、`shared/infrastructure/http/Health.flix` に自作する。

| エンドポイント | 用途 | 検査内容 | 応答 |
| :--- | :--- | :--- | :--- |
| `GET /health/live` | Liveness（コンテナ `HEALTHCHECK`） | プロセス応答のみ（DB を見ない） | `200 {"status":"UP"}` |
| `GET /health/ready` | Readiness（ALB ターゲットグループ） | DB 疎通（`SELECT 1`、タイムアウト 2 秒）、マイグレーション適用済み判定 | 正常 `200` / 異常 `503` と失敗要因 |
| `GET /metrics` | 監視 | リクエスト数・レイテンシ・エラー率・DB プール使用率 | CloudWatch 埋め込みメトリクス形式 |

- 詳細情報（失敗した検査名）は認証済みリクエストにのみ返し、未認証には状態のみ返す
- Liveness で DB を見ないのは、DB 障害時にタスクが再起動ループへ陥るのを防ぐためである
...
**認証方式**: 自作のフォームベース認証（サーバ側セッション管理）。詳細は [バックエンドアーキテクチャ](architecture_backend.md) の「セキュリティ設計」を参照
...
- `eff AuditLog` を宣言し、アプリケーションサービスの入口で記録する。効果としたことで「監査記録を要求するユースケース」が型に現れ、記録漏れをレビューで発見できる
...
| 脅威 | 対応方針 |
|---|---|
| A01: 認証の不備 | 自作フォーム認証（BCrypt コスト 12・アカウントロック・セッション ID 再生成）。自作であるため、セキュリティ回帰テストを必須とする（[テスト戦略](test_strategy.md) 8.4） |
| A02: 暗号化の失敗 | TLS 強制・KMS 暗号化・Secrets Manager |
| A03: インジェクション | `PreparedStatement` のみ使用（SQL 文字列連結を `arch-lint` で禁止）・値オブジェクトのスマートコンストラクタによる入力検証 |
| A04: 安全でない設計 | セキュリティレビューをリリース前チェックリストに含める |
| A05: セキュリティ設定ミス | 認可要件をルーティング表の必須項目とし、設定漏れをコンパイルエラーにする。セキュリティヘッダ（HSTS・CSP・X-Content-Type-Options）は共通ミドルウェアで一律付与 |
...
**接続プール設定（HikariCP を Java 相互運用で使用）**:

`Db` 効果のハンドラ内部にプールを隠蔽し、環境変数から以下の値を設定する。

| 設定項目 | 値 | 意味 |
| :--- | :--- | :--- |
| `maximumPoolSize` | 20 | タスクあたり最大接続数 |
| `minimumIdle` | 5 | 最小アイドル接続数 |
| `connectionTimeout` | 3,000 ms | 接続取得のタイムアウト |
| `idleTimeout` | 600,000 ms | アイドル接続の解放時間（10 分） |
| `maxLifetime` | 1,800,000 ms | 接続の最大生存時間（30 分） |
...
| シナリオ | 想定時期 | 対応方針 |
|---|---|---|
| 荷主セルフサービスポータル（Phase 2） | 1 年後 | `Shipper` ロールの権限拡張、認証フローの OAuth2 / OIDC 対応（`Auth` 効果のハンドラ差し替えで実現） |
| 多言語対応（日本語・英語・中国語） | 2 年後 | メッセージ束を `Map[Locale, Map[MessageKey, String]]` として持ち、`Html` DSL の文字列生成を `msg(key)` 経由に統一。URL パスにロケールを追加（`/en/`, `/zh/`） |
