...
>
> **本表の目標値は依然として未検証である**。負荷試験は TS12b として IT8 に割り当てた。
> 特に追跡 API の 1,000 RPS は他の目標より 1 桁高く、DB 参照を伴うため
> コネクション 20 本 × タスク数では不足しうる。**負荷試験を経ずに
> 「達成できる」と扱わないこと**。実測の結果しだいで本表を改訂する（ADR に記録する）。
