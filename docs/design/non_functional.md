---
title: 非機能要件定義 - 国際貨物輸送管理システム（Rust 版）
description: ISO/IEC 25010 品質モデルに基づく非機能要件定義。性能、可用性、セキュリティ、保守性、拡張性の各要件を測定可能な数値で定義する。
published: true
date: 2026-07-06T00:00:00.000Z
tags: design, non-functional, sla, security, performance, rust
---
...
- バックエンド: Rust（axum + tokio）/ sqlx（DDD + ヘキサゴナル + CQRS）
- フロントエンド: SSR テンプレート + htmx（30 秒ポーリング）
...
> **計測方法**: 本番相当の負荷試験環境にて k6 または JMeter で計測する。外部 API 呼び出しを含む操作はスタブを用いた内部計測と外部込みの E2E 計測を両方実施する。

> **Rust 特性による補足**: Rust は GC を持たないため、GC ポーズによるレイテンシスパイク（テールレイテンシの悪化）が発生しない。また JIT ウォームアップも不要であり、デプロイ直後・スケールアウト直後から目標値どおりの性能を発揮する。負荷試験ではウォームアップ期間を設けず、起動直後の計測値をそのまま評価対象とする。
...
- axum + tokio の非同期ランタイムは 1 タスクあたりのスループット余裕が大きいが、目標値は SLA 準拠のため据え置く
...
> **Rust 特性による補足**: アプリケーションは静的リンクされた単一バイナリ（distroless / scratch ベースの軽量コンテナイメージ）であり、起動時間は 1 秒未満である。JVM のようなクラスロード・ウォームアップが不要なため、ECS タスク再起動やスケールアウト時のコールドスタートが実質的に無視でき、RTO 目標に対して大きな余裕を持つ。
...
3. ECS が新しいタスクを別の AZ で起動（Rust バイナリの高速起動により数秒で Ready）
4. 新タスクのヘルスチェック成功後、ALB に再登録
5. 旧タスクを停止（既存接続のドレイン: 30 秒。tokio のグレースフルシャットダウンで処理中リクエストを完了させる）

**ヘルスチェックエンドポイントの実装方針（axum）**:

```rust
// GET /health : Liveness（プロセス生存確認、常に 200）
// GET /health/ready : Readiness（依存リソース確認）
//   - sqlx の接続プールで `SELECT 1` を実行し RDS 接続を確認
//   - 接続不可の場合は 503 を返却し ALB から切り離す
...
**認証方式**: tower-sessions + axum-login によるフォームベース認証（サーバーサイドセッション管理）
...
ロールベースの認可は axum-login の `AuthzBackend`（`has_perm`）とルート単位のミドルウェア（`login_required!` / `permission_required!`）で実装する。

**パスワードポリシー**:

- ハッシュアルゴリズム: Argon2id（推奨・OWASP 準拠パラメーター）または bcrypt（コスト係数 12）
- 最低文字数: 8 文字以上
...
- セッションストア: tower-sessions の PostgreSQL ストア（sqlx 経由）。ECS タスク間でセッションを共有しステートレス性を維持
- セッションタイムアウト: ロール別に設定（最終アクセスから）
  - `ROLE_HANDLER`（荷役作業員）: **2 時間**（港湾・倉庫現場での連続作業・バーコードスキャン業務を考慮）
  - その他全ロール: **30 分**
- CSRF 保護: CSRF 対策ミドルウェア（同期トークンパターン）を全状態変更リクエストに適用（フォーム送信・htmx リクエスト両対応。htmx は `hx-headers` でトークンを送信）
- セッション固定攻撃対策: 認証成功後に `Session::cycle_id()` でセッション ID を再生成
- セッション Cookie 属性: `Secure` / `HttpOnly` / `SameSite=Lax` を必須とする
...
- Rust の所有権・借用検査により、バッファオーバーフロー・Use-After-Free・データ競合といったメモリ安全性起因の脆弱性クラスをコンパイル時に排除する
- `unsafe` ブロックの使用は原則禁止とし、やむを得ず使用する場合はコードレビューで安全性の根拠をコメントとして必須記載する（clippy の `undocumented_unsafe_blocks` を有効化）
...
| ECS〜RDS | 暗号化接続 | SSL/TLS（sqlx 接続文字列で `sslmode=require`） |
| ECS〜外部 API | HTTPS 強制 | TLS 1.2 以上、証明書検証有効（rustls） |

**HTTP セキュリティヘッダー**（tower ミドルウェア / `SetResponseHeaderLayer` で全レスポンスに付与）:
...
**外部 API 接続タイムアウト設定**（reqwest クライアント設定）:
...
| 対象データ | 保護方式 | 管理 |
|---|---|---|
| DB 接続情報（URL、ユーザー、パスワード） | AWS Secrets Manager | アプリケーション起動時に動的取得（ECS タスク定義の secrets 注入） |
...
| メモリ上の秘密情報 | `secrecy` クレート等でラップ | Debug 出力・ログへの誤出力を型レベルで防止 |
...
| エラーログ | アプリケーションエラー・エラーチェーン（`anyhow` コンテキスト） | 90 日 |

**監査ログの実装方針**:

- コマンドハンドラーを監査ログデコレーター（またはミドルウェア層）でラップし、入力を自動記録する
...
| 脅威 | 対応方針 |
|---|---|
| A01: 認証の不備 | axum-login フォーム認証・Argon2id/bcrypt・アカウントロック |
| A02: 暗号化の失敗 | TLS 強制（rustls）・KMS 暗号化・Secrets Manager |
| A03: インジェクション | sqlx のコンパイル時検証クエリ（`query!` マクロ・プレースホルダー必須）・入力値バリデーション（validator クレート） |
| A04: 安全でない設計 | セキュリティレビューをリリース前チェックリストに含める |
| A05: セキュリティ設定ミス | セキュリティヘッダー・Cookie 属性をミドルウェアで一律適用し設定漏れを防止 |
| A06: 脆弱なコンポーネント | GitHub Dependabot + cargo-audit（RustSec Advisory DB）による自動脆弱性スキャン |
| A07: 認証・セッション管理の不備 | セッション固定対策（`cycle_id`）・CSRF 保護・タイムアウト設定 |
| A08: ソフトウェアの整合性 | CI/CD パイプラインでの署名検証・`Cargo.lock` による依存関係ロック・cargo-deny によるライセンス/ソース検証 |
| A09: セキュリティログとモニタリングの失敗 | CloudWatch + 監査ログ・異常検知アラート |
| A10: SSRF | 外部 API URL のホワイトリスト管理・VPC エンドポイント活用 |
| （追加）メモリ安全性 | Rust 言語仕様によりバッファオーバーフロー等をコンパイル時に排除・`unsafe` 原則禁止 |

**定期的な脆弱性管理**:

- GitHub Dependabot: 毎週自動スキャン・PR 自動生成
- cargo-audit: CI ビルドごとに RustSec Advisory DB と照合、既知脆弱性検出時はビルド失敗
- cargo-deny: CI ビルドで advisories / licenses / bans / sources を検査
- CVSS スコア 7.0 以上の脆弱性は即時対応（月次棚卸しで残存確認）
...
| ログレベル | 出力条件 | 例 |
|---|---|---|
| ERROR | システムエラー・外部 API 接続失敗 | DB 接続不可、決済 API エラー、未捕捉エラー |
...
**ログフォーマット（tracing + tracing-subscriber の JSON 構造化ログ）**:

```json
{
  "timestamp": "2025-01-15T10:30:00.123Z",
  "level": "INFO",
  "target": "cargo_tracker::application::booking_command_handler",
  "message": "予約を登録しました",
...
**tracing の Span によるコンテキスト自動付与**（MDC 相当）:

- `requestId`: tower の `TraceLayer` / リクエストミドルウェアで UUID を生成し、リクエスト Span のフィールドとして付与
- `userId`: 認証ミドルウェアでログインユーザー ID を Span に記録
- `traceId`: AWS X-Ray トレース ID（後続の分散トレーシング対応。tracing-opentelemetry で連携）
- Span 階層により、ハンドラー配下の全ログへ上記フィールドが自動伝播する
...
> **補足**: Rust は GC を持たないためメモリ使用量は安定的に推移する。メモリ使用率の継続的な上昇（右肩上がりのトレンド）はリークの兆候として扱い、閾値未満でも調査対象とする。
...
- sqlx 接続プール使用率（アプリケーション側から出力）

### 5.3 コード品質目標

| 指標 | 目標値 | 計測ツール |
|---|---|---|
| テストカバレッジ（ドメイン層） | 85% 以上 | cargo-llvm-cov |
| テストカバレッジ（アプリケーション層） | 80% 以上 | cargo-llvm-cov |
| SonarQube Reliability | A（バグゼロ） | SonarQube |
| SonarQube Security | A（脆弱性ゼロ） | SonarQube |
| コード重複率 | 3% 以下 | SonarQube |
| 技術的負債比率 | 5% 以下 | SonarQube |
| clippy 警告数 | 0 件（`-D warnings` で deny） | clippy |
| rustfmt 差分 | 0 件（`cargo fmt --check`） | rustfmt |
| cargo-audit 既知脆弱性 | 0 件 | cargo-audit |
| cargo-deny 違反 | 0 件 | cargo-deny |

**CI 統合**:

- Pull Request 時に cargo-llvm-cov カバレッジレポートをコメントで自動投稿
- SonarQube Quality Gate 失敗時はマージをブロック
- `cargo clippy -- -D warnings` / `cargo fmt --check` / `cargo audit` / `cargo deny check` の違反はビルドエラーとして扱う
- sqlx のオフラインモード（`cargo sqlx prepare` の検証）で SQL とスキーマの整合性を CI で担保する
...
| スケールアウト速度 | 60 秒で追加インスタンス起動 | Rust バイナリはウォームアップ不要のため起動即戦力化 |
...
| 環境 | CPU | メモリ | 備考 |
|---|---|---|---|
| 本番（通常） | 0.5 vCPU | 1 GB | 2 タスク × 2 AZ。Rust の低メモリフットプリント（常駐数十 MB 規模）により JVM 構成比で小さいタスクサイズで同等性能を確保 |
| 本番（ピーク） | 1 vCPU | 2 GB | Auto Scaling 後の最大値 |
| ステージング | 0.25 vCPU | 0.5 GB | コスト最適化 |

> **補足**: Rust アプリケーションはヒープ設定・GC チューニングが不要であり、メモリ上限はタスクサイズのみで管理する。負荷試験の実測値に基づきタスクサイズは見直すが、性能目標（第 2 章）は変更しない。

### 6.2 DB スケーリング

**接続プール設定（sqlx PgPoolOptions）**:

```rust
let pool = PgPoolOptions::new()
    .max_connections(20)                          // タスクあたり最大接続数
    .min_connections(5)                           // 最小アイドル接続数
    .acquire_timeout(Duration::from_secs(3))      // 接続取得タイムアウト
    .idle_timeout(Duration::from_secs(600))       // アイドル接続の解放時間（10 分）
    .max_lifetime(Duration::from_secs(1800))      // 接続の最大生存時間（30 分）
...
| シナリオ | 想定時期 | 対応方針 |
|---|---|---|
| 荷主セルフサービスポータル（Phase 2） | 1 年後 | ROLE_SHIPPER ロール追加、axum-login のバックエンド拡張（OAuth2 / OIDC 対応。openidconnect クレート等） |
| 多言語対応（日本語・英語・中国語） | 2 年後 | fluent / rust-i18n によるメッセージカタログ国際化、URL パスにロケール追加（`/en/`, `/zh/`） |
| マイクロサービス分割 | 3 年後 | ヘキサゴナルアーキテクチャのポート境界を REST/gRPC（tonic）API 境界に昇格、サービス間認証に JWT 追加 |
...
- セッション延長ボタン: クリックで `/keep-alive` エンドポイントを呼び出し tower-sessions の有効期限をリセット
...
- [ ] sqlx 接続プールの設定が本番環境に適用されていることを確認

**可用性**:

- [ ] ECS ヘルスチェック（`/health`・`/health/ready`）が正しく設定されていることを確認
- [ ] RDS Multi-AZ フェイルオーバーを実際にテストし、RTO 2 分以内を確認
- [ ] tokio のグレースフルシャットダウン（SIGTERM ハンドリング）が機能することを確認
- [ ] メンテナンス通知の送信フローが確立されていることを確認

**セキュリティ**:

- [ ] HTTPS 強制が ALB で設定されていることを確認
- [ ] すべての HTTP セキュリティヘッダーがミドルウェアで設定されていることを確認
- [ ] AWS Secrets Manager 経由での DB 接続が機能していることを確認
- [ ] CSRF 保護が有効であることを確認（フォームおよび htmx リクエスト）
- [ ] セッション Cookie の `Secure` / `HttpOnly` / `SameSite` 属性を確認
- [ ] OWASP Top 10 対応のレビューが完了していることを確認
- [ ] cargo-audit / cargo-deny の検査がゼロ違反であることを確認

**保守性**:

- [ ] tracing の JSON 構造化ログが CloudWatch Logs に出力されていることを確認
- [ ] CloudWatch アラートが適切に通知されることを確認（テスト通知）
- [ ] SonarQube Quality Gate がパスしていることを確認
- [ ] cargo-llvm-cov のテストカバレッジが目標値を達成していることを確認
- [ ] `cargo clippy -- -D warnings` / `cargo fmt --check` がパスしていることを確認

**拡張性**:

- [ ] ECS Auto Scaling のトリガーが設定されていることを確認
- [ ] sqlx の接続プール設定が本番環境に適用されていることを確認
