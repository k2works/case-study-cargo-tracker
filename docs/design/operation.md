...
本ドキュメントは、7 マイクロサービス + React SPA で構成される国際貨物輸送管理システムの運用要件を定義する。AWS ECS (Fargate) 上での運用を前提とし、自動化を基本方針とする。
...
- **ゼロダウンタイムデプロイ**: Blue/Green デプロイメントでサービス停止なくリリースする
...
| 水曜 | セキュリティパッチ確認 | Dependabot アラート + ECR イメージスキャン |
...
| 認証失敗率 | auth_login_failed_total | > 10 件/5 分で Warning | 1 分 |
...
| S3 監査ログ | バージョニング + ライフサイクルポリシー | 即時 | 1 年 | 0 |
| Terraform 状態 | S3 + DynamoDB ロック | 変更時 | 無期限 | 0 |
| Docker イメージ | ECR | ビルド時 | 直近 10 世代 | - |

### 3.2 バックアップ対象の DB 一覧

| DB 名 | サービス | 重要度 | バックアップ方式 |
| :--- | :--- | :--- | :--- |
| `auth_db` | authms | 高 | 自動スナップショット + PITR |
| `booking_db` | bookingms | 最高 | 自動スナップショット + PITR |
| `routing_db` | routingms | 中 | 自動スナップショット + PITR |
| `tracking_db` | trackingms | 高 | 自動スナップショット + PITR |
| `handling_db` | handlingms | 高 | 自動スナップショット + PITR |
| `billing_db` | billingms | 最高 | 自動スナップショット + PITR |
...
if (受入テスト OK?) then (yes)
  :4. リリース前 DB スナップショット取得;
  :5. リリース承認（チームリーダー）;

  if (DB マイグレーションあり?) then (yes)
    :6a. Flyway マイグレーション実行;
    :6b. マイグレーション結果確認;
  endif
...
| ロールバック対象 | 手順 | 所要時間 |
| :--- | :--- | :--- |
| アプリケーション | ALB ターゲットグループを Blue 環境に切り戻し | 1 分 |
...
| 本番 DB 直接アクセス | SSH トンネル経由、DBA のみ | 操作ログ記録必須 |
...
| アプリケーションロール | 四半期 | `ROLE_ADMIN` / `ROLE_OPERATOR` の適切性確認 |
...
| DB 管理 | pgAdmin / DBeaver | DB 操作（SSH トンネル経由） |
