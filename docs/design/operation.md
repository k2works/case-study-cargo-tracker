...
| ALB | HTTPS:443 → ECS:8080（Node.js 24 / NestJS 11） |
...
| Node.js イベントループ遅延 | カスタムメトリクス | 100ms | 500ms | アプリ調査（同期処理のブロック） |
...
> アプリケーションログは pino による JSON 構造化ログを標準出力へ書き出し、ECS の awslogs ログドライバーで CloudWatch Logs に転送する。

#### ログ検索例

```bash
# 直近 1 時間の error ログを抽出（pino の level フィールドで判定）
...
aws logs filter-log-events \
  ...
  --filter-pattern "duration: [d>=1000, ...]" \
  --start-time $(($(date +%s%3N) - 86400000))
```
...
- **Node.js ランタイム**: イベントループ遅延・ヒープ使用量（カスタムメトリクス）
...
> **Node.js 特有の確認ポイント**: タスクが OOM で停止した場合は `--max-old-space-size` の設定値とコンテナメモリの割り当てを確認する。未捕捉の Promise reject（`unhandledRejection`）でプロセスが落ちるケースもあるため、アプリケーションログで該当スタックトレースを確認する。
...
> **注記**: 本プロジェクトのマイグレーションは node-pg-migrate（`npm run migrate up` / `npm run migrate down`）で管理する。ただし本番環境では `migrate down` による破壊的なダウンマイグレーションは原則使用せず、以下の **Forward マイグレーション方式** を採用することで、旧コードと新スキーマの共存を保証する。
...
| フェーズ | 内容 | マイグレーション |
|---|---|---|
| **Expand** | 新しいカラム・テーブルを追加（旧コードと共存可能な形で） | `*-Expand*.ts` |
| **Migrate** | データ移行・新コードへの切り替え | アプリケーションコードの更新 |
| **Contract** | 旧カラム・テーブルを削除（新コード完全切り替え後） | `*-Contract*.ts` |
...
| 対象 | 頻度 | 方法 | 承認フロー |
|---|---|---|---|
| アプリケーション依存ライブラリ（npm） | 月次（Critical は即時） | Dependabot PR + `npm audit` + CI | 通常リリースフロー |
| Docker ベースイメージ（node:24-slim） | 月次 | ECR 再ビルド + デプロイ | 通常リリースフロー |
| RDS マイナーバージョン | 四半期 | AWS コンソール / CLI | インフラ変更フロー |
| RDS メジャーバージョン | 年次（EOL 前） | Blue/Green デプロイ | 重大インフラ変更フロー |
| Node.js LTS メジャーバージョン | 年次（LTS EOL 前） | ベースイメージ更新 + 回帰テスト | 重大インフラ変更フロー |
...
> Node.js はシングルスレッドのイベントループモデルであるため、CPU バウンドな処理でイベントループが詰まる場合は縦方向（vCPU 増強）よりも横方向（タスク数追加）のスケールを優先する。
...
| イベントループ遅延 | Node.js のシングルスレッドイベントループがタスクを処理するまでの遅延。同期的なブロッキング処理の指標 |
