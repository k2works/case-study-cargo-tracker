...
| 頻度 | 担当 | 手順 |
| :--- | :--- | :--- |
| 毎営業日 09:15 | 経路設計者 | 貨物予約一覧（`/bookings`）を開き、**経路設計済（引き渡し済）の予約**を確認する。着手するものは予約詳細から [経路を割り当てる] を開く |
| 毎営業日 09:30 | 営業担当者 | 貨物予約一覧を開き、**自分が引き渡した予約**のうち経路設計済（引き渡し済）のまま前営業日以前から動いていないものを確認する |
...
> 詳細から経路割り当てへ辿れる）。
>
> **状態での絞り込みは依然として無い**。一覧の絞り込みは**予約番号・出発地・仕向地の
> 文字一致だけ**であり（荷主名は列に出るが検索対象ではない。IT6 レビュー M4）、
> 経路設計済の予約だけを抽出できない。
>
> **この手順は未着手の予約が 20 件を超えたあたりで成立しなくなる**。
> 全件を目で見て状態バッジを拾う運用だからである（IT7 レビュー・user-rep）。
> US12（確定経路の通知）と併せて IT8 で必ず入れる。
> 営業側の手順を残すのは、**この限界があるため経路設計者側だけでは取りこぼしうる**からである。
