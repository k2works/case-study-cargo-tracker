---
title: 運用要件定義 - 国際貨物輸送管理システム（Go 版）
description: 国際貨物輸送管理システム（Go 実装）の運用フロー、監視設計、バックアップ・リカバリ、障害対応、変更管理、セキュリティ運用、キャパシティ管理を定義する。
published: true
date: 2026-07-10T00:00:00.000Z
tags: design, operation, monitoring, backup, incident, sre, go, slog, distroless
---

# 運用要件定義 - 国際貨物輸送管理システム（Go 版）

## 1. 概要

### 1.1 目的

本ドキュメントは、国際貨物輸送管理システム（Go 実装）の本番稼働後における運用手順・方針を定義する。運用設計の目標は以下のとおりである。

- **自動化優先**: 繰り返し発生する運用タスクはすべてスクリプト化・自動化し、人的ミスを排除する
- **IaC 管理**: インフラリソースはすべて Terraform で管理し、手動操作による構成ドリフトを防止する
- **観測可能性（Observability）の確保**: CloudWatch によるメトリクス・ログ・アラートを統合し、問題を早期に検知する。アプリケーションは `log/slog` による JSON 構造化ログと Go ランタイムメトリクスを提供する
...
| 障害種別 | RTO | RPO |
|---|---|---|
| ECS タスク異常終了 | 5 分（Auto Recovery。Go バイナリは秒未満で起動するため実復旧はさらに短い） | 0（ステートレス） |
...
| コンテナイメージ | 静的リンク Go バイナリ + distroless（`gcr.io/distroless/static`）。イメージサイズ数十 MB・起動 1 秒未満 |
| RDS | PostgreSQL 16 Multi-AZ（`db.t3.medium`、自動バックアップ 7 日） |
| ALB | HTTPS:443 → ECS:8080（ヘルスチェックパス: `/healthz`） |
| 監視 | CloudWatch Logs・Metrics・Alarms・Dashboard（Go ランタイムメトリクスはカスタムメトリクスまたは prometheus/client_golang で提供） |
...
- [ ] Go ツールチェーン・依存モジュールのセキュリティパッチ確認（`govulncheck`）と適用計画策定（→ 変更管理フローへ）
...
- [ ] Go メジャーバージョンのサポート状況確認（直近 2 バージョンのみサポートのため年次で更新計画を策定）
...
JVM のヒープ・GC 監視に相当するものとして、Go ランタイムメトリクスを監視する。アプリケーションは `prometheus/client_golang` の `collectors.NewGoCollector()` により `/metrics` エンドポイントで公開し、CloudWatch エージェント（Prometheus スクレイプ）またはアプリからの `PutMetricData` で CloudWatch カスタムメトリクス（名前空間 `CargoTracker/Go`）に集約する。

| メトリクス | Prometheus 名 | Warning | Critical | 対応 |
|---|---|---|---|---|
| goroutine 数 | `go_goroutines` | 5,000 | 10,000 | goroutine リーク調査（pprof） |
| GC pause p99 | `go_gc_duration_seconds` | 10ms | 100ms | GOGC / GOMEMLIMIT 調整 |
| ヒープ使用量 | `go_memstats_heap_alloc_bytes` | コンテナメモリの 70% | 85% | メモリリーク調査・スケール |
| GC 頻度 | `go_gc_cycles_total`（増加率） | 10 回/秒 | 30 回/秒 | アロケーション削減・GOGC 調整 |
| DB 接続プール使用率 | `sql.DBStats`（カスタム） | 70% | 90% | `SetMaxOpenConns` 調整 |

**運用上の推奨設定**:

- `GOMEMLIMIT` をコンテナメモリの 90%（約 920 MiB）に設定し、OOM Kill を防止する
- 障害調査用に `net/http/pprof` を内部管理ポート（外部非公開）で有効化する
...
| 操作 | p95 目標 | 計測ポイント |
|---|---|---|
| 貨物追跡検索 | 200ms | ALB アクセスログ + アプリメトリクス |
| 荷役作業登録 | 500ms | ALB アクセスログ + アプリメトリクス |
| 請求書生成 | 1,500ms | ALB アクセスログ + アプリメトリクス |
...
# goroutine 数 Critical アラーム作成（Go ランタイムカスタムメトリクス）
aws cloudwatch put-metric-alarm \
  --alarm-name "cargo-tracker-goroutines-critical" \
  --alarm-description "goroutine 数が 10,000 を超過（リーク疑い）" \
  --namespace "CargoTracker/Go" \
  --metric-name "GoGoroutines" \
  --statistic Maximum \
  --period 60 \
  --evaluation-periods 3 \
  --threshold 10000 \
...
アプリケーションログは `log/slog` の `JSONHandler` による JSON 構造化ログとして標準出力へ出力し、ECS の `awslogs` ドライバ経由で CloudWatch Logs へ集約する（Logback 相当の設定ファイルは不要）。

**ログ共通フィールド**:

| フィールド | 内容 |
|---|---|
| `time` | RFC 3339 タイムスタンプ |
| `level` | `DEBUG` / `INFO` / `WARN` / `ERROR` |
| `msg` | メッセージ本文 |
| `trace_id` | リクエスト単位のトレース ID（ミドルウェアで付与） |
| `component` | ドメイン / ハンドラ等の発生箇所 |
...
|---|---|---|---|
| アプリケーション（slog JSON） | `/ecs/cargo-tracker` | 30 日 | デバッグ・障害調査 |
...
slog の JSON 出力により、CloudWatch Logs の JSON フィルターパターンがそのまま利用できる。

```bash
# 直近 1 時間の ERROR ログを抽出（slog JSON の level フィールドで絞り込み）
...
Spring Boot Actuator の代替として、アプリケーション自身が以下のエンドポイントを実装する。

| エンドポイント | 用途 | チェック内容 |
|---|---|---|
| `/healthz` | ALB ヘルスチェック / Liveness | プロセス生存確認（常に 200） |
| `/readyz` | Readiness（デプロイ時の切替判定） | DB 接続（`db.PingContext`）等の依存確認 |
| `/metrics` | メトリクス公開（内部のみ） | prometheus/client_golang によるランタイム・アプリメトリクス |

ALB ターゲットグループのヘルスチェックは `GET /healthz`（間隔 15 秒、healthy 閾値 2、unhealthy 閾値 3）とする。

### 3.6 CloudWatch ダッシュボード
...
- **Go ランタイム**: goroutine 数 / GC pause / ヒープ使用量の時系列グラフ
...
- ALB ヘルスチェック（`/healthz`）失敗率が 50% を超過
...
# Step 3: アプリケーションログ確認（直近 100 行、slog JSON）
...
# Step 5: 新タスクが正常起動するまで待機（Go バイナリは起動 1 秒未満のため通常 1〜2 分で安定）
...
ECS はタスク停止時に SIGTERM を送信し、`stopTimeout`（30 秒）経過後に SIGKILL する。アプリケーションは SIGTERM を受けて `http.Server.Shutdown` により処理中リクエストを完了してから終了する。

```go
// main.go でのグレースフルシャットダウン実装
...
タイムアウト（25 秒）は ECS の `stopTimeout`（30 秒）および ALB の deregistration delay より短く設定し、リクエスト欠損なくローリング更新できるようにする。
...
**補足**: `database/sql` の接続プールに `SetConnMaxLifetime`（例: 5 分）を設定し、フェイルオーバー後の古い接続が自動的に破棄・再確立されるようにする。
...
:コードレビュー + CI チェック\n（go vet / golangci-lint / go test / govulncheck）;

if (CI 全パス + レビュー承認?) then (Yes)
  :main ブランチへマージ;
  :GitHub Actions が自動起動;
  :静的リンク Go バイナリをビルド\n（CGO_ENABLED=0）;
  :distroless イメージビルド・ECR プッシュ;
  :ECS ステージング自動デプロイ\n（Rolling Update）;
  :ステージング環境での動作確認\n（自動 E2E テスト + 手動確認）;
  ...
  if (ステージング確認 OK?) then (Yes)
    ...
    if (本番デプロイ承認?) then (Yes)
      ...
      :HealthCheck（/healthz）通過後にロールアウト完了;
      :デプロイ後動作確認\n（5 分間のメトリクス確認）;
...
静的リンクバイナリ + distroless によるマルチステージビルドを標準とする。
...
RUN go mod download
COPY . .
RUN CGO_ENABLED=0 GOOS=linux go build -ldflags="-s -w" -o /cargo-tracker ./cmd/server

# 実行ステージ（distroless: シェル・パッケージマネージャなし）
FROM gcr.io/distroless/static:nonroot
COPY --from=build /cargo-tracker /cargo-tracker
USER nonroot
ENTRYPOINT ["/cargo-tracker"]
```

**運用上の効果**:

- イメージサイズが数十 MB となり、ECR プッシュ・Fargate プル時間が短縮される
- distroless により攻撃面が最小化される（シェルが存在しないため ECS Exec でのデバッグは debug タグイメージへの一時切替が必要）
- 起動 1 秒未満のため、ローリングデプロイのタスク入替とドレインが高速化する
...
# Step 4: デプロイ完了を確認（起動が高速なため通常数分で安定）
aws ecs wait services-stable \
  --cluster cargo-tracker-cluster \
  --services cargo-tracker-service
...
# Step 5: ロールバック後の動作確認
curl -f https://<ALB-DOMAIN>/healthz
```

#### DB マイグレーションのロールバック（Forward マイグレーション方式）

> **注記**: マイグレーションツールには `golang-migrate` を使用する。`down` マイグレーション（`{n}_{説明}.down.sql`）は本番では原則使用せず、以下の **Forward マイグレーション方式** を採用する。
...
-- 000003_add_cargo_status_column.up.sql  （本番反映済み）
ALTER TABLE cargos ADD COLUMN status VARCHAR(50) NOT NULL DEFAULT 'PRELIMINARY';

-- 000004_remove_cargo_status_column.up.sql  （ロールバック相当の forward マイグレーション）
...
| フェーズ | 内容 | マイグレーション |
|---|---|---|
| **Expand** | 新しいカラム・テーブルを追加（旧コードと共存可能な形で） | `{n}_expand_*.up.sql` |
| **Migrate** | データ移行・新コードへの切り替え | アプリケーションコードの更新 |
| **Contract** | 旧カラム・テーブルを削除（新コード完全切り替え後） | `{n}_contract_*.up.sql` |
...
# 注: distroless イメージにはシェルが含まれないため、
# debug タグ付きイメージ（gcr.io/distroless/static:debug-nonroot）への
# 一時切替、または pprof / slog による無停止調査を優先する
...
| 対象 | 頻度 | 方法 | 承認フロー |
|---|---|---|---|
| Go モジュール依存 | 月次（Critical は即時） | Dependabot PR + CI（govulncheck） | 通常リリースフロー |
| Go ツールチェーン | パッチリリース時（月次確認） | go.mod の toolchain 更新 + CI 再ビルド | 通常リリースフロー |
| distroless ベースイメージ | 月次 | ECR 再ビルド + デプロイ（更新頻度・脆弱性混入が少なく作業は軽量） | 通常リリースフロー |
...
Go バイナリは起動が高速（1 秒未満）なため、スケールアウト時に新タスクがトラフィックを受け始めるまでの時間は主に Fargate のタスクプロビジョニングと ALB ヘルスチェック間隔で決まる。
...
- RDS 接続数の推移（上限の 70% を超えたら `SetMaxOpenConns` を含む接続プール設定を見直し）
- RDS ストレージ使用量（80% を超えたらストレージ拡張を計画）
- Go ランタイムメトリクスの推移（goroutine 数・ヒープ使用量のベースライン変化を確認）
...
| distroless 軽量イメージ | ECR ストレージ・転送コスト削減（JVM イメージ比 ~90% 減） | 採用済み |
| Go の低メモリフットプリント | タスクサイズ縮小余地（CPU 256 / メモリ 512 MB への削減を実測後に検討） | 月次レビュー時 |
...
- [ ] Go ランタイムメトリクス確認（goroutine / GC pause / ヒープ）
...
| slog | Go 標準ライブラリ `log/slog`。構造化ログ（JSON）を出力する |
| distroless | シェル・パッケージマネージャを含まない最小コンテナイメージ |
| GOMEMLIMIT | Go ランタイムのソフトメモリ上限。コンテナ OOM を防止する |
| pprof | Go 標準のプロファイリングツール。CPU・ヒープ・goroutine を解析する |
| govulncheck | Go 公式の脆弱性スキャナ。依存モジュールの既知脆弱性を検出する |
