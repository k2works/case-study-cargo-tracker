...
- **SLA/SLO 遵守**: `non_functional.md` §3.1 に定める SLA / SLO を満たす状態を維持する

### 1.2 SLA / SLO

**`non_functional.md` §3.1 を正典とする。** 本ドキュメントは独自の値を持たない。

リリースごとの目標値は `docs/development/release_scope.md`「リリース別の非機能目標」を参照すること。`non_functional.md` の値は**最終形**であり、初回リリース要件ではない。

### 1.3 RTO / RPO

**`non_functional.md` §3.2 を正典とする。** 本ドキュメントは独自の値を持たない。

> 旧版は本節に独自の RTO / RPO 表を持ち、AZ 障害（30 分 / 1 時間）・リージョン障害（RPO 24 時間）が `non_functional.md`（10 分 / 0、RPO 1 時間）と食い違っていた。実際の復旧機構に合わせて `non_functional.md` 側を是正したうえで、値の重複定義を解消した。
...
> **本章はシステム基盤運用（ECS・RDS・監視・デプロイ）を扱う。** 業務そのものを回すための運用（未解決例外の棚卸し、通関留置の督促、月次締めなど）は [10. 業務運用](#10-業務運用) に定義する。
>
> **システムが動いていることと、業務が回っていることは別である。** 基盤運用だけを整備しても、誰も未解決例外を見ない運用では貨物は止まったままになる。
...
**アラート閾値は `non_functional.md` §5.2 を正典とする。** 本ドキュメントは閾値を再掲せず、正典に無い項目のみを補う。

> 旧版は本節に独自の閾値表を持ち、ECS メモリ（80%/95% vs 75%/90%）・ALB p95（500ms/1,000ms vs 1,000ms/3,000ms）・RDS 接続数・RDS 空きストレージが `non_functional.md` と食い違っていた。**閾値を 2 箇所で定義すると、どちらのアラートが鳴っているのか運用者が判断できなくなる。**
...
| 監査ログ | CloudWatch → S3 Export | 月次 | 7 年（`non_functional.md` §4.4 が正典） | Lambda 自動 |
...
| **荷降し（`UNLOAD`）** | 荷役種別の 1 つ。**現場が実際に船から降ろした記録**である |
| **荷降し手配** | 輸送中のキャンセルを承認したときに決まる「ここで降ろせ」という**指示**（US30）。**まだ降ろしていない。** 荷役作業一覧の先頭に出し、荷降し（`UNLOAD`）を登録すると消える |

> **「荷降し」と「荷降し手配」を混ぜない。** 同じ言葉が記録と指示の両方を指すと、
> 一覧を見た作業員が「もう降ろしたのか」と読む。IT15 で「キャンセル」（予約を取り消す）と
> 「取り消し」（荷役記録を取り消す）を分けたのと同じ扱いである。

---

## 10. 業務運用

システムを使って**業務を回すため**の運用を定義する。9 章までのシステム基盤運用とは担当者も目的も異なる。

### 10.1 業務運用カレンダー

| 頻度 | 実施時刻 | 作業 | 担当ロール | 放置した場合に起きること |
| :--- | :--- | :--- | :--- | :--- |
| 日次 | 始業時 | **未解決例外の棚卸し** — 例外一覧を確認し、担当と対応期限を確定する | ROLE_TRACKER | 遅延・破損の対応が遅れ、荷主からのクレームで初めて発覚する |
| 日次 | 始業時 | **通関留置の督促** — 留置 3 日以上の申告を確認し、通関業者に督促する | ROLE_TRACKER | 保管料が日割りで発生し、月末に想定外のコストとして現れる |
| 日次 | 始業時 | **期限が迫る未割り当て予約の確認** — 希望期限まで 3 日以内で経路未割り当ての予約を確認する | ROLE_ROUTER | 出港便に間に合わず、納期遅延が確定してから気づく |
| 日次 | 終業時 | **当日の荷役実績の確認** — 予定されていた荷役が未登録のまま残っていないか確認する | ROLE_HANDLER | 「作業したのに未登録」が追跡情報の欠落として荷主に見える |
| 週次 | 月曜 | **船会社スケジュール変更の反映**（US25） — 各社の公開スケジュールを確認し、変更を登録する | ROLE_ROUTER | 経路候補が古いスケジュールで算出され、存在しない便に割り当てる |
| 週次 | 金曜 | **誤配・再ルーティング案件の進捗確認** | ROLE_TRACKER | 誤配された貨物が現在地に滞留したまま忘れられる |
| 月次 | 第 1 営業日 | **前月分の請求書発行状況の確認** — 配達完了済みで請求書が未発行の予約が無いか確認する | ROLE_BILLING | 請求漏れがそのまま売上の欠落になる |
| 月次 | 第 1 営業日 | **未入金の督促** — 支払期限超過の請求書を抽出し、荷主に督促する | ROLE_BILLING | 回収が遅れ、滞留債権になる |
| 月次 | 月末 | **月次締め** — 当月の精算を確定する | ROLE_BILLING | 締めが遅れると翌月の請求サイクル全体がずれる |
| 随時 | 発生時 | **輸送中キャンセルの承認**（US30） — 申請を確認し、陸揚げ地を指定して承認または却下する | ROLE_TRACKER | 貨物が船上にあるまま処理が宙に浮く |

### 10.2 ダッシュボードとの対応

**業務運用の各項目は、担当ロールのダッシュボードから 1 クリックで到達できる。** 手順書に「例外一覧を確認する」と書いても、そこへ辿り着く導線が無ければ実施されない。

| 業務運用項目 | ダッシュボードのカード | 遷移先 |
| :--- | :--- | :--- |
| 未解決例外の棚卸し | 未解決の例外 | `/tracking/exceptions?resolved=false` |
| 通関留置の督促 | 通関で 3 日以上留置中 | `/handling/customs?status=HELD&days=3` |
| 期限が迫る未割り当て予約 | 経路割り当て待ち | `/routing/queue` |
| 当日の荷役実績の確認 | 本日の自分の荷役実績 | `/handling?operator=me&date=today` |
| 未入金の督促 | 支払期限超過 | `/billing/invoices?status=OVERDUE` |
| 誤配案件の進捗確認 | 誤配 | `/bookings?routing=MISROUTED` |

### 10.3 業務運用の記録

- 日次・週次の確認結果は記録を残す。**「確認した」と「確認したことを記録した」は別である。** 記録が無いと、後から「あの日は誰も見ていなかった」ことを立証も反証もできない
- 対応が必要と判断した案件は、例外イベントまたは予約に対応内容を入力する。個人のメモや口頭連絡で完結させない
- 未実施が 3 営業日続いた項目は、担い手または頻度の見直しを検討する。**実施されない運用手順は、あることが害になる**（「やっているつもり」を生む）
