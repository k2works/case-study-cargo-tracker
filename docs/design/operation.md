---
title: 運用要件定義 - 国際貨物輸送管理システム（F# 版）
description: 国際貨物輸送管理システム（F# 9 + Giraffe 版）の運用フロー、監視設計、バックアップ・リカバリ、障害対応、変更管理、セキュリティ運用、キャパシティ管理を定義する。
published: true
date: 2026-07-06T00:00:00.000Z
tags: design, operation, monitoring, backup, incident, sre, fsharp
---
...
本ドキュメントは、国際貨物輸送管理システム（F# 9 + .NET 10 / Giraffe 版）の本番稼働後における運用手順・方針を定義します。運用設計の目標は以下のとおりです。

- **自動化優先**: 繰り返し発生する運用タスクはすべてスクリプト化・自動化し、人的ミスを排除します
- **IaC 管理**: インフラリソースはすべて Terraform で管理し、手動操作による構成ドリフトを防止します
- **観測可能性（Observability）の確保**: CloudWatch によるメトリクス・ログ・アラートを統合し、問題を早期に検知します
- **SLA/SLO 遵守**: 合意した SLA（99.9%）および内部 SLO（99.95%）を常に満たす状態を維持します
...
| ALB | HTTPS:443 → ECS:8080（Giraffe / Kestrel、F# 9 + .NET 10 LTS） |
| 監視 | CloudWatch Logs・Metrics・Alarms・Dashboard |
| ネットワーク | VPC 10.0.0.0/16（ap-northeast-1a / ap-northeast-1c） |

> **補足**: アプリケーションは Giraffe + Giraffe.ViewEngine + htmx で構成し、データアクセスには Donald、スキーマ管理には DbUp を使用します。開発環境は SQLite、本番環境は PostgreSQL 16 を使用します。本ドキュメントの運用手順は本番環境（PostgreSQL 16 / AWS）を対象とします。
...
> **補足**: ECS コンテナのヘルスチェックおよび ALB のヘルスチェックには、ASP.NET Core Health Checks ミドルウェアを Giraffe パイプラインに組み込んで公開する `/health` エンドポイントを使用します。DB 接続（Donald 経由の `SELECT 1` チェック）を含む詳細チェックは `/health/ready`、プロセス生存確認は `/health/live` に分離します。.NET ランタイムの詳細メトリクス（GC ヒープサイズ、ThreadPool キュー長、Gen 0/1/2 コレクション数など）は EventCounters を OpenTelemetry 経由で CloudWatch にエクスポートし、障害調査時には ECS Exec 上で `dotnet-counters` による即時確認も可能とします。

```fsharp
// Giraffe でのヘルスチェックエンドポイント構成例（Program.fs）
let webApp: HttpHandler =
    choose [
        route "/health"       >=> Successful.OK "Healthy"          // ALB 用（軽量）
        route "/health/live"  >=> Successful.OK "Alive"            // プロセス生存確認
        route "/health/ready" >=> HealthHandlers.readiness         // DB 接続を含む詳細チェック
        // ... アプリケーションルート
    ]

// HealthHandlers.fs: Donald による DB 接続チェック
let readiness: HttpHandler =
    fun next ctx ->
        task {
            let factory = ctx.GetService<DbConnectionFactory>()
            ...
            return!
                match result with
                | 1 -> Successful.OK "Ready" next ctx
                | _ -> ServerErrors.SERVICE_UNAVAILABLE "Not Ready" next ctx
        }
```
...
アプリケーションログは Serilog による JSON 構造化ログとして標準出力に出力し、ECS の awslogs ドライバー経由で CloudWatch Logs に転送します。全ログにトレース ID（`traceId`）を付与し、リクエスト単位の追跡を可能にします。Serilog は F# からもそのまま利用でき、Giraffe の HttpHandler パイプラインにリクエストロギングミドルウェアとして組み込みます。

| ログ種別 | CloudWatch ロググループ | 保持期間 | 用途 |
|---|---|---|---|
| アプリケーション（Serilog JSON） | `/ecs/cargo-tracker` | 30 日 | デバッグ・障害調査 |
...
```bash
# 直近 1 時間の Error レベルログを抽出（Serilog の Level フィールド）
...
:コードレビュー + CI チェック\n（Fantomas / FSharpLint / dotnet test / セキュリティスキャン）;
...
**CI チェックコマンド（dotnet CLI + Fantomas + FSharpLint）**:
...
> **注記**: 本プロジェクトのスキーマ管理には DbUp（バージョン付き SQL スクリプト）を使用します。DbUp は forward-only 設計であり Down マイグレーションを持たないため、ロールバックは以下の **Forward マイグレーション方式** で対応します。開発環境（SQLite）と本番環境（PostgreSQL 16）で同一のマイグレーション運用を適用しますが、本番反映前に必ずステージング（PostgreSQL）で適用検証を行います。

**Forward マイグレーション方式**:

スキーマ変更のロールバックは「新しいマイグレーションで元の状態に戻す」ことで実現します。

```bash
# 例: カラム追加のロールバックは新しいマイグレーションスクリプトでカラムを削除する

# Scripts/0042_add_cargo_status_column.sql（本番反映済み）
#   → cargos テーブルに status カラム（NOT NULL DEFAULT 'PRELIMINARY'）を追加

# Scripts/0043_remove_cargo_status_column.sql（ロールバック相当の forward マイグレーション）
#   → ALTER TABLE cargos DROP COLUMN status;
#   → DbUp が journal テーブル（schemaversions）に適用履歴を記録する
```
...
| フェーズ | 内容 | マイグレーション |
|---|---|---|
| **Expand** | 新しいカラム・テーブルを追加（旧コードと共存可能な形で） | `Expand〜` マイグレーション |
| **Migrate** | データ移行・新コードへの切り替え | アプリケーションコードの更新 |
| **Contract** | 旧カラム・テーブルを削除（新コード完全切り替え後） | `Contract〜` マイグレーション |
...
| 対象 | 頻度 | 方法 | 承認フロー |
|---|---|---|---|
| アプリケーション依存ライブラリ（NuGet: Giraffe / Donald / DbUp 等） | 月次（Critical は即時） | Dependabot PR + CI | 通常リリースフロー |
| Docker ベースイメージ（.NET 10 ランタイム） | 月次 | ECR 再ビルド + デプロイ | 通常リリースフロー |
...
| Health Checks | Giraffe パイプラインで公開するヘルスチェックエンドポイント。`/health`（ALB 用）、`/health/ready`（DB 接続含む）、`/health/live`（生存確認）を公開 |
| EventCounters | .NET ランタイムのメトリクス公開機構。`dotnet-counters` や OpenTelemetry で収集する |
| DbUp | forward-only のバージョン付き SQL スクリプトによるスキーマ管理ツール。適用履歴を `schemaversions` テーブルに記録する |
| Fantomas / FSharpLint | F# コードのフォーマッターと静的解析ツール。CI の品質ゲートで使用する |
