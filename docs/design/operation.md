...
（[リリース計画](../development/release_plan.md) の「将来リリース」。
US10・US12・US20 が該当する）。

通知が無いまま引き渡しを行うと、**「引き渡したのに誰も着手していない」**という
状態が誰にも気付かれずに続く。予約は経路設計済（引き渡し済。内部表現は
`ROUTING_ASSIGNED`）になっているため、
営業担当者からは「渡した」ように見える。
...
| 頻度 | 担当 | 手順 |
| :--- | :--- | :--- |
| 毎営業日 09:15 | 経路設計者 | 貨物予約一覧で**状態を「経路設計中」に絞る**（[`/bookings?status=ROUTE_PROPOSED`](#) をブックマークしてよい）。着手するものは予約詳細から [経路を割り当てる] を開く |
| 毎営業日 09:30 | 営業担当者 | 同じ絞り込みを開き、**自分が引き渡した予約**が前営業日以前から残っていないか確認する。荷主を特定して調べる場合は**荷主名の欄**に社名を入れる |
| 毎営業日 09:45 | 営業担当者 | 動いていない予約があれば、予約番号を添えて経路設計者へ口頭・チャットで伝える |
| 毎営業日 10:00 | 営業担当者 | **経路が提案済みの予約**を確認し、荷主へ提案する。返事が来たものは予約詳細から [予約を確定する] を開き、承認なら確定・変更希望なら差し戻し・見送りならキャンセルする（US13） |

**「経路提案済」は状態の絞り込みでは分けられない**（IT9 時点）。
経路の割り当ては `BookingStatus` を変えず `RoutingStatus` を変えるため、
`status=ROUTE_PROPOSED` には**割り当て前と割り当て済みの両方**が入る。
一覧の状態列で「経路設計中」と「経路提案済」を読み分ける。

> **経路状態での絞り込みは IT10 以降で入れる**。IT9 で `RoutingStatus` を
> 永続化したため技術的には可能だが、**絞り込みの軸を増やすのは
> 運用が回り始めてから決めるほうがよい**——どの軸が実際に使われるかは、
> 使ってみないと分からない。09:15 と 10:00 の手順が
> 「状態列を目で追う」ことで回らなくなった時点で入れる。

絞り込みは URL に残る。**状態と荷主名はブックマークできる**ため、毎朝
同じ条件を開き直す手間はない（`GET` のフォームである）。

> **経路設計者の手順を IT7 で復活させた**（IT6 レビュー H10 の解消）。
>
> IT6 当初は「経路設計者が貨物予約一覧を状態で絞り込む」と書いたが、
> **実行できなかった**。貨物予約一覧（`/bookings`）は営業担当者と荷主にしか
> 開いておらず、経路設計者は画面そのものに到達できなかった。
> **書いた手順が実行できるかを確かめずに運用へ載せると、安全網があるつもりで
> 何も見ていない状態になる**——そのため IT6 では担当を営業へ一本化した。
>
> IT7 で経路割り当て（US08）を実装するにあたり、`/bookings` と
> `/bookings/{bookingId}` を経路設計者へ開いた（閲覧のみ。登録・引き渡しは
> 営業のまま）。ダッシュボードと navbar にも「貨物予約」が出る。
> **手順が実行できることは `RouteAssignHttpTest.testRouterReachesAssignmentFromBookingDetail`
> が検証している**（ダッシュボードに入口がある → 一覧が開ける →
> 詳細から経路割り当てへ辿れる）。
>
> **状態と荷主名での絞り込みを IT8 で入れた**（IT7 レビュー M14・IT6 レビュー M4 の解消）。
>
> IT7 までの絞り込みは**予約番号・出発地・仕向地の文字一致だけ**で、
> 経路設計済の予約だけを抽出できなかった。**全件を目で見て状態バッジを拾う運用**は
> 未着手が 20 件を超えたあたりで成立しなくなる（IT7 レビュー・user-rep）。
> 荷主名も列に出るだけで検索対象ではなく、荷主からの電話に応えられなかった。
>
> IT8 で状態の選択欄と荷主名の欄を足し、**上の手順はそのまま実行できる**。
> 実行できることは `BookingHttpTest.testFiltersBookingListByStatusAndShipperName`
> が検証している（状態で絞る → 1 件だけ残る → 荷主名と AND で重なる）。
> 状態の選択肢は `BookingModel.allStatuses()` から組み立てており、
> **状態を足したときに絞り込みの選択肢からだけ漏れることはない**。
>
> 営業側の手順を残すのは、**経路設計者が朝の確認を飛ばした日に誰も気付かない**からである。
> これは絞り込みの有無とは別の理由による。

**この手順は暫定である。** 人が見に行くことに依存する運用は、忙しい日に飛ぶ。
通知機能（US12）を実装した時点で本節を削除する。

> **IT9 で 10:00 の手順を足した**（US13）。予約の確定・差し戻し・キャンセルが
> 実装され、**荷主の返事を受けて操作する担当者が必要になった**ためである。
>
> 受入基準 3（経路設計者への追跡番号発行依頼の通知）と 6（荷主へのキャンセル
> 確認通知）は US12 と同じく将来リリースであり、**この手順が代替である**。
> 担当ロールで実行できることは `BookingConfirmHttpTest` が検証している
> （営業のみが確定でき、経路設計者・荷主は 403）。
