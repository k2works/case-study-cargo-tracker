...
| ALB | HTTPS:443 → ECS:8080（ASP.NET Core / Kestrel） |
...
> **補足**: ECS コンテナのヘルスチェックおよび ALB のヘルスチェックには、ASP.NET Core Health Checks が公開する `/health` エンドポイントを使用する。DB 接続を含む詳細チェックは `/health/ready`、プロセス生存確認は `/health/live` に分離する。.NET ランタイムの詳細メトリクス（GC ヒープサイズ、ThreadPool キュー長、Gen 0/1/2 コレクション数など）は EventCounters を OpenTelemetry 経由で CloudWatch にエクスポートし、障害調査時には ECS Exec 上で `dotnet-counters` による即時確認も可能とする。
...
アプリケーションログは Serilog による JSON 構造化ログとして標準出力に出力し、ECS の awslogs ドライバー経由で CloudWatch Logs に転送する。全ログにトレース ID（`traceId`）を付与し、リクエスト単位の追跡を可能にする。
...
:コードレビュー + CI チェック\n（dotnet format / dotnet test / セキュリティスキャン）;
...
> **注記**: 本プロジェクトのスキーマ管理には DbUp（バージョン付き SQL スクリプト）を使用する。DbUp は forward-only 設計であり Down マイグレーションを持たないため、ロールバックは以下の **Forward マイグレーション方式** で対応する。

**Forward マイグレーション方式**:

スキーマ変更のロールバックは「新しいマイグレーションで元の状態に戻す」ことで実現する。
...
| 対象 | 頻度 | 方法 | 承認フロー |
|---|---|---|---|
| アプリケーション依存ライブラリ（NuGet） | 月次（Critical は即時） | Dependabot PR + CI | 通常リリースフロー |
| Docker ベースイメージ（.NET ランタイム） | 月次 | ECR 再ビルド + デプロイ | 通常リリースフロー |
...
| Health Checks | ASP.NET Core Health Checks。`/health`（ALB 用）、`/health/ready`、`/health/live` を公開 |
