...
> **担当を営業に一本化した理由**（IT6 レビュー H10）: 当初は
> 「経路設計者が貨物予約一覧を状態で絞り込む」と書いたが、**実行できなかった**。
> 貨物予約一覧（`/bookings`）は営業担当者と荷主にしか開いておらず、
> 経路設計者は画面そのものに到達できない。絞り込みも予約番号・出発地・仕向地の
> 文字一致だけで、状態で絞る手段がない。
>
> **書いた手順が実行できるかを確かめずに運用へ載せると、安全網があるつもりで
> 何も見ていない状態になる**。引き渡した側（営業）が追う形なら、
> 今ある画面と権限でそのまま実行できる。
>
> 経路設計者側から追えるようにするには「予約一覧の閲覧権限」と
> 「状態での絞り込み」が要る。US12（確定経路の通知）と併せて IT7 以降で判断する。
