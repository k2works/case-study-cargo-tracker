...
description: 国際貨物輸送管理システム（Rust 版）の運用フロー、監視設計、バックアップ・リカバリ、障害対応、変更管理、セキュリティ運用、キャパシティ管理を定義する。
published: true
date: 2026-07-06T00:00:00.000Z
tags: design, operation, monitoring, backup, incident, sre, rust
---
...
- **観測可能性（Observability）の確保**: CloudWatch によるメトリクス・ログ・アラートを統合し、問題を早期に検知する。アプリケーション側は tracing による JSON 構造化ログと metrics クレートによるメトリクス公開で支える
...
| 障害種別 | RTO | RPO |
|---|---|---|
| ECS タスク異常終了 | 5 分（Auto Recovery。Rust バイナリはサブ秒起動のため実復旧はさらに短い） | 0（ステートレス） |
...
| コンポーネント | 構成 |
|---|---|
| ECS クラスター | `cargo-tracker-cluster`（Fargate、Rust / axum、CPU 256 / メモリ 512 MB、min 2 / max 10） |
| RDS | PostgreSQL 16 Multi-AZ（`db.t3.medium`、自動バックアップ 7 日） |
| ALB | HTTPS:443 → ECS:8080（ヘルスチェック: `/health`） |
...
- [ ] OS・ベースイメージ・依存クレートのセキュリティパッチ確認と適用計画策定（→ 変更管理フローへ）
...
> Rust アプリケーションは GC を持たないため、メモリ使用量は接続数・キャッシュサイズにほぼ比例して推移する。
> メモリ使用率の急増はリークまたは接続プール設定異常のシグナルとして扱い、`metrics` クレートで公開する
> コネクションプール（sqlx `PgPool`）のアクティブ接続数と合わせて調査する。
...
アプリケーションログは `tracing` + `tracing-subscriber`（JSON フォーマッター）で stdout に出力し、
awslogs ドライバー経由で CloudWatch Logs へ転送する。全ログにリクエスト単位の `trace_id` を付与する。
...
# Step 5: 新タスクが正常起動するまで待機
# （Rust バイナリはサブ秒起動のためヘルスチェック通過が律速。通常 1〜2 分）
aws ecs wait services-stable \
  --cluster cargo-tracker-cluster \
  --services cargo-tracker-service
```

> Rust アプリケーションで panic による異常終了が発生した場合、`tracing` の panic フックで
> スタックトレースが ERROR ログとして記録される。`{ $.level = "ERROR" }` フィルターで直前の
> panic メッセージを確認し、根本原因分析に使用する。
...
# （sqlx の PgPool は切断された接続を自動再確立する）
aws ecs describe-services \
  --cluster cargo-tracker-cluster \
  --services cargo-tracker-service \
...
:コードレビュー + CI チェック\n（clippy / rustfmt / cargo test / cargo-audit）;

if (CI 全パス + レビュー承認?) then (Yes)
  :main ブランチへマージ;
  :GitHub Actions が自動起動;
  :Docker イメージビルド（cargo-chef キャッシュ）・ECR プッシュ;
  :ECS ステージング自動デプロイ\n（Rolling Update + sqlx migrate）;
  :ステージング環境での動作確認\n（自動 E2E テスト + 手動確認）;
...
> **注記**: 本プロジェクトのマイグレーションは **sqlx migrate** で管理する。sqlx は
> `migrate add -r` による revert スクリプト（`*.down.sql`）をサポートするが、
> 本番環境で down マイグレーションを直接実行することはデータ損失リスクがあるため原則禁止とし、
> 以下の **Forward マイグレーション方式** を採用する。
...
-- migrations/20260401000003_add_cargo_status_column.sql  （本番反映済み）
ALTER TABLE cargos ADD COLUMN status VARCHAR(50) NOT NULL DEFAULT 'PRELIMINARY';

-- migrations/20260401000004_remove_cargo_status_column.sql  （ロールバック相当の forward マイグレーション）
...
| フェーズ | 内容 | マイグレーション |
|---|---|---|
| **Expand** | 新しいカラム・テーブルを追加（旧コードと共存可能な形で） | `{timestamp}_expand_*.sql` |
| **Migrate** | データ移行・新コードへの切り替え | アプリケーションコードの更新 |
| **Contract** | 旧カラム・テーブルを削除（新コード完全切り替え後） | `{timestamp}_contract_*.sql` |
...
- CI/CD: GitHub Actions OIDC による一時クレデンシャルのみ。長期アクセスキーは発行しない

```bash
# 本番環境の ECS Exec（緊急デバッグ時のみ。承認必須）
# 注意: distroless イメージにはシェルが含まれないため ECS Exec は使用できない。
# 緊急デバッグが必要な場合は debug タグ付きイメージ（debian-slim ベース）に
# 一時的に切り替えてから実行する
...
| 対象 | 頻度 | 方法 | 承認フロー |
|---|---|---|---|
| 依存クレート | 月次（Critical は即時） | Dependabot / cargo-audit（CI 常時実行）+ `cargo update` PR | 通常リリースフロー |
| Rust ツールチェーン | 6 週間ごと（stable リリースに追従） | `rust-toolchain.toml` 更新 + CI 検証 | 通常リリースフロー |
| Docker ベースイメージ（rust:slim / distroless） | 月次 | ECR 再ビルド + デプロイ | 通常リリースフロー |
...
| 脆弱性発覚（Critical） | 24 時間以内にパッチ適用計画を策定（RustSec Advisory の場合は cargo-audit で影響確認） | 開発リード + セキュリティ担当 |
...
> Rust バイナリはウォームアップ不要で起動直後からフルスループットを発揮するため、
> スケールアウト時の新タスクは即座にトラフィックを受けられる。
> JVM のようなウォームアップ待ちを考慮した先行スケールは不要である。
...
- RDS 接続数の推移（上限の 70% を超えたら sqlx `PgPool` の `max_connections` 設定を見直し）
...
| 施策 | 効果 | 実施タイミング |
|---|---|---|
| Rust による小型タスク（CPU 256 / メモリ 512 MB） | JVM 構成比で Fargate コスト約 50% 削減 | 初期構成で適用済み |
...
- [ ] cargo-audit の指摘事項ゼロ（または対応計画策定済み）
- [ ] IAM アクセス権限棚卸完了
...
| cargo-chef | Rust の依存クレートビルドを Docker レイヤーキャッシュ化するツール |
| sqlx migrate | sqlx CLI によるバージョン管理された DB マイグレーション（Flyway 相当） |
| distroless | シェル・パッケージマネージャを含まない最小実行イメージ |
