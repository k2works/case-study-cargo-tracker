...
| BC 間 ACL ポート（`domain-model.md`「BC 間 ACL ポート一覧」が正典） | ユニットテスト（ポートをモック） | 呼び出し側は Mockito でモックし委譲を検証。ポート実装（Adapter）は連携先 BC のサービスへの委譲のみのため、必要に応じて委譲先をモックしたユニットテストで検証 |
...
IT17 の時点で **11 本**ある（走査の支援クラス自体のテスト `SourceScanTest` は本数に数えない）。
...
- **現在の閾値**: 全体行 **75%** / 分岐 **65%**。これは実測カバレッジ（**全体行 96.5% / 分岐 78.9%。計測日: 2026-08-06、IT1 完了時点**）を下回る安全側の値で、「いきなり 85% でビルドが壊れる」事態を避けるための開始点である。**実測値には必ず計測日を併記する。** 日付の無い実測値は、以後どの時点の判断材料にもならない。
- **引き上げ手順**: カバレッジが安定して目標を上回るようになったら、閾値を段階的に引き上げる。レイヤー別目標（§6.1）は**レイヤーごとに分けた `JacocoCoverageVerification` タスク**で強制する（**IT8 で実施済み**）。
  - **`element = 'PACKAGE'` は採らなかった。** パッケージ 1 つずつに閾値を課すと、列挙型 1 個だけのパッケージが分岐 0 件で落ちるなど、**レイヤーの健全性と関係のない理由で赤になる**。レイヤーごとに `classDirectories` を絞ったタスクを分け、レイヤー単位の合計で判定する。
- **手順の目安**: (1) 現在の閾値で緑を維持 → (2) 実測が閾値+5% を安定して超えたら閾値を実測近くまで引き上げ → (3) レイヤー別ルールへ分割。1 度に大きく上げず、実測に追随させる。
- **レイヤー別ルールへの分割**: **IT8 で実施済み**（`development_strategy.md`「品質ゲートの段階的有効化」が定めた時期）。期限は「Release 0.1〜1.0 の最終イテレーション」だったが、**前倒しした**。

| レイヤー | 行（実測 / 閾値 / 目標） | 分岐（実測 / 閾値 / 目標） |
| :--- | :--- | :--- |
| domain | 89.0 / **85** / 85 | 74.9 / **70** / 80（未達） |
| application | 91.3 / **85** / 80 | 64.1 / **60** / 75（未達） |
| interfaces | 92.3 / **85** / 70 | 67.5 / **60** / — |
| infrastructure | 97.6 / **90** / 75 | 81.6 / **75** / — |

  - 実測は **2026-08-08（IT8 タスク 0-3）** の計測値である。
  - **目標値をそのまま閾値にしていない。** 分岐は domain・application とも目標に届いておらず、目標を閾値にすると「閾値を満たすためのテストを書く」ことになる。差が埋まった時点で引き上げる。
  - **閾値が働くことを確認済み**: domain の分岐を 0.99 に上げると 0.74 で落ち、infrastructure の行を 0.99 に上げると 0.97 で落ちる。**レイヤーごとに違う数字が出る**ことをもって、絞り込みが効いていると判断した。
...
> **負荷試験は Release 0.1〜1.0 で追跡 API に 1 本だけ実施する**（`release_scope.md`）。統合テストで性能を測らない。Testcontainers 上の測定値は本番構成を代表しないため、性能の判断材料にならない。
...
| 対象 | どこで確かめるか | 理由 |
| :--- | :--- | :--- |
| 丸め規則・段階丸め | `MoneyTest`（単体） | **設計書の計算例をそのままテストにする。** 実装より先に例を決める — 実装で決めると、例が実装の写しになって検算にならない |
| 順序を固定する必要性 | `MoneyTest`（単体） | **段階丸めと一括丸めがずれる値を明示する。** 設計書の計算例（100,003 円）では差が出ないため、ずれる値（100,007 円）を探して固定した。ずれないなら順序を決める必要が無い |
| 割引の適用可否 | `DiscountPolicyTest`（単体） | 個人荷主にも率 0% の道を通すことを確かめる。**飛ばす実装では請求書の形が 2 種類できる** |
...
> 「押したのに何も起きない」形は、画面にもサーバログにも痕跡が残らない（IT13 で実際に踏んだ）。
