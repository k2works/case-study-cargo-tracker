...
`shared` の testFixtures にある `DialectSmoke` を使い、各サービスに `DialectSmokeTest` を置く。

- MyBatis に登録された**全ステートメント**を列挙し、PostgreSQL（Testcontainers）と H2 の両方で
  `prepare` する。実行はしない。結果の正しさではなく、構文・関数・型がその DB で理解されるかを見る
- 「1 つも SQL が集まっていない場合は落とす」アサートを必ず置く。集まらないまま緑になると、
  この検査は何も守らない
- `prepare` に加えて `getParameterMetaData()` まで呼ぶ。構文は通るがパラメータの型が決まらない
  書き方（`#{keyword} IS NULL` 等）は、prepare だけでは通ってしまい実行時に初めて落ちる。
  IT1 で実際にこの形の欠陥が本番相当の DB でのみ落ち、prepare だけの検査は素通りした
- **違反例で赤になることを確認してから採用する**。ただし H2 の PostgreSQL 互換モードは想像より
  多くを受け入れる。IT3 で確かめたところ、`~`（正規表現マッチ）だけでなく **`DISTINCT ON`・`ILIKE`・
  `TO_CHAR` も H2 が prepare を通す**（doc に「違反例になる」と書いてあった `DISTINCT ON` は、もはや
  違反例ではない）。**存在しないテーブル名・列名**なら両方の DB が確実に落ちるため、検査が働いている
...
**公開経路の観点（IT8 で確立）**: 認証の外にある経路は、認証済みの経路とは別の壊れ方をする。
追跡番号を手に入れた誰もが叩けるため、次の 4 つを検証対象として明示する。

| 観点 | 検証すること | なぜ |
| :--- | :--- | :--- |
| **返さないものを型に持たない** | 公開応答に予約番号・荷主名・作業者・航海番号・例外の詳細が現れない | 「画面に出していない」は守りではない。応答に載っていれば誰でも読める |
| **上限が働く** | 同じフィルタの実体のまま時計を進め、窓がリセットされることと、窓の記録が無制限に増えないことを対で見る | 新しい実体を作って確かめると、リセットの実装を消しても緑になる。総当たりを防ぐ仕組みが別の枯渇経路になっては意味がない |
| **詐称された値を採らない** | `X-Forwarded-For` の**末尾**を採る。先頭を採る実装に戻すと赤になる | 各ホップは末尾に追記するため、先頭はクライアントが送った文字列そのもの。毎回変えるだけで上限を回避できる |
| **ヘルスチェックを除外する** | 上限や排他をかけた経路が、liveness / readiness を巻き込まない | 一律に適用すると、過負荷時にヘルスチェックが弾かれて再起動ループになる |

**照会の記録は成否に関わらず残す**（[ADR-024](../adr/024-tracking-manual-update-and-exceptions.md) 決定 7）。
見つからなかった照会こそ総当たりを見つける材料であり、「見つかったものだけ記録する」実装に
戻すと赤になる形にする。
...
#### SonarQube の指摘に対する方針（IT1 で確定）

Bug・Vulnerability は 0 を維持する。Code Smell のうち、次の指摘は**業務上の理由から残す**。
残す判断を毎回やり直さないため、方針をここに置く。
...
| 対象 | 理由 |
| :--- | :--- |
| `**/SchemaErdDocument.java` | テストではなく ER 図の生成タスク。通常のテスト実行からも除外している |
| フロントの `src/mocks/**` | 動作確認用のモックであり、本番の成果物に含まれない（ADR-006） |
