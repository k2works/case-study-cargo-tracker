...
| JS の静的解析 | Rating A | `ops/scripts/**`（`arch-lint` を含む） | SonarQube（**Flix コードは解析できない**。下記参照） |
...
アプリケーション本体の品質ゲートとしては機能しない。設定（`sonar-project.properties`）の
スキャン対象は **Node.js の運用スクリプト**に限る。
...
| 統制 | 対象 |
| :--- | :--- |
| `arch-lint`（規約 10 件 + メタテスト） | アーキテクチャ規約の違反 |
...
> IT3 で設定を用意した。実行には SonarQube のトークン（`.env` の `SONAR_TOKEN`）が要る。
