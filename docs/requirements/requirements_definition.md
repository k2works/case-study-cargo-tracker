...
| 港湾制約 | 出発地・目的地・経由地の港湾が該当する貨物の取扱いに対応していること。**take-7 ではモデルとして持たない**（[ADR-018](../adr/018-route-search-rules.md)）。対応できる貨物種別は港ではなく航海が持ち、港ごとの取扱い制限は経路設計者の目視確認を運用条件とする |
