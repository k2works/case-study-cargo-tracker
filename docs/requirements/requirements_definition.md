...
- 本システムの全 API と画面は認証必須とする
- 認証方式は JWT Bearer Token とし、`authms` が発行したトークンを API Gateway で検証する
- 認可は RBAC で実施し、業務ロールごとに最小権限を付与する
...
| `ROLE_SALES` | 見積作成、荷主登録、予約登録/確定 | 営業担当者 |
| `ROLE_ROUTING` | 航海スケジュール管理、経路設計、追跡番号発行 | 経路設計者 |
| `ROLE_TRACKING` | 貨物状態更新、例外処理、追跡照会（運用） | 追跡管理者 |
| `ROLE_HANDLING` | 荷役作業登録、荷役履歴照会 | 荷役作業員 |
| `ROLE_BILLING` | 料金算出、割引適用、精算処理 | 経理担当者 |
| `ROLE_SHIPPER` | 自身貨物の予約照会、追跡照会 | 荷主、荷受人 |

> 追跡照会（UC08）は例外なく認証必須とし、`ROLE_SHIPPER` または `ROLE_TRACKING` に限定する。
