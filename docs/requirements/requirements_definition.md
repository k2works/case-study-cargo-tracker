...
status: stable
generated: { by: claude-code/claude-opus-5, at: 2026-09-16T10:38:59Z }
---
...
| 破損 | 輸送中に貨物が破損した状態 |
| 紛失 | 貨物の所在が確認できない状態 |
| 誤配 | 経路に含まれない場所で荷役作業が行われた状態（US28） |
| 税関保留 | 通関状態が留置となり、税関に留め置かれている状態（UC21） |
...
[*] --> 仮受付 : UC02・UC03 荷主・貨物予約登録

仮受付 --> 経路提案中 : UC04 予約情報引き渡し

state 経路提案中 {
  [*] --> 経路検討中
  経路検討中 --> 経路候補算出済 : UC06 経路候補算出
  経路候補算出済 --> 経路選択済 : UC07 経路選択・確定
  経路候補算出済 --> 経路検討中 : UC08 経路条件調整
}

経路提案中 --> 予約確定 : UC11 予約確定\nnote right of 予約確定 : UC09 経路情報紐付け完了後
予約確定 --> 追跡番号発行済 : UC12 追跡番号発行
追跡番号発行済 --> 輸送中 : UC13 荷役作業開始

輸送中 --> 配送完了 : UC13 最終荷役作業完了
配送完了 --> 精算済 : UC18 精算処理完了
精算済 --> [*]
...
受領待ち --> 受領済 : UC13 荷役作業記録（受領）
受領済 --> 積込済 : UC13 荷役作業記録（積込）
積込済 --> 輸送中 : 出港
輸送中 --> 荷降し済 : UC13 荷役作業記録（荷降し）
荷降し済 --> 引取待ち : 最終港着港
引取待ち --> 引取済 : UC13 荷役作業記録（引取）
引取済 --> [*]

受領済 --> 例外発生 : UC16 例外処理（破損・紛失）
積込済 --> 例外発生 : UC16 例外処理（遅延）
輸送中 --> 例外発生 : UC16 例外処理（破損・紛失・遅延）
荷降し済 --> 例外発生 : UC16 例外処理
...
| 種別 | アクター | 役割 |
| :--- | :--- | :--- |
| ヒューマン | 荷主 | 貨物の輸送を依頼し、見積確認・経路選択・予約確定・追跡確認を行う |
| ヒューマン | 荷受人 | 貨物の到着を待ち、追跡情報を確認する |
| ヒューマン | 営業担当者 | 見積作成・荷主情報登録・貨物予約登録・予約確定を行う |
| ヒューマン | 経路設計者 | 最適経路設計・航海スケジュール管理・追跡番号発行を行う |
...
ID は [システムユースケース](system_usecase.md) の採番を正とする。本書の図中の別名（`as UC..`・`as BUC..`）は図ごとの局所的な識別子であり、この ID とは対応しない。

| ID | ユースケース名 | 対応 BUC | 主要アクター |
| :--- | :--- | :--- | :--- |
| UC01 | 輸送見積を作成する | 貨物輸送予約 | 営業担当者 |
| UC02 | 荷主を登録する | 貨物輸送予約 | 営業担当者 |
| UC03 | 貨物予約を登録する | 貨物輸送予約 | 営業担当者 |
| UC04 | 予約情報を引き渡す | 経路設計 | 営業担当者 |
| UC05 | 航海スケジュールを検索する | 経路設計 | 経路設計者 |
| UC06 | 経路候補を算出する | 経路設計 | 経路設計システム（自動） |
| UC07 | 経路を選択・確定する | 経路設計 | 経路設計者 |
| UC08 | 経路条件を調整する | 経路設計 | 経路設計者 |
| UC09 | 経路情報を予約に紐付ける | 経路設計 | 経路設計者 |
| UC10 | 確定経路を荷主に通知する | 経路設計 | 営業担当者 |
| UC11 | 予約を確定する | 貨物輸送予約 | 営業担当者、荷主 |
| UC12 | 追跡番号を発行する | 貨物輸送予約 | 経路設計者 |
| UC13 | 荷役作業を記録する | 荷役・追跡管理 | 荷役作業員 |
| UC14 | 貨物状態を更新する | 荷役・追跡管理 | 追跡管理者 |
| UC15 | 追跡情報を照会する | 荷役・追跡管理 | 荷主、荷受人 |
| UC16 | 例外を処理する | 荷役・追跡管理 | 追跡管理者、荷役作業員 |
| UC17 | 輸送料金を算出する（割引適用を含む） | 精算管理 | 経理担当者 |
| UC18 | 精算を処理する | 精算管理 | 経理担当者、荷主 |
| UC19 | 航海スケジュールを登録する | 経路設計 | 経路設計者 |
| UC20 | ユーザーを認証する | 共通 | システム利用者 |
| UC21 | 通関申告を管理する | 荷役・追跡管理 | 荷役作業員、追跡管理者 |
| UC22 | 予約をキャンセルする | 貨物輸送予約 | 営業担当者、追跡管理者 |
...
| 寄港地の接続 | 経由港で航海を乗り継ぐ場合、同一港湾での接続が必要 |
| 期限遵守 | 荷主が指定した到着期限内に目的地に到達できること |
| 貨物種別対応 | 危険物・冷凍・冷蔵貨物等、貨物種別に対応した航海・設備が利用可能であること |
| 港湾制約 | 出発地・目的地・経由港の港湾が該当する貨物の取扱いに対応していること |
