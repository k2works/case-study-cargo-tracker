...
| US26 | システムにログイン・ログアウトする | 横断 | 高 |

---

## US26: システムにログイン・ログアウトする

**として**: システム利用者（営業担当者・経路設計者・追跡担当者・精算担当者・マスタ管理者）

**したい**: ユーザー ID とパスワードでログインし、業務終了時にログアウトしたい

**なぜなら**: 各役割の操作権限を分離し、業務ログを操作者単位で追跡できるからだ。また個人情報・契約情報を含む荷主データへの不正アクセスを防げるからだ
...
- [ ] 認証成功時は役割（ロール）に応じたトップ画面に遷移する
- [ ] 認証失敗時はエラーメッセージを表示し、入力フォームに戻る
- [ ] 未認証状態で保護リソース（`/bookings/*`・`/voyages/*` 等）にアクセスした場合、ログイン画面にリダイレクトされる
- [ ] セッションタイムアウト（30 分無操作）でログイン画面に戻る
- [ ] ログアウトボタンでセッションを破棄しログイン画面に戻る
- [ ] パスワードはハッシュ化（bcrypt 等）して保存される
- [ ] 公開追跡ページ（`/public/tracking/:trackingNumber`）は認証不要でアクセスできる
