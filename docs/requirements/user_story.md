...
**として**: システム利用者（営業担当者・経路設計者・荷役作業員・追跡管理者・経理担当者・管理者）

**したい**: ユーザー名とパスワードでログインし、自分のロールに応じた機能だけを利用したい

**なぜなら**: 各業務担当者の権限に応じてアクセスを制御し、認可されていない操作を防ぐ必要があるからだ
...
- [ ] 正しいユーザー名・パスワードでログインでき、セッションが確立される
- [ ] 誤った認証情報ではログインに失敗し、エラーメッセージが表示される
- [ ] ログイン後、ロールに応じてナビゲーションメニューの表示が制御される
- [ ] ロールが不足するルートへアクセスすると 403（アクセス拒否）になる
- [ ] ログアウトするとセッションが破棄され、ログイン画面へ戻る
...
| US26 | UC20 | BUC22 | 認可されたアクセス制御・情報セキュリティ |
