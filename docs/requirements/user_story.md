...
- [ ] 追跡番号が存在しない場合、「該当する貨物が見つかりません。追跡番号を確認の上、再度お試しください」と表示される（文言は [UI 設計](../design/ui_design.md) を正典とする）
...
**したい**: ユーザー名とパスワードでログインし、自分のロールに応じた機能を利用したい
...
- [ ] 認証情報が一致しない場合、「ユーザー名またはパスワードが正しくありません」と表示される（文言は [UI 設計](../design/ui_design.md) を正典とする）
- [ ] 認証失敗が 5 回連続するとアカウントが一時ロックされ、利用者に通知される（通知手段はログイン画面での表示。メール送信基盤はスコープ外）
