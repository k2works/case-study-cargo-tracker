...
**したい**: 割引ポリシー（割引方針種別・対象荷主種別・割引率・有効期限）を登録・編集・一覧管理したい

**なぜなら**: 法人割引の適用条件を管理画面で一元管理でき、契約条件の変更に料金計算を追従させられるからだ

**対応 UC**: UC17

**受け入れ基準**:

- [ ] 割引ポリシーの一覧（割引方針種別・対象荷主種別・割引率・有効期限・状態）を確認できる
- [ ] 新規割引ポリシーを登録できる
- [ ] 既存の割引ポリシーを編集できる
- [ ] 割引率は 0〜30% の範囲でのみ設定できる
- [ ] 有効期限切れのポリシーは料金算出時に適用されない
...
| US21, US22, US24 | UC17 | BUC18, BUC19 | 精算業務の正確化 |
