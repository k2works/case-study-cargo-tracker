...
**として**: 業務ユーザー（営業担当者・経路設計者・追跡管理者・荷役作業員・経理担当者・管理者）

**したい**: ユーザー ID とパスワードでシステムにログインし、自分のロールに応じた画面・機能だけを利用したい

**なぜなら**: 業務データ（荷主情報・予約・請求）への不正アクセスを防ぎ、誰がどの操作を行ったかを追跡できるからだ

**対応 UC**: -（全 UC の前提となる横断ストーリー）

**受け入れ基準**:

- [ ] ユーザー ID とパスワードでログインできる（Cookie 認証）
- [ ] 認証失敗時はエラーメッセージが表示され、入力値は保持される
- [ ] 未認証で保護ページにアクセスするとログイン画面へリダイレクトされる
- [ ] 公開貨物追跡ページ（`/public/tracking/{trackingId}`）と `/health` は未認証でアクセスできる
- [ ] ログアウトするとセッションが破棄されログイン画面に戻る
- [ ] ロール（ROLE_ADMIN / ROLE_SALES / ROLE_ROUTER / ROLE_TRACKER / ROLE_HANDLER / ROLE_BILLING）に応じてナビゲーションと機能が制御される
- [ ] パスワードはハッシュ化（BCrypt 相当）して保存される
...
| US26 | -（横断） | -（全 BUC の前提） | 業務データの保護・監査可能性 |
