...
**として**: システムユーザー（営業担当者、経路設計者、荷役作業員、追跡管理者、経理担当者、管理者）

**したい**: ユーザー ID とパスワードを入力してシステムにログインし、自分のロールに応じた機能を利用したい

**なぜなら**: 業務ロールに応じたアクセス制御により、不正操作を防止しつつ各担当者が必要な機能を安全に利用できるからだ

**対応 UC**: なし（横断的基盤機能）

**受け入れ基準**:

- [ ] ユーザー ID とパスワードを入力してログインできる
- [ ] 認証成功時に JWT トークンが発行される（有効期限 1 時間）
- [ ] ログイン後、自分のロール（`ROLE_SALES`・`ROLE_ROUTING`・`ROLE_HANDLER`・`ROLE_TRACKER`・`ROLE_ACCOUNTANT`・`ROLE_ADMIN`）に対応したメニューが表示される
- [ ] 認証失敗時（ID 不正・パスワード不正）は「ユーザー ID またはパスワードが正しくありません」と表示される
- [ ] 5 回連続失敗でアカウントが一時ロックされる
- [ ] ログアウト操作でトークンが無効化される
- [ ] JWT トークン期限切れ時は自動的にログイン画面に遷移する
...
**したい**: ユーザーアカウントの作成・ロール付与・無効化を管理したい

**なぜなら**: 組織変更や人員異動に伴うアクセス権限を適切に管理し、セキュリティを維持できるからだ

**対応 UC**: なし（横断的基盤機能）

**受け入れ基準**:

- [ ] 新規ユーザー（ユーザー ID・パスワード初期値・表示名・ロール）を作成できる
- [ ] 既存ユーザーにロール（`ROLE_SALES`・`ROLE_ROUTING`・`ROLE_HANDLER`・`ROLE_TRACKER`・`ROLE_ACCOUNTANT`・`ROLE_ADMIN`）を付与・剥奪できる
- [ ] ユーザーアカウントを無効化（ログイン不可）・再有効化できる
- [ ] 管理者自身のアカウントは無効化できない
- [ ] パスワードはハッシュ化（BCrypt）して保存される
- [ ] ユーザー一覧（ユーザー ID・表示名・ロール・状態）を確認できる
...
- [ ] **荷主・荷受人へのメール通知に含まれる時限署名トークン付き URL（`/tracking/{tn}?token=<JWT>`）から照会できる**（システム内ログイン不要）
- [ ] **トークンは有効期限 30 日、配送完了後 7 日で自動失効する**
- [ ] **トークン検証失敗時は「リンクの有効期限が切れています」と表示し営業担当者連絡を促す**
