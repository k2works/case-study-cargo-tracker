...
| US32 | 荷主情報を訂正する | UC02 | 高 |
| US33 | ロックされたアカウントを解除する | UC20 | 中 |
| US34 | 荷主が自社の予約を照会する | UC01 | 高 |
| US35 | 引取確認コードを採番して照合する | UC13 | 高 |
| US36 | 引取記録を訂正・取り消しする | UC13 | 中 |
...
- [ ] 登録完了後、予約番号が発行され状態が「仮予約」になる
- [ ] 出発地と目的地が同じ予約、希望着日が過去の予約は登録できない
- [ ] 仮予約の予約はキャンセルできる

> **受入基準から移した 2 項目**（IT2 計画時の突合）。消したのではなく、担当ストーリーが別にある。
>
> | 元の受入基準 | 移した先 | 理由 |
> | :--- | :--- | :--- |
> | 経路設計者に予約登録の通知が送信される | [US06](#us06-予約情報を経路設計者に引き渡す)（受入基準に既にある） | 「経路設計者への引き渡し」は US06 のストーリーそのもの。US04 に置くと二重に持つ |
> | 見積情報との整合性が確認される | Release 2.0（Estimation Context） | 見積は Release 2.0 のスコープ。**存在しない前提への受入基準は満たしようがない** |
...
**したい**: 仮予約の出発地・目的地・期限・貨物仕様を確認し、経路設計者に引き渡したい
...
- [ ] 経路設計依頼を実行すると、予約状態が「経路提案済」に更新される
- [ ] 引き渡した予約が**経路設計者の作業入口（経路割り当て待ち一覧）に現れる**
- [ ] 引き渡し済みの予約は重ねて引き渡せない
- [ ] 誰がいつどの予約を引き渡したかが業務操作ログに記録される（`../design/non_functional.md` §4.4）

> **「通知が送信される」を「作業入口に現れる」に変えた**（IT3 計画時の突合）。
> ADR-006 により外部連携（メール送信）は実装しない。**通知の実体が無いまま
> 「送信される」と書き続けると、永遠に満たせない受入基準が残る。**
> 業務上の「引き渡し」は、経路設計者が次に見る一覧にその予約が現れることである。
>
> あわせて状態名を `BookingStatus` の表示名（「経路提案済」）に揃えた。
> 旧版の「経路設計中」は遷移表（`../design/domain-model.md`）に存在しない名前だった。
...
- [ ] 紐付け後、予約状態は「経路提案済」のまま維持され、経路状態が「割り当て済」になる
...
- [ ] 通知内容（経由港・所要日数・到着予定日・追跡番号）を確認できる
- [ ] 荷主への経路通知を送信できる
- [ ] 通知送信記録が登録される
- [ ] 期限を延長して割り当てた場合、当初の希望期限からの差分が通知に含まれる

> **料金概算を受入基準から外した**（IT9 の開始準備）。概算式（ADR-008）は
> **経路候補の並べ替え用**であり、荷主に見せる金額として設計していない。
> 見せた瞬間に請求額として読まれる。料金の通知は [US21](#us21-輸送料金を算出する) で
> 実際の料金を算出できるようになってから扱う。
>
> あわせて、IT8 の実装が満たしている「期限の延長分を伝える」を受入基準に足した
> （`ui_design.md` 経路割り当て §候補ゼロ時の再算出が求めていたが、基準に無かった）。
...
- [ ] 予約に荷受人（氏名・住所・連絡先メール）を登録でき、引取時の本人確認に用いる

> **荷受人を US04 ではなく本ストーリーで扱う理由**: 貨物予約登録画面は荷受人 3 項目を必須としていたが、
> US04 の受入基準に無く、`cargo` テーブルにも住所のカラムが無い（IT2 計画時の突合）。
> **荷受人の情報を最初に実際に使うのが引取時の本人確認**であるため、担当を本ストーリーに定めた。
> 住所カラムの追加も本ストーリーで行う（`../design/data-model.md`）。
...
- [ ] ロックされたことは**ログイン画面では知らせない**。ログイン画面以外の経路（監査ログと管理者への運用連絡）で把握する
- [ ] ロックは一定時間の経過で解除される（管理者による即時解除は US33 で扱う）
- [ ] 認証成功時に失敗回数がリセットされる
...
**したい**: 登録済みの荷主の名称・連絡先・住所を訂正したい

**なぜなら**: 荷主名の誤字・住所の番地違いは月に数件は必ず起きるが、訂正の手段が無いと請求書と通関書類に誤った情報がそのまま流れるからだ

**対応 UC**: UC02

**受け入れ基準**:

- [ ] 荷主詳細から編集画面を開き、荷主名・メールアドレス・電話番号・住所を訂正できる
- [ ] 荷主コードと荷主種別は変更できない（識別子と区分の変更は訂正ではなく別の荷主である）
- [ ] メールアドレスを他の荷主と同じ値に変更しようとした場合は、訂正されず既存の荷主が提示される
- [ ] 訂正した内容は一覧・詳細に即座に反映される
- [ ] 誰がいつ何を訂正したかが業務操作ログに記録される（`non_functional.md` §4.4）

> **起票の経緯**: IT1 のユーザー代表レビューで「登録内容の修正手段が無いのは業務が回らなくなる一歩手前」と指摘された。**1 イテレーションなら管理者依頼で耐えられるが、IT2 の計画に入れることを条件に受け入れ**とされた（`docs/review/IT1実装_review_20260806.md` L3）。
...
**なぜなら**: ロックの自動解除を待つ間、荷役作業員や追跡管理者は現場で作業を止めることになる。**輸送は待ってくれないため、待ち時間がそのまま業務の停止時間になる**からだ

**対応 UC**: UC20

**受け入れ基準**:

- [ ] 管理者はロック中のアカウント一覧を、ロック日時・失敗回数とともに確認できる
- [ ] 管理者は対象アカウントのロックを解除でき、解除と同時に失敗回数がリセットされる
- [ ] 解除には理由の入力が必須である
- [ ] 誰がいつどのアカウントを解除したかが監査ログに記録される
- [ ] 管理者以外はこの操作に到達できない

> **起票の経緯**: US31 は「ロックは一定時間の経過、または管理者の解除操作で解除される」を受入基準に
> 含んでいたが、IT1 では自動解除のみを実装し、管理者による解除は持ち越しになった
> （`../development/retrospective-1.md` の C2）。**US31 の受入基準に残したままだと「未達なのに完了扱い」
> になる**ため、US31 からは外し、独立したストーリーとして起票した。荷役・追跡が入るイテレーションまでに
> 実装する必要がある。
...
**したい**: 自社が依頼した予約の状況を、担当営業に問い合わせずに自分で確認したい

**なぜなら**: 「あの貨物は今どうなっているか」の電話は毎日発生し、営業担当者の時間を奪う。
荷主が自分で見られれば、双方の待ち時間が消えるからだ
...
- [ ] 利用者アカウントと荷主が紐づけられる
- [ ] 荷主は**自社の予約のみ**を一覧・詳細で確認できる
- [ ] **他社の予約は一覧に現れず、URL を直接指定しても開けない**
- [ ] 荷主は予約の登録・キャンセルはできない
- [ ] 紐づけのない荷主アカウントでは、予約が 1 件も表示されない（他社の予約が漏れない）

> **起票の経緯**: IT2 で貨物予約一覧を ROLE_SHIPPER に開放したが、
> **利用者アカウントと荷主を結びつける手段が無く、他社の予約まで見える状態だった**。
> クローズ前のレビューで気づき、開放を取り消した（`../review/IT2実装_review_20260806.md`）。
> `non_functional.md` は ROLE_SHIPPER を「自社予約・追跡（Phase 2）」と定めており、
> 「自社の」を実現する手段そのものが本ストーリーである。

---

## US35: 引取確認コードを採番して照合する

> Issue: [#509](https://github.com/k2works/case-study-cargo-tracker/issues/509)

**として**: 営業担当者（採番・伝達）／荷役作業員（照合）

**したい**: 予約確定時に引取確認コードを採番して荷主に伝え、引取時に提示されたコードと照合したい

**なぜなら**: 現状の引取記録は「提示された値をそのまま書き写す」だけで、
**照合する相手がシステムの中に無く、引き渡しの証明にならない**からだ

**対応 UC**: UC13

**受け入れ基準**:

- [ ] 予約確定時（US13）に引取確認コードが採番される
- [ ] 採番されたコードを予約詳細で確認でき、営業担当者が荷主へ伝えられる
- [ ] 引取記録（US16）で入力されたコードが採番済みのコードと一致しない場合、記録できない
- [ ] 一致しなかった事実が監査ログに残る（総当たりの検知）
- [ ] コードは追跡番号とは別の値である（追跡番号を知っているだけでは引き取れない）

> **起票の経緯**: IT7 で US16（引取作業を記録する）を実装した際、確認コードは
> **入力された文字列を記録するだけ**で照合していなかった。クローズ前のレビュー H4 と
> ふりかえり P5 で「記録はできるが証明にならない」と判定し、**US 化して IT8 の開始準備で
> 起票する**と決めた（`../development/retrospective-7.md` C1）。
> 採番して荷主へ伝える運用は、追跡番号（US14）と同じ形である。

---

## US36: 引取記録を訂正・取り消しする

> Issue: [#510](https://github.com/k2works/case-study-cargo-tracker/issues/510)

**として**: 荷役作業員（申請）／追跡管理者（承認）

**したい**: 誤って登録した引取記録を訂正・取り消ししたい

**なぜなら**: 引取は輸送の終点であり、誤登録をそのままにすると
**貨物が届いていないのに配送完了として扱われる**からだ

**対応 UC**: UC13

**受け入れ基準**:

- [ ] 登録済みの引取記録を一覧から選び、訂正または取り消しを申請できる
- [ ] 取り消しが承認されると、貨物状態が引取前の状態に戻る
- [ ] 訂正・取り消しの履歴（申請者・承認者・日時・理由）が残り、元の記録は消えない
- [ ] 追跡管理者の承認なしには状態が戻らない
- [ ] 精算済み（`SETTLED`）の予約に対しては訂正・取り消しできない

> **起票の経緯**: IT6 のレビュー H11 が「荷役記録の訂正・取り消しを US 化して IT7 へ」と
> 求めたが、US 化されないまま IT7・IT8 へ送られた（`../development/retrospective-7.md` C2）。
> **無記名のまま繰り越すと、返済枠が「余力次第」になり固定化する。**
...
| US32 | UC02 | BUC03 | 荷主管理の正確化 |
| US33 | UC20 | BUC22 | 業務データの保護・監査証跡の確保 |
| US34 | UC01 | BUC01 | 荷主セルフサービス・問い合わせ削減 |
| US35, US36 | UC13 | BUC14 | 引き渡しの証明・業務データの保護 |
