...
### 設計 DoD（状態機械・ドメイン不変条件）

受入基準を満たすコードが書けても、状態遷移のガードやドメイン不変条件の置き場所を誤ると、ユニットは緑でも実運用（現場の典型ケース）で誤動作する。IT8 で発生した「OVERDUE 塩漬け（期限超過後に入金・調整がブロックされる）」「補償費用の符号方向の取り違え」を再発させないため、以下を集約・値オブジェクトを設計する際の DoD とする。

- **【T43】状態機械のガードは「現場の典型ケース」起点で洗い出す**
  - ガード条件（遷移可否）を受入基準の文面だけから導出しない。**期限後入金・部分入金・取消・再調整・当日時刻付き到着**など、現場で実際に起きる典型ケースを列挙し、各ケースで許可すべき遷移を先に決めてからガードを実装する。
  - 「PENDING のみ許可」のように単一状態を直接判定せず、業務的に等価な状態集合（例: 未精算 = PENDING ∪ OVERDUE）を述語（`unsettled?` 等）として集約に定義し、ガードはその述語を参照する。単一状態のハードコードは状態追加時に塩漬けを生む。
  - テストには受入基準の正常系だけでなく、上記の典型ケース（特に「期限超過後の操作」「当日時刻付きの境界」）を必ず含める。
- **【T44】ドメイン不変条件は VO / 集約に閉じ、アプリ層に置かない**
  - 符号の正規化・値の下限/上限・丸め・単位変換などの不変条件は、**必ず値オブジェクトまたは集約の内部**で保証する。アプリ層（ユースケース）やコントローラで符号反転・絶対値化・下限クリップを行わない。
  - 例: 料金調整の「減額・補償費用はいずれも請求額を減算する（負値に正規化）」という不変条件は `InvoiceLineItem` の生成時に閉じ込め、`AdjustFreight` ユースケースは符号を意識しない（IT8/IT9 実績）。
  - 同じ不変条件の判定・変換ロジックがアプリ層とドメイン層に二重に現れていないかをレビュー観点とする（二重化はドメイン層への一本化のシグナル）。
