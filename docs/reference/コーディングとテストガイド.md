...
#### 4. DTO 型から変数名を導出するネーミング規約

`record` は Java 14+ の予約識別子のため変数名として使用しない（SonarQube `S6212` 警告対象）。`*Record` サフィックスを持つ MyBatis Record / Persistence DTO は、サフィックスを除いた小文字キャメルケースで命名する。

| DTO 型 | 変数名 |
|--------|--------|
| `HandlingActivityRecord` | `activity` |
| `CargoSnapshotRecord` | `snapshot` |
| `ClaimVerificationRecord` | `verification` |
| `CargoStatusHistoryRecord` | `history` |
| `TrackingSummaryRecord` | `summary` |
| `TrackingEventRecord` | `event` |

複数 DTO を同一スコープで扱う場合は意味的識別子（`bookingSnapshot` / `cargoSnapshot` 等）を前置する。

参考: IT5 ふりかえり P3/P4・T3。

### Axon 5.1 Aggregate Command Handler 規約

`@CommandHandler` 注釈付きメソッドの引数規約は以下に従う。Axon の `CommandGateway` が Aggregate にルーティングできないと、E2E でしか検出できない障害になるため設計時に注意する。

#### 引数規約

| 位置 | 種別 | 例 |
|------|------|----|
| 第 1 引数 | Command（必須） | `RegisterHandlingActivityCommand command` |
| 第 2 引数以降 | `EventAppender` または Spring Bean のみ | `EventAppender appender` |

値オブジェクトや DTO を Command Handler に渡したい場合は、**Command 自体のフィールドに含める**。Aggregate static メソッドの追加引数として直接渡すことはできない（Axon は Spring Bean 注入のみサポート）。

```java
// NG: CargoSnapshot は Spring Bean ではないため Axon が解決できず NoHandlerForCommandException
@CommandHandler
public static String register(RegisterHandlingActivityCommand command,
                              CargoSnapshot snapshot,
                              EventAppender appender) { ... }

// OK: CargoSnapshot を Command フィールドに含める
public record RegisterHandlingActivityCommand(
        @TargetEntityId String activityId,
        String trackingNumber,
        CargoSnapshot snapshot,   // ← Command に含める
...
`*Command` を引数に取る Aggregate メソッドには **必ず** `@CommandHandler` を付与する。ArchUnit ルール `CommandHandlerArchitectureTest` で CI 自動検出される（bookingms / handlingms / routingms / trackingms 共通）。

参考: IT5 ふりかえり P1/P2・T1/T2。
