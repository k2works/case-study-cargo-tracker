...
| [開発](./development/index.md) | リリース計画、イテレーション計画、進捗管理 | **IT20 完了**（累計 117SP / 117SP）。**全 36 US が実装完了**し、[Release 2.1（見積）を v2.1.0 として出荷](./development/release_report-2_1_0.md)。IT20 は**出荷と是正の 2 回目**（SP 0）——**育つ負債 2 件**（ADR-024 の代償・`Cargo` の行数）を返し、正典に 2 リリース残っていた未判断 5 件を決着させた（[ADR-025](./adr/025-single-currency-jpy.md)） |
| [運用](./operation/index.md) | 環境構築、デプロイ、運用手順の整理 | `index.md` を整備済み |
| [レビュー](./review/index.md) | 分析・開発レビュー結果の記録 | 26 件作成済み |
| [ジャーナル](./journal/index.md) | 判断の経緯と学びの記録 | 3 件作成済み |
| [ADR](./adr/index.md) | Architecture Decision Records の管理 | 26 件（うち 1 件は置き換え済み） |
...
| [IT15 実装レビュー](./review/IT15実装_review_20260811.md) | IT15（US30 輸送中の予約キャンセル）のレビュー結果（高 5 件・中 8 件・低 3 件。**ADR-009 の規則を既存 5 本が守っていなかったことが検査で判明**） |
| [IT16 実装レビュー（途中）](./review/IT16実装_review_20260811.md) | IT16 実装の途中経過のレビュー結果（高 3 件・中 6 件・低 3 件。**IT16 自身が定めた Try を IT16 の実装が破っていた**） |
| [IT16 クローズ前レビュー](./review/IT16クローズ_review_20260811.md) | IT16 の途中レビュー以降のレビュー結果（高 2 件・中 3 件・低 2 件。**高 2 件はどちらも自分が書いた検査クラス自身の問題**） |
| [IT17 実装 セルフレビュー（中間）](./review/IT17実装_self_review_20260811.md) | IT17 タスク 0〜6 の自己レビュー結果（要修正 2 件・許容 3 件） |
| [IT17 クローズ前レビュー](./review/IT17クローズ_review_20260811.md) | IT17 の実装全体のレビュー結果（高 1 件・中 3 件・低 7 件。**高 1 件は認可変更の副作用**） |
| [IT18 実装 セルフレビュー（中間）](./review/IT18実装_self_review_20260812.md) | IT18 タスク 0〜7 の自己レビュー結果（要修正 1 件・修正済み 2 件・許容 2 件） |
| [IT18 クローズ前レビュー](./review/IT18クローズ_review_20260812.md) | IT18（US01 輸送見積）の実装全体のレビュー結果（高 2 件・中 4 件・低 8 件。**高 2 件は「作った本人には見えない」もの**） |

## 補足

- すべてのカテゴリに実ドキュメントを配置済みです（`design/` 11 件・`development/` 計画 19 件と報告書 19 件・`adr/` 25 件・`manual/` 13 章＋付録 1 件＋用語集）。
