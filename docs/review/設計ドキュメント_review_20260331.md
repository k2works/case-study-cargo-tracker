...
```mermaid
quadrantChart
    title Priority Matrix - Business Impact x Implementation Cost
    x-axis Low Cost --> High Cost
    y-axis Low Impact --> High Impact
    quadrant-1 Phase1 Split Work
    quadrant-2 Phase1 Fix Now
    quadrant-3 Phase2
...
| ID | 内容 | 象限 |
| :--- | :--- | :--- |
| H-01 | 消費税設計（tax_rate / tax_amount） | Phase1 Fix Now |
| H-02 | 見積機能追加（quotation テーブル + 画面） | Phase1 Split Work |
| H-03 | BookingStatus / TrackingStatus 二重管理 | Phase1 Split Work |
| H-04 | 荷主テーブル新設（shipper テーブル） | Phase1 Fix Now |
| H-05 | BookingStatus 列挙値統一 | Phase1 Fix Now |
| H-06 | TransportStatus 列挙値統一 | Phase1 Fix Now |
| H-07 | @TransactionalEventListener 設計注意 | Phase1 Fix Now |
| H-08 | Java 21 LTS / Spring Boot 4.0 移行 ADR | Phase1 Split Work |
| H-09 | htmx セッション切れ認証対応（401 返却） | Phase1 Fix Now |
| H-10 | 見積・荷主登録画面の設計 | Phase1 Split Work |
| H-11 | CargoType から OVERSIZED 削除 | Phase1 Fix Now |
| H-12 | PaymentStatus 表示を PENDING に統一 | Phase1 Fix Now |
| H-13 | ACL ポート 5 件の WireMock 契約テスト定義 | Phase1 Fix Now |
| H-14 | cargo テーブルの欠落カラム（cargo_type / weight_kg） | Phase1 Split Work |
| H-15 | ArchUnit 検証ルール 4 件の明文化 | Phase1 Split Work |
| H-16 | E2E テストフレームワーク（Playwright）追加 | Phase1 Fix Now |
| H-17 | 荷役登録のバーコードスキャン対応 | Phase1 Split Work |
| H-18 | 追跡詳細に推定到着日（ETA）追加 | Phase1 Fix Now |
| H-19 | 追跡詳細に通関ステータス追加 | Phase1 Fix Now |
| H-20 | EXCEPTION 時の荷主アクション案内 | Phase1 Fix Now |
| M-01 | MV（Marnot Vital）期限管理 UI | Phase2 |
| M-02 | DiscountPolicy のドメインサービス昇格 | Re-evaluate |
| M-03 | Billing Context トリガー設計（自動 vs 手動） | Phase1 Split Work |
| M-04 | 荷役作業員モバイル UX 最適化 | Re-evaluate |
| M-05 | 貨物種別変更時の動的フィールド表示 | Phase2 |
| M-06 | 境界値テストシナリオの記述 | Phase2 |
| M-07 | ロール別ダッシュボード対応 | Phase2 |
| M-08 | 引取作業の荷受人確認フィールド追加 | Phase2 |
| M-09 | MISROUTED 状態の業務フロー・UI 設計 | Re-evaluate |
| L-01 | CQRS Read Model の Mapper 責務分離 | Re-evaluate |
| L-02 | 多言語対応（日本語・英語・中国語） | Re-evaluate |
| L-03 | パフォーマンステスト計画 | Phase2 |
