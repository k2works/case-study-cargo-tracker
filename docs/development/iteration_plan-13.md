...
- [ ] US33 の受入基準 4 件が、E2E / API / ドメインのテストで 1:1 に固定されている。
- [ ] 利用者と荷主の紐付けは、authms と bookingms のどちらにも暗黙の文字列一致を置かない。
- [ ] `ROLE_SHIPPER` から見える API は自社貨物に絞られ、未紐付け利用者には問い合わせ先が出る。
- [ ] 無操作タイムアウトは ADR で判定時間・警告有無・入力中データの扱いを決め、破るテストを置く。
- [ ] IT12 の Try 1〜6 を計画と DoD に反映している。
...
- [ ] 一定時間の無操作で認証状態が破棄され、ログイン画面へ戻る
- [ ] 入力中の業務フォームでは、タイムアウト前に警告を出すか、保存されないことを明示する
- [ ] タブを閉じた場合は従来どおり `sessionStorage` のトークンが消える
- [ ] タイムアウト後の API 呼び出しは認証切れとして扱われる
...
| # | タスク | 見積 | 状態 |
| :--- | :--- | :--- | :--- |
| 0.1 | `ROLE_SHIPPER`・`ADR-008`・`US18`・`自分の貨物` を grep し、改訂が必要な設計・マニュアル箇所を一覧化する | 3h | [ ] |
...
| # | タスク | 見積 | 状態 |
| :--- | :--- | :--- | :--- |
| 1.1 | US33 の 4 受入基準を E2E シナリオに翻訳し、赤を確認する | 8h | [ ] |
| 1.2 | TD-01 のタイムアウトシナリオをコンポーネントテストと E2E に翻訳し、赤を確認する | 5h | [ ] |
| 1.3 | ADR-029（利用者と荷主の紐付け・荷主向け追跡境界・無操作タイムアウト）を起票する | 5h | [ ] |
| 1.4 | ADR のコンプライアンス表に検査名を書き、`AdrComplianceTableTest` で実在確認する | 3h | [ ] |
...
| # | タスク | 見積 | 状態 |
| :--- | :--- | :--- | :--- |
| 3.1 | bookingms の CargoSnapshot に荷主 ID を返す内部 API を追加し、`system:trackingms` だけが読めることを固定する | 6h | [ ] |
| 3.2 | trackingms に `ShipperCargoTrackingQuery` を追加し、自社貨物だけを返すテストを赤から通す | 8h | [ ] |
| 3.3 | 他社貨物の追跡番号を指定した荷主向け詳細は 404 とし、公開追跡とは別経路で固定する | 5h | [ ] |
| 3.4 | 一覧項目に状態・現在地・到着予定・未解決例外を出す Read Model を整える | 6h | [ ] |

### Phase 4: フロントエンド（荷主ポータル / タイムアウト）

| # | タスク | 見積 | 状態 |
| :--- | :--- | :--- | :--- |
| 4.1 | `ROLE_SHIPPER` のダッシュボードとサイドバーに「自分の貨物」を追加し、ナビ表示テストを置く | 5h | [ ] |
| 4.2 | `/shipper/tracking` 一覧と `/shipper/tracking/:trackingNumber` 詳細を MSW で実装する | 8h | [ ] |
| 4.3 | 未紐付け利用者の問い合わせ案内画面を実装し、空一覧にしないことをテストする | 4h | [ ] |
| 4.4 | 無操作検知・警告・自動ログアウトをルートガードに実装する | 7h | [ ] |
| 4.5 | 入力中画面でタイムアウト警告が操作を隠さないことを Playwright のスクリーンショットで確認する | 4h | [ ] |
...
| :--- | :--- | :--- | :--- |
| Phase 0: 返済枠と調査 | 0 | 8h | [ ] |
| Phase 1: 受け入れテストと ADR | 1 | 21h | [ ] |
| Phase 2: 利用者と荷主の紐付け | 2 | 25h | [ ] |
| Phase 3: 自社貨物の追跡一覧 | 2 | 25h | [ ] |
| Phase 4: フロントエンド | 1 | 28h | [ ] |
| Phase 5: 設計・マニュアル反映 | 1 | 24h | [ ] |
...
> **設計への反映が必要**: `user_shipper_link.shipper_id` はサービス境界をまたぐ参照であり DB FK は張らない。参照整合は authms の内部 API と管理者 UI の候補検索で守る。
...
> **設計への反映が必要**: `ui_design.md` の画面一覧・ナビゲーション構成・RBAC マトリクスに `/shipper/tracking` と `/shipper/tracking/:trackingNumber` を追加する。`/tracking/:trackingNumber` は公開画面のまま残す。
...
- [ ] US33 / TD-01 の受け入れテストが Red → Green で実装されている。
- [ ] 他社貨物が一覧・詳細に出ないことを API と E2E の両方で固定している。
- [ ] `ROLE_SHIPPER` のナビゲーションとダッシュボード導線が実装され、押した先で 403 にならない。
- [ ] 未紐付け利用者に問い合わせ先が表示される。
- [ ] 無操作タイムアウトの警告とログアウトが動作し、入力中画面を隠さないことを確認している。
