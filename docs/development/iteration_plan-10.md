...
- [x] US19 / US20 の受入基準（**正典を 1 行ずつ指差して確認**。T2）— **US19 の「新しい到着予定日」1 項目が未達**（IT11 / C18）。他は緑
- [x] デモ項目 8 件が動作する（**9 番目の C9 は計画どおり落とした**）
- [x] C10 は決着（例外一覧が待ち行列になり、荷主名が読める）。**C9 は落として IT11 へ送った**

#### 終盤の完了条件（`development_strategy.md`）

- [x] 業務シナリオが通しで動作する
- [x] E2E のクリティカルパス（既存 8 本）＋本 IT の 2 本が緑
- [x] ArchUnit のルール 10 件がすべて有効・緑（**`shared.application` の新ルールを含む**）

#### 品質

- [x] `./gradlew check` が緑（Checkstyle 0 / SpotBugs 0 / ArchUnit / JaCoCo レイヤー別）
- [x] CI が緑・SonarQube Quality Gate が PASS（Bug 0 / Vulnerability 0 / **Code Smell 0** / 重複 0.23% / 新規コードのカバレッジ 87.4%）

#### 主張とテスト（T1。**本 IT の最上位**）

- [x] **「〜しない」「〜に限る」と書いたコメントに、それを破るテストが同じコミットにある** — ただし **V22 のコメントと `ExceptionOccurrence` の実装が矛盾していた**（レビュー H1 で発覚・修正済み）
- [x] **ドメインに足した `throw` 6 件すべてに、画面から踏むテストが対になっている**（IT9 の Try T1）
- [x] 書けない主張はコメントから落とした
...
- [x] 追跡管理者がダッシュボードから未解決例外へ到達できる
- [x] **管理者がエスカレーション中の例外へ到達できる**（US20 の受入基準の受け皿）。**詳細まで開けることはレビューで発覚して修正した**
- [x] `EXCEPTION` の貨物から例外解決へ到達できる（**状態軸**）
- [x] **例外の一覧から「誰に連絡するのか」が読め、その予約へ行ける**（IT9 の Try T2）
- [x] 画面の案内文が指す先を、その文を読むロールで実際に開ける（**レビューで 2 件の 403 が見つかり修正**）

#### ドキュメント

- [x] 設計への反映 9 件を完了
- [x] **マニュアルを更新**（07-追跡管理に 7.5〜7.8）＋キャプチャ 4 枚を生成 spec で再生成
- [x] 早見表に足した 8 行の文言が**実際に画面に出ることを確認**（IT9 の Try T3）
- [x] 用語集・付録 B・索引 3 点を同期

---

## 実績（開発フェーズ）

### 安全装置の破壊検証（DoD。**その装置だけを壊す**）

**10 件すべてが赤になった。空振りは無い。** 装置を 1 つずつ壊し、
対応するテストクラスだけを走らせて確認した。検証は使い捨てのスクリプトで行い、
実行後にすべて元に戻したことを作業ツリーの差分が空であることで確かめている。

| # | 装置 | 壊し方 | 結果 |
| :--- | :--- | :--- | :--- |
| 1 | 発生前の状態を永続化する | 復帰先を `NOT_RECEIVED` 固定に | 赤 |
| 2 | 引取済みには例外を登録できない | 条件を `false` に | 赤 |
| 3 | 二重解決を拒む | 条件を `false` に | 赤 |
| 4 | 紛失はエスカレーションする | `LOST` のフラグを `false` に | 赤 |
| 5 | 遅延・破損はエスカレーションしない | `DAMAGE` のフラグを `true` に | 赤 |
| 6 | 例外の発生で通知が記録される | 購読で記録せず成功を返す | 赤 |
| 7 | 解決の報告は別種別で記録される | `EXCEPTION_RAISED` で積む | 赤 |
| 8 | 追跡管理者以外は例外を登録できない | 認可を `authenticated()` に | 赤 |
| 9 | 管理者以外はエスカレーション一覧を開けない | 認可を `authenticated()` に | 赤 |
| 10 | 未解決の例外は 2 つ並ばない | 条件を `false` に | 赤 |

> C1（`shared.application` の ArchUnit）はタスク 0-1 で個別に検証済み。
> 違反クラスを足すと**他の 9 ルールは緑のまま本ルールだけが赤**になることを確かめた。

### 受入基準の指差し確認（T2。**正典を 1 行ずつ**）

| ストーリー | 受入基準 | 満たした形 |
| :--- | :--- | :--- |
| US19 | 追跡番号と例外種別「遅延」・発生状況（場所・日時・理由）を記録できる | `ExceptionHandlingTest.遅延を発生状況とともに記録できる`。**V22 で発生場所の列を足した** |
| US19 | 記録後、貨物状態が「例外発生」に更新される | `記録すると貨物状態が例外発生になる` |
| US19 | 荷主に遅延発生の通知が送信される | `荷主への発生通知が記録される`（ADR-006 により記録で満たす） |
| US19 | 対応内容を入力して荷主に対応報告を送信できる | `対応内容を入力すると対応報告が記録される`（`EXCEPTION_RESOLVED`） |
| US19 | 例外対応履歴が記録される | `対応履歴が残る`（`resolved_at` / `resolution_notes`） |
| US20 | 追跡番号と例外種別「破損」「紛失」・発生状況を記録できる | `破損を記録できる` |
| US20 | 記録後、貨物状態が「例外発生」に更新される | US19 と同じ経路（`raiseException`） |
| US20 | 紛失は緊急フラグが設定されて管理職への escalation 通知が送信される | `紛失は緊急フラグが立つ` ＋ `管理者がエスカレーション中の例外を見られる`。**外部へは送らず、管理者が見る場所を作って満たした** |
| US20 | 荷主に破損・紛失発生の通知が送信される | US19 と同じ経路（`CargoExceptionRaisedEvent`） |
| US20 | 対応内容（補償方針等）を入力して荷主に報告を送信できる | `紛失の対応報告を記録できる` |

### スコープから落としたもの

| # | 内容 | 理由 |
| :--- | :--- | :--- |
| 3-5 | **C9: 日程が変わった区間に印を出し、遅延例外の登録へ導く** | **リスク 2 で先に決めた順序どおり最初に落とした。** 10SP はベロシティ（8SP）を 2SP 超えており、US19 / US20 の受入基準を優先した。**C9 自体は消さず IT11 の返済枠へ送る**（「余力次第」にはしない） |
| 4-4 | E2E の操作を 1 本ずつ外して赤になることを確かめる | 安全装置の破壊検証（4-3）を 10 件すべて実施し、空振りが無いことを確認したため、本 IT では E2E の逆検証まで行わなかった。**未実施であることを記録する** |

### 実装で決めたこと（設計への反映済み）

- 例外一覧の**検索フォーム・ページ送り・htmx の部分更新は作らなかった**。件数が少なく、未解決だけを既定にすれば 1 画面に収まる。`ui_design.md` に「IT10 で実装した範囲」として明記した
- `ROLE_ADMIN` の権限範囲を `non_functional.md` で「全画面」から実態（アカウント管理＋エスカレーション確認）に直した
- `domain-model.md` のビジネスルール 4（CUSTOMS_HOLD の自動登録）が **ADR-006 と矛盾**していたため書き直した
...
| 2026-08-09 | 開発フェーズの実績を反映。安全装置 10 件の破壊検証・受入基準の指差し確認・落としたもの（3-5 / 4-4）を記録 |
| 2026-08-09 | クローズ。DoD を実績で更新。**レビューで高優先度 10 件（6 件を IT 内で対応・4 件は IT11）**。US19 の受入基準 1 項目が未達であることを明記 |
