...
- [x] 対象ストーリーの受入基準を満たす。**果たせないもの（28-6 の通知は代替）を [ADR-026] と完了報告書に記録する**
- [x] 成功基準 1〜9 を満たす
- [x] **受入基準 28-1〜28-8 それぞれに、達成を確かめる検査がある**（Try 6・成功基準 6）。**DoD のチェックだけで達成と判定しない**
- [x] 全テストが緑。**品質ゲートは CI と同じコマンドで回す**——バックエンドは `./gradlew build`、フロントエンドは **`tsc -b`**（`tsc --noEmit` ではない。Try 3）
- [x] **CI が緑**（run 32829312554）。**2 回続けて赤だった E2E を追った**——CI でだけ追跡番号の入力が再描画で捨てられていた
- [x] **`TZ=UTC` でも緑**（`--rerun-tasks` で強制再実行）
- [x] **ドメイン層のカバレッジが 90% 以上**（6 サービス + shared）
- [x] **品質ゲートの前にフルビルドでレポートを作り直した**（Try 7）
- [x] SonarQube Quality Gate が **両プロジェクトで PASS**（新規カバレッジ 90.5 / 89.0・違反 0）。**指摘 1 件が実際の欠陥だった**——再設計の可否判定が期限の引数を使っておらず、「期限では弾かない」が固定されていなかった
- [x] **ADR-026 の決定ごとに検査があり、1 件ずつ壊して赤を確認した記録がある**
- [x] **この IT で書いた「〜しない」「〜まで確かめる」の数だけ検査があり、壊して赤を確認した**（成功基準 8）
- [x] **検査がリポジトリのファイルを読むなら、入力に宣言し、壊して赤を確認した**（Try 2）
- [x] **新しく使い始めた検査コマンドを、一度壊して赤になることを確かめた**（Try 3）
- [x] **ガードを足したあと、その後ろにある守りの検査が今も踏まれているか確かめた**（Try 4・タスク 4.5）
- [x] **応答の一覧に依存する画面を、画面から踏むテストで確かめた**（Try 1・成功基準 7）
- [x] **新しく足した値が層をまたいで生き延びる**。値ごとに 1 本
- [x] **設計への反映が検査で担保されている**。**注の表の消化ではなく、検査が緑になったことで判定する**
- [x] **ナビゲーションの 4 点が一致している**。**ロール別・状態別の到達性**を確かめた
- [x] **件数から対象一覧へ辿れる**（[横断規約](release_plan.md)）。誤配の件数は実装済みだったが検査が無く、クローズ直前に足して赤を確認した
- [x] **サービス間の呼び出しを実際に 1 往復させた**（kind で予定外の荷役 → 誤配 → 例外 → 組み直し → 解決）
- [x] **実環境で確かめる前にイメージを作り直した**。**`dev:k8s:rollout:image` は同じタグだと Pod を作り直さない**——`images` → `rollout:image` → `rollout:restart` まで踏み、Pod の起動時刻で確かめた
- [x] **デモ項目 10 件をこの順で通した**。**10 件すべてに検査を対応させた**——5（誤配の件数から一覧へ）と 10（解決しても記録が残る）は検査が無く、この時点で足した
- [x] **IT9 時点のコメントが残っていない**（返済枠 0.11）
- [x] **JIG / jig-erd の出力を再生成した**
- [x] ユーザーマニュアル 08 章・04 章を更新し、**キャプチャを再生成して目視した**
- [x] **US33 のリリース配置を決めた**（Release 2.1・IT13）
