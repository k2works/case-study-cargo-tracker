...
**小計**: 2h（実績、元プラン 3h から A2.1 アプローチ変更で 1h 短縮）
...
| A2 fallback UX 改善 | 1 | 2h（実績、元 3h から短縮） | （M9 は ADR 起票判断） |
| A3 staging 環境構築 + E2E | 3 | 16h | H5 / H6 / H7 / H8 / H9 |
| A4 Flyway × enum 同期自動検証 | 1 | 3h | — |
| A5 Release 1.1 正式版昇格 | 1 | 3.5h | M8 / L5 / L6 |
| **合計** | **8** | **34h**（実績、元 35h から A2.1 アプローチ変更で 1h 短縮） | 高 8 件 / 中 1 件 / 低 3 件 |

**1 SP あたり**: 約 4.4h（IT9 比 1.6 倍、IT9 レビュー指摘 12 件統合の影響）
**進捗率**: 21/28 タスク完了（75%）。AI Agent 単独完結部分は上限到達。残 7 タスク（A3.1-A3.5 / A3.9b / A3.10b / A5.2 / A5.3）は staging 実機環境構築フェーズ。
