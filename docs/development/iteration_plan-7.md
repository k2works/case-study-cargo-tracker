...
| # | 内容 | 見積 | 由来 |
| :--- | :--- | :--- | :--- |
| 0.1 | **SonarQube のセキュリティホットスポット 1 件を利用者にレビューしてもらう**（依頼する） | 1h | IT5 から持ち越し・IT6 Try 7 |
| 0.2 | **ArchUnit の規則を「適用する側」から機械的に導く**。どのサービスがどの規則を呼ぶかを手書きの一覧で管理しない | 5h | IT6 Try 1 |
| 0.3 | 発行に失敗したイベントの再実行手段（運用手順書の 1 タスク） | 4h | IT6 レビュー |
| 0.4 | 取りこぼしが見える照会（発行済みだが追跡が無い予約） | 3h | 同上 |
| 0.5 | 交換機に `alternate-exchange` を足す（[ADR-022](../adr/022-domain-event-contract.md) 決定 4 に一行足す） | 2h | 同上 |
| 0.6 | 冪等を read-then-write からやめる | 2h | 同上 |
| 0.7 | **遷移の可否を集約の述語として持ち、応答に載せる。** 画面とモックが状態名を比較する形をやめる | 6h | IT6 Try 5 |
| 0.8 | **画面を割る基準を決め**、`booking-detail-page.tsx`（610 行）を割る | 5h | IT6 Try 4 |
| 0.9 | 所要日数の丸めを 1 か所に寄せる（いま 3 通り） | 2h | IT6 レビュー |
| 0.10 | `CONFIRMED` の表示名を「確定済」に揃える | 1h | 同上 |
| 0.11 | `ignoreFailures = true`（Checkstyle / SpotBugs）の方針を決める | 3h | 同上 |
| 0.12 | 契約の名簿・チャネル名を両側が読む共通定義にする | 3h | 同上 |
| **小計** | | **37h** | |
