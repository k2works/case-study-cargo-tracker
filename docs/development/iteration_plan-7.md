...
| # | 基準 | 測り方 |
| :--- | :--- | :--- |
| 1 | **荷役の記録が追跡に届く** | **達成。** kind で受領 → 積込 → 途中港の荷降し → 目的港の荷降し → 引取 を通し、追跡状態が `NOT_RECEIVED → RECEIVED → LOADED → UNLOADED → AWAITING_CLAIM → CLAIMED` と 1 段ずつ変わることを確認（`TRK-20260823-0001`） |
| 2 | **予定と違う場所の作業が、記録に残る** | **達成。** kind で Singapore（予定外）の荷降しが `off_route = t` で保存されることを確認。判定を殺すと赤になることを確認済み |
| 3 | **引取は荷受人の確認なしに記録できない** | **達成。** 集約・API・画面の 3 層と、実バックエンド E2E で固定。断りを外すと赤になることを確認済み |
| 4 | **新しいサービスが、既存の規則すべてに掛かっている** | **達成。** `ServiceArchitectureTest` の継承へ寄せ、規則の一覧をサービス側に持たせない。継承をやめると赤になることを確認済み |
...
| US15-5 | 記録後、荷主に状態変更通知が送信される | **代替**。通知の仕組みは IT8 で入る。IT7 は追跡の状態に反映されるところまでとし、その旨を画面とマニュアルに明記する |
...
| 5 | **US15-5（荷主への通知）を代替にする** | 受入基準が部分達成 | 通知の仕組み（IT8）で置き換える。**完了報告書に明記** |
...
| # | 見せるもの | 役割 | 対応 |
| :--- | :--- | :--- | :--- |
| 1 | 追跡番号を入力して貨物を特定し、**受領**を記録する | 荷役作業員 | US15-1〜4 |（実施済み。ACL が実際に往復し、予約番号 `BKG-2026000068` が戻った）
...
- [x] 対象ストーリーの受入基準を満たす（**スコープ外にしたものは理由とともに完了報告書へ**）。US15-5 は代替、US16-4 は範囲外（[ADR-023](../adr/023-handling-activity-validation.md)）
- [x] 成功基準 1〜5 を満たす（成功基準の表に達成の根拠を記載）
- [x] 全テストが緑（単体・統合・契約・E2E）。**CI のジョブすべて success**
- [x] **`TZ=UTC` でも緑**（CI の専用ジョブ）
- [x] **ドメイン層のカバレッジが 90% 以上**（5 サービス + shared）
- [x] SonarQube Quality Gate が PASS（**両プロジェクト**。0.1 は修正で閉じたため前提は解消）（**バックエンドは 0.1 のホットスポットが前提**）
- [x] **決定の数だけ検査がある**（レビューで 2 件不足を検出し、追加した）（ADR-023 のコンプライアンス表）
- [x] **壊して赤を確認した**（約 20 回）（荷受人確認の必須・予定外の判定・種別ごとの要件・イベントの往復・デッドレター）
- [x] **新しいサービスが既存の規則すべてに掛かっている**（0.2。handlingms を外すと赤）
- [x] **サービス間の呼び出しを実際に 1 往復させた**。REST は kind、イベントは Testcontainers と kind の両方
- [x] **契約の検査を「本番の呼び出し形」で組み立てた**（Try 2）
- [x] **実環境で確かめる前にイメージを作り直した**（Try 3。Pod の `Started:` と修正コミットの時刻を突き合わせた）
- [x] **画面を割る基準を決めてから割った**（400 行・`max-lines`）（Try 4）
- [x] **モックを足した変更で、本物の該当箇所と読み比べた**（差分をコミットメッセージに記載）
- [x] **前 IT で「まだできません」と書いた箇所を棚卸しした**（4 箇所）
- [x] **ナビゲーションの 4 点が一致している**（検査が 2 回反応した）。ロール別・状態別の到達性を確かめた
- [x] **デモ項目 7 件をこの順で通した**（kind 統合環境）
...
| Phase 2 ACL とイベント | 完了（追跡番号での照会・ACL・イベントの往復。`TrackingStatus` へ改名） |
| Phase 3 永続化と API | 完了（`handling_activity`・4 エンドポイント・方言スモーク） |
| Phase 4 画面 | 完了（`/handling`・ナビの 4 点一致。`/handling/list` は作らない——理由を `ui_design.md` に記載） |
| Phase 5 結合の検査 | 完了（kind でデモ 7 件・実バックエンド E2E 12 本） |
| ユーザーマニュアル | 完了（第 8 章・キャプチャ再生成・前 IT の棚卸し） |
| レビュー手直しの枠 | 完了（[レビュー結果](../review/イテレーション7_review_20260823.md)。高 11 件すべて対応） |
