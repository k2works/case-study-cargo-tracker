...
| 5 | 同時実行上限に達したとき `503` + `Retry-After` が返る | 上限を **1** に落とした構成で同時到達させ、`503` を観測する（計画時は 2 としていたが、確実に上限へ達する 1 を採った） |
...
### 実績（2026-08-04 記入。ここから下はクローズ時の追記である）
...
| N5 | **タイムアウト時の過剰入場**（枠は返るが処理は走り続ける） | `cancel(true)` は割り込み要求にすぎず JDBC は応答しない。恒久対策 2 案（枠の解放をタスク側へ／`setQueryTimeout` の併用）は **TS12b の実測後に決める**。測らずに設計を変えない |
| N6 | 候補一覧に**件数上限・絞り込み・並べ替えが無い** | 候補が 10 件を超えると選べない（user-rep）。少なくとも表示件数の上限を IT8 で入れる |
| N7 | 依頼条件に**重量・容積・危険物クラス・設定温度が無い** | 船・港を選ぶときに実際に見る項目。予約詳細と往復することになる（user-rep） |
| N8 | **最低接続時間**（同一港 24 時間・危険物 48 時間） | 業務からの回答が得られた。設定として外へ出せる形で IT8 に入れる |
| N9 | 合成ルートの肥大（`AppEff` 13 効果・`runWithoutDb` 7 段） | BC 追加のたびに 4 箇所を直す。**IT8 序盤の独立コミット枠**で BC 別へ畳むか、明示的にスコープ外と宣言する（architect） |
...
| 2026-08-04 | 実績・計画になかった対応・ロール × 画面の到達性・型変換経路の突合を追記（クローズ作業） |
| 2026-08-04 | レビュー指摘を反映（成功基準 5 の上限値を実装に合わせ、引き継ぎに N5-N8 を追加） |

> **本書は計画と実績が混在している**（IT7 レビュー・technical-writer 指摘）。
> 「実績」「計画になかった対応」「デモ項目の結果」はクローズ時の追記であり、
> 計画時点の判断ではない。読み分けの手掛かりとして各節に記入時点を添えた。
> **次の IT からは実績を完了報告書へ寄せ、計画書は計画時点で凍結する**
> （ふりかえりの Try とする）。
