...
- [ ] US08: 受入基準 1・2・4・5・6 を充足。3 は費用を除いて充足（未充足を報告書に明記）
- [ ] US07: 受入基準 1 と 3 の一部（寄港地接続）を充足
- [ ] TS12: `503` + `Retry-After` と `504` を**同時到達・遅延ハンドラで観測**
- [ ] TS13: navbar の行き止まり 0 件。ルート × ロール可否表が現在のルートと一致
...
| # | デモ | 受け入れ手段 |
|:--:|------|------------|
| 1 | 引き渡し済みの予約に対し、直行便が最優先の候補として提示される | HTTP 統合テスト |
| 2 | 寄港地を経由する 1 回積み替えの候補が提示される | HTTP 統合テスト |
| 3 | **期限当日に着く航海が候補に残る** | `RouteFinderTest` の単体テスト |
| 4 | 接続時刻が逆転する組み合わせは候補に出ない | `RouteFinderTest` の単体テスト |
| 5 | 条件を満たす経路が無いとき、その旨と協議依頼の導線が出る | HTTP 統合テスト |
| 6 | 同時実行が上限に達すると `503` + `Retry-After` が返り、**その後に後続が成功する** | `RuntimeGuardTest` |
| 7 | navbar のどのリンクも行き止まりにならない | `NavigationReachabilityTest` |
