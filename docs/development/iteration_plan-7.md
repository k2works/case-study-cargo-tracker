...
- [~] **繰り越し品質ゲートの決着（4 IT 連続繰り越しを止める）**: SonarQube SQ-3（S6853 33→0）/SQ-2（S6967 8→1）・セキュリティホットスポット（3→0 SAFE レビュー）は決着（Bug 0・脆弱性 0・全評価 A・重複 1.0%）。カバレッジ 85% ハードゲートを backend-ci.yml に導入（ドメイン層 92.6%・PASS）。Playwright E2E はブラウザ実行環境前提で繰り越し
...
| 4.2 | カバレッジ 85% ハードゲートを CI に段階導入（operating-cicd・全体マージ計測 → ゲート化） | 4h | - | [x]（coverage-gate.py を追加し backend-ci.yml に「ドメイン層 85% ハードゲート」ステップを導入。全テストプロジェクトの cobertura を (ファイル,行) でマージ集計。ローカル完全再現でドメイン層 92.6%・PASS を確認。プレゼン層は Web.Tests 統合で担保のためドメイン層に限定） |
