...
- [x] E2E 異常系シナリオ（OVERDUE・バリデーションエラー）が追加されている
- [x] 「予約確定」状態の予約に追跡番号を発行できる
- [x] 追跡番号は一意に採番される
- [x] 発行後、貨物状態が「追跡番号発行済」に設定される
- [x] 追跡番号で貨物を特定して作業を記録できる
- [x] 記録後、貨物状態が対応する状態（受領済・積込済・荷降し済）に更新される
- [x] テストカバレッジ 80% 以上（新規コード 81.7%）
...
| # | タスク | 見積もり | 担当 | 状態 |
|---|--------|---------|------|------|
| 1.1 | SonarQube スキャン実行・Quality Gate 確認 | 1h | - | [x] |
| 1.2 | SonarQube 指摘事項の修正（Critical/Major） | 2h | - | [x] |
| 1.3 | E2E 異常系シナリオ追加（billing.spec.ts の OVERDUE フロー） | 2h | - | [x] |
| 1.4 | E2E バリデーションエラーシナリオ追加 | 1h | - | [x] |
...
| # | タスク | 見積もり | 担当 | 状態 |
|---|--------|---------|------|------|
| 2.1 | Tracking コンテキスト: `TrackingNumber`・`TrackingBookingId` 値オブジェクト実装（TDD） | 2h | - | [x] |
| 2.2 | Tracking コンテキスト: `TrackingNumberIssuer` ドメインサービス実装（TDD） | 2h | - | [x] |
| 2.3 | Booking → Tracking: 追跡番号発行イベント連携（ACL ポート） | 2h | - | [x] |
| 2.4 | Tracking コントローラ: 追跡番号発行 API エンドポイント実装 | 1h | - | [x] |
| 2.5 | Thymeleaf: 追跡番号発行画面実装 | 1h | - | [x] |
| 2.6 | E2E テスト: 追跡番号発行シナリオ作成 | 2h | - | [x] |
...
| # | タスク | 見積もり | 担当 | 状態 |
|---|--------|---------|------|------|
| 3.1 | Tracking コンテキスト: `TrackingActivityEvent`・`TrackingEventType` 実装（TDD） | 2h | - | [x] |
| 3.2 | Tracking コンテキスト: `TrackingActivity` 集約・`TrackingStatus` 遷移実装（TDD） | 2h | - | [x] |
| 3.3 | Tracking コンテキスト: 荷役記録アプリケーションサービス実装（TDD） | 2h | - | [x] |
| 3.4 | DB マイグレーション: V14 tracking_handling_event テーブル + MyBatis マッパー実装 | 1h | - | [x] |
| 3.5 | 荷役作業記録 API エンドポイント実装 | 1h | - | [x] |
| 3.6 | Thymeleaf: 荷役作業記録画面（作業種別選択・場所入力）実装 | 2h | - | [x] |
| 3.7 | E2E テスト: 荷役作業記録シナリオ（受領・積込・荷降し）作成 | 2h | - | [x] |
| 3.8 | E2E テスト: 異常系シナリオ（追跡番号不在・場所不一致警告）作成 | 1h | - | [x] |
