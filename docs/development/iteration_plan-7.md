...
| ID | ストーリー | SP | 結果 |
| :--- | :--- | :--- | :--- |
| US18 | 追跡情報を照会する | 3 | **完了**（認証つき / 公開の 2 画面・ACL ポート・レートリミット） |
| US16 | 引取作業を記録する | 3 | **完了**（引取確認・配送完了への伝播・荷受人登録）。署名は範囲外として明記 |
| US03 | 法人荷主を登録する | 2 | **完了**（契約番号・割引率）。US22 の `ShipperDiscountPort` は範囲外として明記 |
| | **合計** | **8** | **達成率 100%** |
...
| # | 内容 | 結果 |
| :--- | :--- | :--- |
| 0-1 | 購読側の失敗を見る手段（C10 / T10） | `EventualConsistencySkips` と `operation.md` §3.2.2。あわせて `@CheckReturnValue` で T2 の規律を SpotBugs に移した |
| 0-2 | `TrackingSequence` の置き場と H2 方言スモーク（C5） | `infrastructure/repositories` へ移設。`nextval` を検査対象に追加 |
| 0-3 | 確定の `CONFLICTED`・荷役種別の網羅（C6） | 追加。種別を 1 つ足すと `switch` がコンパイルエラーになる形 |
| 0-4 | 荷役一覧の追跡番号・スキャナ入力（C3 / H12 / H14） | `V13` で列追加。前後空白・改行・大小文字を整える |
| 0-5 | 予約一覧の追跡番号検索（C1 / H9） | 部分一致・大小文字を問わない検索 |
...
| コミット数 | 15 |
| 本番コード | 222 → **267** ファイル |
| テスト | 50 → **67** ファイル（**794 テスト**） |
| 行カバレッジ | **93.1%**（分岐 73.1%） |
| `./gradlew clean check` | **緑**（Checkstyle 0 / SpotBugs 0 / ArchUnit 6 ルール） |
| CI（Backend CI） | **緑**（run 31245066674） |
| SonarQube Quality Gate | **PASS**（Bug 0 / Vulnerability 0 / Code Smell 0 / 重複 0.5% / 新規カバレッジ 88.0%） |
| E2E | **5 件緑**（クリティカルパス 3 本を達成） |
| マイグレーション | 12 → **14**（V13 / V14） |

### 壊して赤を確認した安全装置（11 件）

**入口と出口の両方で回した**（IT6 ふりかえり T1 への対応）。

| 装置 | 壊し方 | 結果 |
| :--- | :--- | :--- |
| 方言スモーク（採番） | `nextval` を PostgreSQL 専用構文に | 赤 1 件 |
| 購読側の取りこぼし計測 | `skips.recordSkip` を握りつぶしに | 赤 2 件 |
| 戻り値の握りつぶし検出 | 呼び出し元で戻り値を捨てる | SpotBugs `RV_RETURN_VALUE_IGNORED` |
...
| # | 内容 | 対応 |
| :--- | :--- | :--- |
| 1 | **テスト基盤が MockMvc にアプリのフィルタを組み込んでいなかった** | Security だけを適用していたため、横断的な防御を足しても「テストからは存在しない」状態だった。同ファイルのコメントが Security について警告していたのとまったく同じ欠陥であり、すべての `Filter` Bean を組み込む形に直した |
| 2 | **`@CheckReturnValue` 導入でテストの Arrange 11 件が握りつぶしていた** | 楽観的ロックの結果を捨てており、失敗した準備の上でアサートしていた。すべて検証する形に修正 |
| 3 | **公開追跡へ未認証利用者が到達できなかった** | ロール別到達性は `sec:authorize` で動くため認証済み利用者にしか働かない。ログイン画面とポータル（`apps/www/index.html`）に導線を追加 |
| 4 | Checkstyle が 3 回、設計上の合図を出した | パラメータ数上限 2 回（`HandledCargo`・`HandlingDetails`）と循環的複雑度 1 回（`apply` の boolean 3 個）。いずれも上限を緩めず概念を見直した |
| 5 | E2E とキャプチャが開発サーバとポートを奪い合っていた | 専用ポート（18080 / 18081）に分離。開発サーバを止めずに回せるようにした |
