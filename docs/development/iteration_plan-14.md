...
| # | タスク | 見積 | 状態 |
| :--- | :--- | :--- | :--- |
| 3.1 | デモ利用者としてログインし、Gateway 経由で呼ぶ実行基盤を作る（**内部 API を使わないことを検査で固定**） | 8h | [x] |
| 3.2 | 予約登録 → 経路確定 → 予約確定 → 追跡番号発行の 4 工程を実装する | 8h | [x] |
| 3.3 | 荷役 → 通関 → 引取 → 料金算出 → 精算の 5 工程を実装する | 8h | [x] |
| 3.4 | 二重実行の拒否と、工程ごとの独立トランザクション（巻き戻さない）を固定する | 5h | [x] |

### Phase 4: 由来の識別と除外（US34）

| # | タスク | 見積 | 状態 |
| :--- | :--- | :--- | :--- |
| 4.1 | 荷主コードの `SIM-` 帯を追加する（**列は置かない**——帯と列が食い違う行が生まれるため、[ADR-030](../adr/030-business-simulation-execution.md) 決定 3 を改訂） | 4h | [x] |
| 4.2 | 精算の締め対象・荷主向け一覧から、シミュレーション由来を除外する | 6h | [x]（**未解決例外一覧は IT15 へ持ち越し**。trackingms は荷主コードを持たない。[ADR-030](../adr/030-business-simulation-execution.md) に明記） |
| 4.3 | **除外側の検査**を置く（シミュレーション由来を 1 件作り、各一覧に出ないことを確かめる） | 5h | [x]（実 DB で 4 件。名指しの照会では返ることも固定） |
...
- [x] US34 / US35 の受け入れテストが Red → Green で実装されている。
- [x] シミュレーションが内部 API を呼んでいないことを検査で固定している（**ArchUnit ではなくソースを読む検査**。ArchUnit には文字列で書かれた URL の向き先が映らない）。
- [x] 本番環境で起動が失敗することを検査で固定している。
- [x] シミュレーション由来が業務一覧から除外されることを、除外側の検査で固定している（**精算の締め対象・荷主一覧**。追跡の未解決例外一覧は IT15 へ持ち越し。[ADR-030](../adr/030-business-simulation-execution.md) に明記）。
- [x] 失敗しても途中の業務データが残ることを、失敗を注入するテストで固定している。
- [x] `ROLE_ADMIN` のナビゲーションと遷移導線が実装され、押した先で 403 にならない。
...
- [x] **最後のコミットに対して** SonarQube を回し、Quality Gate が PASS である（IT13 Try 5'）。
- [ ] 5 視点レビューを実施し、高優先度をすべて対応または明示的に送っている（**クローズ時に実施**）。
