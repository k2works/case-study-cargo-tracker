...
- [ ] US23 の受入基準（**受入基準ごとに「その道を実行したテスト」を名指しする**）
- [ ] デモ項目 15 件が動作する
- [ ] **返済枠 10 件（C1〜C7 / C12 / C14 / C16）を返した**

#### 精算局面の完了条件（`development_strategy.md`）

- [ ] 精算までの業務シナリオが通しで動作する（予約 → 追跡 → 引取 → 料金算出 → 請求 → 精算）
- [ ] ArchUnit の全ルールと ADR-015 の検査が緑
- [ ] **ROLE_BILLING の月次業務が実際に回せる**（T3。R1 に書いた作業をテストで固定する）

#### 品質

- [ ] `./gradlew check` が緑
- [ ] **`TZ=UTC ./gradlew check` が緑**（支払期限は日付である）
- [ ] CI が緑・SonarQube Quality Gate が PASS

#### 主張とテスト

- [ ] **T1: 表示用の型に計算を書いていない。** View の値は集約が計算したものである
- [ ] **T4: 「実装しなかった守り」を数えた。** 受入基準・マニュアル・**本番コードの Javadoc・テンプレートのコメント**が対象
- [ ] **T2: 組み合わせを数え上げた**（片方ずつは通っているが両方は踏んでいない道）
- [ ] **T5 / T6: 破壊検証を既存のテストにも向けた。** テストが赤になったかで判定した
...
- [ ] 経理担当者がダッシュボードから**督促対象一覧**へ到達できる（navbar とカードの両方）
- [ ] **確定済みの請求書から発行を開けること**を確認した（状態軸の到達性）
- [ ] **下書きの請求書からは開けないこと**を確認した
- [ ] **入口があるロールに「利用できる機能はありません」と併記されない**（C16。全ロールで機械的に確かめる）

#### ドキュメント

- [ ] 設計への反映を完了（12 件）
- [ ] **ADR を 2 件起票**（一部入金を認めない判断・期限超過の判定を画面で行う判断）
- [ ] **マニュアル 11 章に発行・入金確認・督促を追記**＋**キャプチャを再生成**（生成 spec 経由）
- [ ] 用語集・付録 B・索引を同期し、`mkdocs build` の警告が 0
