...
tags: [plan]
status: draft
generated: { by: claude-code/claude-opus-5, at: 2026-09-06T10:08:02Z }
---
...
**クローズ時点の実績です。** 満たせなかったものは印を付けず、理由を書きます。

- [x] デモ項目の受け入れテストがすべて緑（7 件すべてに本文のアサーションが対応）
- [x] `TZ=UTC ./gradlew build` が緑（JaCoCo の層別閾値を含む。クローズ時に実測）
- [x] フロントの `npm run test`（256）・`npx tsc -b`・`npm run build` が緑
- [x] `./gradlew :acceptance-tests:test` が緑（8 feature）
- [ ] ~~**層別カバレッジと SonarQube を、US ごとに 1 度回した**~~ → **守れませんでした（IT5 に続き 2 回目）。** 層別カバレッジ（JaCoCo）は US ごとでなく**CI が落ちてから**回し、2 回赤にしました。SonarQube はクローズで初めて回しています。Day 4・Day 5・Day 8 のタスクとして置いたのに、実際には踏んでいません
- [x] **実環境で症状を見たときに、テストを直す前に実装を疑った**（IT5 の T1）。クラスタ E2E で旅程が消えたときは実装を直しました。航海の欄名でテストを直した 1 件は、H.3 で欄名を意図的に変えたためで、実装の欠陥を隠していません
- [x] **javadoc・設計文書・テスト名に「〜する」と書いたら、同じ変更でその検査を書いた**（IT5 の T2）。**ただし完全ではありません**——`ui_design.md` に書いた「旅程は残ります」は検査が無く、クラスタ E2E で破れているのが見つかりました（レビュー前に自分で発見・修正済み）
- [x] **画面から呼ぶ API を、HTTP の層で 1 本は通した**（IT5 の T3）。US10・US12 の全経路で正常系と固有の異常系を通しています
- [x] **受入基準を 1 行ずつ「本 IT で満たす / 未達（前提は何）」に割り当てた**（IT5 の T5）。**ただし「等」でまとめられた列挙を数え落とし**、US10 §2 の「貨物種別変更」をクローズの自前点検で追加しました
- [x] **追加・変更した画面を、それを見る他のロールから何が読めるか確かめた**（IT5 の T6）。通知履歴は経路設計者も読め、操作は出ません
- [x] **内部の列挙名を利用者に見せていない**（IT5 の T7）。さらに**呼び名が設計の要素表と一致することを canon テストで固定**しました（実装だけ逸れていたのを見つけたため）
- [x] **クラスタ E2E を Day 9 に 1 度、クローズ前にもう 1 度回した**（14/14 を 2 回）
- [x] SonarQube の Quality Gate がバックエンド・フロントエンドとも PASS
- [x] `npx gulp okf:check` が ERROR 0
- [x] ユーザーマニュアルの該当章が更新され、画面キャプチャが再生成されている（05 章の撮り直しを含む。**10 章を新設**）
- [x] **並列レビューをクローズの最初に起動し、切れていないか確かめてから統合した**（IT5 の T8）。**ただし返着前に「返らない」と誤判断し、誤った記録を 3 文書に書きました**（返着後に全面訂正）。2 視点は本文が上限で切れたので、短い形で送り直してもらっています
