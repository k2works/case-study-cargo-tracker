...
- [x] US10・US11・US12 の受入基準（`user_story.md`）を満たす。**ただし US12 §受入基準 2 の「料金概算」と US10 §受入基準 2 の「経由地追加」は未達**（前者は US21・IT13、後者は探索の入力に「必ず通る港」が無い。完了報告書に記録）
...
- [ ] ~~クラスタに対して E2E が緑（Day 9 とクローズ前の 2 回）~~ → **1 回のみ**（Day 9 に 14/14 緑）。クローズ前の 2 回目は、Day 9 のあとバックエンドを変えていない（クローズで触ったのはフロントの JSX 2 行と Sonar 抑制コメント）ため回していません。**2 回と書いたのに 1 回なので、印は付けません**
