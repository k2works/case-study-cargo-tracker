...
| IT14 見積・精算 | [計画](iteration_plan-14.md) | — | — | **計画済み**（US01 見積・US23 精算。8 SP + 引き継ぎ枠 3・負債枠 2）。**見積から精算まで金額の流れが 1 本につながる IT**——新設は `Quotation` 1 つで、US23 は `Invoice` に発行・入金・取消を足し `Cargo` に `SETTLED` を通す。IT13 の引き継ぎ 12 件のうち**発行より前に要る 3 件**（確認の跡・算出漏れ・調整の取り消し）を序盤の枠で消化する |
| IT15 | 未作成 | — | — | 未着手 |
