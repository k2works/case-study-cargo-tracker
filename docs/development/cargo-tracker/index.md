...
| IT16 業務シミュレーション | [計画](iteration_plan-16.md) | — | — | **計画済み**（US33 シナリオの自動実行・US34 工程ごとの結果。8 SP + 引き継ぎ枠 2・負債枠 2）。**Release 3.0 の最初**で、`simulationms` を新設し**本番と同じ API（Gateway 経由）**で予約から精算までを通す。新しい業務ロジックは足さず、**呼ぶ順序と待ち方**を組み立てる。注 7 件（ES にするか・HTTP 越境の前例が無い・由来の印をどう運ぶか ほか） |
