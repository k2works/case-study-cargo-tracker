...
| IT10 引取と遅延例外 | [計画](iteration_plan-10.md) | — | — | **計画済み**（US16(3)・US19(4) + 負債枠 1）。荷受人の確認を取って引取を記録すると「引取済」になり精算が始まる。遅延を起票すると「例外発生」になり、解決すると**例外前の状態へ戻る**。着手前の検証で**正典側の欠落を 4 件**（注 N1〜N4）見つけ、`ShipperNotifiedEvent` の再利用が実装不能であることを止めた |
| IT11〜IT15 | 未作成 | — | — | 未着手 |
