...
| IT15 キャンセル承認 | [計画](iteration_plan-15.md) | — | — | **計画済み**（US30 キャンセル承認。6 SP + 引き継ぎ枠 4・負債枠 2・仕上げ枠 2）。**Release 2.0 の最後**で、bookingms → trackingms → handlingms → trackingms → billingms の 4 サービス連鎖を通す。**承認しても追跡は閉じない**——陸揚げ地での荷降しが記録されてから閉じる。IT14 の未達（入金が予約へ届かない）の修復を**最優先タスクに置き、SP には数えない** |
