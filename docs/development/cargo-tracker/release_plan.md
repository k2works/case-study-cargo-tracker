...
| IT15 | 仕上げ 2 + **引き継ぎ 3** | 記事第 5 章の参照元としての整理（README・実行手順）。**引き継ぎは IT14 の中 3 件**——(1) **`dead_letter_entry` の業務向け読み口**（運用の gulp タスクと actuator はあるが、退避されたことに気づけるのは運用担当だけ）、(2) **Processing Group を書くテーブルの単位に分け直す**（いまはパッケージ単位なので、リプレイと退避の単位が正典より粗い。token の移行が要る）、(3) **入金の取り消し**（誤って記録した入金を戻す操作。マニュアル 17 章が「いまは運用で引き取る」と書いている）。**「余力次第」にせず行として立てます** |
...
| IT14 | 8 | 8 | 100% | **完了**（[計画](iteration_plan-14.md) / [ふりかえり](retrospective-14.md) / [完了報告書](iteration_report-14.md)）。US01・US23。**業務の輪が閉じた**——見積 → 予約 → 輸送 → 引取 → 発行 → 入金 → 精算済。引き継ぎ枠 3 件を**繰越ゼロで返済**（10 回連続）。**クローズのレビューで高 9 件**（8 件を修正・1 件は事実誤認）。最重は**受け入れテストが書かれていたのに 1 つも回っていなかった**こと——回したら実欠陥が 3 件出た。**フルビルドも前のコミットから赤のまま**だった（カバレッジゲート 2 モジュール）。ADR-0017 を起票 |
| IT15 | 6 | — | — | 未着手 |
| **累計** | **122** | **116** | **100%**（IT1〜IT14 の 116 SP に対して） | |
...
| 2026-09-12 | IT14 クローズ。実績 SP 8（達成率 100%・累計 116・**14 回連続**）と引き継ぎ枠 3 件の返済（**10 回連続で繰越ゼロ**）を記録。**Release 2.0 の 22 SP のうち 16 を消化**。**成功基準が 2 か所で事実に反していた**——受け入れテストはステップ定義が 1 つも無く、フルビルドはカバレッジゲート 2 モジュールで赤のままだった。どちらも一覧の上では揃って見える形。IT15 へ 3 件を引き継ぐ（`dead_letter_entry` の業務向け読み口・Processing Group の分け直し・入金の取り消し）。**「余力次第」にせず行として立てる** | claude-code/claude-opus-5 |
