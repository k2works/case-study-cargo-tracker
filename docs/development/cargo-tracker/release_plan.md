...
| IT15 | 6 | — | — | **計画済み**（[計画](iteration_plan-15.md)）。US30 キャンセル承認。**Release 2.0 の最後**。4 サービス連鎖（booking → tracking → handling → tracking → billing）。**承認しても追跡は閉じず**、陸揚げ地の荷降しで閉じる。**IT14 の未達（入金が予約へ届かない）の修復を最優先タスクに置き、SP には数えない**。引き継ぎ枠 4 + 負債枠 2（IT13 から 3 IT 目の 7 件）+ 仕上げ枠 2 |
| **累計** | **122** | **115** | **99%**（IT1〜IT14 の 116 SP に対して 115） | |
