...
1. IT5 の計画を作る（`opening-iteration`）。[IT4 のふりかえり](retrospective-4.md) の引き継ぎを返済枠に配分する
2. IT4 で先に置いた `VoyageCancelledEvent` の**発行側（航海キャンセル）をストーリーにする**。今は不変条件 5 に到達する手段がイベントの直接適用しかない
3. IT5 でも **Day 8 あたりでクラスタ E2E を 1 度回す**（IT4 では 2 つの実害をここでだけ捕まえた）
4. `syncing-github-project` で IT4 の Issue をクローズし、IT5 の Status を更新する
