...
| IT5 | 10 | — | — | **開発中**（[計画](iteration_plan-5.md)）。US08・US09 の実装完了。引き継ぎ枠 6 件は**繰越ゼロで返済**。残るはクラスタ E2E とキャプチャ生成 |
| IT6〜IT15 | 77 | — | — | 未着手 |
| **累計** | **122** | **35** | **100%**（IT1〜IT4 の 35 SP に対して） | |
...
1. ~~IT5 の計画を作る（`opening-iteration`）~~ **完了**（[IT5 計画](iteration_plan-5.md)）。IT4 の引き継ぎ 14 件は、引き継ぎ枠 6 件と「扱わない」8 件（行き先つき）に配分した
2. ~~IT5 の **Day 4 で契約クエリの往復を確かめる**~~ **実施済み**。`ContractQueryRoundTripIT` が bookingms → routingms を同じ JVM で通す。[ADR-0001](../../adr/cargo-tracker/0001-cqrs-es-with-axon-in-microservices.md) 決定 4 の見直しは不要だった
3. IT5 の **Day 9 でクラスタ E2E を 1 度回す**（IT4 では 2 つの実害をここでだけ捕まえた）
4. **IT5 は 10 SP で、過去最大（9）を超える。** 未達で終わるなら US08 を完成させ、US09 は「未着手」と正直に記録する
