...
| IT6 | 8 | — | — | **計画済み**（[計画](iteration_plan-6.md)）。US10・US11・US12。IT5 引き継ぎ枠 3 件（SP 対象外） |
| IT7〜IT15 | 69 | — | — | 未着手 |
| **累計** | **122** | **45** | **100%**（IT1〜IT5 の 45 SP に対して） | |
...
1. ~~IT5 を完了する~~ **完了**（[ふりかえり](retrospective-5.md) / [報告書](iteration_report-5.md)）。10 SP を達成し、引き継ぎ枠 6 件も返済した
2. ~~IT6 の計画を作る（`opening-iteration`）~~ **完了**（[IT6 計画](iteration_plan-6.md)）。IT5 の引き継ぎ 12 件は、引き継ぎ枠 3 件と「扱わない」8 件（行き先つき）に配分した
3. ~~US10 を IT6 に置くか判断する~~ **置く**。リリース計画の当初どおりで、US09 §受入基準 4 の未達は US10 の完成で解消する
4. **クラスタ E2E は IT6 でも 1 度回す。** IT5 では、モックと単体では出ない実害をここで捕まえた
