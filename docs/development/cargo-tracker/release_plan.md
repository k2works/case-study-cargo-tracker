...
| IT5 | 負債（追加） | **当初は置いていません。** IT4 のふりかえりで高 2 件（航海キャンセルの操作が無い・US32 の「何を変えたか」の読み口が無い）が出たため、IT5 に限って置きます。「余力次第」にはせず、序盤の独立コミットで消化します |
...
| IT5 | 10 | — | — | **計画済み**（[計画](iteration_plan-5.md)）。引き継ぎ枠（SP 対象外） |
| IT6〜IT15 | 77 | — | — | 未着手 |
...
2. IT5 の **Day 4 で契約クエリの往復を確かめる**。ここで詰まったら [ADR-0001](../../adr/cargo-tracker/0001-cqrs-es-with-axon-in-microservices.md) 決定 4（サービス越しの問い合わせを Query Bus で行う）を見直す
