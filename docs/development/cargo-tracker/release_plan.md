...
2. IT6 の計画を作る（`opening-iteration`）。IT5 のふりかえりが挙げた引き継ぎ（05 章のキャプチャ・航海キャンセルの影響範囲・時刻表記の統一）に行き先を与える
3. **US10（経路条件を調整して再算出する）を IT6 に置くか判断する。** US09 §受入基準 4 の未達は US10 が前提で、経路設計の一連の作業はそこまで揃って初めて回る
