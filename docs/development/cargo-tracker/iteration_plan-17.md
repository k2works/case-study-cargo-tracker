...
| 2 | 実行間隔・同時実行数・例外の発生比率を設定でき、**設定した上限を超えて実行しない** | `SimulationScheduleTest`（上限を 1 超える要求を断る）・`SimulationSchedulePropertiesTest`（設定名を明示的に受ける） |
...
| 6 | 本番環境では起動しない | `SimulationSchedulePropertiesTest`（既定は無効）・**無効な設定の実物を立てた受け入れ**（IT16 と同じ形） |
| 7 | 実行中も**ヘルスチェックと実利用者の操作は劣化しない** | `HealthProbeUnderLoadIT`（継続実行中に `/actuator/health` が 200 を返し続ける）・クラスタ E2E（継続実行中に営業の予約登録が通る） |
| 8 | 実行の統計（件数・成功／失敗の内訳・失敗した工程の分布）が US34 と同じ形式で読める | `SimulationStatisticsIT`・S94 の画面テスト |
...
| 3 | 一度出た知らせは、**別の端末で開き直しても**もう一度出ない | `NoticeReadPositionIT`（**既読はサーバが持つ**。ブラウザの記憶を消しても出ない） |
| 4 | 他社の貨物の知らせは、追跡番号を知っていても届かない | `TrackingNoticeQueryIT`（荷主 ID で絞る）・`RoleAuthorizationDeclarationTest` |
...
- [ ] US35・US36・US37 の受入基準 19 件がすべて緑（未達があればその旨を明記）
- [ ] **受入基準の「落とし先」列に空欄が無く、書いた検査名が実在する**（**Try T2 の前提**。IT16 は空欄が無くてもフロントの検査を 1 つも名指ししていなかった）
- [ ] **入口を足した変更では「意図した相手以外が通る道」を書き出した**（**Try T1**。US36 の開始・停止、US37 の知らせの読み口が対象）
- [ ] **複数の行き先・複数の一覧が出てくる受入基準は、検査を数え上げる形で書いた**（**Try T2**。US37 §2 の行き先、US36 §5 の読み口）
- [ ] **検査を書いたら、その場で壊して赤を見た。コミット本文に壊し方を 1 行書いた**（**Try T3/T7**・4 IT 目。IT16 は 13/30 コミット）
- [ ] **除外・絞り込みを足したら、同じ列を読む読み口を数え上げて表にした**（**Try T4**。ADR-0020 決定 4 の表を「未」ゼロにする）
- [ ] **待ち条件は「自分を数えていないか」を確かめてから回した**（**Try T5**）
- [ ] `TZ=UTC` の分割フルビルドが緑
...
- [ ] **マニュアル 20 章の更新（継続実行）と荷主向けの知らせの節**。画面キャプチャを生成 spec で撮る
...
| 引き継ぎ 2 | **S45 自社予約一覧・S46 自社予約の進み具合のルートが無い**（正典にはある） | IT16 計画 D2（**6 IT 連続で起票されず**） | T0。**US37 のポップアップの行き先**になる |
...
| R6 | **既読をブラウザに持つ誘惑** | US37 §3 を `NoticeReadPositionIT` で固定する（ブラウザの記憶を消しても出ない） |

## DoD

- [ ] US35・US36・US37 の受入基準 19 件がすべて緑（未達があればその旨を明記）
- [ ] **受入基準の落とし先に空欄が無く、書いた検査名が実在する**
...
- [ ] CI が緑
- [ ] SonarQube の Quality Gate が両プロジェクト PASS
- [ ] **マニュアル 20 章の更新（継続実行）と荷主向けの知らせの節**・キャプチャ
- [ ] 正典（`domain-model` / `data-model` / `ui_design` / `architecture_backend`）に `SimulationSchedule`・`notice_read_position`・S94・S45/S46 を反映した
- [ ] **ADR-0020 決定 4 の表から「未」が消えた**（3 つの読み口を閉じた）
- [ ] ADR-0021（知らせを押し出さない）を起票した
- [ ] **`IterationPlanChecksExistTest` の `CURRENT_PLAN` を `iteration_plan-17.md` へ進めた**（**クローズのときに進める**）
- [ ] 引き継ぎ 3 件・負債 2 件を返した
- [ ] `npx gulp okf:check` が ERROR 0
