...
- [ ] **レビューは 5 通そろうまでクローズしない。返らない視点には再送した**（**Try T7**。IT11 は user 視点が 1 時間後に返り、**高 3 件**を持っていた） — **未**（クローズのステップ 1 で実施する）
- [ ] **レビューの「達成と書いたが実務では足りない」欄を、受入基準の表に突き合わせた**（**Try T9**。返着を待ってから実績表を確定する） — **未**（クローズのステップ 1 で実施する）
