...
| N3 | **シミュレーション由来の印をどう運ぶか** → **決めた: `ShipperRegisteredEvent` に `simulated` を足す** | 当初は「契約の版上げに Upcaster が要る」と見ていたが、**`golden-legacy` の仕組みが既にある**（`LegacyContractPayloadTest` が「項目が無い古い JSON を読む」ことを固定している）。既存のイベントは `simulated = false` になり、**業務上正しい**（既存の荷主は本物である）。**専用の荷主種別にしない**——由来と荷主種別は直交する（シミュレーションでも法人割引を踏みたい）し、`ShipperType` は割引の判断に使われている | T7 で実装。**古い形の JSON を `golden-legacy` に足して**、読めることを固定する。除外は**各 BC の読み口の側**に置く（印だけでは混ざる・注 N7） |
