...
| # | タスク | 見積 | 状態 |
| :--- | :--- | :--: | :--: |
| S.1 | ADR-0001 決定 5 第 7 項：S3 へエクスポートした Event Store からの差分再投入。RPO の根拠を実測する | 2h | [ ] |
| S.2 | 結果を ADR-0001 決定 5 と `non_functional.md`（「未検証」の記述）に反映 | — | [ ] |
| | 小計 | 2h | |
...
| 4 | US31 §受入基準 3（ロックが利用者に通知される）は送信基盤がスコープ外。監査ログと S90 での可視化で満たす解釈を明示する必要がある | [ユーザーストーリー](../../requirements/user_story.md) の通知に関する注記 | 注記の対象に US31 を含める |
