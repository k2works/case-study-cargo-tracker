...
| §3 | ルート候補ごとに「経由港・所要日数・概算料金・航海番号」が表示される | 受け入れ「候補ごとに経由港・所要日数・概算料金・航海番号が出る」・`QuotationScreens.test.tsx`・`QuotationEstimatorTest`（**経由港はクローズで直した**——画面が出発地と目的地を繋いでおり、どの候補も同じ経路に見えていた。V020 で候補に持たせた） |
...
- [x] **検査を足した変更では、その検査を 1 度赤にした**（**Try T1**。IT13 の最重の問題。**コミット本文に「どう壊して赤を見たか」を 1 行書く**）
- [x] **赤の理由を推測しなかった**（**Try T2**。赤が出たコマンドの出力を、原因の行まで引用してから直す）
- [ ] **記録を書いた変更で、同じ変更に読み口の検査を置いた**（**一部達成**。読み口は置いたが、`quoted_amount` は書き込みが無いまま列・読み口・画面が揃っていた）（**Try T3**。`invoice_notification` に書くなら S62 が読む経路の検査を対で置く）
- [x] **画面の中のリンク先の到達性を検査した**（**Try T4**・**2 IT 目**。S13 → S21、S46 → S62、S61 → S13。**リンク先のロール許可を突き合わせる検査**を置く。IT13 は未達だった）
- [x] **判別できるデータで検査を書いた**（**Try T5**。並び・絞り込みでは、**その条件を外すと赤になること**をコミット本文に書く）
- [x] **正典を引くときも 3 点を回った**（**Try T6**。`domain-model` / `data-model` / `architecture_backend` を `grep` した結果を残す）
- [ ] **US ごとにクラスタ E2E を 1 度回した**（**一部達成**。spec は書いたが、実走は IT の途中でなくクローズ時のみ）（**Try T7**・**5 IT 目**。US01 と US23 で独立したタスク行を立てる）
- [x] **レビューはクローズの最初に起動して待った**（切れた 2 本の続きも待って統合した）（**Try T8**。全通そろってから確定する）
- [x] **「後の IT で作る」と書いたコメントを、序盤に `grep` で回収した**（33 件を一覧化。残骸は 1 件のみ）（**Try T9**・**2 IT 目**。IT13 は未達で 29 件積み増した。**T0 で一覧にして計画に貼る**）
- [ ] **コミットの品質ゲート欄を空のままにしなかった**（**未達**。クローズ中の 6 コミットすべてが空。IT15 の Try へ）（**Try T10**。IT13 は 11 件中 2 件。埋められないならその理由を 1 行書く）
...
- [x] 受入基準の表がすべて埋まっている（US23 §2 は**スコープ外**と明記）
- [x] デモ項目 12 件が受け入れテストかクラスタ E2E で緑（**受け入れのステップ定義が 1 つも無かったのをクローズで書いた**。D4・D8・D12 は画面から踏む検査が見る）
...
- [x] クラスタ E2E が緑（US ごとに 1 度 + 通し）——**23 件緑・1 件 fixme**。E2E 側の不備 3 件（待ち方・ラベルの重複・候補の打ち切り）を直し、通しで緑。fixme は US23 §4 の実欠陥（`ContractEventRoundTripIT#paymentRecordedReachesBooking` が赤で示す。IT15 の最優先課題）
- [x] SonarQube の Quality Gate がバックエンド・フロントエンドとも PASS（新規指摘 0 件。4 度赤を返してから通した）
- [x] CI が緑（run 34697770614）
...
- [x] **マニュアル 18 章を新設し、17 章に発行・入金・取消と S62 の節を追記した**。キャプチャも撮った（17 章 2 枚・18 章 2 枚。本文が「候補ごとに違います」と書いている経由港を見本データにも入れた）
- [x] `npx gulp okf:check` が ERROR 0 / WARN 0
- [x] 引き継ぎ枠 A・B・C と負債枠 1・2 を消化した（**10 回連続で繰越ゼロ**）
- [x] **前 IT（IT13）の Try T1〜T11 を採点した**（Try T11。[ふりかえり](retrospective-14.md) の採点表）
- [ ] 各タスクの成果を意味のある単位でコミットした（**品質ゲートの結果を書く欄を埋める**。Try T10）——**コミットの単位は守ったが、欄は 6 件中 1 件しか埋めていない**（IT15 の Try T6 へ）
