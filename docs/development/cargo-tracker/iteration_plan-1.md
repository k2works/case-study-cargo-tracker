...
status: stable
generated: { by: claude-code/claude-opus-5, at: 2026-09-02T14:21:43Z }
verified:
...
| # | タスク | 見積 | 状態 |
| :--- | :--- | :--: | :--: |
| 3.1 | Vite + React + TypeScript、`features/` のディレクトリ型、ESLint の `import/no-restricted-paths` | 3h | [ ] |
| 3.2 | 共通レイアウト（左サイドナビ + トップヘッダ）、認可ガード（`RequireRole`）、403 画面、ポータル（`/portal`）。**アクセシビリティの型**：送信中は `disabled` でなく `aria-disabled` でフォーカスを保つ、状態の変化は `role="status"`・エラーは `role="alert"`、反映中の経過秒数は live region の外、色トークンをコントラスト比つきで定義 | 6h | [ ] |
...
| 3.5 | 認証ストア（Zustand + `sessionStorage`）、無操作 15/20 分（荷役画面は 60 分） | 2h | [ ] |
...
| 6.2 | crypto-shredding：荷主ごとの KMS 鍵で name/email/phone/address を暗号化し、鍵が無ければ復号結果を `null` にする（ADR-0003） | 5h | [~] 暗号本体と鍵ポートは完了。Axon シリアライザへの配線は 6.3 と対で行う |
...
| 6.5 | `ShipperController`（`POST /api/v1/booking/shippers`・`GET`）。`201` + 識別子、詳細は投影が無ければ `202` | 3h | [ ] |
