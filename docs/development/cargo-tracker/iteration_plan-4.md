...
- [ ] **クラスタ E2E を Day 8 に 1 度、クローズ前にもう 1 度回した**（IT2・IT3 と 2 回続けて未達。3 回目の宿題）
- [ ] **足した検査を壊して赤を見た**、かつ**検査の Javadoc に書いた方針どおりでない書き方を 1 つ食わせて赤を見た**（IT3 の T4）
- [ ] **書いた保証（「〜で守る」「〜まで比べる」）を、その変更の中で破って赤を見た**（IT3 の T3）
- [ ] **値オブジェクトが本番経路を通ることを検査した**（IT3 の T5）
- [ ] **記録する先を足したら読み口も同じ変更で足した**（IT3 の T6）
- [ ] **画面に出す件数・一覧について「これはその人の仕事か」を確かめた**（IT3 の T7）
- [ ] SonarQube の Quality Gate がバックエンド・フロントエンドとも PASS
- [ ] `npx gulp okf:check` が ERROR 0
- [ ] ユーザーマニュアルの該当章が更新され、画面キャプチャが再生成されている
- [ ] **並列レビューをクローズの最初に起動し、切れていないか表の最終行を確かめてから統合した**（IT3 の T1・T2）
...
- [ ] US25・US07・US32 の受入基準（`user_story.md`）を満たす（US07 の港湾制約・経路探索は US08（IT5）。この線引きを計画に明記済み）
- [ ] デモ項目の受け入れテストがすべて緑。**対応はテスト名でなく本文のアサーションで確かめる**
- [ ] IT3 の引き継ぎのうち負債 6 件が返済されている、または「落とす順序」に従って送った理由がふりかえりに書かれている
- [ ] **R.1・R.2（2 回繰越）が返済されている**
- [ ] 本 IT で足した検査を壊して赤を見た
- [ ] **検査の Javadoc に書いた方針どおりでない書き方を 1 つ食わせて赤を見た**（IT3 の T4）
- [ ] **書いた保証を、その変更の中で破って赤を見た**（IT3 の T3）
- [ ] **足した値オブジェクトが本番経路を通ることを検査した**（IT3 の T5）
- [ ] `./gradlew build` が緑
- [ ] `TZ=UTC ./gradlew cleanTest test` が緑
- [ ] フロントの `npm run test`・`npx tsc -b`・`npm run build` が緑
- [ ] **新しい経路が `RoleAuthorization` に宣言され、そのロール以外は 403 になることを検査した**
- [ ] UI 設計・navbar・ダッシュボード・到達性テストの 4 点が一致している。**一覧・詳細から開く画面（S34・S24）のロール制御も検査されている**
