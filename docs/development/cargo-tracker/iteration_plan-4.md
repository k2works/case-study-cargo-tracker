...
- [x] **Release 0.1 の完了報告書を作成した**（IT3 の DoD からの持ち越し）
...
- [x] `npx gulp okf:check` が ERROR 0
- [x] SonarQube の Quality Gate がバックエンド・フロントエンドとも PASS
- [x] **ユーザーマニュアルの該当章が更新され、画面キャプチャが再生成されている**（航海の更新・検索、予約の修正。R.6 もここで返済）
- [x] **並列レビューをクローズの最初に起動し、結果が切れていないか確かめてから統合した**
- [x] **設計への反映が必要な事項 5 件が `docs/design/` に反映されている**
- [x] ふりかえり（`retrospective-4.md`）と完了報告書（`iteration_report-4.md`）を作成した
- [x] **Release 0.1 の完了報告書を作成した**（IT3 からの持ち越し）
...
| 2026-09-05 | クローズ。マニュアル（Q.3・R.6）を反映し、[ふりかえり](retrospective-4.md)・[完了報告書](iteration_report-4.md) を作成。DoD の残りは Release 0.1 完了報告書のみ | claude-code/claude-opus-5 |
...
| R.6 | マニュアル 08 のキャプチャが 05 と同一 | **返済**（d2ddd03b4）。クローズの Q.3 に合流させ、08 章は「引き渡したあと」の状態を写す形にした。**返済枠 6 件はすべて返済** |
...
| # | 内容 | 結果 |
| :--- | :--- | :--- |
| Q.1 | 並列レビューをクローズの最初に起動 | **実施**。5 視点が返着し、切れていた分は続きを求めて統合した |
| Q.2 | 指摘の反映 | **実施**。高 10 件を全件、中 12 件・低 5 件を対応（[レビュー](../../review/cargo-tracker/IT4実装_review_20260905.md)） |
| Q.3 | マニュアルとキャプチャ | **実施**（d2ddd03b4）。07 章に検索・詳細・更新、05 章に修正の節を足し、08 章に引き渡し列を書いた。キャプチャは 3 枚追加・5 枚撮り直し。R.6 もここで返済 |
| Q.4 | クラスタ E2E（2 回目）・SonarQube・CI | **実施**。上表のとおり |
| Q.5 | Release 0.1 の完了報告書 | **実施**（[リリース完了報告書 0.1](release_report-0_1_0.md)）。IT3 の DoD からの持ち越しを解消 |
| Q.6 | ふりかえり・完了報告書・GitHub 同期・ドキュメント同期 | **実施**（[ふりかえり](retrospective-4.md)・[完了報告書](iteration_report-4.md)） |
