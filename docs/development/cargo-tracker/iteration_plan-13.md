...
tags: [plan]
status: draft
generated: { by: claude-code/claude-opus-5, at: 2026-09-10T21:37:33Z }
---
...
| §4 | 割引計算の**根拠（割引率・基本料金・割引後料金）が精算書に記載される** | `InvoiceLineItem`（`BASE` / `DISCOUNT`）と S61。**請求書は作成時の割引率を持つ**（`invoice.discount_rate`）——作成後に荷主の契約が変わっても、出した請求書は変わらない | `InvoiceProjectionIT`・`InvoiceScreens.test.tsx`・受け入れテスト | |
...
| 4 | 期限超過は列に持たず `overdue(today)` で判定。**期限当日は超過ではない** | T3。本 IT では `INVOICED` に進まないので `dueDate` は入らないが、**判定は集約に置く**（US23 で読む側が来る）。※読む側の無い配線は敷かない方針との兼ね合いは注 N4 |
...
| # | 注 | 反映先 | 本 IT での対応 |
| :--- | :--- | :--- | :--- |
| N1 | **`CargoDeliveredEvent` だけでは料金を計算できない。** 運ぶのは追跡番号・予約 ID・引渡時刻・場所だけで、**区間・重量・貨物種別が無い**。`TrackingInitializedEvent` は区間・貨物種別・出発地・目的地を運ぶが**重量が無い** | `domain-model.md` の契約イベント表・`data-model.md`（`billing_cargo_snapshot`） | **T1 で決める。** 候補は (a) `TrackingInitializedEvent` に `weightKg` を足す（追記専用なので過去のイベントには入らない。旧ペイロードが読めることを `LegacyContractPayloadTest` で固定する）、(b) Upcaster、(c) 算出を断って要確認に出す。**(a) + (c) を採る**——足りない重量で安い請求を黙って出さない |
...
| N4 | **`overdue` と `dueDate` の読む側が本 IT に無い**（発行は US23・IT14） | — | **判定は集約に置き、投影の列は置かない**（`data-model.md` が「`overdue` 列は持たない」と書いている）。`dueDate` は `INVOICED` になるときに確定するので、本 IT では値が入らない。**読む側の無い画面は作らない** |
...
- **輸入港の国で営業日を数える**（いまは `CountryCode("JP")` 固定が 2 か所。`HolidayCalendarTest` の国別分岐が本番経路で一度も踏まれない）→ **本 IT で `HolidayCalendar` を共有カーネルへ移す**判断とあわせて T1 で扱う。ADR-0013 が立てた問い（「billingms が保管料を数え始めたらそのとき移す」）の答えを出す

残りの 7 件（`AdrHasChecksTest` の対応数検査・ADR の独立化・図の細部など）は IT14 へ送ります。
...
| T1 | **入力の調達を決める**（注 N1）。`billing_cargo_snapshot` を新設し `TrackingInitializedEvent` を購読。**重量を運ぶかを決めて実装**し、旧ペイロードが読めることを固定する。**`HolidayCalendar` を共有カーネルへ移す**（ADR-0013 の問いに答える。輸入港の国で数える） | US21 | 8h |
...
| T4 | **`Invoice` 集約**（`calculate` / `applyDiscount` / `adjust` / `overdue`）と不変条件 1・2・4・7 | US21・US22 | 8h |
| T5 | **投影とクエリ**（`invoice`・`invoice_line_item`・`billing_cargo_snapshot`。`FindInvoicesQuery` / `FindInvoiceQuery`）。**不変条件 2 の三段目**（`attention_item` に拒否を記録。宛先は経理）。**`BillingReplayIT`**（規約テストが要求する。リスク R3） | US21 | 8h |
...
| T7 | **連鎖**（`BillingReactionHandler`。`CargoDeliveredEvent` 購読 → `CalculateInvoiceCommand`）。**割引は算出の中で当てる**（別コマンドにすると割引の無い請求書が一瞬見える） | US21・US22 | 6h |
| T7e | **クラスタ E2E（US21・US22）**。引取済 → 請求書ができる → 割引が入る → 調整を入れる | US21・US22 | 4h |
| T8 | **認可**（`ROLE_ACCOUNTANT`（利用者 `accountant01`）に `/api/v1/billing/**`。荷主は自社分だけ＝S62 は US23・IT14 なので本 IT では開けない） | US21 | 4h |
| T9 | **調整の根拠**（`basisExceptionId` から例外へのリンク。US28 §8 の受け側）。**留置営業日数を根拠にする**（IT12 で載せた `heldBusinessDays` の最初の読み手） | US21 | 6h |
...
| `attention_item`（billingms） | **ある**（V001 で作成済み。書き手はまだ無い） | T5 で不変条件 2 の三段目に使う |
| `HolidayCalendar` | **ある**（handlingms。共有カーネルには置いていない） | T1 で共有カーネルへ移す（ADR-0013） |
...
| 日 | 内容 |
| :--- | :--- |
| 1 | T0（引き継ぎ枠 A・B・負債枠。独立コミット） |
| 2 | P1（業務シナリオを赤で置く）・T1（入力の調達・`HolidayCalendar` 移送） |
| 3 | T2（値オブジェクトと料率表） |
| 4 | T3（料金計算・割引・輸出免税） |
| 5 | T4（`Invoice` 集約） |
| 6 | T5（投影とクエリ）・T7（連鎖） |
| 7 | T6（S60・S61 と注 N6・N7） |
| 8 | T8（認可）・T9（調整の根拠） |
| 9 | T7e（US ごとのクラスタ E2E）・T10（受け入れ・マニュアル 17 章） |
| 10 | クローズ（レビューは**最初に起動して逐次と並行**。Try T4） |

## ADR

| # | 判断 | 起票 |
| :--- | :--- | :--- |
| ADR-0013 の問いに答える | **`HolidayCalendar` を共有カーネルへ移す**。billingms が保管料（留置営業日）を数え始めるので、「全 BC で同じでなければならない業務上の理由」が立つ（ADR-0001 決定 4 の名簿に足す） | **ADR-0013 を改訂**（新規 ADR は立てない。同じ問いの続きである） |
| 料率の置き場 | `application.yml` の `RateTable`。見積（US01・IT14）と同じ値を読むことを契約テストで固定する | **ADR-0015 を起票**（「料率は設定に置き、見積と請求で同じものを読む」）。決定ごとに検査を対応させる |
...
| R5 | **`HolidayCalendar` を共有カーネルへ移すと handlingms の参照が変わる。** 移送そのものは機械的だが、名簿（ADR-0001 決定 4）と `SharedKernelScopeTest` の更新が要る | **T1 で ADR-0013 を改訂してから移す。** 検査を先に直すと、移す前に赤になって手が止まらない |
...
- [ ] **ADR-0013 を改訂し、ADR-0015 を起票した**（決定ごとに検査を対応させる）
...
- [ADR-0013](../../adr/cargo-tracker/0013-shared-kernel-holds-the-error-mapping.md)（`HolidayCalendar` の置き場の問い）・[ADR-0014](../../adr/cargo-tracker/0014-poison-events-are-parked-not-blocking.md)（引き継ぎ枠 A・B）
