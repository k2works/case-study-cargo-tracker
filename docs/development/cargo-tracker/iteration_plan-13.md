...
tags: [plan]
status: stable
generated: { by: claude-code/claude-opus-5, at: 2026-09-11T08:29:24Z }
verified:
...
- [x] デモ項目の受け入れテストがすべて緑（11 件。D11 は gatewayms の検査とクラスタ E2E で見る——理由は完了報告書）
- [x] **`TZ=UTC` でモジュールを分けて `build` が緑**（`dev:backend:full:split`。5 群すべて緑）
- [x] フロントの `npm run test`（453 件）・`npx tsc -b`・`npm run build` が緑
- [x] **受入基準の表を、実装を始める前に作った**（継続）
- [ ] **未達。画面から画面への繋ぎ目を、件数・リンクを作った時点で 1 度踏んだ**（**Try T1**。href を検査するだけでなく、**着いた先が期待の絞り込みで開くか**をテストに書く。本 IT では S02 → S60、S22 → S61、S61 → 例外）。**踏めていなかった**——レビューで「調整の根拠リンクが経理に 403」（IT7 と同じ形）が高指摘として出た。ふりかえりの Try T4 に引き継ぐ
- [ ] **未達。「後の IT で作る」と書いたコメントを、序盤に `grep` で回収した**（**Try T2**。`IT13`・`US21`・`US22`・`US23` で探す）——**序盤に回収した記録が無い**。本 IT で新たに書いた IT14 向けの注記は 29 件あり、回収の対象は次 IT へ積み上がっている
- [ ] **一部達成。自分が書いた javadoc・SQL コメントの「〜する」を、同じ変更の中で赤にできる検査と対にした**（**Try T3**・**3 IT 目**。**手立てを変える**——`grep` で「写す」「返す」「読む」「数える」を含むコメントを列挙し、対応する検査を表にしてから書く）——**表は作らなかった**。レビューで「ADR-0015 の理由が実装で成立していない」（billingms は営業日を読んでいない）が高指摘として出た
- [x] **レビューは 15 分で逐次に切り替え、そのうえで返着を待って統合した**（**Try T4**。IT11・IT12 と 2 IT 続けて遅着分にしか出ない欠陥があった。**クローズを確定する前に、届いた分をもう一度突き合わせる**）——5 通とも 3〜6 分で返着したため逐次への切り替えは不要だった。64 件（高 18・中 27・低 19）を統合し、高はすべてクローズ前に修正
- [x] **受け入れフィクスチャの日付を「今」から導いた**（**Try T5**。handling・tracking に残る固定日付を直し、**固定日付を書かせない規約テスト**を置く）——`AcceptanceFixtureTime` と規約テスト 2 本。ただし**その規約テストは一時期 1 度も走っていなかった**（下記 Try T8）
- [x] **処理の列を分け、退避からの再処理入口を作った**（**Try T6**。引き継ぎ枠 A・B）——`@SequencingPolicy` と `/actuator/deadletters`。**IT12 の「この版に手立てが無い」は誤りだった**（ADR-0014 決定 4）
- [x] **US ごとにクラスタ E2E を 1 度回した**（**Try T7**。US21 と US22 で独立したタスク行を立てる）——通しで 22/22 緑。**層ごとの検査では出ない実欠陥を 2 件**（請求書 ID のカラム長超過・調整明細の重複）出した
- [x] **手立てを機械に移した**（**Try T8**。IT11 の `.gitmessage`・IT12 の `dev:backend:guard` に続く形。本 IT では「受入基準の表に書いたクラス名が実在するか」を検査にする）——`IterationPlanChecksExistTest` を置いた。ただし**入力宣言のパス誤りで 1 度も走っていなかった**（`:shared:test` が構成時に落ちていた）。直した直後に実在しないクラス名 2 件を検出。本 IT 最重の問題としてふりかえりの Try T1・T2 に引き継ぐ
- [x] **受入基準の表の「検査の所在」に書いたクラス名・メソッド名が実在する**（IT12 レビューの懸念。`AdrHasChecksTest` と同じ作りの検査を 1 本置く）——`IterationPlanChecksExistTest`。検出した 2 件（`BillingReactionHandlerTest` → `BillingReactionHandlerIT`・`BillingReplayIT` → `ReplayIT`）を修正済み
- [x] **料率をハードコードしていない**（`billing-rates.yml` を `spring.config.import` で読む。ADR-0016）
- [x] **丸めが `Money` の中 1 か所だけ**（正典。`Money#roundToUnit` のみ）
- [x] SonarQube の Quality Gate がバックエンド・フロントエンドとも PASS（新規コードのカバレッジ 94.4% / 94.5%、重複 0.27% / 0.0%、Bug 0・Vulnerability 0。**3 度赤を返してから通した**）
- [x] `npx gulp okf:check` が ERROR 0
- [x] **ユーザーマニュアル 17 章（請求を組み立てる）を新設し、画面キャプチャを生成 spec で撮った**
...
- [x] 受入基準の表がすべて埋まっている（未達は**未達と書く**）——US21 §2 を「一部達成」として記録（距離・荷役作業実績は意図して出さない）
- [x] デモ項目 11 件が受け入れテストかクラスタ E2E で緑
- [x] **受入基準の表の「検査の所在」に書いたクラス名が実在する**（検査で固定）
- [x] ナビゲーション整合（`ui_design.md` の構成表・`navigation.ts`・S02・検証テストの 4 点が一致）
- [x] **`TZ=UTC` で分割 `build` が緑**
- [x] フロントの `npm run test`・`npx tsc -b`・`npm run build` が緑
- [x] クラスタ E2E が緑（US ごとに 1 度 + 通し。22/22）
- [x] SonarQube の Quality Gate がバックエンド・フロントエンドとも PASS
- [x] CI が緑（run `34577514020`。6 ジョブすべて success）
- [x] **注 N1〜N13 を設計ドキュメントに反映した**（計画作成時に `N1〜N9` と書いたが、注は N13 まである。`domain-model.md`・`data-model.md`・`architecture_backend.md`・`ui_design.md` の 4 点に反映済み）
- [x] **ADR-0015・ADR-0016 を起票した**（決定ごとに検査を対応させる）。**ADR-0013 は改訂しない**（営業日に触れていない）
- [x] **マニュアル 17 章を新設し、キャプチャを生成 spec で撮った**（37/37 緑）。**書いた手順を画面で 1 度なぞった**——項目表と操作手順は実装と突き合わせたが、逐条でなぞった記録は残していない
- [x] `npx gulp okf:check` が ERROR 0 / WARN 0
- [x] 引き継ぎ枠 A・B と負債枠 1・2 を消化した（できなければ**理由をふりかえりに書く**）——4 件すべて完了。**9 回連続で繰越ゼロ**
- [ ] **一部達成。** 各タスクの成果を意味のある単位でコミットした（品質ゲートの結果を書く欄を持つテンプレートで）——コミットは 11 件で単位は保てたが、**品質ゲートの結果を書いたのは 2 件だけ**。テンプレートの欄を埋める規律は定着していない
...
| 2026-09-11 | **クローズ（実績反映）**。成功基準 18 件・DoD 15 件を実績で埋めた。**未達 2 件・一部達成 2 件**を記録する——(1) Try T1「画面の繋ぎ目を作った時点で踏む」は**踏めておらず**、レビューで「調整の根拠リンクが経理に 403」が高指摘として出た（ふりかえりの Try T4 に引き継ぎ済み）、(2) Try T2「『後の IT で作る』コメントの序盤回収」は**実施記録が無い**（本 IT で新たに 29 件積み増した。**ふりかえりの Try に未反映**）、(3) Try T3「コメントと検査を対にする」は表を作らず、ADR-0015 の理由が実装で成立していない指摘が出た、(4) コミットの品質ゲート欄は 11 件中 2 件しか埋めていない（**ふりかえりの Try に未反映**）。あわせて**注の表が空行 2 つで 3 分割されていた**のを直し、DoD の「注 N1〜N9」を実際の範囲（N13 まで）に訂正した | claude-code/claude-opus-5 |
