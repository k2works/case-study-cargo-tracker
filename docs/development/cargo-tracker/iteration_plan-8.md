...
- [x] デモ項目の受け入れテストがすべて緑（6 シナリオ。`:acceptance-tests:trackingAcceptanceTest`）
...
- [x] **タスクに着手する前に、その名前で `grep -r` して既にあるか探した**（IT7 の T1。**`process_state` の一式を上書きしかけた**）
- [x] **行数の基準に合わせるために、判断の理由を書いたコメントを削らなかった**（IT7 の T3）
- [x] **イベントに載せる値を「購読側の投影が作れるか」で決めた**（IT7 の T2。**受け側の列を先に並べて確かめる**）
- [x] **要確認一覧・知らせの宛先を「対処できる人」に、リンクを「その人の次の一手」に合わせた**（IT7 クローズの高 2 件）
- [x] **クラスタ E2E が自分の作ったデータを名指しで探していない**（IT7 の T5）
- [x] **US を終えたコミットのメッセージに、回した品質ゲートの結果を 1 行書いた**（IT6 の T5b。IT7 で守れた）
- [x] **分岐を足したタスクの終わりに `jacocoTestCoverageVerification` を回した**
- [ ] **US ごとに SonarQube を 1 度回した**（IT7 の T7。クローズで 11 件まとめて直すことになった）。**守れなかった**——クローズで初めて回し、新規コードの指摘 4 件をまとめて直した（IT7 と同じ形）
- [x] **利用者に見せる文字列を、設計の要素表と突き合わせる canon テストで固定した**。`TransportStatus` はバックエンド（`TransportStatusTest`）に加え、**フロントの選択肢も要素表と突き合わせる検査を足した**（`transportStatusLabels.test.ts`）。`ExceptionType`・`HandlingType` は本 IT に出ていない
- [x] **値を足したら「集約 → イベント → 投影 → 読み口 → 画面」を 1 本読み直した**
- [x] **モックは URL で出し分けた**
...
- [x] **全体ビルドで落ちたテストは、単独で再実行して切り分けたうえで記録した**（`authStore.test.ts` は Node 25 の組み込み `localStorage` が原因と特定して直した）
- [x] **クローズで計画を更新するとき、文書内の `- [ ]` を全部数えてから埋めた**（51 件）
...
| T7 | **S41 追跡詳細・管理**（状態の履歴・手動更新）と **S40 追跡一覧**（`/tracking`）。**追跡管理者と荷主の両方の入口**（`ui_design.md:144-145` は S40・S41 のロールを「追跡、**荷主（自社のみ）**」と定める）。navbar・ダッシュボード・到達性テストの 4 点を**両ロールで**合わせる。**完了**（S40 一覧・S41 詳細と手動更新。navbar・ダッシュボード・到達性検査は NAVIGATION 由来で 4 点一致。T3 の集約もここで REST に繋いだ） | US17・US18 | 7h |
| T8 | 荷主の絞り込み（`ROLE_SHIPPER` は自社の追跡だけ）。**`X-Shipper-Id` ヘッダは伝播済み**（authms → Gateway）なので、trackingms 側で突き合わせる。**完了**（`X-Auth-Shipper-Id` を一覧・詳細の両方で突き合わせる。他社のものは「見つからない」） | US18 | 4h |
| T9 | 認可の宣言（公開経路・追跡管理者・荷主）と HTTP の配線。**完了**（`/tracking/trackings/**` は追跡・荷主、`POST /status` は追跡だけ。公開経路は PUBLIC_PATHS が持つ） | US17・US18 | 3h |
...
- [x] US18・US17 の受入基準（`user_story.md`）を満たす。**US18 §2（現在位置）と §3（作業種別）・US17 §4（通知）は一部未達**（理由を完了報告書に記録）
- [x] デモ項目の受け入れテストがすべて緑。**対応はテスト名でなく本文のアサーションで確かめる**（6 シナリオ。`:acceptance-tests:trackingAcceptanceTest`）
- [x] 引き継ぎ枠 H.1〜H.3 が返済されている（Day 1 で 3 件とも完了）
- [x] 本 IT で足した検査を壊して赤を見た（例外中の更新・localStorage の保存先・荷主の差し込み）
- [x] **追跡番号が `TRK-` + 大文字英数字 10 桁になっている。連番でないことを検査した**
- [x] **公開の応答に荷主名・予約番号・社内メモが入らないことを検査した**
- [x] **同一 IP からの総当たりが `429` で止まることを検査した**（`ui_design.md:1298`。**認証不要経路の唯一の防御**）
- [x] **存在しない追跡番号と権限の無い追跡番号を区別しないことを検査した**
- [x] **遷移表が正典と一致していることを canon テストで固定した**（部分的に写していない）
- [x] **例外発生中は手動更新で状態が動かないことを確かめた**（不変条件 5 の下地）
- [x] **`tracking_event` が再配送で増えないことを確かめた**（`event_id` が PK）
- [x] **`shipperId` が契約 3 本すべてを生き延びることを確かめた**（集約 → イベント → 投影 → 読み口 → 画面）
- [x] **荷主は自社の追跡だけが見えることを確かめた**（他社の番号で 404 と同じ応答）
- [x] `./gradlew build` が緑・`TZ=UTC ./gradlew build` が緑
- [x] フロントの `npm run test`・`npx tsc -b`・`npm run build` が緑
- [x] **新しい経路が `RoleAuthorization` にメソッド込みで宣言され、そのロール以外は 403 になることを検査した**（公開経路は除外の宣言も検査する）
- [x] UI 設計・navbar・ダッシュボード・到達性テストの 4 点が一致している
- [x] **追跡管理者と荷主の両方が S40・S41 にたどり着ける**（navbar・ダッシュボード。`ui_design.md:144-145`）。**状態軸の到達性**も——どの状態の追跡からでも開けるか
- [x] **認証不要の画面の入口が、ログイン画面とポータルにある**（IT7 の教訓）
- [x] **内部の列挙名を利用者に見せていない**
...
## 本 IT で見つかった、次 IT へ送る欠陥

**荷主が上限を超えると、新しく登録した荷主で予約が取れません。** クラスタ E2E で実測しました（荷主 219 件 / 一覧の上限 200 件）。

- 一覧（S10）は荷主コード順なので、新しく採った荷主ほど後ろに回り、**1 ページ目には決して出ません**
- 予約登録（S21）の荷主の選択肢も同じ上限で作られるため、**登録した荷主を選べません**。荷主を登録した当日から、その荷主の予約が取れなくなります
- `ShipperListPage` のコメントは「載らなかった荷主は、予約登録の選択肢にも出ないので、その日から予約が取れなくなる」と**予告していました**。予告どおりに起きました

**本 IT では半分だけ直しました。** 正典（`ui_design.md` S11「登録後は S10 に反映中の行を差し込む」）が約束していた差し込みが未実装だったので実装し、登録直後の荷主は一覧に出るようにしました。**残る半分（予約登録の選択肢）は絞り込みの実装が要る**ので、IT9 へ送ります（`ShipperListPage` の「絞り込みは次のイテレーションで入ります」の約束を果たすこと）。

**クラスタ E2E は 14 緑・2 赤で終えています。** 赤 2 件（`営業が荷主と貨物予約を登録し、一覧と詳細に出る`・`営業が仮受付の予約を修正できる`）は、この欠陥をそのまま踏んだものです。**テストを緩めていません**——緩めると、実際に予約が取れない状態が緑で隠れます。
