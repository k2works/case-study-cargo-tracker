...
tags: [plan]
status: draft
generated: { by: claude-code/claude-opus-5, at: 2026-09-12T22:40:46Z }
---
...
| 引き継ぎ 1 | **`dead_letter_entry` の業務向け読み口**（運用の gulp タスクと actuator はあるが画面が無い） | IT14 レビュー architect | T0 |
| 引き継ぎ 2 | **Processing Group を書くテーブルの単位に分け直す**（token の移行が要る） | IT14 レビュー architect | T0。**移行手順を先に書く**（ブローカーのトポロジ変更と同型） |
| 引き継ぎ 3 | **入金の取り消し**（誤って記録した入金を戻す。17 章が「いまは運用で引き取る」と書いている） | IT14 レビュー writer | T0 |
| 負債 1 | **IT13 から送った 7 件**（一覧の絞り込みと合計、経理宛の要確認件数、明細の表示順、コマンド失敗の行き先、連鎖が投影を追い越したときの検知、要素表の 3 行、IT12 から引き継いだ H.5 の 2 件） | IT13 → IT14 → IT15 | T0。**3 IT 目なので今回で片づける** |
| 負債 2 | **キャンセル料の料率を正典と設定に書く**（注 N1） | 本計画 | T0 |
...
| # | タスク | 対象 | 見積 |
| :--- | :--- | :--- | :--- |
| **T-1** | **精算の連鎖の修復**（引き継ぎ 0）。**赤い往復テストから始める**（`@Disabled` を外す）。Axon 5 のイベント配送を、購読側の token・`EventCriteria`・payload 型の 3 点から切り分ける | US23 | 8h |
...
| 日 | 内容 |
| :--- | :--- |
| 1-2 | **T-1（精算の連鎖の修復）**。ここが通るまで US30 に入らない |
| 3 | T0（枠の消化と `grep` 回収） |
...
| **候補** | **`PaymentRecordedEvent` が届かなかった原因と対処** | **T-1 の調査結果しだいで起票する。** Axon 5 のイベント配送に関する決定（購読の書き方・`EventCriteria`・Processing Group の粒度）になるなら、**同じ形の欠陥を次に入れないための検査**と対で起票する |
...
| # | リスク | 対処 |
| :--- | :--- | :--- |
| R1 | **T-1（精算の連鎖）に時間が読めない**。原因が Axon 5 の中にあると、切り分けだけで数日かかりうる | **2 日で切る。** 直らなければ US30 を先に進め、ふりかえりで 2 IT 連続の未達として書く。**「余力次第」にしない** |
