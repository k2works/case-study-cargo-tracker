...
verified:
  - { by: human:k2works, at: 2026-09-14T06:02:18Z }
---
...
- [x] **US23 §受入基準 4 が緑**（T-1 で修復。`ContractEventRoundTripIT` の `@Disabled` を外した）
- [x] デモ項目の受け入れテストがすべて緑。**5 スイート / 128 シナリオ緑**（test 58・routing 20・handling 19・billing 18・tracking 13。うち US30 は `輸送中キャンセルの承認.feature` の 9 シナリオ）
- [x] **`TZ=UTC` の分割フルビルドが 5 群すべて緑**
- [x] **触ったモジュールの検査を各コミットで回した**（pre-commit フックが Checkstyle + SpotBugs まで回す。フルビルドは IT の節目で 2 度）
- [x] フロントの `test`（517 件）・`tsc -b`・`build` が緑
- [x] **受入基準の表を、実装を始める前に作った**（T1 で `.feature` を赤で置いた）
- [x] **検査を足した変更では、その検査を 1 度赤にした**（コミット本文に記録。**1 度は判別していない検査を書いてしまい**、書き直して赤を見た——取り消した荷役の現在地）
- [x] **記録の側も検査した**（`cancellation_request`・`cancellation_discharge_unlocode` とも投影 IT で書いて読んだ）
- [x] **入口ごとに違う扱いをしなかった**（陸揚げ地の判定は `DischargeCandidates` 1 本。集約と読み口が同じ関数を呼ぶ）
- [x] **定義済み未使用を `grep` した**（本 IT で足した public 要素 34 件。フレームワークが呼ぶもの以外で参照 0 件は `Cargo.hasPendingCancellation` の 1 件——レビューでも指摘され、削った）
- [x] **コミットの品質ゲート欄を空にしなかった**（3 IT 目・達成）
- [x] **クラスタ E2E を IT の途中で 1 度回した**（**Try T7**・**6 IT 目**。US30 の実装が終わった時点で回し、結果をコミット本文に書く）
- [x] **レビューは切れる前提で出力形を指定して起動した**（4 通が 2〜3 分で返着。writer は長さ制限で切れ、同じ形で再取得して全文を得た）
- [x] **BC をまたぐ契約イベントに向きごとの往復テストを置いた**（`CancellationRoundTripIT` が 3 購読先の表を 1 本で見る）
- [x] **前 IT の Try を、クローズの最初に採点した**（T1〜T9 を本欄で採点）
- [x] **画面の中のリンク先の到達性を検査した**（S02 → S23 は本 IT のクローズで足した。S23 → S22 は `CancellationScreens.test.tsx`）
- [x] SonarQube の Quality Gate がバックエンド・フロントエンドとも PASS（新規指摘 0 件。**15 件を直した**——うち 1 件は検出が誤りで理由を書いて抑制）
- [x] `npx gulp okf:check` が ERROR 0
- [x] **ユーザーマニュアル 19 章（キャンセルを申請して承認する）の新設**と 17 章へのキャンセル料の追記。画面キャプチャを生成 spec で撮る
...
- [x] US30 の受入基準 10 件がすべて緑（§7 の落とし先と §4 の件数の検査は**クローズのレビューで未達が判明し、同じ IT のうちに塞いだ**）
- [x] **US23 §受入基準 4 が緑**（IT14 の未達を返済）
- [x] デモ項目 12 件が緑（受け入れ `輸送中キャンセルの承認.feature` 9 シナリオ + クラスタ E2E の US30 通し 1 本）
- [x] `TZ=UTC` の分割フルビルドが 5 群すべて緑
- [x] フロントの `test` / `tsc -b` / `build` が緑
- [x] クラスタ E2E が緑（**IT の途中で 1 度**・25 件すべて緑。クローズの通しは下記の注のとおり）
- [x] CI が緑（**IT14 から続いていた赤を解消**。原因は受け入れフィクスチャの固定日付で、規約検査が捕まえた）
- [x] SonarQube の Quality Gate が両プロジェクト PASS（新規カバレッジ backend 93.9% / frontend 92.7%・重複 0.5% / 0.0%）
- [x] **ユーザーマニュアル 19 章を新設し、17 章にキャンセル料を追記した**。キャプチャを撮った
- [x] 正典に注 N1〜N7 を反映した（N8 は Release 3.0 へ送った）
- [x] `npx gulp okf:check` が ERROR 0
- [x] 各タスクの成果を意味のある単位でコミットした（30 コミット・品質ゲート欄はすべて記入）
- [x] **Release 2.0 の完了報告書を作成した**（仕上げ 2。[release_report-2_0_0.md](release_report-2_0_0.md)）
...
| 2026-09-14 | **分割フルビルド 5 群すべて緑**（33 分）・**フロント 517 件緑・`tsc -b`・`build` 緑**。1 回目は赤で、**規約検査が本 IT で書いた固定日付を捕まえた**——`CancellationSteps` の 5 か所（`2026-09-20` など）。時限式なので `AcceptanceFixtureTime` から導く形に直した。**IT13 の負債枠で置いた検査が、2 IT 後の新しいコードで実際に働いた** | claude-code/claude-opus-5 |
| 2026-09-14 | **IT15 クローズ。** 実績 6/6 SP（100%・累計 121）。**Release 2.0 のスコープ 22 SP を消化しきった**。並列 5 視点のレビューで**高 13 / 中 25**——高はすべてクローズ前に修正した。**4 件は「記録はあるのに読み口が無い」形**で、集約・投影・往復テスト・受け入れ・クラスタ E2E がすべて緑のまま残っていた。品質ゲートは**分割フルビルド 5 群緑・フロント 525 件緑・CI 緑（IT14 から続いた赤を解消）・SonarQube 両プロジェクト PASS**（新規指摘 15 件を直し、1 件は検出が誤りとして理由を書いて抑制）。GitHub の Issue #599 をクローズし、take-8 の Open はゼロになった | claude-code/claude-opus-5 |
| 2026-09-14 | **T12e（クラスタ E2E）完了・25 件すべて緑**。6 イメージを作り直して配り直し、US30 の通し（申請 → 承認待ち一覧 → 陸揚げ地を指定して承認 → 追跡は開いたまま → 陸揚げの荷降しで閉じる → キャンセル料が請求に載る）を書いた。**4 サービスの連鎖は一度で通った**が、期待した明細の文言が違っていた（実際は「キャンセル料（輸送中・50%）」）のでマニュアルもあわせて直した。**通しで US10 の差し戻しが赤**——ダッシュボードの見直し一覧は申し出の古い順で上限 50 件なので、作り直さないクラスタでは差し戻した直後の予約が載らない（**E2E は自分のデータを名指ししない**の 2 度目）。知らせと導線で確かめ、理由は予約詳細で読む形に直した | claude-code/claude-opus-5 |
| 2026-09-14 | **T13（マニュアル 19 章の新設・17 章へのキャンセル料の追記・キャプチャ 2 枚）完了**。**仕上げ 1（README にアプリケーションの入口と実行手順）完了**。キャプチャを回したところ **17 章の請求詳細が 1 枚も撮れていなかった**——見本の請求書に `payments` が無く、画面が `payments.length` で落ちていた（モックが本物より足りない）。`satisfies InvoiceView` を付けて型検査で分かるようにした。あわせて **T12e の下ごしらえで実欠陥 1 件**——追跡を閉じた履歴が「例外発生」と表示され、記録者欄に `CANCELLED` が入っていた（`new_status` が NOT NULL で既定が `EXCEPTION`）。閉じた時点の状態を書き、画面は `eventType` が `CLOSED` の行を「追跡終了」と読ませる | claude-code/claude-opus-5 |
