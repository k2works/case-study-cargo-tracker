...
- [x] **US を終えたコミットのメッセージに、回した品質ゲートの結果を 1 行書いた**（IT6 の T5b。**IT5・IT6 と 2 回続けて守れなかったので、置き方を「タスク表」から「コミットの本文」に変える**。書けないなら回していない）
- [x] **分岐を足したタスクの終わりに `jacocoTestCoverageVerification` を回した**（IT6 の T5）
- [x] **設計ドキュメントに書いた「〜は残る」「〜になる」を、その変更の中で赤にした**（IT6 の T1）
- [x] **利用者に見せる文字列を、設計の要素表と突き合わせる canon テストで固定した**（IT6 の T2。`BookingStatus`・**`TransportStatus`** に広げる）
- [x] **値を足したら「集約 → イベント → 投影 → 読み口 → 画面」を 1 本読み直した**（IT6 の T3）
- [x] **層をつなぐ組み立てを 1 本踏む検査を置いた。スタブは受け取った引数を捕捉し、捨てない**（IT6 の T8c）
- [x] **モックは URL で出し分けた**（IT6 の T4）
- [x] **受入基準を 1 項目ずつ表にして、満たす／未達を個別に書いた**（IT6 の T7。「等」でまとめない）
- [x] **列を足したら、同じ変更の中で読み口まで作った**（IT6 の T8b）
- [x] **追加・変更した画面を、それを見る他のロールから何が読めるか確かめた**
- [x] **内部の列挙名を利用者に見せていない**
- [x] **クラスタ E2E を Day 9 に 1 度、クローズ前にもう 1 度回した。Pod を作り直したあと 1 度空回ししてから測った**（IT6 の T8d）
- [x] SonarQube の Quality Gate がバックエンド・フロントエンドとも PASS
- [x] `npx gulp okf:check` が ERROR 0
- [x] ユーザーマニュアルの該当章が更新され、画面キャプチャが再生成されている
- [x] **並列レビューをクローズの最初に起動し、45 分は待った。返着するまでレビュー結果に関する記述を書かなかった**（IT6 の T8）
  - **起動はクローズの最初にできた。1 時間半待ったが返着しなかった**（問い合わせを 3 度送っても応答なし）。
    返着していない事実をそのまま記録し、**自己レビューで補完**した（[レビュー](../../review/cargo-tracker/IT7実装_review_20260907.md)）。
    IT6 では早く「返らない」と判断して誤ったので、待った時間も書いている
- [x] **クローズで計画を更新するとき、文書内の `- [ ]` を全部数えてから埋めた**（IT6 の T8e）
...
**判断（T2 完了時点）: IT8 へ送ります。** US13 §4 の「経路提案中に戻せる」は**通知済みの予約**の話で（`ReturnToRoutingCommand`）、引き継ぎ 8 は**差し戻された（`condition_review`）予約**の話です。**同じ画面ですが同じ操作ではありません。** 営業が打てる手（見直しを終えたことを記録して経路設計者へ返す）には**新しいコマンドとイベントが要り**、US13 の実装で当たりませんでした。**IT8 で US18 の荷主向け画面と一緒に扱います**（引き継ぎ 9「経路設計者が自分がもう差し戻したかを S30 で読めない」と同じ受け皿の話なので、まとめて設計します）。
...
- [x] US13・US14 の受入基準（`user_story.md`）を満たす。**ただし US13 §3・§5・§6 と US14 §4 は未達または一部未達**（理由を完了報告書に記録）
  - **満たした 5 項目・一部未達 2 項目・未達 2 項目。** `user_story.md` に 1 項目ずつ実績を書いた
- [x] デモ項目の受け入れテストがすべて緑。**対応はテスト名でなく本文のアサーションで確かめる**
- [x] 引き継ぎ枠 H.1〜H.3 が返済されている、または送った理由がふりかえりに書かれている
- [x] 本 IT で足した検査を壊して赤を見た
- [x] **通知していない予約は確定できないことを、集約と HTTP の両方で確かめた**
- [x] **追跡番号を二重に発行できないことを確かめた**（不変条件 8）
- [x] **連鎖が最後まで通ることを、クラスタで 1 度確かめた**（Testcontainers では両サービスを同時に起こさない）
- [x] **連鎖が途中で止まったときに、止まった位置が `process_state` から SQL で読めることを確かめた**（ADR-0010 決定 4）
  - **通った連鎖は SQL で読めた**（クラスタで `current_step`・`completed_steps` を確認）。
    **止まった連鎖そのものはクラスタで再現していない**——trackingms を落として確かめる手順は
    IT8 へ送る（検査としては `BookingReactionHandlerTest#startsTheProcessBeforeSending` が
    「届いていないときに段を進めない」ことを固定している）
- [x] **上限を超えた連鎖が要確認一覧に出ることを確かめた**（同上。`RevertTrackingNumberCommand` + `attention_item`）
  - **検査（`BookingReactionHandlerTest#compensatesAfterTheLimit`）で確かめた。クラスタでは未確認**
    （trackingms を落とす手順が要る）。IT8 へ送る
- [x] **`TrackingNumberIssuedEvent` に `legs[]` を載せた**（購読側が未実装でも落とさない。IT9 の材料）
- [x] **契約のゴールデン JSON を bookingms・trackingms の両側に置き、赤から始めた**（開発戦略の Phase 0。US14 は名指し）
- [x] **契約に列挙型・識別子型を載せていない**（文字列・数値・日付のみ。ArchUnit で固定）
- [x] `./gradlew build` が緑・`TZ=UTC ./gradlew build` が緑
- [x] フロントの `npm run test`・`npx tsc -b`・`npm run build` が緑
- [x] **新しい経路が `RoleAuthorization` にメソッド込みで宣言され、そのロール以外は 403 になることを検査した**
- [x] **契約コマンドの名簿を ArchUnit で固定し、同じ変更で更新した**
  - **ArchUnit ではなく走査で固定した。** `ContractCommandGoldenTest` がパッケージを走査して
    名簿を導き、ゴールデンの無い契約と検査していないゴールデンの両方で赤くなる。
    加えて `ContractCarriesOnlyPlainValuesTest` が列挙型・識別子型を断る。
    **手書きの名簿にしないという意図は同じ**で、走査のほうが載せ忘れに強い
- [x] UI 設計・navbar・ダッシュボード・到達性テストの 4 点が一致している
- [x] **内部の列挙名を利用者に見せていない**。`TransportStatus` の呼び名も canon テストで固定した
- [x] **kind クラスタで動く**：イメージを作り直して載せ直し、全 Pod が Ready
- [x] **クラスタに対して E2E が緑（Day 9 とクローズ前の 2 回）**
- [x] `npx gulp okf:check` が ERROR 0
- [x] SonarQube の Quality Gate がバックエンド・フロントエンドとも PASS
- [x] **ユーザーマニュアルが更新されている**（~~05 章に確定、追跡番号の説明~~ → **11 章を新設**）
  - 確定（営業）と発行（経路設計者）は**別の人の仕事**なので、05 章（予約の登録）に足すと
    読者が混ざる。索引・業務フロー・ナビゲーションも合わせ、キャプチャを 2 枚生成した
- [x] **設計への反映が必要な事項 6 件が `docs/design/` に反映されている**
- [x] ADR-0010 を作成し、**決定の数だけ検査を対応させた**
- [x] ふりかえり（`retrospective-7.md`）と完了報告書（`iteration_report-7.md`）を作成した
...
| 日付 | 更新内容 | 更新者 |
| :--- | :--- | :--- |
| 2026-09-06 | IT7 計画を作成 | claude-code/claude-opus-5 |
| 2026-09-06 | 着手前の検証（ステップ 3・4）の結果を反映。**「そもそも動かない」計画を 2 件直した**——`@Saga` は Axon 5 に API が無くコンパイルできない（ADR-0001 決定 6）、追跡番号の自動発行は `ui_design.md:351` が定める経路設計者の操作を消す。見積 60h | claude-code/claude-opus-5 |
| 2026-09-06 | 開発フェーズの実績を反映。DoD・成功基準 47 件を処理 | claude-code/claude-opus-5 |
| 2026-09-07 | クローズの実績を反映。**本節（更新履歴）が欠けていた**ことを IT8 の検証が指摘したため遡って追加 | claude-code/claude-opus-5 |
