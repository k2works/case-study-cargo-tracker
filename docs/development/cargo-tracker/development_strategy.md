...
| **終盤（続き）** | IT16〜IT17 | 3.0 | アウトサイドイン | **業務そのものではなく、業務が成立していることを確かめる手段**を作る。新しい業務ロジックを足さず、既存の API を呼ぶ順序と待ち方だけを組み立てる |
...
- [ ] **その IT で実装した画面**とロール別ナビが対で揃い、到達性が E2E で固定されている（**全ルートのプレースホルダは置きません**。中身の無い画面は動くと誤解されて問い合わせを増やすため、IT1 のレビュー C1 でこの方針を追認しました。画面はそれを使うストーリーの IT でナビと対で足します）
...
| IT | US | SP | 中核の判断 |
| :--- | :--- | :--: | :--- |
| IT4 | US25(3), US07(3), US32(2) | 8 + 負債 2 | 航海スケジュールの整合（時刻昇順・港の連結）、**更新できる状態の判断**（キャンセル済みの航海・仮受付より先へ進んだ予約） |
...
## 終盤の続き: 業務を確かめる手段（IT16〜IT17 / Release 3.0）

**新しい業務ロジックを足さない局面です。** US33〜US36（業務シミュレーション）が作るのは、既にある API を**本番と同じ経路で**順に呼び、結果を待ち、どこまで進んだかを記録する仕組みです。集約も不変条件も増えません。

**それでもアウトサイドインです。** 入口は「シナリオを 1 本流したときに何が見えるか」で、そこから逆に「どの工程で何を待つか」を決めます。内側から作ると、**待ち方（結果整合の連鎖をどこまで待つか）が後付けになり**、失敗が「連鎖の欠落」なのか「待ち不足」なのか判別できない実行結果ができます。

**この局面に固有の危険が 3 つあります。**

| # | 危険 | 手立て |
| :--- | :--- | :--- |
| 1 | **シミュレーション専用の書き込み経路を作ってしまう** | 呼び出し元だけを自動化し、呼ばれる側は本番の実装を使う。Gateway を通る（認可も本番と同じ） |
| 2 | **生成データが実データに混ざる** | 印を付け、業務の読み口（経理の締め・追跡の対応一覧・請求検索）が既定で除外する。**除外は読み口の側に置く**——印を付けるだけでは混ざる |
| 3 | **負荷をかける側が業務を止める** | ヘルスチェックと実利用者の操作を劣化させないこと（IT17 の US36）。IT7 で横断的な防御が liveness を殺した形と同型である |
...
| 画面の型 | 共通レイアウト（左サイドナビ + トップヘッダ）、ロール別ナビ、403 画面。**プレースホルダは置かず**、画面はそれを使うストーリーの IT でナビ・到達性テストと対で足す（IT1 レビュー C1） |
...
| 日付 | 更新内容 | 更新者 |
| :--- | :--- | :--- |
| 2026-09-05 | IT4 の対象ストーリーに US32（IT3 で US06 から切り出し）を反映。6 SP → 8 SP。中核の判断に「更新できる状態の判断」を追加 | claude-code/claude-opus-5 |
| 2026-09-02 | 初版作成。IT1〜IT15 を序盤（IT1-3）・中盤（IT4-10）・終盤（IT11-15）に割り当て | claude-code/claude-fable-5-1 |
| 2026-09-03 | IT2 開始準備の横断検証。序盤の完了条件とウォーキングスケルトンの骨格から「全ルートのプレースホルダ」を外し、画面はストーリーの IT でナビと対に足す方針へ改める（IT1 レビュー C1 の追認を反映） | claude-code/claude-opus-5 |
