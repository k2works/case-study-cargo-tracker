...
| # | タスク | 由来 | 見積 | 状態 |
| :--- | :--- | :--- | :--: | :--: |
| R.1 | **契約テストの往復（2 サービス起動）。** 対象は **bookingms ↔ billingms**（ゴールデンは `ShipperRegisteredEvent.json` 1 本）。**routingms は購読側なので契約は増えません**（契約イベント 11 本の発行元は bookingms/handlingms/trackingms/billingms）。ゴールデンの一致だけでなく**実際に届くこと**を検査する | 引き継ぎ 1 | 6h | [x] |
| R.2 | **クラスタ E2E の順序依存を外す。** 各テストが自分で前提を作る。**前回実行の残骸でも緑になる**箇所がある | 引き継ぎ 7 | 3h | [x] |
...
| # | タスク | 見積 | 状態 |
| :--- | :--- | :--: | :--: |
| 4.1 | 対象章：07 航海スケジュールを登録する（新規）・08 経路設計に引き渡す（新規）・05 に危険物と冷凍の節を追加・00 の「担当と使える画面」を更新 | 5h | [x] |
| 4.2 | 画面キャプチャの再生成（S30・S32・S33）。**本文の更新は 1.4・1.5・2.6 の変更の中で行い、ここは撮り直しだけ**（IT2 T7：画面を変えたらその変更の中でマニュアルを見る。IT の終わりにまとめて見ない） | 2h | [x] |
| | 小計 | 7h | |
...
**落とした実績（2026-09-04）。** 順序どおり 1・3・5 を落としました。順 2（S32 の検索条件）は最初から入れていないので落とすものがありません。順 4（マニュアル）は今回も落ちませんでした（3 IT 連続）。

| 順 | 落としたもの | 送り先 | 落とした時点で分かっていたこと |
| :--- | :--- | :--- | :--- |
| 1 | 3.4 危険物の絞り込みを画面から確かめる | IT5 | 絞り込みそのものは受け入れ層（`航海スケジュールの登録.feature`）と投影の IT で固定済み。画面から踏む分は US08 の経路候補と同じ IT で見るほうが実際の使われ方に近い |
| 3 | R.7 `attention_item` の識別子の形 | IT4 | 実害が無い。内容から導く形は routingms でも同じにしたので、直すなら 2 サービス分を一度に直す |
| 5 | Q.4 値を捨てる分岐・設定の重複を探す | IT4 | 「探す」枠なので対象が増える側に置くほうが実入りが大きい。ただし US05 の付帯情報については受け入れ層で確かめた（3.5）ので、その 1 件は IT3 で潰している |

**R.9（ADR-0004 の承認と `verify`）は人の署名なので代筆しません。** 内容は承認できる状態です。
