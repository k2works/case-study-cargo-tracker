...
> **ベロシティ表記の統一注記 (H-09)**:
>
> - **計画 SP (拡張)**: 18 = 本体 10 + Try 8 (T-01〜T-09 + T-10 HPC)
> - **計画 SP (本体のみ)**: 10 = 当初イテレーション計画の本体 4 ストーリー
> - **実績 SP**: 18 (本体 10 + Try 8 完了) + arch-check Rule 4 (2 SP) を横断扱いで加算した場合 22
> - **達成率 100%**: 拡張計画 18 SP に対する実績 18 SP (本ドキュメント採用)
> - **達成率 180%**: 当初本体目標 10 SP に対する実績 18 SP (Try 込み、`release_plan.md` 採用)
>
> 本ドキュメントは「拡張計画 18 比 100%」表記。`release_plan.md` / `iteration_report-2.md` は「本体目標 10 比 180%」表記。数学的に整合する (10 本体 + 8 Try = 18)。
