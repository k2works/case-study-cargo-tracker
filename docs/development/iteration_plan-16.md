...
- [x] US38 の受入基準 6 件が、E2E / API / ドメインのテストで 1:1 に対応している。
- [x] **合計が取り消し済みを含まないこと**を、赤伝を 1 件置いた状態のテストで固定している。**ただしレビューで「一覧からも消えていた」ことが判明し、合計だけに効かせる形へ直した**。
- [x] TD-02 の一覧について、**実業務の案件を 1 件置いた状態**で `contains(実業務).doesNotContain(SIM)` を確かめている。**対象は 5 つではなく 4 つだった**（荷役の履歴は名指しの照会）。
- [x] `replicas: 1` を 2 に変えるとマニフェストの検査が赤くなる（確認済み）。
- [x] IT15 のふりかえり Try 5 件を計画と DoD に反映している。
- [x] 画面を伴うため、ユーザーマニュアルを更新した。**「夜間の継続実行を禁じる」記述を外した**（TD-02 完了）。**画面キャプチャの再生成は残作業**。
- [x] テストカバレッジ 80% 以上、ドメイン層 90% 以上（JaCoCo の検証が緑）。
...
| # | タスク | 見積 | 状態 |
| :--- | :--- | :--- | :--- |
| 2.1 | 検索条件の値オブジェクト（部分一致・発行月）をドメインに置く | 3h | [x] |
| 2.2 | `InvoiceMapper` に検索・件数・合計を足す。**同じ条件を 1 か所で組み立てる** | 4h | [x] |
| 2.3 | 取り消し済みを合計から外す。**赤伝を 1 件置いたテストで固定** | 2h | [x] |
| 2.4 | 検索 API（`GET /api/v1/billing/invoices?...`）。**読めない発行月は 400 で断る** | 3h | [x] |
| 2.5 | 画面に検索欄・発行月の絞り込み・件数と合計を出す | 4h | [x] |
| 2.6 | 方言スモーク（PostgreSQL / H2 の両方で文が通る）を通す | 1h | [x] |
...
| # | タスク | 見積 | 状態 |
| :--- | :--- | :--- | :--- |
| 3.1 | **既存の Snapshot 契約に由来を 1 項目足す**（新しいポートは作らない） | 4h | [x] |
| 3.2 | 通関・通関の期限超過から由来を外す（**荷役は名指しの照会なので対象外**と判明） | 3h | [x] |
| 3.3 | キャンセル承認待ち・陸揚げ待ちの 2 つから SQL で外す（bookingms 内なので越境しない） | 2h | [x] |
| 3.4 | **4 つとも実業務の案件を 1 件置いた状態**で除外を確かめる | 4h | [x] |
...
| # | タスク | 見積 | 状態 |
| :--- | :--- | :--- | :--- |
| 4.1 | 実行一覧に日付での絞り込みを足す（SQL に降ろす） | 3h | [x] |
| 4.2 | 過去セッションの一覧（種・設定・期間）。**成否の内訳は載せていない**——統計は動いているセッションのものを出しており、過去分は実行一覧から辿る | 4h | [x] |
| 4.3 | 実行一覧にセッション・種の列を出す | 2h | [ ] **未着手**——実行の集約が種を持たず（永続化のみ）、読み取り専用のモデルが要る。**過去セッションの一覧で種は読めるため、再現の手立ては確保できている** |

### Phase 5: 単一 Pod の前提（TD-04・1 SP）

| # | タスク | 見積 | 状態 |
| :--- | :--- | :--- | :--- |
| 5.1 | ~~ADR-033 に決定として残す~~ **[ADR-032](../adr/032-shipper-notification-delivery.md) 決定 7 に記した**（US39 と同じ ADR に置く方が、読む人が 1 か所で済む） | 1h | [x] |
| 5.2 | マニフェストを読む検査で `replicas: 1` を固定する。**2 に変えて赤を見る** | 3h | [x] **確認済み** |
