...
| 対象ストーリー | US38（請求書を検索する）＋ IT15 から送った負債 3 件（TD-02・TD-03・TD-04） |
| 計画 SP | **8**（US38 = 3・TD-02 = 2・TD-03 = 2・TD-04 = 1） |
...
- [ ] TD-02 の 3 つの一覧について、**実業務の案件を 1 件置いた状態**で `contains(実業務).doesNotContain(SIM)` を確かめている（IT15 で空振りした形を繰り返さない）。
...
| [TD-02](https://github.com/k2works/case-study-cargo-tracker/issues/564) | シミュレーション由来を 3 つの一覧から外す | 2 | — |
...
#### TD-02: シミュレーション由来を 3 つの一覧から外す（2 SP）

キャンセル承認待ち（bookingms）・荷役（handlingms）・通関（handlingms）の 3 つ。
...
| # | 決めること | 組み合わせて効く既存決定 | 本 IT での決定（案。Phase 1 で ADR-032 に固定する） |
| :--- | :--- | :--- | :--- |
...
| 1.2 | ADR-032（請求書検索の絞り込みと合計・継続実行の単一 Pod 前提）を書く | 3h | [ ] |
...
| 3.2 | 荷役の一覧・通関の一覧から由来を外す | 3h | [ ] |
| 3.3 | キャンセル承認待ちの一覧から SQL で外す（bookingms 内なので越境しない） | 2h | [ ] |
| 3.4 | 3 つとも**実業務の案件を 1 件置いた状態**で除外を確かめる | 3h | [ ] |
...
| # | タスク | 見積 | 状態 |
| :--- | :--- | :--- | :--- |
| 5.1 | ADR-032 に決定として残す（前提の外にあるものも書く） | 1h | [ ] |
...
| 6.3 | 実環境で通す（検索・3 つの一覧・継続実行を一晩回す） | 4h | [ ] |
...
> **効くのは発行月の絞り込みが先に掛かる場合**であり、そのことを ADR-032 に書く
...
| ID | 内容 |
| :--- | :--- |
| ADR-032 | 請求書検索の絞り込みと合計・継続実行の単一 Pod 前提 |
...
（`validating-iteration-plan` / `validating-design` の実施結果をここに記録する）
...
- [ ] 3 つの一覧の除外を、**実業務の案件を置いた状態**で確かめている（空の一覧で確かめない）
...
| 3 | 継続実行を一晩回した翌朝、荷役・通関・キャンセル承認待ちの一覧が実業務だけ | 結合 + 実環境 |
