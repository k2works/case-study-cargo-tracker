...
### 2. R2 / T4: 所有表を突き合わせ、ADR に届く範囲を書く — 5h
...
- [ ] **`NOT_SPLIT_YET` と `NOT_MEASURED_YET` が空である** — 整流局面を終わらせる条件
...
- [ ] **T1: 着手時に数え直した**（R8 は 8 → 10 だった）
- [ ] **T2: 自分の除外が支援クラスの既定になっている**
- [ ] **T3: `NOT_*` の表が空になった**（名簿を最後の手段にする）
- [ ] **T4: ADR の守り手にすべて対象範囲が書かれている**
- [ ] **T5: レコード分割で変換前後の要素数を突き合わせた**
- [ ] **T6: 正典の食い違いを着手前に決着させた**（A1）
- [ ] **破壊検証で空振り 0**
...
- [ ] **追跡管理者がダッシュボード・navbar の両方から荷役作業一覧へ到達できる**
- [ ] **荷役作業員の到達性が変わっていない**
- [ ] **追跡管理者が荷役登録・訂正申請へ進めない**（403）

#### ドキュメント

- [ ] `ui_design.md` の書式を通常に戻した（R4）
- [ ] `data-model.md` の所有表と検査が突き合わされている（R2）
- [ ] ADR の守り手に対象範囲が書かれている（T4）
- [ ] **マニュアル 8.8 のキャプチャを撮り直した**（R10。落としたなら記録する）
