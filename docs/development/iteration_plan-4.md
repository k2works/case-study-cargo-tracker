...
| # | タスク | 見積もり | 担当 | 状態 |
|---|--------|---------|------|------|
| 4.1 | 経路設計画面（`/routing/requests/{bookingId}`）に**経路候補の選択・確定→予約紐付け**（US09/US11）を追加 + 受入テスト | 4h | - | [ ] |
| 4.2 | **条件調整・再算出**（US10・期限延長等で computeRoutes 再実行）+ 受入テスト。営業への条件協議依頼導線 | 3h | - | [ ] |
| 4.3 | 予約詳細（`/bookings/{bookingId}`）に**確定経路表示・荷主通知（US12）・予約確定（US13）・差し戻し・キャンセル**を追加 + 受入テスト（状態遷移確認）| 4h | - | [ ] |
