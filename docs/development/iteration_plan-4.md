...
| 4.2 | **条件調整・再算出**（US10・期限延長等で computeRoutes 再実行）+ 受入テスト。営業への条件協議依頼導線 | 3h | - | [x] |
...
#### 実装で確定した設計判断（セルフレビュー反映）

- **US10 期限調整の意味論**: 経路設計画面の到着期限調整は**候補探索（再算出）専用**であり、集約の `RouteSpecification` 期限は変更しない。緩和期限で表示された候補を確定しようとすると、ドメインが**元の期限**で検証して棄却する（`BusinessRuleViolation("RouteSpecification")` → 400）。期限そのものの変更は営業を通じた荷主との条件協議（別アクション）に委ねる。この挙動は受入テストで固定済み。将来、正式な期限変更コマンド（`RouteSpecification` 更新）を導入する場合は別途 ADR で扱う。
- **post-commit イベント発火はベストエフォート**: `RouteAssignment.applyCommand` は永続化コミット後にのみ発火し、発火失敗（`Async.Catch`）は確定済み結果を巻き戻さない。実消費（Tracking 等）への差し替え時はディスパッチャ側でリトライ/DLQ を担う。
...
- [x] カバレッジゲート（ドメイン 85%／全体 80%）が緑（全体 94.1%／ドメイン 90.2%）
