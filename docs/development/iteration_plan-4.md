...
- [ ] ルーティング表が 1 つに統合され、同一パスが 2 表に現れうる構造が消えている（または成立しない場合は ADR で「認可の正典」を再定義している）
- [ ] パス一致・メソッド不一致で **405** が返る（IT3 では 404 だった）
- [ ] `Clock` 効果が導入され、AU-11（ロール別セッションタイムアウト）がテストで固定されている
- [ ] `Booking` の `domain` が `Shipper` の `domain` を参照していない（`arch-lint` 規約で機械検査される）
- [ ] 判断を含むユースケース（荷主登録の重複判定・予約登録の必須項目判定）に**単体テストが先にある**（Try T3）
- [ ] CI が緑（build / test / arch-lint / trace-lint / E2E / Trivy）
- [ ] 手順書だけで新規参加者が荷主登録・予約登録まで到達できる（Try T5）
...
| # | タスク | 見積もり | 状態 |
|---|--------|:---:|:---:|
| 0.1 | T6: テストヘルパーを点検し、失敗時に値を返さず `Assert.fail` で停止させる（`loginAndGetCookie` ほか） | 1.5h | [ ] |
| 0.2 | ロック境界値のテスト分割・Cookie の攻撃者視点ケース（IT3 レビュー M11・M13） | 1.5h | [ ] |
| 0.3 | CSRF のクロスセッション統合テスト（IT3 レビュー H17） | 1h | [ ] |
| 0.4 | ロール・無効化の文言によるユーザー列挙の受容を ADR-0002 へ追記（IT3 レビューの矛盾 1） | 0.5h | [ ] |
| 0.5 | `requiresCsrf` のテスト（IT3 レビュー M14）。`Anonymous` な POST ルートが増えた瞬間に無防備になるため | 0.5h | [ ] |
| 0.6 | CSRF トークン比較を定数時間に（M13/L5）、`Session.login` の戻り値を `Unit` へ（L6） | 1h | [ ] |
...
| # | タスク | 見積もり | 状態 |
|---|--------|:---:|:---:|
| 1.1 | 【実験】lift 案（A）: ポートへ no-op 操作を足して効果を揃える。小さく試して記述量を実測する | 2h | [ ] |
| 1.2 | 【実験】クロージャ案（B）: ハンドラ適用済みの関数を表へ載せる。A が不成立の場合のみ | 1.5h | [ ] |
| 1.3 | 【RED→GREEN】単一表への統合。二重定義の検出テストと、パス一致・メソッド不一致で 405 を返すテスト | 3h | [ ] |
| 1.4 | `/health/ready` をルーティング表へ載せる（IT3 レビュー M9） | 1h | [ ] |
| 1.5 | 【RED→GREEN】`TxMode`（読み取り専用 / 書き込み）をルート定義に持たせ、読み取り専用でコミットしない | 2h | [ ] |
| 1.6 | 【RED→GREEN】`Clock` 効果の導入。AU-11（ロール別タイムアウト）とロック解除を注入時刻で検証する（Try T7）。ポート一覧へ反映 | 3h | [ ] |
| 1.7 | A・B が不成立の場合は ADR-0005 を起票し、認可の正典を「表の集合」と再定義する | 1.5h | [ ] |
| 1.8 | ルート定義の `RequiredRole` と設計の認可可否表を照合するテスト（IT3 レビュー M17）。単一表になれば安く書ける | 1.5h | [ ] |
...
| # | タスク | 見積もり | 状態 |
|---|--------|:---:|:---:|
| 2.1 | `V4__add_shipper.sql`。データモデルの是正 3 点（`address` 追加・割引率上限 30%・`shipper_id UUID` 併置）を同一コミットで反映 | 2h | [ ] |
| 2.2 | 【RED→GREEN・単体】`Shipper` 集約と値オブジェクト（`ShipperCode` / `ShipperName` / `Email` / `Phone` / `Address` / `ContractNumber` / `DiscountRate` / `ShipperType`）。不変条件は `Result[DomainError, t]` | 4h | [ ] |
...
| # | タスク | 見積もり | 状態 |
|---|--------|:---:|:---:|
| 4.1 | Playwright のセットアップと E2E シナリオ（公開追跡照会・ログイン → ダッシュボード） | 4h | [ ] |
| 4.2 | `main` への push で E2E を実行するジョブ | 1h | [ ] |
| 4.3 | 日次の実 PostgreSQL 統合テストジョブ | 2h | [ ] |
| 4.4 | Trivy スキャンを PR ジョブへ追加 | 1h | [ ] |
...
- [ ] ルーティング表が単一化されている（または ADR-0005 で再定義され、二重定義の検出テストがある）
- [ ] `Clock` 効果が導入され、AU-11 がテストで固定されている
- [ ] **判断を含むユースケースに単体テストが先にある**（Try T3）: 荷主の重複判定・予約の荷主存在確認が、DB なしで固定されている
- [ ] **受入基準に「壊れ方」の項目がある**（Try T2）: 各ストーリーで 1 項目以上
- [ ] **詰まったときの次の行動が画面にある**（Try T4）: 重複メール・入力エラー・荷主未登録の 3 箇所
- [ ] **手順書だけで荷主登録・予約登録まで到達できる**（Try T5）
- [ ] **テストヘルパーが失敗時に停止する**（Try T6）
- [ ] `arch-lint` が違反 0 件（BC 独立性の規約を含む）、メタテストが全件成功
- [ ] `npm run dev:verify` が全件成功する
- [ ] CI が緑（build / test / arch-lint / trace-lint / E2E / Trivy）
- [ ] **実画面で確認する**: 荷主登録 → 予約登録をブラウザで通し、スマートフォン幅でも崩れないこと
- [ ] **表示ラベルを業務の言葉として読み上げる**: 画面に列挙値（`PRELIMINARY` 等）が生のまま出ていないこと
- [ ] 設計ドキュメントへの反映（データモデルの是正 3 点・UI 設計の是正 2 点・荷主 2 画面の詳細設計・`Clock` ポート・実装状況注記）が実装と同一コミットで行われている
- [ ] ビジネスルール ⇄ テスト対応表が更新されている（AU-11 が「済」）
