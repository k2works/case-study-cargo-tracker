...
| 0.7 | 追跡詳細画面に「対応取消し」「補足コメント追記」動線を追加 + Controller `cancelExceptionResolution` / `appendResolutionComment` アクション（H9 / 業務代表者指摘） | 5h | [x] **完了** (2026-06-24): `TrackingActivity` に `cancelExceptionResolution` / `appendExceptionComment` メソッド追加 (Error 拡張: NotResolved / EmptyComment)、Repository に `clearExceptionResolution` / `updateExceptionNotes` 追加 (PK 直接更新)、`TrackingCommandService` に `cancelExceptionResolution` / `appendResolutionComment` + 2 コマンド追加、Controller に 2 アクション + AppendCommentFormData 追加、routes に 2 行追加、`detail.scala.html` に「対応取消し」(confirm 付) / 「補足追記」フォーム追加。Unit 31 件 Green (+4 件) |
| 0.8 | 例外記録モーダルに Delay 選択時のみ「新到着予定日 datetime-local + 対応方針 select (定型 4 種) + 詳細理由 textarea」を JS 表示制御で追加。`logDelayNotification` を意味ある値に置換（H10 / T7 / P8） | 4h | [x] **完了** (2026-06-24): `RecordExceptionFormData` に `newEstimatedArrival` / `responsePlan` フィールド追加、`DelayResponsePlan` object 新設 (Reroute / Express / BondedWarehouse / ContactShipper 定型 4 種 + displayNames)。`detail.scala.html` の例外モーダルに Delay 専用フィールドブロック (`#delayOnlyFields`) 追加 + JS で exceptionType=Delay 時のみ表示制御。Controller の Delay ブランチで ETA を ISO 形式に変換、responsePlan を displayName で日本語化して `logDelayNotification` に渡す形に変更 (従来「未確定」固定 → 実値)。Unit 87 件 Green |
...
| 0.12 | 設計ドキュメント反映（T6 / docs/design/data-model.md + domain-model.md + ui_design.md）: IT7 差分 (V18-V22 + TrackingExceptionEvent + ItineraryLeg + InvoiceLineItem + RecipientConfirmationType + 例外記録 UI) + IT8 差分 (Payment テーブル列 `amount BIGINT` 単通貨整合 + `due_date` / `version` 追加 + `transaction_reference` → `reference_code` 統一、ui_design.md L82 画面一覧 + L209 画面遷移図 (精算フロー) の **両方** に Payment 系 4 画面 (`/billing/invoices/:id/issue-payment` / `/billing/payments` / `/billing/payments/:paymentId` / `/billing/payments/:id/confirm`) を追加、Accountant→Settlement / Admin→MasterAdmin Role 統一 (実装側 Role.scala: Sales / RouteDesigner / Tracker / Settlement / MasterAdmin 準拠)) を正式反映 | 6h | [x] **完了** (2026-06-24): ADR 0019 案 B 反映を中心に最小スコープで実施。**data-model.md**: invoice テーブルに `paid_at` / `payment_reference` / `version` 列追加 + ADR 0019 注記、payment テーブルに「IT8 V25 で drop 予定」歴史的記録ヘッダー追加 + `transaction_reference` → `reference_code` 名称統一。**ui_design.md**: Role 全 6 箇所 `Accountant` → `Settlement` / `Admin` → `MasterAdmin` 統一 (定義 + ナビ + ロール制御 + アクセス制御)、画面一覧に精算 2 アクション (issue-payment / confirm-payment) 追加。**domain-model.md**: Invoice クラス図に `dueDate` / `paymentReference` / `version` 追加 + `issuePayment` / `markOverdue` メソッド追加 + ADR 0019 note 追記。IT7 差分の TrackingExceptionEvent.id 追加は 0.5 で反映済 |
...
| # | タスク | 見積もり | 状態 |
|---|--------|---------|------|
| 2.1 | `Invoice` に `dueDate: Option[LocalDate]` / `paymentReference: Option[String]` フィールド追加 + `PaymentStatus` enum 拡張 (NotIssued / Pending / Overdue / Confirmed / Refunded) + `issuePayment(dueDate, referenceCode)` / `confirmPayment(paidAt)` / `markOverdue(now)` メソッド追加。`Invoice.Snapshot` (ADR 0014) もフィールド追随 | 4h | [x] **完了** (2026-06-24): PaymentStatus enum に NotIssued 追加 (順序: NotIssued/Pending/Overdue/Confirmed/Refunded)、Invoice にフィールド 2 個 + メソッド 3 個追加、`Invoice.InvalidPaymentStateTransition(from, to)` Error 追加、Invoice.Snapshot にも追随、issue 直後の初期 status を Pending → **NotIssued** に変更。InvoiceSpec に状態遷移テスト 6 件追加 (NotIssued→Pending / 不正遷移 / Pending→Confirmed / Overdue→Confirmed / NotIssued→Confirmed NG / markOverdue 期日内 NG/期日超過 OK/NotIssued NG)。Unit 19 件 Green |
| 2.2 | Flyway V23: `invoice` テーブルに `due_date DATE NULL` / `payment_reference VARCHAR(64) NULL` 追加 (corporate_discount_policy 新設と同一 migration に統合) | 2h | [x] **完了** (2026-06-24): V23 作成 = invoice に `due_date DATE NULL` + `payment_reference VARCHAR(64) NULL` 追加 + `ck_invoice_payment_status` CHECK 制約を NotIssued 含む形 (NotIssued/Pending/Overdue/Confirmed/Refunded) に差し替え。**corporate_discount_policy 新設は IT9 申し送り** (US22 は Shipper.discountRate 直接参照で機能完結しており本テーブル不要) |
| 2.3 | `ScalikeJdbcInvoiceRepository.save` / `update` を新フィールド対応に拡張（withOptimisticLock 適用、PaymentRepository は新設しない） | 3h | [x] **完了** (2026-06-24): `rowToInvoiceAndId` で `due_date` / `payment_reference` を Snapshot に追加読込、`save` UPDATE 文に両列の SET 追加 (楽観ロック維持)。Unit 19 件 Green。**withOptimisticLock** ヘルパは既存 OptimisticLockException 例外 throw + try/catch パターンが Repository 層で確立しているためここでは未適用 (Application Service 側で適用済) |
| 2.4 | `BillingCommandService.issuePayment(invoiceId, dueDate, referenceCode)`: Confirmed Invoice → `Invoice.issuePayment` → MailNotificationPort 経由で荷主メール送信 + PaymentRequested 通知ログ | 4h | [x] **完了** (2026-06-24): `BillingCommandService.issuePayment(IssuePaymentCommand)` 追加、`BookingPublicApi.logPaymentRequested` 経由で通知記録、withOptimisticLock 適用。Unit 22 件 Green (+3 件)。**MailNotificationPort 経由のメール送信は 2.9 で実装** |
| 2.5 | `BillingCommandService.confirmPayment(invoiceId, paidAt)`: 手動入力された paidAt で `Invoice.confirmPayment` → BookingPublicApi 経由で `Cargo.markSettled` 遷移 + PaymentConfirmed 通知 | 3h | [x] **完了** (2026-06-24): `Cargo.markSettled()` メソッド追加 (Delivered → Settled、それ以外は InvalidStatusTransition)、`BookingPublicApi.markSettled` 追加 + `BookingCommandService.markSettled` 実装、`BillingCommandService.confirmPayment(ConfirmPaymentCommand)` 追加 (invoice 状態遷移 + markSettled + PaymentConfirmed 通知)。Unit 79 件 Green (+2 件 confirmPayment 成功 / NotIssued NG) |
| 2.6 | `BillingCommandService.detectOverdue(now)` (Cron スケジューラ想定、IT8 はバッチ未着手で API のみ、IT9 で Pekko Scheduler 連携): 期限超過 Invoice を `Invoice.markOverdue` で Overdue 化 + OverdueAlerted 通知 | 3h | [x] **完了** (2026-06-24): `BillingCommandService.detectOverdue(now: LocalDate): Int` 追加。全 Invoice を findAll で取得し markOverdue 成功分のみ save + OverdueAlerted 通知、戻り値は Overdue 化された件数。Cron 連携は IT9 申し送り。Unit +2 件 (期限超過 / 期限内スキップ) 計 16 件 Green |
| 2.7 | NotificationType に PaymentRequested / PaymentConfirmed / OverdueAlerted 追加、ペイロード + JSON + Flyway V24 (CHECK 拡張) | 3h | [x] **完了** (2026-06-24): NotificationType に 3 種追加、NotificationPayload に 3 case class 追加、NotificationPayloadJson に 3 case 追加、BookingPublicApi に logPayment* / logOverdueAlerted の 3 メソッド追加、BookingCommandService に実装。**Flyway は計画上 V24 だったが V26 (LossEscalated rename) 先行作成済のため Out-Of-Order 回避で V27 として実装** (notification_log CHECK 制約に 3 種追加) |
...
| 2.9 | `MailNotificationPort` (handling と同じ ACL パターン) + `MailNotificationAdapter` (Pekko Mail or print logger)、ADR 0018 候補 | 3h | [x] **完了** (2026-06-24): ADR 0018 起票・承認、`billing/domain/model/ports/MailNotificationPort` trait 新設 (3 メソッド: sendPaymentRequested / sendPaymentConfirmed / sendOverdueAlert)、`LoggingMailNotificationAdapter` (Play Logger による INFO 出力、IT9 で Pekko Mail/SES 連携予定) 実装、Module bind、BillingCommandService に Inject + 3 メソッド (issuePayment/confirmPayment/detectOverdue) で sendXxx 呼出、NoopMail テストダブル、Unit 16 件 Green |
| 2.10 | BillingCommandServiceSpec 拡張 (issuePayment / confirmPayment / detectOverdue 各 2 件) + InvoiceSpec 拡張 (issuePayment / confirmPayment / markOverdue 状態遷移 6 件) + ScalikeJdbcInvoiceRepositoryIT 拡張 (新フィールド永続化) + Playwright E2E 3 件 (発行 / 入金 / 期限超過) | 8h | [x] **完了 (Unit のみ、IT/E2E は IT9 申し送り)** (2026-06-24): 各タスクで増分 Spec 作成済 + 本タスクで MailNotificationPort 送信検証 3 件追加 = BillingCommandServiceSpec 16 → 19、InvoiceSpec は 2.1 で 6 件追加済 (10 件) = Billing 計 29 件 Green。**ScalikeJdbcInvoiceRepositoryIT 拡張 / Playwright E2E 3 件は IT9 申し送り** (Testcontainers PostgreSQL 起動環境と Shipper 法人マスタ登録 UI が IT8 範囲外で前提条件不足) |
