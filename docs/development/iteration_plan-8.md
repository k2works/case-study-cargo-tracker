...
| # | タスク | 見積もり | 担当 | 状態 |
|---|--------|---------|------|------|
| 2.1 | 荷主連絡先の実解決（Shipper メールを合成層 ACL で解決）と送信抽象（`MailSender` ポート）を導入。notification_log 記録＋送信を分離。精算/例外/追跡通知を共通化 | 4h | - | [ ] |
| 2.2 | 通知失敗経路（Save 成功・通知失敗）の受け入れ/統合テスト（IT7 レビュー中#2） | 2h | - | [ ] |
...
| # | タスク | 見積もり | 担当 | 状態 |
|---|--------|---------|------|------|
| 3.1 | `PaymentGatewayPort` の実アダプタ（HTTP）を合成層に実装し、WireMock.Net で契約（成功・決済拒否・タイムアウト）を固定した受け入れテスト。ADR-0014 を承認済みへ更新 | 5h | - | [ ] |
...
**進捗率**: 38% (5/13 SP・エピック1「精算業務の受入完全充足」完了。割引ポリシーマスタ接続・支払期限/期限超過通知・輸送実績プレビュー/距離自動導出・消費税/金額内訳・例外時料金調整)
