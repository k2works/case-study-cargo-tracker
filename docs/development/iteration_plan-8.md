...
- [x] 対象ストーリーの受入基準を満たす。US17-4・US19-3・US20-4 の通知は**代替**、`ROLE_SHIPPER` 開放は US18 では果たせないことを [ADR-024] 決定 10 に記録（完了報告書へ）
- [x] 成功基準 1〜6 を満たす（下記）
- [x] 全テストが緑（バックエンド 7 サービス・フロントエンド 338 件・E2E 58 件）。**CI も緑**（run 32669396922）。**ただし最初の CI は赤だった**——ローカルで `./gradlew test` しか回しておらず、`build` に含まれる SpotBugs を抜かしていた（ふりかえり Problem 7）
- [x] **`TZ=UTC` でも緑**（`--rerun-tasks` で強制再実行）
- [x] **ドメイン層のカバレッジが 90% 以上**（trackingms 94.6 / bookingms 97.4 / handlingms 99.0 / routingms 98.7 / authms 90.5 / shared 100.0）
- [x] SonarQube Quality Gate が PASS（**両プロジェクト**。Bug 0・Vulnerability 0・新規指摘 0）
- [x] **決定の数だけ検査があり、検査の場所が ADR に書かれている**（Try 3）。**ただしレビューで 3 件の空振りが判明した**——表を書くだけでは足りない（ふりかえり Try 1 へ）
- [x] **「〜しない」と書いたコメントの数だけ、検査がある**（Try 1・成功基準 4）。**ただし 2 件は検査が誤った実装を固定していた**（X-Forwarded-For・窓のリセット）
- [x] **値が層をまたいで生き延びる**（Try 2・成功基準 5）。**推定到着日は 3 層連続で潰れていた**——実環境とレビューで見つけた
- [x] **壊して赤を確認した**（発生前状態の永続化・例外の復帰・交換機の引数・認可の呼び出し・推定到着日・章の目次登録ほか）
- [x] **画面を開いたロールが「何をできるか」を E2E で 1 本ずつ持つ**（Try 4）
- [x] **同じ失敗を 2 回見たら原因を追った**（Try 5）。コンテナの資源不足は**IT7 から 3 IT 連続**のため、土台を 1 つに寄せた
- [x] **サービス間の呼び出しを実際に 1 往復させた**（kind で予約 → 発行 → 公開照会）
- [ ] **実環境で確かめる前にイメージを作り直した**
- [ ] **ナビゲーションの 4 点が一致している**。ロール別・状態別の到達性を確かめた
- [ ] **認証不要の画面は、入口も認証の外に置いた**（ポータル・ログイン画面）
- [x] **件数から対象一覧へ辿れる**（[横断規約](release_plan.md)。件数を出すだけは完了としない）
- [ ] **認証の外へ出す項目を絞り、返さないことを検査で固定した**
- [ ] **照会ログが残る**（UC14・UC15 の最低保証）
- [ ] **デモ項目 8 件をこの順で通した**
- [ ] **JIG / jig-erd の出力を再生成した**
- [ ] ユーザーマニュアルを更新し、**キャプチャを再生成して目視した**
- [ ] **「設計への反映が必要な箇所（注）」13 件をすべて反映した** → **未達（2 / 13）**。`data-model.md` の tracking_db 全面とストーリー番号の誤りのみ。**残り 11 件は IT9 の返済枠へ**
- [x] **[ADR-024] で「決めること」A〜D に答えを出した**（決定は 11 件になった）
