...
|-----------|:--:|:--:|------|
| TS14 | 5 | 5 | 完了（1.1-1.5 すべて。返済 12 件 + 状態・荷主名の絞り込み） |
| US01 | 5 | 5 | 完了（2.1-2.8 すべて。受入基準 6（危険物申告フォーム）は見積の段階で不要と判断し未実装） |
| TS12b | 1 | 1 | 完了（3.1-3.3。3.3 は実測のうえ「現状維持」を選択） |
| **合計** | **11** | **11** | 縮退 0 件 |

> **実績の詳細は完了報告書へ書く**（Try T5。本書は計画時点で凍結する）。
> 本表と下記の注記のみ、クローズ時に追記した。

#### 計画から変えた判断（3 件）

| # | 計画 | 実際 | 理由 |
| :--: | :--- | :--- | :--- |
| 1 | 運賃モデルは Estimation Context に置く | **共有カーネル**（`SharedFreightTariff`）へ置いた | タスク 2.7（経路割り当ての費用列）で Routing からも同じ計算が要ることが判明した。運賃表は全社で 1 つであり、画面によって違う金額を出すことはあり得ない |
| 2 | `EstimateId` は UUID | **`EST-XXXXXXXX`**（採番時に確定） | 受入テストで URL に UUID が現れることに気付いた。利用者が電話口で読み上げる番号と URL が別物になる。予約の `BK-` と同じ形へ揃えた |
| 3 | 未知の状態語は 0 件（軸 1） | **一度「絞らない」で実装し、計画に合わせて戻した** | 実装時に計画の壊れ方の観点を確認せず、逆の判断をしていた。計画が正典である（コミット `9bdf186a`） |

#### 受入基準の未充足（1 件・意図的）

US01 受入基準 6「危険物が含まれる場合、危険物申告情報の入力フォームが表示される」は
**実装していない**。見積の段階で必要なのは種別だけであり（割増はそれで決まる）、
申告情報（クラス・UN 番号・品名）は予約時に要る（US05 で実装済み）。
**要らない情報を先に聞くと、電話口で即答するという利用場面が壊れる**。
