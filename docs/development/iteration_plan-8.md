...
{+
  ...
  {#
    **請求 ID** | **予約 ID** | **荷主** | **請求金額** | **ステータス** | **支払期限** | **操作**
    b3c7d2e1 | B-2026-0512-001 | 田中物産 | ¥115,000 | CALCULATED | — | [詳細]
    a1b2c3d4 | B-2026-0515-003 | 個人荷主 | ¥32,000 | INVOICED | 2026-09-10 | [詳細]
    f9e8d7c6 | B-2026-0518-007 | 山田商事 | ¥280,000 | OVERDUE | 2026-08-31 | [詳細] [督促]
  }
  ---
  [S25 督促一覧へ]
}
...
{+
  ...
  {#
    **種別** | **説明** | **金額**
    BASIC | 基本輸送料金 | ¥150,000
    DISCOUNT | 法人割引（20%）| ▲¥30,000
    ADJUSTMENT | 遅延補償 | ▲¥5,000
    —— | **合計** | **¥115,000**
  }
  ---
  {
    [料金を算出・確定] | [精算書を発行（S24）] | [← 一覧へ戻る]
    ' PENDING の場合: [料金を算出・確定] のみ有効
    ' CALCULATED の場合: [精算書を発行] が有効
  }
  ---
  {(算出成功)
...
{+
  ...
  {#
    **請求番号** | **予約 ID** | **荷主** | **請求金額** | **支払期限** | **超過日数** | **操作**
    INV-20260831-000003 | B-2026-0518-007 | 山田商事 | ¥280,000 | 2026-08-31 | 20 日 | [督促送信] [詳細]
    INV-20260820-000001 | B-2026-0501-002 | 鈴木商店 | ¥45,000 | 2026-08-20 | 31 日 | [督促送信] [詳細]
  }
  ---
  [全件に督促送信] | [← 請求一覧へ]
  ---
