...
| 2.4 | Cross-BC: 入金確認 → Cargo.Settled 連動 (FK 制約なし、`booking_id` TEXT 照合を Application 層で実施) + 精算書発行 → Notification BC 通知レコード + 期限超過 → 未払い通知 | 3h | [~] 主要部完了 (`286e1923`、BookingStatus に Settled 追加 (ADR-0014 宿題) + markSettledByBookingId + isClaimedByBookingId + cargo CHECK 潜在バグ修正。**残**: Pricing resolveConfirmedCost 実装 + Notification 通知ポート実装 + Main 配線) |
