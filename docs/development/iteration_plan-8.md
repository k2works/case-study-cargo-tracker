...
- [x] US20: 追跡番号と例外種別「破損」または「紛失」・発生状況を記録できる
- [x] US20: 例外記録後に貨物状態が「例外発生（EXCEPTION）」に更新される
- [x] US20: 紛失時に緊急フラグが設定され管理職への escalation 通知が送信される
- [x] US20: 対応内容（補償方針等）を入力して荷主に報告を送信できる
- [x] US21: 「引取済」状態の予約に対して料金算出を開始できる
- [x] US21: 輸送実績（経路・距離・重量・貨物種別）が表示され、基本料金が自動計算される
- [x] US21: 算出した料金を確定（保存）できる
...
| 1.6 | FE: US20 の FE テストを追加する | 1h | - | [x] |
...
| 2.7 | FE: US21 の FE テストを追加する | 1h | - | [x] |
...
| カテゴリ | SP | 理想時間 | 状態 |
|---------|----|----|------|
| US20: 破損・紛失例外処理（IT7 レビュー対応含む） | 8 | 8.5h | [x] 完了 |
| US21: 輸送料金算出 | 8 | 9.5h | [x] 完了 |
| **合計** | **16** | **18h** | |
