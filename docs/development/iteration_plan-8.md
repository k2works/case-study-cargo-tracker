...
| # | タスク | 見積もり | 担当 | 状態 |
|---|--------|---------|------|------|
| 4.1 | 精算完了の Settled 同期を `BookingSettled` イベント駆動の集約更新へ移行（`Booking.Application.settle` 経由で Delivered→Settled ガードを通す）。射影 `syncBookingStatus` を廃止 or 補助化。ADR-0013 を改訂 | 4h | - | [x] |
| 4.2 | booking_status の実値検証を E2E/統合テストに追加（IT7 レビュー高#1・射影の実値検証欠落の解消） | 2h | - | [x] |
...
| # | タスク | 見積もり | 担当 | 状態 |
|---|--------|---------|------|------|
| 5.1 | US-ADM-01 有効期限フィルタ・割引率フォームバリデーション（`max` 30%）・確定前割引率表示 | 3h | - | [x] |
| 5.2 | `Money.add` 異通貨拒否テストの是正（第二通貨導入 or 契約明確化）・`IssueRefund` の Web 返金導線・採番の `last_insert_rowid()` 化 | 3h | - | [x] |
| 5.3 | Release 1.1 正式出荷（`developing-release`: 品質ゲート→バージョンバンプ→CHANGELOG→tag）・リリース完了報告書の更新 | 3h | - | [x]（注: ユーザー判断により品質ゲート確認＋報告書更新に限定。バージョンバンプ・git tag は保留） |
...
**進捗率**: 100% (13/13 SP・全エピック完了。品質ゲート合格〔全体 89.0%・ドメイン 88.8%〕・release_report-1.1 更新済み。注: task5.3 の出荷はユーザー判断でバージョンバンプ・git tag を保留し、品質ゲート確認＋報告書更新に限定)
