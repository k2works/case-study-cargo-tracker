...
- [x] US23 の受入基準 5 件・US01 の受入基準 6 件を、**1 項目ずつ画面と突き合わせた**（Try 2）
- [x] **「代替で満たす」ものを「満たす」と書いていない**（23-2・23-3・23-5）
- [x] 距離（地域区分）・輸出免税が入り、IT11 の未達 2 件が閉じた
- [x] デモ項目 14 件すべてに**対応する検査がある**（`billing.spec.ts` 19 件・`estimate.spec.ts` 8 件・`navigation.test.ts`・`QuoteMatchesInvoiceIntegrationTest`）
- [x] **Phase の完了を宣言する前に、その Phase のタスク表を 1 行ずつ読んだ**（Try 1）。Phase 0.7 は「送った」——Phase 5 と同じ段取りであり 2 度起こす意味が無い
- [x] 料金の式が 1 か所にあり、US21 と US01 が同じ計算を通ることが検査で固定されている（統合 + 実環境）
- [x] ADR-028 の決定ごとに「破られたら何が起きるか」と検査が対応する（Try 8）
- [x] `BookingStatus` に値を足した箇所を、**扱う場所すべてを回る検査**で固定した（網羅 switch がコンパイルで止め、`status-labels.test.ts` が赤になった）
- [x] `location.region` を 4 サービス（booking / routing / handling / tracking）に配り、`LocationSeedReplicaTest` が緑
- [x] **マニュアル 12 章に入金確認・取り消しを追記し、13 章「見積管理」を新設**。キャプチャを生成 spec 経由で撮り直した（**また欠陥を見せた**——4 IT 連続）
- [x] **一気通貫 E2E（予約 → 経路 → 荷役 → 通関 → 引取 → 料金 → 入金 → 精算済）が緑**
- [x] 実環境（kind）でも同じ 1 本を通した（27 件緑。**実環境でしか出ない設定漏れを 1 件見つけた**）
- [x] **JIG / jig-erd を再生成し、設計と実装の乖離を差分で確かめた**
- [x] 注 1〜13 の設計反映が済んでいる
- [x] 全テスト緑・`TZ=UTC` 緑・CI 緑
- [x] **SonarQube PASS（両プロジェクト）**——新規違反 0・Bug 0・Vulnerability 0・重複 0.1%・
      新規カバレッジ 89.7%（バックエンド）／ 83.7%（フロントエンド）。バックエンドは
      **Security Hotspot 1 件がレビュー待ちで一度 ERROR になった**が、**利用者が UI で手動承認**
      して PASS になった（クローズ後）。対象は Flyway の Java マイグレーションで、
      **中身は読んで安全と確認済み**（DB から読んだ識別子を SQL に混ぜる前に形を検査する）。
      `// NOSONAR` は Hotspot に効かない。**毎回 UI 操作が要る形そのものは IT13 の申し送り**
- [x] 返済枠を **8 件すべて消化した**（0.7 は Phase 5 の実環境テストで一巡させた
      ——`unbilled` → `calculations` → `calculate` → `invoices`）。IT13 へ送る分は申し送りに記載済み
- [x] **マルチパースペクティブレビューの高 14 件をすべて対応した**
      （[レビュー](../review/イテレーション12_review_20260827.md)）
...
| クローズ（レビュー対応） | **完了**（高 14 件すべて対応。中 27 件は対応分と IT13 送りを分けた） |
