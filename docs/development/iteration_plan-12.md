...
- [ ] US35 / US36 の受入基準（**受入基準ごとに「その道を実行したテスト」を 1 つ名指しする**。T6）
- [ ] デモ項目 14 件が動作する
- [ ] **返済枠の高 4 件（C28〜C31）と 3 IT 繰り越し 4 件（C6 / C11 / C12 / C16）を返す**

#### 終盤の完了条件（`development_strategy.md`）

- [ ] 業務シナリオが通しで動作する
- [ ] E2E のクリティカルパス（既存 12 本）＋本 IT の 2 本が緑
- [ ] ArchUnit のルールがすべて有効・緑（**C28 の新しい検査を含む**）
...
- [ ] **「〜する」と書いたコメントを数え上げ、その全件を壊して赤にする。** とくに「同じ〜を使う」「〜を持ち回る」「〜の内部にある」など**構造を主張する**もの
- [ ] 壊せない主張はコメントから落とす
- [ ] **マイグレーションのコメントに「読み戻す側の責務」を書いたら、その IT のうちに読み戻すテストを書く**（T4）
...
- [ ] **Mapper の SQL が触るテーブルの所有 BC を検査する仕組みが動く**（実コードの形のフィクスチャで確かめる）
- [ ] 既存の越境 2 本が解消している（または理由を明記して残す）

#### 到達性（ロール別・状態別・**発生時点**）

- [ ] 営業担当者が予約詳細で確認コードを読める
- [ ] **荷主が自分の予約で確認コードを読める**
- [ ] 荷役作業員が荷役作業一覧から訂正申請へ到達できる
- [ ] **追跡管理者がダッシュボードから承認待ちへ到達できる**
- [ ] **荷受人が追跡照会で通関状態を読める**（C30）
- [ ] 「無いこと」を見るアサートの前に「開けたこと」を見ている

#### ドキュメント

- [ ] 設計への反映 10 件を完了（**ADR を 1 件起票する**。C28）
- [ ] **マニュアルを更新**（04 に確認コード・08 に訂正申請・07 に承認）＋キャプチャを生成 spec で再生成。**実装を見てから書く**（T7）
