...
- [ ] **SonarQube PASS**——**新規違反 0・Bug 0・Vulnerability 0・重複 0.1%・新規カバレッジ
      89.7%（バックエンド）／ 83.7%（フロントエンド）でフロントは PASS**。バックエンドは
      **Security Hotspot 1 件がレビュー待ち**で ERROR のまま。UI で「レビュー済み」にするには
      管理者の資格情報が要る（手元のトークンでは列挙もできない）。対象は Flyway の Java
      マイグレーションで、**中身は読んで対処済み**（DB から読んだ識別子を SQL に混ぜる前に
      形を検査する）。**IT13 の申し送りに載せた**
