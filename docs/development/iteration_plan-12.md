...
- [ ] **SonarQube PASS**——指摘は 33 件 → 0 件にしたが、**Security Hotspot 1 件がレビュー待ち**。
      UI で「レビュー済み」にするには管理者の資格情報が要る（手元のトークンでは読み取りもできない）。
      対象は Flyway の Java マイグレーションで、**中身は読んで対処済み**（DB から読んだ識別子を SQL に混ぜる前に検査する）
- [x] 返済枠を消化し（0.1-0.6・0.8）、**0.7 は Phase 5 に寄せた**。IT13 へ送る分は申し送りに記載済み
