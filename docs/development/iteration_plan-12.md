...
| 項目 | 計画 | 実績 |
| :--- | :--- | :--- |
| SP | 11 | **11**（TS18 3 + US19 8。縮退 0 件） |
| テスト件数 | — | **886**（IT11 終了時 872 から +14）+ E2E 3 本 |
| 縮退 | — | **0 件** |
| 品質ゲート | 5 検査 | すべて緑（`arch-lint` は規約 13 件・メタテスト 36 件へ） |
| CI | 緑 | **緑**（E2E ジョブを含む） |
...
| # | 内容 |
| :--- | :--- |
| 1 | **例外の誤登録の取り消し（2.5）は実装していない**。代わりに TS18-1 で「引取済への荷役を拒む」を入れ、**壊れる経路そのものを塞いだ**。取り消しは US20（将来リリース）と同じ土俵で設計するのが妥当と判断した |
| 2 | **対応の記録と解決を別の操作に分けた**。計画は 1 つの「対応内容の入力」として書いていたが、**到着予定が決まっても遅延が解消したとは限らない**——現場では別の操作である |
| 3 | **`ExceptionType` は `DELAY` のみ実装した**。設計は 4 値だが、US20（DAMAGE / LOST / CUSTOMS_HOLD）は将来リリースであり、**選べるのに動かない選択肢を出さない** |
| 4 | **規約 13 を実装した**（タスク 3.1）。IT11 レビュー M15。4 本目で実際に赤くなることを実測した |
| 5 | **間欠失敗が再現した**（1 度）。ただし診断は明確で「アプリ以外が応答しました / 本文: Proxy key is incorrect」と出た——**Try T6 の狙いは達成している**。IPv4 固定だけでは解消しておらず、環境側の要因が残る |
| 6 | **ローカルの E2E 実行環境（タスク 0.3）は未修復**。CI では緑であり、シナリオを 3 本へ増やした |
