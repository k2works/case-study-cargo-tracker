...
- [ ] US30 の受入基準（**受入基準ごとに「その道を実行したテスト」を名指しする**）
- [ ] デモ項目 18 件が動作する
- [ ] **先行して直す穴 3 件（X1 / X2 / X3）を直した**
- [ ] **返済枠を返した**（落としたものは名前とリリース後の起票先を記録した）

#### 精算局面の完了条件（`development_strategy.md`）

- [ ] 精算までの業務シナリオが通しで動作する（予約 → 追跡 → 引取 → 料金算出 → 請求 → **入金確認**）
- [ ] **丸め後の金額が永続化され、税率を変えても発行済み請求書の金額が変わらない**
- [ ] ArchUnit の全ルールと ADR-015 の検査が緑（**Billing と Booking の新テーブルを所有表に登録済み**）
...
- [ ] **T1: 新設した ACL ポートの戻り値を呼び出し側が使っている**（状態の伝播はイベントにした）
- [ ] **T4: 「実装しなかった守り」を数え、主張が到達可能かまで確かめた**
- [ ] **T2: 組み合わせを数え上げた**
- [ ] **T5: 「キャンセル」と「取り消し」の語が混ざっていない**（イベント名を含めて grep で確認）
- [ ] **T6: タスク 3 完了時点でレビューを 1 度回した**
- [ ] **破壊検証を行い、テストが赤になったかで判定した**
...
- [ ] 追跡管理者がダッシュボードから**承認待ち一覧**へ到達できる（navbar とカードの両方）
- [ ] **追跡管理者が承認画面を開き、承認・却下の POST が通る**（X2。403 にならない）
- [ ] **承認・却下の後、承認待ち一覧へ戻れる**（予約詳細で行き止まりにしない）
- [ ] **輸送中の予約から申請を開けること**を確認した（状態軸の到達性）
- [ ] **輸送開始前の予約からは申請ではなく即時キャンセルが開けること**を確認した
- [ ] **`DELIVERED` 以降ではどちらも開けないこと**を確認した
- [ ] **入口があるロールに「利用できる機能はありません」と併記されない**

#### ドキュメント

- [ ] 設計への反映を完了（15 件）
- [ ] **ADR を 3 件起票**（料率 / 状態伝播をイベントへ / 請求書の種別）
- [ ] **マニュアルに輸送中キャンセルの申請と承認を追記**＋**キャプチャを再生成**
- [ ] 用語集・索引を同期し、`mkdocs build` の新規警告が 0
- [ ] **Release 2.0 の完了報告書を作成した**
