...
- [x] US36 の受入基準 4 件・US37 の受入基準 8 件が、E2E / API / ドメインのテストで 1:1 に固定されている（[レビュー](../review/イテレーション15_review_20260901.md)の対応表）。
- [x] 乱数の種を固定した再現テストが緑である（**同じ種 → 同じシナリオ列**）。並行して取り出しても変わらないことも固定した。
- [x] 実行間隔・同時実行数・例外比率の上限を超えないことを、破るテストで固定している。**間隔は当初効いておらず、レビューで指摘されて実装した**。
- [x] 継続実行中もヘルスチェックが劣化しないことを、負荷をかけた状態で確認している（実行のスレッドを上限まで埋めた状態で liveness / readiness が通る。実環境でも再起動 0）。
- [x] IT14 のふりかえり Try を計画と DoD に反映している。
- [x] 画面を伴うため、ユーザーマニュアルと画面キャプチャを更新する。
- [x] テストカバレッジ 80% 以上、ドメイン層 90% 以上（SonarQube 新規コード 89.8%）。
...
- [x] 例外シナリオ（遅延・破損・誤配・税関保留・輸送中キャンセル）を選んで実行できる
- [x] 例外が記録され、対応（例外解決・経路の組み直し・キャンセル承認）まで含めて実行される
- [x] 誤配シナリオでは、現在地からの経路が組み直されて輸送が再開する（**組み直し用の航海の登録が要ることは実環境で分かった**）
- [x] 例外シナリオの結果も、US35 と同じ形式で工程ごとに確認できる（**分母が選択中シナリオ固定だったのをレビューで指摘され修正**）
...
- [x] バックエンドが乱数でシナリオ・出発地・目的地・貨物種別・重量・期限を選び、一定間隔で実行し続ける（**選んだ入力が業務 API まで届いていなかったのを自己レビューで発見。間隔もレビュー指摘で実装**）
- [x] 実行間隔・同時実行数・例外の発生比率を設定でき、設定した上限を超えて実行しない（**画面から設定できなかったのをレビューで指摘され修正**）
- [x] 使った乱数の種（seed）が記録され、同じ種を指定すると同じ並びを再現できる（**停止後に種を読む手段は IT16**）
- [x] 管理者が開始・停止でき、停止すると進行中の実行は最後まで終えてから止まる
- [x] 生成されたデータは、US34 と同じ印でシミュレーション由来と識別できる（**キャンセル承認待ち・荷役・通関の一覧はまだ混ざる。IT16**）
- [x] 本番環境では起動しない
- [x] 実行中も**ヘルスチェックと実利用者の操作は劣化しない**（**ただし実利用者の一覧への混入は 3 か所残る。IT16**）
- [x] 実行の統計（実行件数・成功／失敗の内訳・失敗した工程の分布）が US35 と同じ形式で確認できる（**分布から対象へ辿れなかったのをレビューで指摘され修正**）
...
| 0.9 | 実行詳細画面で**識別子の種別を表示し、コピーできる形にする**（管理者が営業に番号を伝えるため） | 報告書 課題 5.5 | 3h | [x] |
...
| 1.2 | US37 の 8 受入基準を E2E とコンポーネントテストに翻訳し、赤を確認する | 8h | [x] |
...
| # | タスク | 見積 | 状態 |
| :--- | :--- | :--- | :--- |
| 5.1 | 実行件数・成功／失敗の内訳・失敗した工程の分布を集計する | 5h | [x] |
| 5.2 | 継続実行の開始・停止・状態と統計を `/admin/simulations` に出す | 6h | [x] |
| 5.3 | 種の表示と、種を指定した再実行の導線を作る | 4h | [x] |
...
| 6.5 | **最後のコミットに対して** SonarQube を回す。**違反 0・カバレッジ 89.8%・重複 0.04%**。Quality Gate は **Hotspot のレビュー率だけで FAIL**（承認に `SONAR_ADMIN_TOKEN` が要る。IT14 からの引き継ぎ） | 2h | **一部** |
| 6.6 | **git タグと `CHANGELOG.md` を作る**（`developing-release`。**5 リリース連続の欠落を解消する**） | 5h | [x] |

### 見積もり合計

| カテゴリ | SP | 理想時間 | 状態 |
| :--- | :--- | :--- | :--- |
| Phase 0: IT14 から送った負債の返済枠 | 0 | 31h | [x] |
| Phase 1: 受け入れテストと ADR | 1 | 20h | [x] |
| Phase 2: 例外シナリオ | 2 | 23h | [x] |
| Phase 3: ランダム実行 | 2 | 23h | [x] |
| Phase 4: 業務を止めない | 1 | 16h | [x] |
| Phase 5: 統計と画面 | 2 | 15h | [x] |
| Phase 6: 設計・マニュアル・品質ゲート・リリース | 0 | 31h | **一部**（SonarQube の Hotspot 承認のみ未達） |
| **合計** | **8** | **159h** | |
...
- [x] US36 / US37 の受け入れテストが Red → Green で実装されている。
- [x] 同じ種で同じ並びが再現されることを、並行実行下のテストで固定している。
- [x] 実行間隔・同時実行数・例外比率の上限を、**破るテスト**で固定している。
- [x] ヘルスチェックがセマフォの対象外であることを検査で固定している。
- [x] 例外の注入に専用 API を使っていないことを、実際の操作を踏むテストで固定している。
- [x] `domain-model.md`、`data-model.md`、`ui_design.md`、ユーザーマニュアルを更新している。
- [x] マニュアル用キャプチャを撮り直し、目視で UI 欠陥がないことを確認している。
- [x] **push 前に `--tests` で絞らないフルビルド**（`TZ=UTC ./gradlew build`）を回している（Try 1）。
- [ ] API の要求に項目を足した変更では、**その項目を送らない要求のテスト**を同じ変更の中で足している（Try 2）。**守れなかった**——契約に simulated を基本型で足し、IT14 と同じ形で既存の応答が読めなくなった（契約テストが捕まえた）。
- [x] ADR のコンプライアンス表に書いた検査は、**書いた直後に対象を壊して赤を確かめている**（Try 5）。決定 6 件すべてで確認。
- [x] 接続先の取り違えの検査は、**3 本目を作らず `ServiceEndpointConfigTest` / `ServiceDatabaseUrlTest` に足している**（Try 4）。
- [x] ADR-031 に「**この決定の前提の外にあるもの**」を書いている（Try 6）。
- [x] IT14 から送った負債 7 件を、返済または**スコープ外として理由つきで明記**している（全件返済）。
- [x] `./gradlew build`、`TZ=UTC ./gradlew test`、frontend test / build、E2E（101 件）、JIG / jig-erd が完了している。
- [ ] **最後のコミットに対して** SonarQube を回し、Quality Gate が PASS である。**一部**——違反 0・カバレッジ 89.8%・重複 0.04% だが、Hotspot のレビュー率が 33%（承認に `SONAR_ADMIN_TOKEN` が要る。IT14 からの引き継ぎ）。
- [ ] 5 視点レビューを実施し、高優先度をすべて対応または明示的に送っている。**一部**——独立したレビューは利用者代表 1 つだけ（他 4 体が無応答）。高 10 件のうち 9 件対応・1 件は IT16 へ明示的に送った。
- [x] **git タグ（`java/take-7/v2.2.0`）と `CHANGELOG.md` を作成している**（5 リリース連続の欠落を解消）。原因は `release.js` が `gulpfile.js` に読み込まれていなかったこと。
