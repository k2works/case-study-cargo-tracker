...
- [x] US26・US27・US31・US02 の受入基準（`docs/requirements/user_story.md` の該当節）をすべて満たす
...
| # | タスク | 見積もり | 状態 |
|---|--------|---------|------|
| 6.1 | `docs/manual/index.md`（構成・執筆テンプレート・キャプチャ方針）を作成 | 2h | [x] |
| 6.2 | 「ログインする」「ログアウトする」「ロックされたとき」の節を執筆（読者は業務担当者） | 2h | [x] |
| 6.3 | 「荷主を登録する」の節を執筆（メールアドレス重複時の選択を含む） | 2h | [x] |
| 6.4 | 画面キャプチャの自動生成を Playwright に配線（出力先 `docs/manual/assets/`）し、上記 4 節分を生成 | 3h | [x] |
...
| カテゴリ | SP | 理想時間 | 状態 |
|---------|----|----|------|
| US26 ログイン（+ロール確定・ポータル・共通レイアウト） | 3 | 22h | [x] |
| US27 ログアウト | 1 | 4h | [x] |
| US31 アカウント保護 | 2 | 9h | [x] |
| US02 荷主登録 | 3 | 15h | [x] |
| 品質ゲート実配線（SP 対象外） | - | 15h | [x] |
| ユーザーマニュアル初版（SP 対象外） | - | 9h | [x] |
| **合計** | **9** | **74h** | |
...
- [x] 対象 4 ストーリーの受入基準（`docs/requirements/user_story.md` の該当節）をすべて満たす
- [x] `./gradlew build` が緑（ユニット・統合・ArchUnit・カバレッジ検証を含む。静的解析は `ignoreFailures` のため判定に使わない — 警告として扱う）
- [x] `TZ=UTC ./gradlew test` が緑
- [x] フロントエンドのテスト・ビルド・E2E スモークがパス
