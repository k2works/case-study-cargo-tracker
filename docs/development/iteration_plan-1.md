...
- [x] 追跡番号を指定して貨物の状態・現在地・イベント履歴が HTML で表示される
- [x] 存在しない追跡番号でエラーメッセージが表示される（HTTP 200 で画面内表示）
- [x] H2（PostgreSQL 互換モード）に対する統合テストが通る
- [x] JDK `HttpClient` による HTTP 統合テストが通る
- [x] `npm run dev:verify` が全件成功する（22 件）
- [x] **検証記録**（下記）を測定した
...
| # | タスク | 見積もり | 担当 | 状態 |
|---|--------|:---:|------|:---:|
| 1.1 | `flix.toml` に Flyway・HikariCP を追加し、依存解決を確認する | 1h | - | [x] |
| 1.2 | `V1__init.sql` を作成（`location` / `tracking_activity` / `tracking_handling_event`） | 2h | - | [x] |
| 1.3 | `Migration.flix`（Java 相互運用で `Flyway.migrate()` を呼ぶ）を実装する | 2h | - | [x] |
| 1.4 | `R__seed_dev.sql`（開発シード）を作成する | 1h | - | [x] |
| 1.5 | H2 でマイグレーションが適用されることをテストする | 2h | - | [x] |
...
| # | タスク | 見積もり | 担当 | 状態 |
|---|--------|:---:|------|:---:|
| 2.1 | 【RED】HTTP 統合テストを書く（JDK `HttpClient` で `/public/tracking/{n}` を叩き 200 と本文を検証） | 3h | - | [x] |
| 2.2 | `Request` / `Response` / `HttpMethod` の ADT を定義する | 2h | - | [x] |
| 2.3 | `Router`（`Route` ADT・パスパターン照合・`RequiredRole` を含む）を実装する | 4h | - | [x] |
| 2.4 | `HttpServer`（JDK 内蔵）の起動と Executor 構成（仮想スレッド・同時実行上限）を実装する | 3h | - | [x] |
| 2.5 | `Html` ADT と `render`（エスケープ既定）を実装する | 3h | - | [x] |
| 2.6 | `Tracking.Pages.publicShow`（Read Model を受け取り `Html` を返す純粋関数）を実装する | 3h | - | [x] |
| 2.7 | 画面関数の単体テスト（表示項目・エスケープ・未検出時の文言） | 2h | - | [x] |
| 2.8 | `eff ReadDb` を宣言し、`TrackingQuery.findByTrackingNumber` を実装する | 2h | - | [x] |
| 2.9 | インメモリ `ReadDb` ハンドラでクエリサービスの単体テストを書く | 2h | - | [x] |
| 2.10 | `eff Tx`（`connection` / `afterCommit`）と `transactional`（読み取り専用対応）を実装する | 4h | - | [x] |
| 2.11 | `JdbcReadDb` ハンドラと `TrackingRow` デコーダ（`ResultSet` → Read Model）を実装する | 4h | - | [x] |
| 2.12 | H2 に対する永続化統合テスト（JOIN を含む読み取りクエリ） | 3h | - | [x] |
| 2.13 | 合成ルート（`Composition.flix`）でハンドラを結線し、`Main` から起動する | 2h | - | [x] |
| 2.14 | 【GREEN】HTTP 統合テストを通す | 2h | - | [x] |
| 2.15 | 【REFACTOR】重複除去・命名整理。**検証記録の測定**（効果数・行数・ビルド時間） | 3h | - | [x] |
...
| # | タスク | 見積もり | 担当 | 状態 |
|---|--------|:---:|------|:---:|
| 3.1 | ADR-0001: Flix 採用とバージョン固定方針 | 1h | - | [x] |
| 3.2 | ADR-0002: Web・セキュリティ基盤の自作とその補償策 | 1h | - | [x] |
| 3.3 | `business_rule_traceability.md` の雛形作成（[テスト戦略](../design/test_strategy.md) 6.1） | 1h | - | [x] |
...
| カテゴリ | SP | 理想時間 | 状態 |
|---------|:--:|:---:|:---:|
| TS06 Flyway マイグレーション基盤 | 3 | 8h | [x] |
| TS01 ウォーキングスケルトン | 13 | 42h | [x] |
| 設計レビュー残作業（返済枠） | 0 | 3h | [x] |
| **合計** | **16** | **53h** | |
...
アーキテクチャの仮定が実装として成立するかを実測した。

### 効果によるポート注入

| 測定項目 | 実測値 | 評価 |
|---------|-------|------|
| 効果宣言の数 | 2（`Tx`・`ReadDb`） | ポート単位に保てている |
| 効果を要求する `pub` 関数 | 17 個中 15 個が `\ IO` のみ | **効果の伝播は想定より穏やか** |
| 最も複雑なシグネチャ | `readOnly(ds, f: Unit -> a \ ef + Tx): a \ (ef - Tx) + IO` | 効果の加減算が読みにくいのはハンドラ定義のみ。利用側には現れない |
| アプリケーション層のシグネチャ | `findByTrackingNumber(n: String): Option[TrackingDetailView] \ ReadDb` | 必要な副作用が 1 つだけ現れ、意図が明快 |

**判定: 破綻の兆候なし。** ただし効果が 2 つの段階での測定であり、
書き込み系（`CargoRepo`・`EventBus`・`Clock`）が加わる IT4 で再測定する。

### 効果ハンドラによるテストダブル

`InMemoryHandlers.withInMemoryReadDb` を 20 行で実装し、**DB もサーバも起動せずに**
アプリケーション層を検証できた（`TrackingQueryTest` の 3 件）。
本番の JDBC ハンドラと同じ効果を実装するため、アプリケーション層のコードは一切変わらない。

**判定: 仮定どおり成立。** モックライブラリ不要という設計上の主張が実証された。

### `Tx` 効果によるトランザクション境界

`readOnly` がコネクションを 1 本借り、`Tx.connection()` 経由でのみ JDBC ハンドラへ供給する構造が成立した。
`transactional` の外でリポジトリ効果を使うとコンパイルエラーになることも確認済み。

**判定: 成立。** 書き込み + `afterCommit` の検証は IT4・IT8 で行う。

### `Html` DSL の記述量

| 対象 | 行数 |
|------|-----|
| `Html.flix`（DSL 本体） | 89 行 |
| `TrackingPublicPages.flix`（1 画面 + ステータス定義） | 121 行 |
| src 合計 | 1,076 行 |
| test 合計 | 370 行 |

1 画面あたり約 120 行はテンプレートより多いが、**ステータスラベル・バッジ定義（約 30 行）を含む**。
純粋関数のため単体テストが容易で、XSS 対策が既定で効く利点と釣り合う。

**判定: 許容範囲。** ただしフォーム・レイアウトを含む画面は IT2 で再評価する。

### ビルド時間

| 操作 | 所要時間 |
|------|---------|
| `flix build`（キャッシュあり） | 約 18 秒 |
| `flix test`（22 件） | 約 6.5 秒 |

**判定: TDD サイクルには支障ないが速くはない。** 編集中は `dev:check`（型検査のみ）を使う。

## 実装中に判明した Flix の制約

| # | 制約 | 対処 | 影響 |
|---|------|------|------|
| 1 | 同名トップレベルモジュールを複数ファイルで宣言できない | モジュール名は階層をドットではなく**接頭辞**で表現（`SharedDbPool`） | 設計のモジュール構成表を要更新 |
| 2 | ドット区切りの `mod A.B.C` は参照解決できない | 入れ子 `mod` か接頭辞命名を使う | 同上 |
| 3 | 型エイリアスがモジュール境界を越えて公開されない | `enum` で包む | 実装規約に追加 |
| 4 | `resources/` は fatjar には入るが `flix test` の classpath には載らない | Flyway の location に `classpath:` と `filesystem:` の両方を指定 | 実装済み |
| 5 | テストランナーが stderr 出力を失敗として扱う | `slf4j-nop` でライブラリのログを抑止 | 依存 1 つ追加 |
| 6 | Flyway 10 に H2 用モジュールが存在しない | 9.22.3 を採用 | 技術スタック更新済み |
...
### 設計判断: 業務データのシードをマイグレーションに置かない

開発用の追跡サンプルを Flyway の Repeatable マイグレーション（`R__seed_dev_tracking.sql`）として
追加したところ、**テストが自前で用意するデータと一意制約で衝突し、7 件が失敗した**。

マイグレーションはテストでも本番と同じものを適用するため、業務データを含めると
テストのデータ前提と干渉する。したがって以下を規約とする。

| 種別 | 配置 | 理由 |
| :--- | :--- | :--- |
| マスタデータ（`location`） | Flyway の Repeatable マイグレーション | 全環境で必要。テストの JOIN 検証にも使う |
| 業務データ（追跡サンプル等） | アプリ起動時（開発環境のみ） | テストが自前で用意するデータと衝突するため |
...
|------|---------|--------|
| 2026-07-31 | 初版作成 | - |
| 2026-08-14 | IT1 完了。検証記録と Flix の制約を追記 | - |
