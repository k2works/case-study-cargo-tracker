...
| 1.5 | 航海スケジュール登録画面（`/routing/voyages/new`）。寄港地を順序付きで追加・削除できる。**未入力箇所を明示する**（ネイティブ検証に任せず自前のメッセージ。IT2 Try 3） | 5h | [x] |
| **小計** | | **24h** | |
...
| 2.3 | 登録画面に差分確認の段を足す（`/routing/voyages/new` で既存の航海番号 → 差分 → 上書き）。**更新前と更新後を並べて見せる**（何が変わるか分からないまま押させない）。一覧の [更新] は航海番号を引き継いでこの画面へ | 5h | [x] |
| **小計** | | **12h** | |
...
| # | タスク | 見積 | 状態 |
| :--- | :--- | :--- | :--- |
| 3.1 | `Voyage.supports(CargoType)` / `connects(origin, destination)` を単体テストで。**危険物・冷凍は対応可能な航海だけが残る**ことを、判定を壊すと赤になる形で | 4h | [x] |
| 3.2 | 検索クエリ（出発地・目的地・出発期間・貨物種別）。**件数の上限を必ず置き、切ったことを黙らない**（IT2 と同じ形） | 4h | [x] |
| 3.3 | `GET /api/v1/voyages` + MockMvc テスト。**出発地・目的地は UN/LOCODE で指定できる** | 3h | [x] |
| 3.4 | 航海スケジュール一覧・検索画面（`/routing/voyages`）。**条件を満たす航海が無いとき、条件を緩めて再検索できる**ことを示す。あわせて `ui_design.md` に画面を定義し（**salt のワイヤーフレーム本体と仕様の記述を同時に直す**。仕様だけ直して図が旧内容のまま残るドリフトを作らない）、「take-7 で定義済みの画面」表に追加（設計反映 #3） | 6h | [x] |
| **小計** | | **17h** | |
...
| 4.2 | 予約詳細画面（`/booking/:bookingId`）+ `GET /api/v1/bookings/{bookingId}`。出発地・目的地・期限・貨物仕様を確認できる（設計反映 #4） | 5h | [x] |
| 4.3 | 「経路設計を依頼する」（`POST /api/v1/bookings/{bookingId}/routing-request`）。**営業担当者のみ**。依頼済みの予約に再依頼できないこと | 3h | [x] |
| 4.4 | 経路設計者の**気づく手段**: ダッシュボードに「経路設計待ち N 件」を出し、**そこから絞り込み済みの一覧へ行ける**ようにする。件数だけ出して終わりにしない | 4h | [x] |
| **小計** | | **16h** | |
...
| # | タスク | 見積 | 状態 |
| :--- | :--- | :--- | :--- |
| 5.1 | 「05 航海スケジュール」章を新設（一覧・検索・登録・更新）。**04 章の様式に揃える**（項目表・操作手順・よくある入力の誤り・注意） | 4h | [x] |
| 5.2 | 「04 貨物予約」に予約詳細と経路設計依頼の節を追加。「01 業務フロー」の工程 4・6 の対応表を更新 | 2h | [x] |
| 5.3 | 画面キャプチャを生成 spec 経由で再生成し、`manual:build` で HTML を作って目視 | 2h | [x] |
| **小計** | | **8h** | |
...
- [x] US24・US25・US07・US06 の受入基準のうち、**IT3 スコープ内のもの**をすべて満たす（スコープ外は上表のとおり）
- [x] `./gradlew build` が緑（ユニット・統合・ArchUnit・カバレッジ検証）
- [x] `TZ=UTC ./gradlew test` が緑
- [x] フロントエンドの lint・テスト・ビルド・E2E が緑
- [x] **本番相当ビルドの検査**（`test:e2e:production`）が緑
- [x] CI が緑（全ジョブ success）
- [x] SonarQube Quality Gate が **PASS**（両プロジェクト。Bug 0 / Vulnerability 0 / 重複 3% 未満 / 新規カバレッジ 80% 以上）
- [x] **追加した検査を壊して赤になることを確認済み**（IT2 Try 14）
- [x] **判定を本番と検査で共有している**（テスト側に判定を書き直していない。IT2 Try 13）
- [x] **復元経路で不変条件を検査していない**（値オブジェクトに `restore` がある。IT2 Try 16）
- [x] **新しい Mapper について方言スモークが通っている**
- [x] **設計に書かれていない判断を ADR に起こし、同じ変更で検査に落とした**（IT2 Try 15。落とせないものは本文に「未実装」と書く）
- [x] **「設計への反映が必要な点」6 件をすべて反映した**（表は 6 件。見出しの「5 件」は誤記だった）
- [x] 画面を追加した US について、`ui_design.md` のナビゲーション表・サイドバー実装・ダッシュボード導線・到達性テストの **4 点一致**
- [x] **ナビゲーションの `available` を true にした**（航海スケジュール）
- [x] **業務フロー章の対応表を更新した**
- [x] ユーザーマニュアルの該当章を執筆し、**キャプチャを再生成し `manual:build` で HTML を作って目視した**
- [x] kind 統合環境で Gateway 経由の動作確認済み（**bookingms と routingms の 2 サービスを通す**）
- [x] 開発環境（Heroku）へデプロイし、`deploy:dev:health` の全 URL が 200。**加えてその環境で業務を 1 本通した**（200 は「動く」ではない。IT2 Try 11）
- [x] ドキュメント更新完了（release_plan の進捗・JIG / jig-erd 再生成）

## 実施記録（開発完了時点）

| 確認 | 結果 |
| :--- | :--- |
| `./gradlew build`（ユニット・統合・ArchUnit・カバレッジ） | 緑 |
| `TZ=UTC ./gradlew test` | 緑 |
| フロントエンド lint / test（156 件）/ build | 緑 |
| E2E（26 件。IT3 分 7 件を追加） | 緑 |
| 本番相当ビルドの検査 | 緑 |
| SonarQube Quality Gate | 両プロジェクト PASS（backend 新規カバレッジ 91.3 / frontend 83.5、違反 0） |
| kind 統合環境（Gateway 経由・bookingms + routingms） | 業務 1 本通過。実バックエンド E2E 3 件も緑 |
| 開発環境（Heroku） | 3 サービスをデプロイし、業務 1 本通過（航海登録 → 種別で絞り込み → 引き渡し → 再依頼 409 → 経路設計者の一覧） |

### 実環境でだけ見つかった欠陥（2 件）

| 欠陥 | 現れ方 | 対応 |
| :--- | :--- | :--- |
| 更新のはずの保存が新しい予約を作る | 引き渡すと元の予約は「未依頼」のまま残り、依頼済みの別番号が増える。単体は偽の保存先、統合は新規保存しか通しておらず、どちらも判別しなかった | `save()` を id の有無で分岐。既にある予約を保存し直すと行が増えないことを統合テストで固定（先に赤を確認） |
| 権限の検査より入力の検査が先に走る | 権限の無い呼び出しでも本文が不正なら 400。本人には「入力を直せ」と伝わり、権限が無いはずの相手に入力仕様を教える | 認可のあとに明示的に検証する形へ。bookingms・routingms の両方 |

> **どちらもモックと単体では緑だった。** 「実物を 1 本通すまで動くと言わない」（IT1 Try 8）が、
> IT3 でも同じ形で効いた。

### クローズ時のレビューで見つかった欠陥（2026-08-21）

開発フェーズの終了時点では気づけず、[マルチパースペクティブレビュー](../review/イテレーション3_review_20260821.md)で
見つけたものです。高優先度 11 件のうち 10 件を本 IT で対応しました。

| 欠陥 | 現れ方 |
| :--- | :--- |
| 時刻だけの変更が差分に出ず、**画面から更新できない** | 差分が先頭区間の出発日時しか比べていなかった。実務でいちばん多い「本船が遅れて到着がずれた」で「変更はありません」と出て終わる。US25 の中核シナリオが全緑のまま動いていなかった |
| 出発日の絞り込みが実バックエンドに通らない | 画面は日付、サーバは日時で受け取る。モックが文字列の前方比較で通していたため単体も E2E も緑 |
| 「認可より入力検証が先」の回帰テストが、欠陥の起きた bookingms に無い | 順序を戻しても全緑（実測）。直した場所と守られている場所がずれていた |
| 隣接する 2 つの時刻境界が、包含を反転しても緑 | テストデータが常に「厳密に前 / 厳密に後」しか使っていなかった |
| ADR-014 の決定 2（テーブルの形）が検査されていない | 列だけ足して種データを変えない `ALTER` が素通りする |

> **実環境でだけ見つかる欠陥は、この IT で合計 3 件になった。** kind で 2 件、
> クローズのレビュー中に 1 件。3 件目は「モックが本物より甘い」形で、
> IT2 の Try 5 が再発している。
...
| 2026-08-21 | クローズ時のレビュー結果と、高優先度 10 件の対応を追記 |
