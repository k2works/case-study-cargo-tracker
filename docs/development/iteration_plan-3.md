...
- [ ] US09: 経路候補を選択して CargoItinerary を割り当てられる API が動作する（bookingms）
- [ ] US09: 経路設計画面で候補選択・確定操作ができる
- [ ] US11: 確定経路を予約に紐付けられる API が動作する
- [ ] US11: 紐付け後、予約状態が `ROUTE_PROPOSED` に更新される
- [ ] US13: 予約確定 API が動作する（bookingms）
- [ ] US13: 予約確定画面で確定操作ができる
...
- [ ] コードレビュー完了（AI ペアレビュー）
- [ ] ユニットテストがパス（routingms + bookingms）
- [ ] ArchUnit テストがパス
- [ ] E2E テストがパス（予約→経路→確定の一連フロー）
