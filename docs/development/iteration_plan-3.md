...
- [x] US08・US09 の全受入基準を満たす（受入基準をテストケースに 1:1 対応させて実証）
- [x] `RouteCandidateCalculator` の単体テストが Red-Green-Refactor で作成されている（直行・単純接続・多段接続・期限超過・貨物種別不適合を網羅）
- [x] 経路設計・割り当て画面の HTTP フローテストが green（算出・0 件時・選択確定）
- [x] US07 の出発期間検索・予約番号からの貨物仕様確認を実装しテストで実証（IT2 繰り越し）
- [x] `VoyageRepository::search` を SQL WHERE 絞り込みへ改善し、N+1 を解消（IT2 レビュー中 #7）
- [x] CI に `cargo llvm-cov` カバレッジ計測・`cargo sqlx prepare --check` を組み込む（IT2 Try #4）
- [x] `cargo clippy --workspace -- -D warnings` と `cargo fmt --check` が全 green・ドメイン層カバレッジ 85% 以上
...
| # | タスク | 見積もり | 担当 | 状態 |
|---|--------|---------|------|------|
| 0.1 | `VoyageRepository::search` を SQL WHERE 絞り込みへ改善（全件ロード + Rust フィルタを廃止）・統合テストで検証 | 3h | - | [x] |
| 0.2 | CI に `cargo llvm-cov`（カバレッジゲート）と `cargo sqlx prepare --check` を組み込む | 3h | - | [x] |
| 0.3 | 受入基準 → テストケースの 1:1 対応表を US08/US09 で作成してからタスク着手（Try #1） | 1h | - | [x] |
...
| # | タスク | 見積もり | 担当 | 状態 |
|---|--------|---------|------|------|
| 1.1 | `RouteLeg`・`RouteCandidate` 値オブジェクト（Routing 固有・経由港・所要日数・航海番号列・到着予定・期限充足フラグ）の単体テスト → 実装 | 3h | - | [x] |
| 1.2 | `RouteCandidateCalculator`（直行便探索）の単体テスト → 実装 | 3h | - | [x] |
| 1.3 | `RouteCandidateCalculator`（単純接続 1 経由・寄港地接続評価）の単体テスト → 実装 | 4h | - | [x] |
| 1.4 | `RouteCandidateCalculator`（多段接続・貨物種別対応絞り込み・推奨順ソート・期限超過警告）の単体テスト → 実装 | 4h | - | [x] |
| 1.5 | US07 繰り越し: `VoyageRepository::search` に出発期間条件を追加し、予約番号から貨物仕様（出発地・目的地・期限・貨物種別）を取得する ACL/クエリを実装 | 3h | - | [x] |
| 1.6 | app-routing に経路候補算出ユースケース（予約番号 → 貨物仕様 → Voyage 検索 → 候補算出）を実装・単体テスト | 3h | - | [x] |
| 1.7 | interface-web に経路設計・割り当て画面（`/bookings/{bookingId}/route`）ステップ 1-2（貨物仕様表示・航海検索・候補算出）＋ HTTP フローテスト | 4h | - | [x] |
| 1.8 | ナビゲーション整合: 予約詳細（`/bookings/{id}`）から `/bookings/{id}/route` への導線＋検証テスト | 2h | - | [x] |
...
| # | タスク | 見積もり | 担当 | 状態 |
|---|--------|---------|------|------|
| 2.1 | 確定経路（`SelectedRoute` / 経路確定状態）のドメイン表現と選択・確定ロジックの単体テスト → 実装 | 3h | - | [x] |
| 2.2 | app-routing に経路確定ユースケース（候補選択 → 確定・永続化）を実装・単体テスト | 3h | - | [x] |
| 2.3 | 経路設計・割り当て画面ステップ 3（候補ラジオ選択・htmx 部分更新・[確定] ボタン）＋ HTTP フローテスト（確定・0 件時の条件調整導線） | 4h | - | [x] |
...
| カテゴリ | SP | 理想時間 | 状態 |
|---------|----|----|------|
| IT2 Try 返済枠（SP 外） | - | 7h | [x] |
| 経路探索ドメイン・算出（US08） | 8 | 26h | [x] |
| 経路選択・確定（US09） | 3 | 10h | [x] |
| **合計** | **11** | **43h** | |
...
IT2 ふりかえり Try #1 に基づき、受入基準を実装前にテストケースへ 1:1 対応させる。各基準を「どの層のどのテストで実証するか」を明示し、実装漏れ・テスト漏れを防ぐ。

### US08: 経路候補を算出する

| # | 受入基準 | 実証テスト（層） |
|---|---------|-----------------|
| 1 | 検索結果と出発地・目的地・期限から経路候補が自動算出される | `route::直行便を最優先候補として算出する`（domain）/ `予約番号から貨物仕様と経路候補を算出する`（app）/ `経路設計画面は貨物仕様と経路候補を表示する`（web） |
| 2 | 寄港地の接続可能性が評価される | `route::単純接続の1経由経路を算出し寄港地を評価する`・`接続時刻が逆転する経路は候補にならない`（domain） |
| 3 | 候補ごとに所要日数・経由港・費用・航海番号が表示される | `route::直行便を…（transit_days/transit_ports/voyage_numbers）`（domain）/ web の route-candidates 表（費用は後続・リスク表参照） |
| 4 | 候補が推奨順に並ぶ | `route::直行便は経由便より推奨順で上位になる`（domain） |
| 5 | 直行便が最優先候補として提示される | `route::直行便は経由便より推奨順で上位になる`（domain）/ web の ★1 表示 |
| 6 | 期限内に到達可能な経路がない場合その旨が通知され条件調整が促される | `route::期限超過の候補も返るが期限内判定で区別できる`（domain）/ web の 0 件時 `route-empty` 警告 |
| （US07 統合）| 出発期間検索・予約番号からの貨物仕様確認 | `検索は出発期間で絞り込める`（infra）/ `経路設計画面は貨物仕様と経路候補を表示する`（web・cargo-spec） |

### US09: 経路を選択・確定する

| # | 受入基準 | 実証テスト（層） |
|---|---------|-----------------|
| 1 | 候補一覧（経由港・所要日数・費用・航海番号）を確認できる | `経路設計画面は貨物仕様と経路候補を表示する`（web） |
| 2 | 最適な候補を 1 件選択できる | web の route-form ラジオ選択 / `経路を選択して確定すると保存される`（app） |
| 3 | 選択後、経路状態が「確定」になる | `確定経路を保存して存在確認できる`（infra・status=SELECTED）/ `経路を選択して確定すると予約詳細へリダイレクトする`（web） |
| 4 | 最適な候補がない場合、経路条件調整（US10）に進める | web の 0 件時警告に条件調整導線（US10 の再算出は IT4） |
| （異常系）| 範囲外の候補選択は拒否される | `範囲外の候補選択はinvalidcandidateになる`（app）/ route_confirm の 422 |
