...
| # | タスク | 見積もり | 担当 | 状態 |
|---|--------|---------|------|------|
| 1.1 | 統合テストのログイン共通ヘルパー化とセッション確立アサート追加（retry 依存解消） | 4h | - | [ ] |
| 1.2 | dependency-cruiser の no-cross-context 例外を ACL 実態に合わせて厳格化 | 2h | - | [ ] |
| 1.3 | Routing Context の `ValidationError` パターンと日付単位比較ヘルパーの設計確認 | 2h | - | [ ] |
...
| # | タスク | 見積もり | 担当 | 状態 |
|---|--------|---------|------|------|
| 2.1 | `Voyage` 集約・`VoyageNumber`・`Schedule`・`CarrierMovement` の単体テスト（順序・接続・同一港禁止・日付整合） | 8h | - | [ ] |
...
| # | タスク | 見積もり | 担当 | 状態 |
|---|--------|---------|------|------|
| 3.1 | `/voyages` 一覧・検索フォーム、`/voyages/new` 登録フォーム、`/voyages/{voyageNumber}/edit` 更新フォーム TSX | 8h | - | [ ] |
| 3.2 | `VoyageController` の GET/POST/PRG と htmx 検索フラグメント統合テスト | 6h | - | [ ] |
| 3.3 | `VoyageQueryService` で出発地・目的地・期間・貨物種別による SQL 絞り込みを実装 | 6h | - | [ ] |
| 3.4 | 予約番号指定時に `ROUTING_IN_PROGRESS` 予約の条件を表示し、検索条件へ引き継ぐ | 4h | - | [ ] |
...
| カテゴリ | SP | 理想時間 | 状態 |
|---------|----|----------|------|
| IT2 Try 返済・基盤調整 | 0 | 8h | [ ] |
| Routing Context ドメイン・DB | 5 | 28h | [ ] |
| 航海スケジュール UI・検索 | 3 | 24h | [ ] |
| 経路候補算出 | 5 | 34h | [ ] |
| **合計** | **13** | **94h** | |
