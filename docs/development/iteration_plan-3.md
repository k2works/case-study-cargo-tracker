...
> IT3 終了時のふりかえり（2026-04-02 実施）

### Keep（続けること）

- **TDD サイクルの徹底**: Red → Green → Refactor のサイクルを守ることで、278 テスト全通過を維持したまま 4 ストーリーを追加できた
- **コードレビュー + UI/UX レビューの二重チェック**: コードレビュー（高 8 件）と UI/UX レビュー（高 6 件）を分けて実施することで、設計品質とユーザビリティの両面を網羅できた
- **ACL（TrackingLookupPort）パターン**: BC 間の依存をポートインターフェース + アダプターで分離するパターンは `ShipperExistencePort` と同様に機能し、テストの差し替えが容易だった

### Problem（やめること・改善すること）

- **ドメインイベント多重発行**: `Booking.register()` でテストフィクスチャを作成すると `BookingRegisteredEvent` が残留し、後続テストで `TooManyActualInvocations` が発生した。`reconstitute()` を使う規約を早期に確立すべきだった
- **コミット前のテスト確認**: `times(1)` への変更時に import が不足したまま push した。pre-commit フックに加えてローカルビルド確認を習慣化する
- **E2E アサーションと UI 表示の乖離**: `BookingStatus` の displayName 対応で E2E アサーションの一括更新が必要になった。UI 表示テキストは定数として E2E テストと共有する仕組みを検討する

### Try（次で試すこと）

- **テストフィクスチャポリシーの確立**: IT4 以降は「永続化後の再構成オブジェクトは `reconstitute()`、新規作成は `register()`」をルールとして iteration_plan に明記する
- **IT4 ベロシティ注意**: IT4 は 13 SP（平均 10.7 を超える）のため、US10/11/12/13 のタスク分解を細かく行い早期にリスクを検知する
- **追跡機能の拡充**: US09 で発行した追跡番号は現在 REST API のみ。IT4 の US13（追跡情報照会）で Web UI と連携し、detail.html の「追跡機能準備中」を解消する
...
|------|------|
| 完了日 | 2026-04-02 |
| 実績 SP | 12（計画 12、達成率 100%） |
| バックエンドテスト | 278 件全通過 |
| E2E テスト | IT3 受入テスト（E07〜E11）全通過 |
| コードレビュー | 高優先 8 件対応済み |
| UI/UX レビュー | 高優先 6 件対応済み |
| 主要コミット | `bee248b` US09 追跡番号発行、`0b5cb72` レビュー指摘対応 |
...
| 2026-04-02 | IT3 完了・KPT ふりかえりを追加（278 テスト Green、コードレビュー・UI/UX レビュー対応済み） | Copilot |
