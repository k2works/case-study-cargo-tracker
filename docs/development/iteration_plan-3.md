...
- [x] US26・US27 の受入基準がすべて満たされる
- [x] セキュリティ回帰テスト 7 項目（[テスト戦略](../design/test_strategy.md) 8.4）が自動テストとして存在する（タイムアウト・クロスセッション CSRF は IT4 で強化）
- [x] 認可の可否表（全ルート × 全ロール）がテストで固定されている（`decide` の網羅。ルート定義との照合は IT4）
- [~] スマートフォン幅で崩れずに表示される（Bootstrap 導入・HTTP レベルで確認。ブラウザ拡張が未接続のため実表示は未確認）
- [x] `arch-lint` に規約 10（`shared` は BC を参照しない）が追加され、違反 0 件
- [x] CI が緑（build / test / arch-lint / trace-lint の 4 段）
- [x] SonarQube の適用範囲を確定し設定を用意（**Flix は解析対象外**であることが判明。実行は要トークン）
...
| 5.4 | T6: `trace-lint`（US 番号とトレーサビリティ表の突合）を実装し CI へ載せる | 3h | [x] |
...
| ~~5.4 `trace-lint`~~ | ~~3h~~ | **IT3 で実施した**（認証が計画より早く収束したため） |
...
- [x] US26・US27 の受入基準がすべて満たされる
- [x] セキュリティ回帰テスト 7 項目が自動テストとして存在し、緑である
- [x] 認可の可否表（全ルート × 全ロール）がテストで固定されている（`decide` の網羅。ルート定義との照合は IT4）
- [x] `arch-lint` が規約 10 を含めて違反 0 件、メタテストが全件成功
- [x] `npm run dev:verify` が全件成功する
- [x] CI が緑
...
- [x] 設計ドキュメントへの反映（`sessions` テーブル・ロック用カラム・認可可否表・規約 10）が実装と同一コミットで行われている
- [x] ビジネスルール ⇄ テスト対応表が更新されている
...
| 2026-08-31 | IT3 の実装を完了。`trace-lint` は IT4 送りの予定だったが実施。E2E・Trivy・日次 PostgreSQL は計画どおり IT4 へ |
| 2026-09-11 | クローズ。レビュー指摘 47 件のうち高優先度 13 件をクローズ前に対応。実績 14/15 SP（達成率 93%） |
