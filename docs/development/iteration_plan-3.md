...
|------|------|
| **イテレーション** | 3 |
| **期間** | 2026-08-24 〜 2026-09-06（計画 Week 5-6） / 2026-07-29（実績記録） |
| **局面** | 中盤（インサイドアウト） |
| **ゴール** | Routing Context の中核ドメインを実装し、航海スケジュール登録・更新・検索から経路候補算出までを利用者導線で確認できる状態にする |
| **目標 SP** | 13 |
...
- [x] `US24` / `US25` / `US07` / `US08` の受入基準をテストで 1:1 に確認する。
- [x] 中盤方針どおり、ドメイン層の不変条件・経路算出ルールを単体テストから Red-Green-Refactor で進める。
- [x] 日付・期限判定は日付単位比較を既定にし、当日着と時刻付き到着の境界テストを含める。
- [x] Routing Context の入力エラーは `ValidationError` 系で分類し、利用者提示エラーと内部障害を分離する。
- [x] `npm run verify` と IT3 デモ E2E が green である。
...
| # | タスク | 見積もり | 担当 | 状態 |
|---|--------|---------|------|------|
| 4.1 | `RouteCandidateFinder` ドメインサービスの単体テスト（直行優先・寄港地接続・期限内到着・該当なし） | 10h | - | [x] |
| 4.2 | `ExternalRoutingServicePort` とフォールバック実装を定義し、IT2 のスタブ ACL 返済方針を実装に接続 | 6h | - | [x] |
| 4.3 | 外部経路サービス契約テスト（nock 相当または fetch mock）とフォールバック時の統合テスト | 6h | - | [x] |
| 4.4 | `/routing/candidates` htmx フラグメントで候補テーブル（所要日数・経由港・費用・航海番号）を返す | 6h | - | [x] |
| 4.5 | IT3 デモ E2E（航海登録 → 検索 → 経路候補算出） | 6h | - | [x] |
...
**1 SP あたり**: 約 7.2h
**進捗率**: 100%（94/94h 完了、タスク 1.1〜1.3・2.1〜2.4・3.1〜3.4・4.1〜4.5 完了）
...
- [x] `US24` / `US25` / `US07` / `US08` の受入基準が単体・統合・E2E のいずれかで確認されている。
- [x] `npm run verify` がパスしている。
- [x] IT3 デモ E2E（航海登録 → 検索 → 経路候補算出）が green である。
- [x] dependency-cruiser が green で、Routing Context 追加後も BC 独立性が保たれている。
- [x] `data-model.md` / `domain-model.md` / `ui_design.md` の IT3 差分が実装と同期している。
- [x] GitHub Project の IT3 Issue は Todo から開発着手時に In Progress へ更新できる状態になっている。

### デモ項目

- [x] 経路設計者が `/voyages` に到達し、航海スケジュールを登録できる。
- [x] 同一航海番号の登録時に既存との差分を確認して更新またはキャンセルできる。
- [x] 予約番号または検索条件から利用可能な航海を検索できる。
- [x] 危険物・冷凍貨物では対応可能な航海だけが表示される。
- [x] 経路候補が所要日数・経由港・費用・航海番号付きで推奨順に表示される。
- [x] 期限内に到達可能な経路がない場合、条件調整を促すメッセージが表示される。
...
| 2026-07-29 | IT3 クローズレビュー後の実績期間・完了状態を反映 | Codex |
