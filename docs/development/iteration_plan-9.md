...
- [ ] US34 / US05 / US25 の受入基準（**正典を 1 行ずつ指差して確認**。T2）が緑
- [ ] デモ項目 8 件が動作する
- [ ] **IT2 から積み上がった「US34 で開放する」の約束**（4 件）に決着が付いている

#### 終盤の完了条件（`development_strategy.md`）

- [ ] 業務シナリオが通しで動作する
- [ ] E2E のクリティカルパス 3 本（US13 / US15 / US18）が緑（既存）＋本 IT の 2 本
- [ ] ArchUnit の 9 ルールがすべて有効

#### 品質

- [ ] `./gradlew clean check` が緑（Checkstyle 0 / SpotBugs 0 / ArchUnit / JaCoCo レイヤー別）
...
- [ ] 荷主がダッシュボードから自社の予約へ到達できる
- [ ] 経路設計者が航海一覧から編集へ到達できる
- [ ] **運航変更が起きたことに気づく手段がある**（影響する予約の警告）
