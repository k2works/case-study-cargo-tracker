...
| # | タスク | 見積もり | 担当 | 状態 |
|---|--------|---------|------|------|
| 4.1 | 荷役登録→追跡イベント記録の一貫性方針を確定（ADR-0012: 荷役を正・ベストエフォート＋冪等の結果整合・retro-5 Try#1／レビュー中#5） | 3h | - | [x] |
...
| 4.7 | US16 荷役登録の確認ステップ明示（中#4・確認チェックボックス＋引取必須ヒント）・`DateTimeOffset.Parse` 例外経路のテスト追加（中#6） | 2h | - | [x] |
