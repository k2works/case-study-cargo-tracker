...
引き継ぎ表の「本 IT」9 件に対し、タスク表の対応行を数えた。

| # | 引き継ぎ項目 | タスク行 | 状態 |
|:--:|------------|---------|------|
| 1 | 予約一覧の絞り込みと荷主名列（M15） | 1.2・1.3 | ✅ |
| 2 | 荷主と利用者の紐付け（P3） | 1.1・1.4 | ✅ |
| 3 | `X-App-Instance` の追加 | 5.1・5.2 | ✅ |
| 4 | `arch_lint_rules.md` のフィクスチャ一覧（L4） | 5.4 | ✅ |
| 5 | 二重引き渡しの並行テスト（M8） | **無し** | ⚠️ **3.5 を新設**（下記） |
| 6 | 温度単位を摂氏固定にするか | 5.3 | ✅ |
| 7 | PostgreSQL での `JdbcCargoRepoTest`（M11） | 2.3 | ✅ |
| 8 | 通知の暫定運用の明記 | 5.3 | ✅ |
| 9 | ADR の承認日の並び（L3） | ADR-0007 起票時 | ✅ |

> **突合で 1 件の取りこぼしを見つけた。** M8（US06 の二重引き渡しの並行テスト）は
> 当初「US25 の版の照合と同じ関心のため 3.3 に統合」としていたが、
> **3.3 は航海スケジュールの更新であり、予約の引き渡しとは別の経路**である。
> 「統合」と書いた時点で独立した行が無くなり、IT5 の M9 と同じ形
> （引き継ぎ表に書いたがタスクに落ちず消える）になっていた。
> **タスク 3.5 として独立させた。**
>
> Try T2 は「1:1 で落とす」ことを求めており、**「別のタスクに統合」は 1:1 ではない**。
> 関心が近くても経路が違えば別のテストが要る。
...
| 3.5 | **US06 の二重引き渡しの並行テスト**（IT5 レビュー M8）。`assign` は read-modify-write であり、同時到達で両方成功しうる。302 がちょうど 1 本であることを固定する | 2h |
...
|-----------|:--:|:--:|------|
| TS11（IT5 レビューの返済） | 2 | 2 | 完了 |
| US24（航海スケジュールの登録） | 3 | 3 | 完了 |
| US25（航海スケジュールの更新） | 3 | 3 | 完了 |
| US07（航海スケジュールの検索） | 3 | 3 | **部分完了**（受入基準 1 は IT7） |
| **合計** | **11** | **11** | |

テスト件数 481 → **570 件**（+89）。うちレビュー対応で +11。

**縮退は 1 件も行わなかった**（成功基準 7 を達成）。

#### 計画になかった対応

| 内容 | きっかけ |
|------|---------|
| `voyage` / `carrier_movement` テーブルの新規作成 | data-model.md に定義があったが V1 で作られておらず、**設計にあるから実装にもあると考えていた** |
| `SharedCalendarText` の新設（日付・UN/LOCODE の検証を Booking から移設） | Routing が同じ検証を要し、BC 間で共有できないため |
| ADR の承認日の訂正（ADR-0003/0004/0005） | ADR-0007 起票時に「承認日の並び」を決める必要が生じた |
| `SELECT ... FOR UPDATE` の導入 | レビュー H4。楽観的ロックが逐次実行でしか成り立っていなかった |
| `VoyagePagesTest` の新設（エスケープ検証を含む） | レビュー H14。XSS の検証がどのレベルにも無かった |
| 間欠異常の新仮説 H7 の記録 | 行ロックの検証中に、無関係なテストが落ちる現象を 2 回観測した |
...
- [x] TS11 の受入基準 5 項目がテストで確認されている
- [x] US24 の受入基準 6 項目がテストで確認されている
- [x] US25 の受入基準 5 項目がテストで確認されている（キャンセルはレビュー後に追加）
- [ ] US07 の受入基準（本 IT の範囲 6 項目）がテストで確認されている
  → **受入基準 1（予約番号から確認）が未充足**。IT7 で US08 と一体で扱う（レビュー M5）
- [x] 対象外とした受入基準に**理由と充足予定の IT** が明記されている
- [x] 全テストが green（570 件。単体・統合・HTTP フロー・E2E）
- [x] `arch-lint` / `trace-lint` / ESLint の違反が 0 件（メタテスト 27 件成功）
- [x] CI が緑（全ジョブ）
- [x] `domain-model.md` / `data-model.md` / `ui_design.md` / `business_rule_traceability.md` に本 IT の変更が反映されている
- [x] ADR-0007・ADR-0008 が作成されている
- [x] リリース計画に再配分（Phase 5 のスコープ外化・TS07 の延期・IT7 の分割）が反映されている
- [x] **索引の突合**（`docs/index.md`・各 `index.md`・`mkdocs.yml` と実体の一致）。Try T8
- [ ] レビュー高優先度が 11 件未満（成功基準 6）
  → **未達（14 件）**。新 BC の立ち上げと Try T1 の「他の場所」調査で潜在欠陥が
  可視化されたため。**14 件すべて対応した**が、件数の基準としては未達である
- [x] 縮退 0 件（成功基準 7）
...
| 二重引き渡しの並行テスト（M8） | **本 IT**（**3.5 として独立**。下記の突合を参照） |
