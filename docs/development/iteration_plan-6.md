...
| 2026-07-27 | 開発進捗（インサイドアウト・バックエンド）: ドメイン + application 層を完了。共有 TransportStatus（注1）、Handling domain/app（IsValidFor・ConsigneeConfirmation 注3）、Tracking domain/app（採番・CQRS 照会）、Booking `Cargo.IssueTrackingNumber` + `AssignTrackingNumberService`（US14）、pgx リポジトリ、migration 000012-014 + sqlc（per-BC 分離 T7・arch-lint 宣言）。全単体テスト green・`make arch` green・ドメイン層カバレッジ 98-100%。 |
| 2026-07-27 | 開発完了（US14/US15/US16/US18 実装完了・16 コミット）: interfaces（web handler + テンプレート tracking/handling/public・予約詳細の発行ボタン）、main.go 合成ルート配線（cargoSnapshotAdapter=Booking ACL・handlingEventAdapter=Handling→Tracking・trackingNumberIssuerAdapter=日次採番・trackingCreatorAdapter=Booking→Tracking）、Booking repo の UpdateTracking、ロール別到達性（T1・navbar/ルート）、設計ドキュメント是正（注1〜5・実装と同時反映）、デモ E2E スペック追加。`make check`（build/test/lint/govulncheck/arch）全 green。**残（環境依存）**: デモ E2E 実行（要 app+DB・CI/staging）、SonarQube Quality Gate（要 SonarQube）。T2/T3 協議依頼/通知待ちワークリストは容量制約により IT7 へ繰越。 |
