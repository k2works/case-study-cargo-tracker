...
イテレーション開始時に行を追加します。次は `opening-iteration` で IT1 の計画を作成します。
...
| フェーズ | 内容 | SP | 完了 SP | 状態 |
|---------|------|-----|---------|------|
| Phase 1 | 基盤とウォーキングスケルトン（IT1-2） | 31 | 0 | 未着手 |
| Phase 2 | 認証と CI/CD（IT3） | 17 | 0 | 未着手 |
| Phase 3 | 予約と経路設計（IT4-7） | 60 | 0 | 未着手 |
