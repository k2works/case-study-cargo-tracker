...
- [x] US02: 荷主（個人）を登録・参照できる
- [x] US03: 法人荷主（契約番号・割引率）を登録・参照できる
- [x] US04: 標準貨物の予約を登録できる
- [x] US05: 危険物・冷凍貨物の予約を登録できる
...
| # | タスク | 見積もり | 担当 | 状態 |
|---|--------|---------|------|------|
| 5.1 | HazardInfo・TemperatureCondition value object 追加 | 3h | - | [x] |
| 5.2 | CargoSpecification に特殊貨物情報統合 + バリデーション | 3h | - | [x] |
| 5.3 | フロントエンド: 貨物種別選択による入力フィールド表示切替 | 3h | - | [x] |
| 5.4 | テスト（バリデーション・フィールド必須チェック） | 4h | - | [x] |

**小計**: 13h（理想時間）

### 6. E2E テスト整備（DoD 対応、0 SP）

DoD「E2E テストがパス」を満たすための整備タスク。SP には含めず受け入れ前品質ゲートとして実施する。

| # | タスク | 見積もり | 担当 | 状態 |
|---|--------|---------|------|------|
| 6.1 | US02/US03 荷主登録 E2E シナリオ（個人 / 法人 / 割引率エラー） | 2h | - | [x] |
| 6.2 | US04 一般貨物予約登録 E2E シナリオ（荷主選択→登録→一覧反映） | 2h | - | [x] |
| 6.3 | US05 危険物 / 冷凍貨物予約 E2E シナリオ（条件フィールド表示・必須・送信） | 2h | - | [x] |
| 6.4 | gatewayms `local-h2` プロファイルに bookingms ルートを追加 | 0.5h | - | [x] |
| 6.5 | E2E 実行手順をフロントエンド README に追記 | 1h | - | [x] |
| 6.6 | Navigation に「荷主管理」「予約管理」リンクを追加（ROLE_ADMIN/ROLE_SALES）+ ユニット / E2E テスト | 1h | - | [x] |
| 6.7 | 荷主・予約一覧にページネーション追加（共通 `Pagination` + Page 形式 API） | 2h | - | [x] |

**小計**: 10.5h（理想時間）。`booking.spec.ts` 18 件 PASS（2026-05-25）。

### タスク合計

| カテゴリ | SP | 理想時間 | 状態 |
|---------|----|---------|----|
| bookingms 基盤構築 | 0 | 8h | [x] |
| US02: 荷主登録 | 2 | 16h | [x] |
| US03: 法人荷主登録 | 2 | 11h | [x] |
| US04: 貨物予約登録 | 3 | 17h | [x] |
| US05: 危険物・冷凍貨物予約登録 | 3 | 13h | [x] |
| E2E テスト整備（DoD） | 0 | 10.5h | [x] |
| **合計** | **10** | **75.5h** | |

**1 SP あたり**: 約 6.5h

**進捗率**: 100% (10/10 SP) — 2026-05-25: 基盤 + US02 + US03 + US04 + US05 完了。バックエンド 46 + フロントエンド 58 + E2E 18 = 計 122 件 Green。DoD「E2E テスト」達成。ナビゲーション・荷主/予約一覧ページネーション追加。
