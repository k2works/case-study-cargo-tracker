...
- [x] 見積作成画面で出発地・目的地・希望期限・貨物種別・重量を入力し、見積番号を発行できる
- [x] 外部ルート照会の結果として、経由港・所要日数・概算料金・航海番号を含む複数候補を表示できる
- [x] 希望期限に間に合うルートがない場合のメッセージ表示と代替候補提示ができる
- [x] ルート照会失敗時に stub / 代替候補で動作確認できる（StubQuoteRouteProviderAdapter 実装済み）
- [x] backend テスト、E2E、SonarQube Quality Gate が Green を維持する（239 テスト Green ✅）
- [x] テストカバレッジ 80% 以上（現在 89.8% ✅）
...
| 1.6 | 見積作成画面と候補一覧 UI、統合テスト、E2E を追加する | 2h | Copilot | [x] |
...
| 2.3 | 希望着日不一致・危険物 / 冷凍貨物の絞り込みルールを実装しテストする | 4h | Copilot | [x] |
| 2.4 | ルート候補一覧 UI、代替条件での再検索導線、404 / 業務エラー表示を実装する | 4h | Copilot | [x] |
| 2.5 | REST API / MVC テスト、E2E、Swagger 表示確認を追加する | 3h | Copilot | [x] |
| 2.6 | SonarQube と docs 更新を含めた品質ゲート確認を行う | 2h | Copilot | [x] |
...
| カテゴリ | SP | 理想時間 | 状態 |
|---------|----|---------|------|
| US01 輸送見積 | 5 | 20h | 完了（6/6 タスク完了） |
| US06 最適ルート検索 | 5 | 20h | 完了（6/6 タスク完了） |
| **合計** | **10** | **40h** | |
...
| 2026-04-01 | タスク 1.6 完了を反映（US01 全完了、進捗 67%） | Copilot |
| 2026-04-02 | 成功基準・カバレッジ実績値を更新（220 テスト Green、カバレッジ 89.3%） | Copilot |
| 2026-04-02 | タスク 2.3・2.4 完了を反映（232 テスト Green、進捗 83%） | Copilot |
| 2026-04-02 | タスク 2.6 完了を反映（239 テスト Green、カバレッジ 89.8%、SonarQube Quality Gate 修正済み、IT2 100% 完了） | Copilot |
