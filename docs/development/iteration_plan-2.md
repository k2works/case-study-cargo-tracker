...
- [ ] US05: 危険物を選択すると危険物申告情報の入力が必須となる
...
| # | タスク | 見積もり | 担当 | 状態 |
|---|--------|---------|------|------|
| 2.1 | `HazardousDeclaration` 値オブジェクト + `TemperatureRequirement` 値オブジェクト実装 | 2h | - | [ ] |
| 2.2 | `Cargo` 集約に特殊貨物情報フィールド追加 + `BookCargoCommand` 拡張 | 2h | - | [ ] |
| 2.3 | Flyway マイグレーション（cargo テーブルに `hazardous_class`・`temperature_min`・`temperature_max` カラム追加） | 1h | - | [ ] |
| 2.4 | `CargoBookingCommandService` 特殊貨物バリデーション追加 | 1h | - | [ ] |
| 2.5 | REST API 拡張（`BookCargoResource` に特殊貨物フィールド追加） | 1h | - | [ ] |
| 2.6 | 予約登録画面に貨物種別連動の動的フォーム実装（htmx `hx-get` でフラグメント切替） | 2h | - | [ ] |
| 2.7 | 単体テスト + API E2E テスト（危険物・冷凍各パターン） | 2h | - | [ ] |
| 2.8 | Playwright E2E テスト（貨物種別切替・必須バリデーション） | 1h | - | [ ] |
