...
| 0.8 | M7: `enabled` の判定を集約の述語に寄せる／M8: CORS プリフライトの扱いを決めテストで固定／M9: npm script を運用手順書に記載 | 3h | [x] M8 は通過を外すと赤になることを確認済み |
...
| # | タスク | 見積もり | 状態 |
|---|--------|---------|------|
| 1.1 | `ContractNumber` / `DiscountRate` 値オブジェクト（TDD。**割引率は 0〜30%**。境界値: -0.1 / 0 / 30 / 30.1） | 2h | [x] |
| 1.2 | `Shipper` に法人契約情報を持たせる（**個人で契約情報を渡したら拒否する**。法人は契約番号必須・割引率任意＝[ADR-012](../adr/012-value-object-granularity.md)。**復元では検査しない**） | 2h | [x] 保存を壊すと赤になることを確認済み |
| 1.3 | Flyway で `shipper` の既存行を壊さないことを確認（列は IT1 で用意済み。**復元では検査しない**） | 1h | [x] マイグレーション追加なし。復元の非検査をテストで固定 |
| 1.4 | 荷主登録画面: 「法人」を選ぶと契約情報の入力欄が出る。個人に戻したら入力値を捨てる | 2h | [x] E2E 4 本が緑 |
...
| # | タスク | 見積もり | 状態 |
|---|--------|---------|------|
| 2.1 | `location` テーブルの Flyway と初期データ（UN/LOCODE。共有カーネルの `Location` を使う） | 2h | [x] |
| 2.2 | `cargo` テーブルの Flyway（`V3__init_booking_cargo.sql`）。**新しいマイグレーションとして足す**（適用済みの V2 は編集しない）。**予約番号のシーケンスとフォーマットをマイグレーションに書く**（0.1 の判断 8。リスク表に書くだけでは規律にならない） | 3h | [x] |
| 2.3 | `BookingId` / `RouteSpecification` / `Quantity` / `Description` / `Dimensions` 値オブジェクト（TDD） | 3h | [x] |
| 2.4 | `Cargo` 集約（TDD。**予約番号は本番経路で採番**。仮受付状態で生成。到着期限は業務タイムゾーンで判断し、過去日付を拒否）。**可変 Clock でテストする**（固定 Clock では日付境界をまたぐ振る舞いを通れない。Try 9） | 4h | [x] |
| 2.5 | 予約登録ユースケース（荷主の存在確認・**存在しない荷主 ID を拒否**） | 3h | [x] |
| 2.6 | `CargoMapper` と永続化（**方言スモークを同じコミットで通す**。IT1 の Try 3） | 3h | [x] |
| 2.7 | `CargoBookingController`（`POST/GET /api/v1/bookings`）と MockMvc テスト。**登録は ROLE_SALES のみ、参照は ROLE_SALES と ROLE_SHIPPER**（ui_design の権限マトリクス）。荷主が自分の予約を参照できることと、**荷主が登録できないこと**を、認可を外すと赤になる形で検証 | 4h | [x] |
| 2.8 | フロントエンド: 貨物予約登録画面（荷主を選ぶ・貨物仕様・輸送条件） | 4h | [x] |
| 2.9 | フロントエンド: 貨物予約一覧（**新しい順・件数・絞り込み・上限**。M6 統合） | 3h | [x] |
...
| # | タスク | 見積もり | 状態 |
|---|--------|---------|------|
| 3.1 | `HazardousDeclaration` / `TemperatureRequirement` 値オブジェクト（TDD。**温度は下限 ≦ 上限**） | 2h | [x] |
| 3.2 | `Cargo` の不変条件: 種別が危険物なら申告が必須・冷凍なら温度条件が必須。**一般貨物にそれらを付けたら拒否する**（付け忘れと同じく、付けすぎも誤り） | 3h | [x] |
| 3.3 | フロントエンド: 種別を変えると追加項目が出る／消える。**種別を戻したときに前の入力が残らない** | 3h | [x] |
| 3.4 | 危険物・冷凍の予約が保存され、**経路設計が読める形になっている**ことの結合テスト（IT3 の入力になる） | 2h | [x] |
...
| # | タスク | 見積もり | 状態 |
|---|--------|---------|------|
| 4.1 | 「04-貨物予約.md」を執筆（予約の登録・一覧・危険物と冷凍の扱い） | 3h | [x] |
| 4.2 | 「03-荷主管理.md」に法人荷主の節を追加 | 1h | [x] |
| 4.3 | 業務フロー章の対応表を更新（工程 3 を「使えます」に。**検査が赤になるので忘れられない**） | 1h | [x] |
| 4.4 | 画面キャプチャの再生成と**目視確認**（Try 5） | 2h | [x] |
...
- [x] US03・US04・US05 の受入基準のうち、**IT2 スコープ内のもの**をすべて満たす（スコープ外は上表のとおり）
...
- [x] **各ストーリー着手時に正典を読み、差分を同じ変更で直した**（Try 6）
- [x] **「定義したが呼んでいない」関数が無いことを確認した**（Try 7。IT1 で全緑をすり抜けた配線漏れの型）
- [x] **MSW を実物に差し替え、E2E が実バックエンドで緑**（Try 8。モックで検証した機能は、実物を 1 本通すまで「動く」と言わない）
- [x] 画面を追加した US について、`ui_design.md` のナビゲーション表・サイドバー実装・ダッシュボード導線・到達性テストの **4 点一致**
- [x] **ナビゲーションの `available` を true にした**（引き継ぎ事項）
- [x] **業務フロー章の対応表を更新した**（工程 3）
- [x] ユーザーマニュアルの該当章を執筆し、**キャプチャを再生成して目視した**（Try 5）
- [ ] kind 統合環境で Gateway 経由の動作確認済み
  - **未達**。作業機のメモリ（Docker に 7.75 GB）で 10 サービス分の JVM が起動しきらず、
    新しいイメージの Pod が起動を完了しない。イメージのビルドとロードまでは成功している
  - **代わりに実施したこと**: Gateway（8080）・authms・bookingms をローカルで起動し、
    Gateway 経由で「ログイン → 法人荷主の登録 → 危険物の予約 → 過去日付の拒否」を通した。
    画面からも `npm run test:e2e:real` で 1 本通している（Try 8）
  - **残す理由**: 通したのは同じ経路（Gateway → サービス → DB）だが、kind の
    Ingress・Service・NetworkPolicy を経由していない。**確かめていないものを確かめたと書かない**
