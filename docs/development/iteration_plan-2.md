...
| # | タスク | 見積もり | 担当 | 状態 |
|---|--------|---------|------|------|
| 3.1 | Routing コンテキスト初期化（パッケージ + ArchUnit ルール追加） | 1h | - | [ ] |
| 3.2 | `Voyage` 集約 + `VoyageNumber`（opaque type）/ `Schedule` / `CarrierMovement` 実装（domain-model.md 585-625 準拠）。`Schedule.apply` で順序・連続性検証、`CarrierMovement` で `arrival_date > departure_date` 検証 | 4h | - | [ ] |
| 3.3 | Flyway V7: `voyage`（`id BIGSERIAL PK`・`voyage_number UK`）+ `carrier_movement`（`id BIGSERIAL PK`・`voyage_id FK → voyage.id`・`departure_location_unlocode FK`・`arrival_location_unlocode FK`・`seq_number`）作成。両テーブルに `version` カラムも付与 | 2h | - | [ ] |
| 3.4 | ScalikeJdbcVoyageRepository 実装（`findByVoyageNumber` / `findByCriteria` / `save` upsert + 楽観ロック）。`AssignToRoutingCommand` 用通知記録の参照 API も用意 | 4h | - | [ ] |
| 3.5 | VoyageController + `RegisterVoyageCommand` / `UpdateVoyageCommand` アプリケーションサービス実装（一覧 `/voyages` / 新規 `/voyages/new` / 編集・差分確認 `/voyages/:voyageNumber/edit` の 3 画面、ui_design.md 85-87 準拠） | 5h | - | [ ] |
| 3.6 | Twirl テンプレート（一覧・登録・差分確認）。ナビバーは layout/nav.scala.html でロール別表示（RouteDesigner にのみ「航路管理」メニュー表示、ui_design.md 130 準拠） | 4h | - | [ ] |
...
| # | タスク | 見積もり | 担当 | 状態 |
|---|--------|---------|------|------|
| 4.1 | DFS + 深さ制限の経路探索プロトタイプ（純関数版） | 4h | - | [ ] |
| 4.2 | サンプル航海データでの探索結果検証（テスト 3 ケース） | 2h | - | [ ] |
| 4.3 | ADR 0005 経路探索アルゴリズム選定を作成 | 2h | - | [ ] |
