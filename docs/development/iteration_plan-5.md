...
| # | タスク | 見積もり | Ralph 適性 | 状態 |
|---|--------|---------|-----------|------|
| 2.1 | PostgresItineraryRepository / PostgresLegRepository / PostgresRouteCandidateRepository 実装 | 4h | AI 完結可 | [x] iter 10 完 (PostgresItineraryRepository が leg も一括担当 iter 6、RouteCandidate は既存 PostgresEstimateRepository で Estimate 集約内エンティティとして永続化済 = IT2 実装、独立ポート不要と判定) |
...
| 4.3 | HTTP + UI: 予約詳細画面 + 通知メールへの表示配線 | 3h | AI 完結可 | [x] iter 26 完了 (Step 3 で HTTP 結線、Step 4 で View 表示: handlerShow に TrackingRepository 引数追加 + TrackingPort.findByBookingId で TrackingActivity 取得 + Maybe Text 変換して bookingShowPage 渡し、BookingShowView に「追跡番号」行追加 code_ タグ + 未発行は「未発行」ミュート表示、stack build 成功)。**US14 完了** ✅ |
...
| 2026-07-01 | **Ralph Loop iter 5**: IT4 繰越 task 2.1 の migration 具体化。Domain 実装 (`Itinerary.hs` / `Leg.hs` / `ItineraryId.hs`) 完了確認 → `20260831110000_create_itinerary_and_leg.sql` + `20260831110100_extend_cargo_for_confirmation.sql` 作成。iteration_plan-4.md §4.3 DDL を忠実に再現 (itinerary_id UUID UK / leg (itinerary_id, seq_number) UNIQUE / load_time < unload_time CHECK / cargo に itinerary_id + cancellation_* + confirmed_at + cancelled_at 追加)。data-model.md 適用済 migration 一覧を 2 行追加 | AI Agent |
| 2026-07-01 | **Ralph Loop iter 6**: task 2.1 の `PostgresItineraryRepository.hs` 実装 (saveItinerary / findByBookingId / findById、T-02 準拠で Tx 開始なし、executeMany で leg 一括挿入、findBy* は itinerary_id で SELECT → mkItinerary で接続性再検証)。package.yaml に module 登録、cabal は hpack 自動同期済み。`stack build --fast` 成功で型整合性検証済み | AI Agent |
