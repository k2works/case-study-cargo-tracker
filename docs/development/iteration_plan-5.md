...
2. **荷役・引取の記録と状態自動遷移（US15/US16）**: 荷役作業員が追跡番号で貨物を特定し、受領・積込・荷降し・引取を記録すると、`TransportStatus` が対応状態へ自動遷移する。
...
- [ ] Tracking の `TransportStatus` 状態遷移（NotReceived→Received→Loaded→OnboardCarrier→Unloaded→AwaitingClaim→Claimed）が FsCheck を含むユニットで網羅検証される
...
| # | タスク | 見積もり | 担当 | 状態 |
|---|--------|---------|------|------|
| 1.1 | `TransportStatus`（9 ケース DU）を Shared へ配置し `toString`/`ofString`（網羅 + Error フォールバック）を実装 + FsCheck。`TrackingNumber`（単一ケース DU・一意採番） | 3h | - | [ ] |
| 1.2 | `TrackingActivity` 集約（追跡番号・BookingId・現在 TransportStatus・`TrackingActivityEvent list`）と、イベント追加で状態を導出する遷移（`recordHandling`/`recordManualUpdate`）を `create`/`execute` で保証 + FsCheck（状態遷移網羅） | 4h | - | [ ] |
| 1.3 | 追跡番号発行ファクトリ（`issue`・Confirmed 予約から NotReceived で生成・`TrackingNumberIssued` イベント）+ ユニット | 2h | - | [ ] |

**小計**: 9h（理想時間）

> **注（TransportStatus の配置）**: domain-model は `TransportStatus` を Shared Kernel に置く（Booking の `Delivery` も参照）。Booking の `BookingState` DU（IT4）とは別概念のため型を分ける。状態を持つ集約のため状態遷移図を「設計」節に掲載する。

#### 2. Handling ドメイン（US15/US16）

| # | タスク | 見積もり | 担当 | 状態 |
|---|--------|---------|------|------|
| 2.1 | `HandlingType`（RECEIVE/LOAD/UNLOAD/CLAIM 等の DU）・`HandlingActivity` 集約（追跡番号・種別・場所〔Location〕・実施日時・荷受人確認〔引取時〕）を `create` で検証 + FsCheck | 4h | - | [ ] |
...
TrackingActivityEvent --> TransportStatus
HandlingActivity --> HandlingType
HandlingActivity ..> TrackingActivity : HandlingActivityRegistered\n（合成層 ACL 経由）
@enduml
```

> **注（domain-model 反映・要）**: Tracking/Handling は現状プレースホルダー。実装時に `TransportStatus` を Shared へ、`TrackingActivity`/`HandlingActivity` の要素・遷移を domain-model の要素表へ反映する。`CargoItinerary` と同様、`TrackingActivityEvent list` は `list` + 不変条件保証で実装する。

### 状態遷移（IT5 スコープ: TransportStatus）

```plantuml
@startuml
title TransportStatus 状態遷移（US14/US15/US16/US17）

[*] --> NotReceived : 追跡番号発行（US14・Confirmed 予約から）
...
- [ ] TransportStatus 状態遷移・TrackingActivity/HandlingActivity が FsCheck 含めて網羅検証
...
- [ ] ドキュメント更新完了（release_plan 進捗・ADR-0011/0002・data-model 0009/0010 反映・domain-model の Tracking/Handling 反映・TransportStatus を Shared へ）
