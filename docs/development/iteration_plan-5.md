...
- [x] SonarQube ローカルスキャンを実行し Quality Gate PASS を確認できる
- [x] `Cargo.requireStatus()` が抽出され、状態遷移ガードが 1 箇所に集約されている
- [x] ナビゲーション順序が業務フロー順（見積管理→予約管理→荷主管理）に変更されている
- [x] 経路候補一覧から「選択」ボタンをクリックすると予約に経路が紐付き、詳細画面で確認できる
- [x] 経路条件（期限・出発地・目的地）を変更すると候補が再算出されて表示される
- [x] `BookingStatus` が `ROUTE_PROPOSED`（経路提案済）に遷移し、予約詳細画面に経路情報が表示される
- [x] Java テスト全パス・E2E テスト全パス（250 件全パス）
...
|---------|----|---------|------|
| IT4 申し送り改善 | 2 | 7h | [x] |
| US09: 経路を選択・確定する | 3 | 13h | [x] |
| US10: 経路条件を調整して再算出する | 3 | 9h | [x] |
| US11: 経路情報を予約に紐付ける | 2 | 8h | [x] |
| **合計** | **10** | **37h** | |
...
- [x] 機能がローカル環境で動作確認済み
- [x] ドキュメント更新完了（iteration_plan-5.md 進捗更新）
