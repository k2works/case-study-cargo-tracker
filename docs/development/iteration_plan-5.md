...
4. 対応内容（新しい到着予定日・対応方針）は記録時に同時入力する（Phase 1 では送信機能なし、記録のみ）
5. 例外対応履歴が記録される
...
5. 対応内容（補償方針等）は記録時に同時入力する（Phase 1 では送信機能なし、記録のみ）
6. 例外対応履歴が記録される
...
| # | タスク | 見積もり | 担当 | 状態 |
|---|--------|---------|------|------|
| 2.1 | DAMAGE/LOSS 種別処理・緊急フラグ設定ロジック（`CargoException.isUrgent()`）とユニットテストを実装する | 3h | Copilot | [x] |
| 2.2 | 破損・紛失例外登録 Web フォーム UI（`/exceptions/damage`, `/exceptions/loss`）と登録フローを実装する | 4h | Copilot | [x] |
| 2.3 | 例外処理の統合テスト・E2E テスト（E15: 遅延記録で状態が例外発生になる、E16: 紛失記録で緊急フラグが立つ）を追加する | 5h | Copilot | [x] |
