...
| # | 基準 | 結果 |
|:--:|------|------|
| 1 | 登録・更新の結果が次の画面で利用者に見える | ✅ フラッシュメッセージ。US02-3・US04-4 を充足 |
| 2 | 荷主コードを画面から探して予約に持ち込める | ✅ 絞り込みと導線。**着地先で選択済み**まで検証 |
| 3 | 危険物・冷凍貨物が申告なしでは登録できない | ✅ 集約で必須性を守り、エラーが画面に出ることを検証 |
| 4 | 引き渡しで状態が変わり、戻せない遷移が守られる | ✅ 許可・拒否の両方を全状態で固定 |
| 5 | E2E シナリオが CI で動く | ✅ 5 シナリオ・CI ジョブとも緑 |
| 6 | 間欠 403 の**原因が特定されている** | ❌ **未達**。360 本の混在並行 × 3 回でも再現せず。観測手段（`X-Route`）と再現手段は残した |
| 7 | 見積もりが 40h に収まる | ⚠️ 計画 59h。実績時間は未計測（下記） |

> **基準 6 は未達である。** 「解消または起こり得ないことの証明」を求めていたが、
> どちらも得られていない。残ったのは次回の切り分け材料だけであり、
> レビューでは「ポート再割り当ての仮説は `X-Route` では切り分けられない」
> （インスタンス識別子が要る）とも指摘された。TS10 を「完了」と呼ぶのは
> **観測手段を残すという受入基準に照らしてであり、原因特定ではない**。
...
| # | タスク | 見積 |
|:--:|-------|:---:|
| 0.1 | **壊れ方の観点表を作る**（入力の境界・外部システム・表示経路・並行の 4 軸。IT5 レビューで軸 5「実行環境・テストの前提」を追加）。`docs/reference/` に置き、受入基準を書くときに当てる | 1h |
...
**成功基準 7（実績時間）は評価できない。** 本イテレーションは AI エージェントの
単独実行であり、人日ベースの稼働時間という概念が対応しない。IT1-IT4 の
「計画時間 vs 実時間」も同じ前提で記録されたものであり、**係数 1.9 の根拠自体が
人の稼働を前提としている**。次 IT では時間ではなく **SP と縮退件数**で
見積もり精度を測る形へ改めるべきである（ふりかえり Try で扱う）。

### レビュー対応（クローズ時）

5 視点のレビューで**高 11 件**を検出し、すべてクローズ前に対応した。
詳細は [IT5 実装レビュー](../review/IT5実装_review_20260803.md) を参照。

| 内容 | 性質 |
| :--- | :--- |
| 寸法が DB へ 10 倍で保存されていた | 往復テストは緑。**アプリの外から読む人だけが誤った値を見る** |
| 荷主ロールに他社の予約が全件見えていた | 情報漏洩。利用者と荷主の紐付けが無く絞れないため営業限定へ |
| 予約詳細に荷主が出ていなかった | US06-1 の実質的な中身 |
| 到着期限の過去日が通っていた | IT4 レビュー M9。**IT5 計画で対応予定としながら漏れていた** |
| エラー表示の受入テスト 5 件が空振り | 検証文字列が入力欄のラベルに一致 |
| 表示フォーマッタが 3 箇所・振る舞いも不一致 | 入力側だけ共有化していた |
| `X-Route` がテスト実行で無効 | TS10 の投資が狙った場面で回収できない |

テスト件数は 472 → **481 件**（レビュー対応で +9）。
