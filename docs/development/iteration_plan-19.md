...
| **D5** | **集約ルート以外から `entities` の生成・変更メソッドを呼ばないルール**（ADR-024 の代償） | **4 メソッド**（`ProposedRoute` 2・`TrackingExceptionEvent` 2）。**実測の違反は 0 件** | ADR-024 で公開した操作を、いまは<strong>コメントだけが守っている</strong>。**0 件のうちに ArchUnit へ落とす**。クローズ前レビューで「本文に『名前で記録した』と書きながらどこにも記録が無い」と指摘されて起票した |

> **D1〜D4 は IT16 で記名されたあと、IT17・IT18 の計画に節ごと現れませんでした**（3 IT 行方不明）。
> **書く場所を決めていなかったのが原因**です。本 IT でこの節を置き、以後の計画に引き継ぎます。
>
> **D1 は本 IT で返しました**（`Cargo` の引取まわりを `CargoClaim` へ）。計画は「本 IT は `Cargo` を
> 触らない」と書いていましたが、モデル分割で `Cargo` が 500 行を超えたため、**触ることになった時点で
> 記名どおり返しました**。
...
| # | タスク | 見積 | 状態 |
| ---: | :--- | ---: | :--- |
| 0 | **開発環境の起動をマニュアルと同じ状態にする**（**利用者の依頼で追加**） | 12h | **完了** `7591e796a` |
| 1 | Release 2.1 を出荷として確定させる | 6h | **完了** `af90744af` |
| 2 | T3: H2 スモークの名簿方式をやめる（**育つ**） | 8h | **完了** `97282b5f7` |
| 3 | T5: 名簿の幽霊エントリを検出する（**育つ**） | 4h | **完了** `8dd6fccc2` |
| 4 | C6b: ADR-022 に検査を付ける（**育つ**） | 6h | **完了** `7907d3923` |
| 5 | T1: 予約登録フォームの往復テスト | 4h | **完了** `bb27f9c49` |
| 6 | T4: キャプチャをデータのある状態で撮る | 5h | **完了**（タスク 0 で解決。63 件を再生成） |
| 7 | C3 / C5: 期限切れの見積から作り直せるようにする（**正典未達**） | 6h | **完了** `bdaaa5108` |
| 8 | C4: 見積一覧の絞り込み 4 条件（**正典未達**） | 8h | **完了** `bdaaa5108` |
| 9 | C2 / C6a / C6c / C6d | 10h | **完了** `ed7bfe684` / `bb27f9c49` |
| 10 | ドキュメント（正典 3 件の反映・マニュアル・インデックス） | 7h | **完了** `2460d88c0` |
| C7 | **package-info の欠落を検査で止め、JIG に説明を載せる**（**利用者の依頼で追加**） | 4h | **完了** |
| 11 | 途中レビュー | 3h | **省略**（クローズ前レビューで代替。5 視点並列） |
| 12 | 品質ゲートと破壊検証 | 8h | **完了** |
| | **合計** | **91h** | **11h 超過**（10 営業日 = 80h。**タスク 0 と C7 が計画外**） |

### 0. 開発環境の起動をマニュアルと同じ状態にする — 12h（**計画外**。利用者の依頼）

**着手後に利用者から依頼された。** 計画には無い。

- `db/demo` の SQL が作れるのは荷主・予約・航海までで、**追跡・荷役・請求・見積の章はすべて空の画面**だった
- **SQL では足せない**。追跡の記録も請求書も荷役を登録した結果として `AFTER_COMMIT` の
  ドメインイベントが作る派生データであり、採番はシーケンスと衝突する
...
**着手前に潰した前提の誤り 3 件に加え、実装中にさらに 3 件見つかった。**

| # | 計画の前提 | 実際 |
| :--- | :--- | :--- |
| T3 | Bean を自動で集めて全件叩く | **叩き方は人が書き、載せ忘れだけを機械が見る形にした。** 引数の要るメソッドに値をこしらえると「叩けなかったものを黙って飛ばす」形になり、名簿方式に戻る |
| T4 | 既存データに依存するキャプチャを spec 内で作り直す | **タスク 0（シード投入）で解決した。** 撮影では投入を切る —— 入れたままだと「見積がまだありません」の図が撮れない |
| — | （計画に無し） | **ソースを読む検査がソースを入力にしていなかった。** Javadoc だけの変更ではバイトコードが変わらず `compileJava` が UP-TO-DATE になり、**検査そのものが走らないまま緑になる**（壊れた `@link` を入れて実測） |
| — | （計画に無し） | **`Result.declarationId` / `Result.exceptionId` は成功時も常に `null`。** Javadoc は「受け付けたときの ID」と書いている。**戻り値を捨てないようにして初めて分かった** |
| — | （計画に無し。利用者の依頼） | **74 パッケージ中 14 に `package-info.java` が無かった。** Billing Context はほぼ全域が欠けており、**BC を 1 つ足したときにまとめて漏れる**形だった。JIG の図では名前だけの箱になっていた |
...
| 2026-08-12 | タスク 1 完了（`java/take-6/v2.1.0` タグ・`CHANGELOG.md`・[リリース完了報告書](release_report-2_1_0.md)）。**タスク 0 を計画外で追加**（利用者の依頼。開発環境の起動をマニュアルと同じ状態にする。12h）。**投入済みの判定を 2 度間違えた** —— 「見積があれば」は見積を作る別のテストが先に走ると投入が飛び、「動作確認用の荷主が居れば」は `V900` がその荷主を SQL で作るため一度も走らなかった。**拒まれた結果を捨てないようにしたところ、紛失の例外の起票がずっと失敗していた**ことが分かった |

| 2026-08-12 | クローズ。**5 視点レビューで高 11 件**（検査自身の穴・マニュアルの手順が 403・ADR の記述と実態のずれ）を指摘され、**すべて対応**した。`entities` の誤分類と識別子の置き場も是正（`entities` に残るのは同一性を持つ 2 クラスのみ）。品質: `check` / E2E 21 件 / キャプチャ 63 件 / CI / SonarQube **PASS** |
| 2026-08-12 | タスク 2〜10 を完了。**育つ負債 3 件（T3・T5・C6b）を先に返した**。正典未達の C3・C4 を返し（Issue #517）、返済枠 C2 / C6a / C6c / C6d と Try T1 も返した。**実装中に前提の誤りがさらに 3 件見つかった**（上表）。品質: `check` 緑・E2E 21 件緑・マニュアルのキャプチャ 63 件緑 |
