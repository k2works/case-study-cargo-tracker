...
### 未配置: US33（荷主の担当者が自分の貨物だけを追跡する）

| ID | ユーザーストーリー | SP | BV | C | KA | RR | 優先度（正典） | 主なサービス |
|----|-------------------|----|----|---|----|----|--------|-------------|
| US33 | 荷主の担当者が自分の貨物だけを追跡する | 5 | 中 | 中 | 中 | 中 | 中 | authms（利用者と荷主の紐付け）+ trackingms（自社の貨物の一覧） |

> **IT9 で起票しました**（[ADR-024](../adr/024-tracking-manual-update-and-exceptions.md) 決定 10 の
> 残課題・IT8 引き継ぎ 4）。公開の追跡照会は認証を求めない設計にした一方、**荷主が自社の
> 貨物を横断して見る手段が無い**ままです。追跡番号を 1 件ずつ控える運用は、貨物が増えるほど
> 控え漏れを生みます。
>
> **リリースへの配置は次の見直しで決めます。** ただし「次の見直しで」と書き続けると
> いつまでも起票されないため、**起票そのものを IT9 のタスクにしました**（タスク 8.1）。
> 配置が決まるまで、この節が US の全件突き合わせ（US01〜US33）の受け皿になります。
...
xychart-beta
    ...
    line "実績" [105, 96, 88, 78, 70, 62, 53, 43, 34, 24]
```
