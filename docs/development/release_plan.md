...
| 8 | 5 | - | - | 計画済み（US23 精算・ハードニング・非機能受け入れ・Release 1.1 完成・整合性検証済み） |
...
xychart-beta
    ...
    line "実績" [97, 81, 70, 59, 45, 31, 18, 5]
```

> 実績: IT1 完了時点で残 81 SP（16 SP 消化）。IT2 完了時点で残 70 SP（11 SP 消化）。IT3 完了時点で残 59 SP（11 SP 消化）。IT4 完了時点で残 45 SP（14 SP 消化）。IT5 完了時点で残 31 SP（14 SP 消化）。IT6 完了時点で残 18 SP（13 SP 消化）。IT7 完了時点で残 5 SP（13 SP 消化）。計画ラインと一致し、ベロシティは安定推移（累計 92/97 SP・95%）。Phase 3 継続（破損紛失例外・料金算出・法人割引）、Billing Context 本格実装、Release 1.1 へ前進。
