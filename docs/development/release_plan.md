...
gantt
    ...
    IT2 予約・航海登録（実装中） :active, a2, after a1, 14d
```
...
|---------------|---------|---------|--------|------|
| IT1 | 16 | 14 | 88% | 完了 |
| IT2 | 14 | - | - | 実装中（バックエンド完了・フロントエンド UI 未着手、2026-05-15 中間集計） |
...
>
> **IT2 中間進捗（2026-05-15）**: バックエンド側は US00-r1/US00-r2/US-UI-r/US04/US05/US24 の受入条件達成（14 SP 相当のバックエンド作業完了）。残作業は (a) フロントエンド UI（タスク 5）、(b) PIT/Checkstyle ブロック化/SonarQube 安定化（タスク 7.2-7.4）、(c) data-model.md 同期。実績 SP は IT2 完了時に確定する。
