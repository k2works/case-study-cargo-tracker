...
| IT13 | 5（+ バッファ 2） | - | - | [計画済み](iteration_plan-13.md)（予備枠） |
...
1. IT13 のふりかえりと完了報告書を作成し、Release 2.1 完了報告書へ入力する。
2. Release 2.2 候補バックログの先頭に、請求書検索・発行月絞り込み・検索結果合計を置く。
3. IT13 の実環境確認で踏んだ `dev:k8s:apply -> dev:k8s:rollout:image` の順序を、以降の実バックエンド E2E の前提として使う。
4. ベロシティは直近の実績で再計算し、予備枠を使い切った後の次リリース計画に反映する。
