...
1. **v1.0.0 リリースタグ付与**: IT6 完了により全 18 ストーリー（US01〜US18）が実装済み。`git tag v1.0.0` でリリースタグを付与する
2. **IT6 完了報告書・ふりかえり**: イテレーション完了報告書（`iteration_report-6.md`）とふりかえり（`retrospective-6.md`）を作成する
3. **GitHub Project 同期**: US17・US18 を「Done」に更新し、Milestone v1.0.0 をクローズする
