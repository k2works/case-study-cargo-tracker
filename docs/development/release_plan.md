...
| IT7 | 経路候補算出（Datalog）+ **HTTP ランタイムの防御**（セマフォ・タイムアウト）と追跡 API の負荷試験 | US08, TS12 | 11 |
...
| **累計** | **152**（145 + TS08 3 + TS09 3 + TS10 1） | **81** | **53%** | |

> **分母の読み方**（IT6 レビュー M13）: 152 SP には**将来リリース 21 SP と
> 延期 5 SP が含まれる**。v1.0.0 のスコープは **126 SP** であり、
> IT6 終了時点の進捗は **81 / 126 = 64%** である。
> 152 だけを見ると v1.0.0 の規模を過大に読む。
...
xychart-beta
    ...
    y-axis "残 SP" 0 --> 145
    line "計画" [152, 136, 119, 105, 91, 80, 69, 56, 44, 32, 26]
    line "実績" [152, 136, 119, 105, 93, 82, 71, 71, 71, 71, 71]
```

実績線は IT6 まで（残 **71 SP**）。IT7 以降は未着手のため横引きしています。
総 SP は 152（145 + TS08 3 + TS09 3 + TS10 1）で、y 軸の上限もこれに合わせています。
