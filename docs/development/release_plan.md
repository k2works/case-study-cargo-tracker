...
gantt
    ...
    IT8 (計画済)                  :active, a8, after a7, 14d
```
...
xychart-beta
    ...
    line "実績" [194, 160, 136, 118, 97, 75, 57, 33]
```
...
1. IT8: 破損・紛失例外処理（US20）・輸送料金算出（US21）の BE + FE 実装（16 SP）
2. IT9: 精算完了（US22/US23）・経路再算出（US10）・Phase 2 統合テスト（21 SP）
