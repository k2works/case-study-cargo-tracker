...
| IT6 | 航海スケジュールと外部 ACL + 航海スケジュール検索 | US24, US25, TS07, US07 | 14 |
| IT7 | 経路候補算出（Datalog）と経路確定 + 輸送見積 + **HTTP ランタイムの防御**（セマフォ・タイムアウト）と追跡 API の負荷試験 | US08, US09, US01 | 18 |
| IT8 | 経路の紐付けと予約確定・追跡番号発行 | US10, US11, US12, US13, US14 | 13 |
| IT9 | 荷役記録と追跡連携 | US15, US16 | 13 |
| IT10 | 例外処理 | US17, US19, US20 | 11 |
| （枠外） | **精算（Phase 5）** | US21, US22, US23 | 13 |

> **Phase 5（13 SP）は現在の 10 イテレーション枠に収まりません**。IT4 のスコープ変更で
> バッファを 5 SP 前借りしたため、**IT6 終了時に「Phase 5 をスコープ外とする」か「イテレーションを追加する」かを
> 明示的に判断します**（[リスク管理](#スケジュールリスク)の対応方針どおり）。「余力次第」とはしません。
>
...
| IT9 | 13 | - | - | 暫定 |
| IT10 | 11 | - | - | 暫定 |
| （枠外） | 13 | - | - | Phase 5。**IT5 クローズ時に一度確認する**（ふりかえりの指摘） |
...
xychart-beta
    ...
    y-axis "残 SP" 0 --> 145
    line "計画" [152, 136, 119, 105, 91, 78, 64, 46, 33, 20, 9]
...
**IT6-IT10 の 5 イテレーションで 82 SP を消化する計画**ですが、実効ベロシティ
12-13 SP では **60-65 SP 程度**しか入りません。IT7 は単独で 18 SP を計画しており、
これも実効ベロシティを 5-6 SP 超えています。

**Phase 5（精算・13 SP）はスコープに入りません**。IT6 終了時としていた判断時期を
待たず、IT5 のクローズ時点で「入らない見込み」を確定として扱います。
IT6 計画時に (a) Phase 5 をスコープ外とする (b) イテレーションを追加する
のどちらかを選び、あわせて IT7 の分割を決めます。
