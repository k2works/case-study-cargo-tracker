...
1. **IT3 完了報告書作成**: `planning-releases --report` で IT3 のふりかえりと報告書を作成する
2. **IT4 実装開始**: US10（荷役作業記録）→ US11（引取作業記録）→ US12（手動更新）→ US13（追跡照会）の順で TDD で進める
3. **ベロシティ確定**: IT1=10 SP・IT2=10 SP・IT3=12 SP の実績から平均ベロシティ 10.7 SP として IT4 以降の計画を維持する（IT4 は 13 SP でやや高めのため注意）
