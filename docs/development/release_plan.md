...
| **累計** | **153**（145 + TS08 3 + TS09 3 + TS10 1 + TS13 1） | **92** | **60%** | |

> **分母の読み方**（IT6 レビュー M13）: 153 SP には**将来リリース 21 SP と
> 延期 5 SP が含まれる**。v1.0.0 のスコープは **127 SP** であり、
> IT7 終了時点の進捗は **92 / 127 = 72%** である。
> 153 だけを見ると v1.0.0 の規模を過大に読む。
>
> **残 SP の内訳**（IT7 レビュー・technical-writer 指摘への是正）:
> v1.0.0 の残りは 127 - 92 = **35 SP**。一方 IT8-IT10 の計画は
> 14 + 12 + 12 = **38 SP** であり、**3 SP 超過している**。
> これは IT8 に TS12b（1 SP・IT7 から分割）と IT6/IT7 レビューの返済枠（2 SP 想定）を
> 積んだためで、いずれも当初の 127 SP に含まれていない。
> **IT8 の計画時に、超過分を吸収するか v1.0.0 のスコープを 130 SP へ改めるかを判断する**。
>
...
xychart-beta
    ...
    y-axis "残 SP" 0 --> 153
    line "計画" [153, 137, 120, 106, 92, 81, 70, 59, 45, 33, 21]
    line "実績" [153, 137, 120, 106, 94, 83, 72, 61, 61, 61, 61]
```

実績線は IT7 まで（残 **61 SP**）。IT8 以降は未着手のため横引きしています。
