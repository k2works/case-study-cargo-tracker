...
| IT4 | 8 | 8 | 100% | [開発完了](iteration_plan-4.md)（クローズ作業中） |
| IT5 | 8 | - | - | 未着手 |
| IT6 | 9 | - | - | 未着手 |
| IT7 | 10 | - | - | 未着手 |
| IT8 | 9 | - | - | 未着手 |
...
xychart-beta
    ...
    line "実績" [105, 96, 88, 78]
```

> **実績が計画と一致しています（IT1〜IT3 で 27 SP／計画どおり）。** ただしこれは
> 「見積もりが当たった」ことのみを意味し、余力の有無は示しません。IT3 は
> [レビュー](../review/イテレーション3_review_20260821.md)で高優先度 10 件の手直しが
> 発生しており、その工数は SP に計上されていません。

---

## 次のステップ

1. IT4 のクローズ（レビュー・ふりかえり・完了報告書）。その後 IT5（US09・US10・US11）を開始する
2. IT3 レビューの中優先度のうち、着手条件が IT4 に来たもの（`connects()` の往復航海ほか 7 件）は
   IT4 の返済枠 33h に計上済み。荷主編集まわり（#550・`save` 分岐・`Shipper.email`）は **IT5 に確定**した
3. ベロシティは実測 9 SP / IT。実効 8 SP を据え置く（IT3 終了時点で検証済み）
