...
1. **IT3 実装開始**: US05（CargoSpecification 拡張）→ US07（ルート確定）→ US08（予約確定）→ US09（追跡番号）の順で TDD で進める
2. **品質維持**: backend 239 テスト Green、E2E 17 件、SonarQube Quality Gate PASS を維持したまま IT3 機能を追加する
3. **ベロシティ再確定**: IT3 終了時に 3 イテレーション実績でベロシティを更新し IT4 以降の計画を調整する
