...
gantt
    ...
    IT4 荷役作業記録（US10 完了・23%進行中）      :active, it4a, 2026-04-02, 1d
    section Phase 2
```
...
| IT4 | 2026-05-12〜05-25 | 13 | 3 | 23% | **進行中** 🔄 |
...
1. **IT4 継続実装**: US11（引取作業記録・RECEIVE 重複防止）→ US12（MANUAL_UPDATE・管理者フォーム）→ US13（追跡照会 REST・公開追跡ページ）の順で TDD で進める
2. **ベロシティ評価**: US10 完了済み（3 SP）、残り 10 SP。US11 完了後で 13 SP 達成可否を評価し、遅延時は US12 を IT5 に延期する
3. **H-3 対応方針**: `HandlingEventRecordedEvent` のリスナー実装（Tracking BC 連携）は US13 設計時に合わせて決定する
