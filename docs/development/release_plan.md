...
| フェーズ | 内容 | ストーリー数 |
|---------|------|---------------|
| Phase 1（Release 1.0 / MVP） | 見積・荷主・予約・経路設計・追跡・荷役 | 20 US |
...
| Phase 1 | 63 SP | 1-5 |
| Phase 2 | 19 SP | 6-7 |
| **合計** | **82 SP** | 7 イテレーション + 予備 1 |

> **Note**: IT1 にはストーリー外の基盤タスク（DbUp 起動時配線・Unit of Work + post-commit イベント基盤・認証の骨格。ADR-0001〜0003 の実装）を含めるため、IT1 のストーリー SP は少なめに設定しています。
...
**ゴール**: 技術基盤（DbUp・UoW + post-commit イベント・認証骨格）を確立し、荷主登録と見積で最初の業務価値を届ける。
...
xychart-beta
    ...
    y-axis "残 SP" 0 --> 90
    line "計画" [82, 72, 62, 48, 36, 19, 13, 0]
```
