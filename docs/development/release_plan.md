...
> 突き合わせる相手は明細ではなく **US の全件（US01〜US33）** です。全 33 件がいずれかのリリースに属することを、リリース変更のたびに確認してください。
...
| IT14 | 8（US34 5 + US35 3） | - | - | [計画済み](iteration_plan-14.md) |
...
2. **Release 2.2（業務シミュレーション・16 SP）を IT14〜IT15 で実施する。** 計画は
   ...
   **3 度目・4 度目の申し送りのうち 2 件を計画に入れた**——SonarQube Hotspot の走査トークン権限
   ...
   壊れる条件を自分で作るため）。請求書検索（3 度目）は IT16 の先頭に置く。
3. IT13 の実環境確認で踏んだ `dev:k8s:apply -> dev:k8s:rollout:image` の順序を、以降の実バックエンド E2E の前提として使う。
4. **ベロシティの最終実績は 8.6 SP / イテレーション**（112 SP / 13 IT）。次リリース計画のコミットメントはこの値を基準にする。
