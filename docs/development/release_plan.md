...
gantt
    ...
    section Release 2.1
    IT13 荷主のセルフサービス   :p6-1, after p5-2, 14d
...
- **予備イテレーション**: IT13（1 イテレーション）。**IT10 で US33（5 SP）を割り当てた**
  ため、残る余力は 3 SP 相当

> **バッファを使い切ってはいません。** US33 は 5 SP で、IT13 の実効ベロシティ（8 SP）に
> 3 SP の余地が残ります。**予備を「空けておく」ために US33 を無配置のままにするのは、
> 別の形の先送り**です（[IT9 の学び](retrospective-9.md)）——居場所を決めたうえで、
> 残りをバッファとして持ちます。

### バッファ消費ルール

1. フィーチャバッファ（上表の順）を先に消費
2. スケジュールバッファ（IT13 の残り 3 SP）は最後の手段
