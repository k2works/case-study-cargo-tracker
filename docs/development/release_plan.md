...
- [ ] IT3-改善: SonarQube スキャン・RouteCandidateProvider ポート抽出・ArchUnit ルール追加
- [ ] US07: 航海スケジュールを検索する
- [ ] US08: 経路候補を算出する（基本実装）
...
1. IT4 開発を開始する（US07: 航海スケジュール検索・US08 基本実装）
2. GitHub Project に同期する（`syncing-github-project --sync`）
3. IT4 完了後にふりかえりと完了報告書を作成する（`planning-releases --retrospective`）
