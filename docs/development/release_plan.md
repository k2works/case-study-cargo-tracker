...
xychart-beta
    ...
    line "実績" [97, 81, 70, 59, 45, 31, 18]
```

> 実績: IT1 完了時点で残 81 SP（16 SP 消化）。IT2 完了時点で残 70 SP（11 SP 消化）。IT3 完了時点で残 59 SP（11 SP 消化）。IT4 完了時点で残 45 SP（14 SP 消化）。IT5 完了時点で残 31 SP（14 SP 消化）。IT6 完了時点で残 18 SP（13 SP 消化）。計画ラインと一致し、ベロシティは安定推移（累計 79/97 SP・81%）。Phase 3 開始（見積・公開照会・遅延例外）、Release 1.1 へ着手。
