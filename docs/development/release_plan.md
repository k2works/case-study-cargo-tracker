...
| IT12 | **11**（うち 3 は US21 の再実施） | - | - | **計画済み** |
| IT13 | 5（+ バッファ 2） | - | - | 未着手（予備枠） |
...
xychart-beta
    ...
    line "実績" [110, 101, 93, 83, 75, 67, 58, 48, 39, 29, 22, 13]
```
...
1. IT5 の開発着手。[イテレーション 5 計画](iteration_plan-5.md)に従い、返済枠（Day 1-2）→ 旅程のドメイン（Phase 1）の順に進める
2. IT5 の返済枠に、荷主編集まわり（[#550](https://github.com/k2works/case-study-cargo-tracker/issues/550)・`save` 分岐・`Shipper.email`）と、
   経路候補の判断材料（船名・運送会社・積み替えの待ち時間・条件を URL に残す）を最初から入れる
   （[IT4 完了報告書の残作業](iteration_report-4.md#課題と残作業)）
3. ベロシティは実測 8.75 SP / IT（IT1〜IT4 平均）。実効 8 SP を据え置く
4. **レビュー手直しの枠を IT5 の見積もりに入れる。** IT3・IT4 と 2 回続けて高優先度 10 件超の
   手直しが SP 外で発生している
