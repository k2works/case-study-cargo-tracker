...
### Phase 1: MVP — 予約・経路設計・追跡（IT1〜IT5）
...
|---------|-------|-------|---------|---------------|
| Phase 1（MVP） | 60 | 38 | 98 | IT1〜IT5 |
| Phase 2（荷役・精算） | 36 | 22 | 58 | IT6〜IT8 |
| Phase 3（見積・荷主） | 11 | 10 | 21 | IT9〜IT10 |
| **合計** | **107** | **70** | **177** | 10 イテレーション |
...
gantt
    ...
    IT5 MVP統合・E2E              :crit, p1-5, after p1-4, 14d
    Release 0.1.0 MVP             :milestone, m1, after p1-5, 0d
    section Phase 2 (荷役・精算)
    IT6 荷役拡張・例外処理         :p2-1, after p1-5, 14d
    IT7 精算・料金計算             :p2-2, after p2-1, 14d
    IT8 統合・品質向上             :p2-3, after p2-2, 14d
    Release 0.2.0                 :milestone, m2, after p2-3, 0d
    section Phase 3 (見積・荷主)
    IT9 見積機能                   :p3-1, after p2-3, 14d
    IT10 荷主管理・完成             :p3-2, after p3-1, 14d
    Release 1.0.0                 :milestone, m3, after p3-2, 0d
```
...
gantt
    ...
    IT5 (予定)                    :a5, after a4, 14d
    Release 0.1.0                 :milestone, ma1, after a5, 0d
```
...
- [ ] US04: 貨物予約を登録する（BE 8 + FE 5 = 13 SP）
- [ ] FE 基盤: 認証画面・AppLayout・API クライアント統合（5 SP）

**目標 SP**: 18（BE 8 + FE 10）

### イテレーション 3（Week 5-6: 2026-05-26〜2026-06-06）

**ゴール**: 経路候補算出・選択・確定の API + 画面を完成する

**主なタスク**:

- [ ] US08: 経路候補を算出する（BE 8 + FE 3 = 11 SP）
- [ ] US09: 経路を選択・確定する（BE 5 + FE 3 = 8 SP）

**目標 SP**: 19（BE 13 + FE 6）

### イテレーション 4（Week 7-8: 2026-06-09〜2026-06-20）

**ゴール**: 予約確定・追跡番号発行・荷役記録の API + 画面を実装する

**主なタスク**:

- [ ] US11: 経路情報を予約に紐付ける（BE 3 + FE 2 = 5 SP）
- [ ] US13: 予約を確定する（BE 3 + FE 2 = 5 SP）
- [ ] US14: 追跡番号を発行する（BE 3 + FE 2 = 5 SP）
- [ ] US15: 荷役作業を記録する（BE 5 + FE 3 = 8 SP）

**目標 SP**: 23（BE 14 + FE 9）※バッファ消費を含む

### イテレーション 5（Week 9-10: 2026-06-23〜2026-07-04）

**ゴール**: 追跡照会・通知で MVP を完成し、E2E テストを実施する

**主なタスク**:

- [ ] US17: 貨物状態を手動更新する（BE 3 + FE 2 = 5 SP）
- [ ] US18: 追跡情報を照会する（BE 5 + FE 5 = 10 SP）
- [ ] US06: 予約情報を経路設計者に引き渡す（BE 3 + FE 2 = 5 SP）
- [ ] US12: 確定経路を荷主に通知する（BE 3 + FE 2 = 5 SP）

**目標 SP**: 25（BE 14 + FE 11）※バッファ消費を含む
...
**ゴール**: 法人荷主管理と全機能の統合テストで完成する

**主なタスク**:

- [ ] US03: 法人荷主を登録する（5 SP）
- [ ] 全体統合テスト・品質向上・ドキュメント整備
...
xychart-beta
    ...
    y-axis "残 SP" 0 --> 180
    line "計画" [177, 159, 141, 122, 99, 74, 53, 37, 16, 0, 0]
```
...
1. イテレーション 1 計画を FE 開発分を含めて更新する
2. IT1 の開発を開始する
