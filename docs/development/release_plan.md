...
- [ ] US01〜US15 の受入基準をすべて満たす（US14・US15 は IT5 で実装予定）
...
| IT5 | 2026-05-26〜06-08 | 11 | 3（進行中） | 27% | 進行中 🚧 |
...
1. **IT5 実装開始**: US14（遅延例外）→ US15（破損・紛失例外）→ US16（輸送料金算出）の順で TDD で進める
2. **v0.1.0 リリース準備**: IT4 完了により Phase 1 コア機能（US01〜US13）が全実装。コードレビュー・SonarQube 確認後にリリースタグ付与
3. **IT4 完了報告書**: イテレーション完了報告書とふりかえりを作成する
