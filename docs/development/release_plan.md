...
| 6 | 13 | - | - | 計画済み（US01・US18・US19・終盤アウトサイドイン） |
| 7 | 13 | - | - | 未着手 |
| 8 | 5 | - | - | 未着手 |
...
xychart-beta
    ...
    line "実績" [97, 81, 70, 59, 45, 31]
```

> 実績: IT1 完了時点で残 81 SP（16 SP 消化）。IT2 完了時点で残 70 SP（11 SP 消化）。IT3 完了時点で残 59 SP（11 SP 消化）。IT4 完了時点で残 45 SP（14 SP 消化）。IT5 完了時点で残 31 SP（14 SP 消化）。計画ラインと一致し、ベロシティは安定推移（累計 66/97 SP・68%）。Phase 2 コア輸送フロー完了、Release 1.0 MVP 完成。
