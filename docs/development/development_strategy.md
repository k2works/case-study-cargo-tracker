...
[リリース計画](release_plan.md) で確定した IT1〜IT20 を **序盤・中盤・終盤・精算・整流・見積・出荷と是正** の 7 局面に分け、各局面で採用する TDD アプローチと横断的な進め方を定義します。
...
| **見積** | IT18 | アウトサイドイン | **最後の未実装 BC（Estimation）を立ち上げる。** 設計・テーブル・画面定義は揃っており、**未実装なのはコードだけ**である（下記） |
| **出荷と是正** | IT19〜IT20 | アウトサイドイン | **全 36 US が実装完了した後の局面である。** Release 2.1 を出荷として確定させ、**正典に届いていない実装**を返す（下記）。**IT20 も同じ局面である** —— 対象は育つ負債（ADR-024 の代償・`Cargo` の行数）と、正典に残った未判断である |
...
| 出荷と是正 | **すべて実装済み。ドメインは変えない** | 正典（`ui_design.md`）に書いてあって実装に無いものを返す。画面と読み取り側だけの変更であり、**条件は整流と同じ** |
| 見積 | **BC は新設だが、設計は確定済み** | 集約・値オブジェクト・テーブル・画面・遷移がすべて既存の文書にある。**未知なのは業務ではなく配線である。** 画面から入り、既にある Routing の探索と料金算出へ繋ぐ形が最短である |
...
## 出荷と是正（IT19〜IT20）: アウトサイドイン

**US01〜US36 のすべてが実装完了した後の局面である。** 新しいユーザーストーリーは無い。

### なぜ整流と呼ばないか

整流（IT16〜IT17）は「**この局面を繰り返さないことが目的である**」と定め、IT17 のふりかえりは「2 回目・最終」と宣言している。**その宣言は取り消さない。**

| 整流（IT16〜IT17） | 出荷と是正（IT19） |
| :--- | :--- |
| 数え上げた**技術的負債**を返す（据え置きの表を空にする） | **正典に届いていない実装**を返す |
| 対象は自分たちが作った据え置き | 対象は**受入基準と正典との差** |

整流の表（`NOT_SPLIT_YET` / `NOT_MEASURED_YET`）は IT17 で空になり、**再び埋まっていない**。

### この局面ですること

1. **出荷を確定させる。** タグ・CHANGELOG・リリース完了報告書を揃え、「何が出荷済みか」を曖昧にしない
2. **正典未達を返す。** `ui_design.md` が定めながら実装に無いもの（見積の再見積導線・一覧の絞り込み）
3. **人の記憶に預けている規律を検査に落とす。** 2 イテレーション続けて外した網（H2 スモークの載せ忘れ）は、人ではなく方式の問題である

### 完了条件

- [ ] **タグ・CHANGELOG・リリース完了報告書がある**
- [ ] **`ui_design.md` に書いてあって実装に無いものが無い**
- [ ] **前イテレーションの Try のうち、検査にできるものはすべて検査になっている**
- [ ] **落としたものが名前で記録されている**
...
**新しい BC を立てる回だが、中盤（インサイドアウト）は選ばない。**

中盤でインサイドアウトを選んだのは、**業務規則が未知**で、集約の形をテストで探る必要があったからである。見積はそうではない。

| 要素 | 状態 |
| :--- | :--- |
| 集約・値オブジェクト（`Estimate` / `RouteCandidate` / `EstimateStatus`） | `domain-model.md` に確定済み |
| テーブル（`estimate` / `route_candidate`） | **V1 から存在する**（マイグレーション不要） |
| 画面（`/estimates` の一覧・作成・詳細）と遷移 | `ui_design.md` に確定済み |
| 経路の探索 | Routing に実装済み（US08） |
| 概算料金 | Routing の `FreightEstimator` に実装済み（ADR-008） |

**未知なのは業務ではなく配線である。** 画面から入り、既にある部品へ繋ぐ形が最短で、かつ「繋がっていない」ことに最も早く気づける。

### この局面で決めること

**着手前に 1 件、正典の食い違いを決着させる**（IT17 の Try T6）。

`domain-model.md` は「ルート候補はスタブ実装（固定値）で生成される。将来、外部ルーティングサービスとの連携時に置換予定」と書いている。しかし **ADR-006 は外部連携を採らないと決めており**、**Routing には US08 で本物の探索が入っている**。

固定値を返す見積は、**画面は動くのに数字が現実と無関係**という状態になる。「動作するきれいなゴミ」そのものである。

**ACL ポートで Routing に問い合わせる形を第 1 案とし、IT18 の開始準備で決着させる。** 決めてから着手する。

### 完了条件

- [ ] **US01 の受入基準 6 件をすべて満たす**（正典は `../requirements/user_story.md` の US01）
- [ ] **見積の数字が Routing の探索と料金算出から来ている**（固定値でない）。スタブに留める判断をした場合は、**その旨を画面に明示する**
- [ ] **見積から予約へプリフィルで進める**（`ui_design.md` の画面遷移図）
- [ ] **ナビゲーションとダッシュボードに営業担当者の入口がある**（ロール別到達性）
- [ ] **IT17 の引き継ぎ（C1・C2）に着手している** — 育つ負債を先に返す
>
