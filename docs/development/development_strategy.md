...
| 終盤（強化・予備） | IT8 | アウトサイドイン | Billing・Booking・通知/決済 | Release 1.1 の受入残を充足し実務品質へ引き上げる。新規ドメインなし、受け入れテスト起点で既存集約・ポート・合成層を補強（レビュー/ふりかえりの先送り消化・正式出荷） |

```mermaid
graph LR
    A["序盤 IT1-2<br/>アウトサイドイン<br/>基盤 + 縦切り"] --> B["中盤 IT3-5<br/>インサイドアウト<br/>複雑ドメイン中核"]
    B --> C["終盤 IT6-7<br/>アウトサイドイン<br/>業務シナリオ結合"]
    C --> E["終盤強化 IT8<br/>アウトサイドイン<br/>受入残充足・出荷品質"]
    A -.->|Release 1.0 MVP| B
    E -.->|Release 1.1 正式出荷| D["完成"]
```
