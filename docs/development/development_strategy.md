...
| イテレーション × ユーザーストーリー割り当て | [リリース計画](release_plan.md)（**SSoT**。本戦略の局面割り当てはこれに追従する） |
...
> **注記**: 本戦略の局面割り当て（IT × US）は [リリース計画](release_plan.md) を SSoT とする。リリース計画のイテレーション構成が変わった場合は、本ドキュメントの割り当て表・対象 US を追従させること。
...
| 中盤 | IT3-6 | Phase 2 経路設計・追跡 | インサイドアウト | 経路算出・追跡の複雑ドメインを domain / data 層から堅牢に作り込み、貧血ドメインモデルを回避 |
| 終盤 | IT7-8 | Phase 3 精算・例外処理 | アウトサイドイン | 実装済みの集約を業務シナリオ起点で結合し、精算・例外処理でリリース全体の一貫性を担保 |

```mermaid
graph LR
    A[序盤 IT1-2<br/>アウトサイドイン<br/>歩けるスケルトン] --> B[中盤 IT3-6<br/>インサイドアウト<br/>複雑ドメイン作り込み]
    B --> C[終盤 IT7-8<br/>アウトサイドイン<br/>業務シナリオ結合]
```
...
## 中盤: インサイドアウト（IT3-6 / Phase 2 経路設計・追跡）
...
| IT | US | 概要 | 主 BC |
| :--- | :--- | :--- | :--- |
| IT3 | US24 航海スケジュールを新規登録する / US25 既存航海スケジュールを更新する / US07 航海スケジュールを検索する / US01 輸送見積を作成する / US06 予約情報を経路設計者に引き渡す | 航海スケジュール・見積・ハンドオフ | routing, estimation, booking |
| IT4 | US08 経路候補を算出する / US09 経路を選択・確定する | 経路探索（複雑） | routing |
| IT5 | US10 経路条件を調整して再算出する / US11 経路情報を予約に紐付ける / US12 確定経路を荷主に通知する | 経路調整・紐付け・通知 | routing, booking, shared |
| IT6 | US14 追跡番号を発行する / US15 荷役作業を記録する / US16 引取作業を記録する / US18 追跡情報を照会する | 追跡・荷役 | tracking, handling |
...
## 終盤: アウトサイドイン（IT7-8 / Phase 3 精算・例外処理）
...
| IT | US | 概要 | 主 BC |
| :--- | :--- | :--- | :--- |
| IT7 | US17 貨物状態を手動更新する / US19 遅延例外を処理する / US20 破損・紛失例外を処理する | 例外処理・状態更新 | tracking, handling |
| IT8 | US21 輸送料金を算出する / US22 法人割引を適用する / US23 精算を処理する | 精算・割引 | billing, shipper |
...
| 2026-07-24 | `release_plan.md` 作成に伴い局面割り当てを確定（中盤 IT3-6・終盤 IT7-8、全 8 イテレーション）。リリース計画を SSoT とし、中盤・終盤の対象 US 表を追従。 |
