...
| **終盤** | **IT8-IT12** | 業務フローの結合と例外系の作り込み | **アウトサイドイン** |
...
> **【改訂】IT8 計画時（2026-08-04）にイテレーションを 10 → 12 へ延長しました。**
> 実効ベロシティが想定の 14 SP ではなく 11 SP で安定し、残スコープ 45 SP（返済枠込み）に対し
> IT8-IT10 の枠 33 SP では約 1 イテレーション分が不足したためです。
> 判断の根拠は [リリース計画](release_plan.md) を参照してください。

> **【注意】終盤でも新しい Bounded Context を立ち上げる場合がある**（IT8 の Estimation）。
> 局面のアプローチ（アウトサイドイン）をそのまま当てると噛み合いません。
> 新 BC の立ち上げは「コンテキストをまたぐ振る舞い」ではなく
> **ドメインモデルの構築**だからです。
>
> **この場合は次のように混ぜます**。
>
> | 対象 | アプローチ | 理由 |
> | :--- | :--- | :--- |
> | 受入テスト（E2E・HTTP） | **先に書く**（アウトサイドイン） | 「業務として成立するか」を最初に固定する |
> | ドメイン層の実装 | **中盤の規律を守る**（効果を要求しない・現在時刻は引数） | 新 BC のドメインは外部依存なしで組み上げる方が速く、テストも軽い |
>
> IT7 でも同じ形を採りました（`RouteFinder` は効果なしの純粋関数、画面は後）。
> **局面はテストの入口を決めるものであり、ドメイン層の規律を上書きするものではありません。**
...
| 終盤 | E2E シナリオ（Playwright）。5 本のシナリオがそのままデモになる。**ただし新しい Bounded Context を立ち上げる IT は HTTP 受入テストで代替してよい**（IT8 Estimation・IT10 Handling）——業務フローが端から端まで通っていない段階で E2E を書くと、中途半端なシナリオが残る |
