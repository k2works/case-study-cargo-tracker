...
本ドキュメントは、計画済みイテレーション（IT1〜IT12 + 予備 IT13、Release 2.2 の IT14〜IT15、Release 2.3 の IT16）を**序盤・中盤・終盤・拡張・仕上げ**の 5 局面に分け、各局面で採用する TDD アプローチと横断的な進め方を定義する。
...
| **仕上げ** | **IT16** | **Release 2.3** | **アウトサイドイン** | 締めの業務（請求書検索）を画面から作り、拡張局面が残した「シミュレーション由来が実利用者の一覧に混ざる」を塞ぐ。**新しいドメイン規則を作らない IT** であり、外側（一覧・画面）から入るのが素直である |
...
## 仕上げ: アウトサイドイン（IT16 / Release 2.3）

### 目的

締めの業務を画面から完結させ、拡張局面が**運用（夜間の継続実行の禁止）で守っている状態**を実装で塞ぐ。

- **新しいドメイン規則を作らない。** 請求書検索は読み取り、一覧からの除外は問い合わせ条件、単一 Pod の前提は配備の制約である。内側から作り込むものが無いため、外側（画面・一覧）から入る
- **運用で守っている状態を次のリリースへ持ち越さない。** マニュアルの禁止事項は、実装が追いつくまでの仮の橋である

### 対象ユーザーストーリー

| IT | US | SP | アプローチ |
| :--- | :--- | :--- | :--- |
| IT16 | US38, TD-02, TD-03, TD-04 | 8 | アウトサイドイン |

### この局面で守ること

| # | 規律 | 理由 |
| :--- | :--- | :--- |
| 1 | **除外の検査は、実業務の案件を 1 件置いた状態で書く** | 空の一覧に対する `doesNotContain` は、何も判別しないまま緑になる（IT15 で実際に起きた） |
| 2 | **同じ絞り込みを一覧・件数・合計の 3 か所で別々に持たない** | 1 か所だけ直して緑になる形を作らない |
| 3 | **配備の前提は文章ではなく検査で残す** | ADR-009 は 7 IT のあいだ半分しか守られなかった |
