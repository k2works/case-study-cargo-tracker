...
[リリース計画](release_plan.md) で確定した IT1〜IT15 を **序盤・中盤・終盤・精算** の 4 局面に分け、各局面で採用する TDD アプローチと横断的な進め方を定義します。
...
| **精算** | IT13〜IT15 | **インサイドアウト** | 金額を扱う新規 BC（Billing）をドメイン層から固める。**中盤と同じ理由で同じ選択をする** |
...
| IT14 | US23 | 精算書の発行・入金確認・精算完了 |
| IT15 | US30 | 輸送中キャンセルの承認（**キャンセル料が US21 に依存する**） |
