...
本ドキュメントは、計画済みイテレーション（IT1〜IT12 + 予備 IT13、および Release 2.2 の IT14〜IT15）を**序盤・中盤・終盤・拡張**の 4 局面に分け、各局面で採用する TDD アプローチと横断的な進め方を定義する。
...
| **拡張** | **IT14〜IT15** | **Release 2.2** | **アウトサイドイン**（IT14）→ **インサイドアウト**（IT15） | 完成した業務の**呼び出し元**を自動化する。IT14 は「1 本の業務シナリオが端から端まで通る」を E2E から作り、IT15 は乱数・上限・再現性というドメインの規則を内側から作り込む |

予備 IT13 は消化する局面（終盤）のアプローチに従う。

> **拡張局面が既存 3 局面と違う点**: 対象が**業務そのものではない**。simulationms が作るのは
> 新しい業務ではなく、既にある業務を実利用者と同じ経路で呼ぶ仕組みである。
> したがって**新しいドメイン概念は既存 BC に足さない**——足すと、業務の集約が
> 「シミュレーションかどうか」を知ることになる。
...
## 拡張: アウトサイドイン → インサイドアウト（IT14〜IT15 / Release 2.2）

### 目的

完成した業務の**呼び出し元**を自動化し、「実際の経路を踏んで初めて分かる欠陥」を人手を介さずに踏む。

- IT14（アウトサイドイン）: 「1 本のシナリオが端から端まで通る」を E2E から作る。**先に外側を固定しないと、9 工程のどこまでを実装すれば終わりなのかが決まらない**
- IT15（インサイドアウト）: 乱数の種・上限・再現性は**ドメインの規則**であり、外から作ると「たまたま動く」実装になる。`Seed`・`ScenarioGenerator`・`ContinuousRunPolicy` をドメイン層から作り込む

### 対象ユーザーストーリー

| IT | US | SP | アプローチ |
| :--- | :--- | :--- | :--- |
| IT14 | US34, US35 | 8 | アウトサイドイン |
| IT15 | US36, US37 | 8 | インサイドアウト |

### この局面で守ること

| # | 規律 | 理由 |
| :--- | :--- | :--- |
| 1 | **既存 BC に「シミュレーションかどうか」を持ち込まない**（例外は bookingms の荷主 1 列のみ） | 業務の集約が呼び出し元を知ると、業務のテストが呼び出し元に依存する |
| 2 | **出口は Gateway 1 本**（内部 API・`system:` principal を使わない） | 専用経路を作ると「シミュレーションは通るのに実際の操作は通らない」を検出できない |
| 3 | **新しい結合方式を発明しない** | 既存の REST 契約テストの型を踏襲する。必要になったら ADR を先に起票する |
...
- **終盤 → 拡張（IT13 → IT14）**: 拡張局面は**新しいサービスを 1 つ増やす**。IT2（bookingms）・IT3（routingms）・IT7（handlingms）・IT11（billingms）で確立した立ち上げの型（Gradle サブプロジェクト・Flyway・ArchUnit・k8s マニフェスト・`APP_*_SERVICE_BASE_URL` の検査）をそのまま踏襲し、型の再議論はしない
...
| 2026-08-31 | 拡張局面（IT14〜IT15 / Release 2.2）を追加。IT14 = アウトサイドイン、IT15 = インサイドアウト。局面の規律 3 件と終盤 → 拡張の移行方針を明記 | - |
