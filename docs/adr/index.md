...
| [0004](0004-us08-route-candidate-bc-placement.md) | US08 経路候補は IT3 で Routing の一時計算値、永続化は IT7 の Estimation。外部経路 ACL は Routing に HTTP アダプタで配置しフォールバック | 承認 |
| [0005](0005-adjustment-amount-sign-convention.md) | 料金調整（減額・補償費用）は種別によらず減算方向（負値）に正規化。符号規約は InvoiceLineItem に閉じる。COMPENSATION は当社負担クレジット | 承認 |
