...
- 未解決: IT4 の実装中に、公開追跡の `GET` が 1 度だけ 403 を返す事象を観測した。
  ...
  **IT6 レビュー後の追記（2026-08-03）**: レビューで
  「識別子を応答へ添えても、**期待値が無ければ受け取った値を判断できない**」と
  指摘された。`X-Route` と同じ形の見落としである。`RunningServer` に
  起動ごとの識別子を持たせ、`TestApp.withAppInstance` がテスト本体へ渡し、
  `SharedHttpTestClient.assertInstance` が突き合わせる形に直した。
  これで H5/H6 が起きたときに**名指しで失敗する**。

  **新しい仮説 H7（書き込み競合の波及）**: 楽観的ロックの修正
  （ADR-0008・行ロック）を検証する過程で、**行ロックを外すと無関係な
  `ShipperHttpTest` のログインが `Set-Cookie` を得られずに落ちる**ことを
  2 回続けて観測した。行ロック無しで同じ行へ 8 本の書き込みトランザクションが
  同時到達すると競合し、待機がコネクションプールを占有する。

  | # | 仮説 | どう見えるか | 切り分ける手段 | 状態 |
  | :--: | :--- | :--- | :--- | :--- |
  | H7 | **書き込み競合でプールが枯渇し、後続の別テストが応答を得られない** | 無関係なテストが「応答が空」「Set-Cookie が無い」で落ちる | 競合する書き込みを行ロックで直列化し、差を観測する | **IT6 で実測**（ロック有 561 件緑 / ロック無 1 件赤） |

  IT4・IT5 の 2 例はいずれも**フルテストの中で 1 度だけ**起き、
  「そのルートでは起こり得ない応答」だった。H7 は
  「アプリケーションの論理ではなく、**実行環境の資源**が原因」という
  これまでと別の系統の仮説であり、観測された事実に基づく点で
  H5/H6 より確度が高い。IT7 で、プールの枯渇時に何が起きるか
  （待機・例外・空応答のいずれか）を確かめる。
