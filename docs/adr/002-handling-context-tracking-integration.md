...
本 ADR は **[ADR-010](010-handling-as-independent-context.md) に置き換えられており、守るべき規則を持たない。**

| 守るもの | 守り手 | 対象範囲（何を見ているか） |
| :--- | :--- | :--- |
| （なし。決定は失効している） | — | **守る対象そのものが無い。** ADR-010 が本決定を置き換えた |

置き換え後の規則の守り手は ADR-010 を参照すること。**失効した ADR に守り手を書くと、
どちらが生きているのか読み手に分からなくなる。**
