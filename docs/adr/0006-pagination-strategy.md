...
採用 (2026-06-29、IT3 L-11 で起票 → IT3 post-nav レビュー H-03 で昇格、IT4 Phase 1 で実装着手)

採用判断の根拠: IT3 で `findAllEstimates` を新規追加した際、本 ADR を「提案」のまま据え置くと
旧パターン (`m [a]` + `LIMIT 100` ハードコード) の callsite が無自覚に増殖し、Phase 2 移行コスト
が線形に膨らむ自己矛盾が判明した。よって本 ADR を「採用」に昇格し、`findAll*` 系 Port は
`{-# DEPRECATED #-}` プラグマで「IT4 で `findCargosPaged` 等に移行する旨」を **コンパイラ警告で
強制的に可視化する**。
