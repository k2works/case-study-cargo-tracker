...
## 追記（2026-08-09 / IT10）: `shared.application` の位置づけ

本 ADR は共有カーネル本体（`shared.domain.model`）の範囲を決めたが、**`shared.application` については何も述べていなかった**。IT9 のレビュー（M1）で、このパッケージが ArchUnit のどのルールにも守られていないことが判明した。

ここは共有カーネル以上に肥大化しやすい。「全 BC のアプリケーション層から使えるもの置き場」に見えるうえ、**BC 間参照を禁じるルール 4 が `..shared..` を依存先から除外している**ため、ここに置いたものは BC 間結合の検査を素通りする。実際 `CurrentUser` / `ShipperScopedPrincipal` は、Security が実装し Booking が読むという**越境をここで受けている**。

**`shared.application` に置いてよいのは、次の 2 種類の「約束」だけとする。**

| 種類 | 現在の要素 | 置いてよい理由 |
| :--- | :--- | :--- |
| 一覧の見せ方の約束 | `Page` / `PageRequest` / `PageLinks` / `AuditValue` | 全 BC の一覧・監査ログが同じ規則で動くための境界の計算であり、業務判断を含まない |
| BC をまたがずに文脈を伝える interface | `CurrentUser` / `ShipperScopedPrincipal` | 実装は提供側の BC、利用は別の BC。両者が相手のクラスを知らないための境界であり、介するのは共有カーネルの `ShipperId` だけである |

**業務ロジックは置かない。**「全 BC から使える」は置く理由にならない（`UserAccount` / `Role` を `security` へ出したのと同じ判断）。

## コンプライアンス

- ArchUnit ルールにより、共有カーネルのパッケージ（`shared.domain.model`）に `Location` と `ShipperId` 以外のクラスが追加されていないことを検証する（**共有カーネルは放置すると必ず肥大化するため、人間のレビューではなくテストで固定する**）
  - `shared.infrastructure` 配下は横断的な技術基盤（TypeHandler・共通画面）であり共有カーネルではないため、検査対象に含めない
  - **IT1 で有効化済み**。違反クラスを一時的に追加して実際に落ちることを確認した。設定したことと働いていることは別である
- ArchUnit ルール `共有アプリケーション層はBC横断の約束のみ` により、`shared.application` に上記 6 クラス以外が追加されていないことを検証する（上記の追記）
  - **IT10 で有効化済み**。`shared.application.paging` に違反クラスを一時的に追加し、**他の 9 ルールは緑のまま本ルールだけが赤になる**ことを確認した
- 認証・認可（`UserAccount` / `Role`）は共有カーネルに置かず、支援サブドメイン `security` パッケージに置く。全 BC から使うことは共有カーネルに置く理由にならない（IT1 で `shared` から移動）
