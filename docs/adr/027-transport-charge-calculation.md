...
| 8 輸出免税 | 国際輸送の荷主に、本来かからない 10% を請求し続ける | `TaxRateTest#exempt`・`InvoiceTest#国が異なれば税額は 0 円`（**同じ条件で国内なら 10% が付くことと対で見る**）・`InvoicePersistenceIntegrationTest#tax_exempt を読み戻す`・`billing.spec.ts#デモ 9` |
