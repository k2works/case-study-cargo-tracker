...
承認済み（決定 1〜8）。**決定 1 と決定 8 は IT12 で改訂した**（下記の追補）。
...
>
> **この粗さは IT12 で直した**（下記「追補」）。経理担当者のレビューが
> 「これでは荷主が納得しない」と述べ、実際に使われない機能になるところだった。
...
IT11 の完了報告書は、受入基準 21-2（距離）と決定 8（税区分）を**未達として記録した**。
経理担当者のレビューは前者に「これでは荷主が納得しない」、後者に
「国際輸送を扱うシステムとしては目立つ誤り」と述べた。**US01（見積）が同じ式を使う以上、
見積にもそのまま伝染する**ため、見積を作る前に直す。

### 決定 1 の改訂: 区間係数を地域区分で重み付けする

区間数だけでは、**東京 → 横浜（1 区間）と東京 → ロサンゼルス（1 区間）が同額**になる。
緯度経度は依然として無く、全港の座標整備は本 IT の範囲を超える。
**区分なら 1 列で足りる。**

```text
区間係数 = Σ（区間ごとの地域係数）

  地域区分（location.region）= DOMESTIC / NEAR_SEA / OCEAN
  地域係数 = 両端の区分の重いほう
             DOMESTIC 1.0 / NEAR_SEA 2.5 / OCEAN 6.0
```

**重いほうを採る**のは、片端が国内でも太平洋を渡れば遠洋の費用がかかるからである。
改訂前は「区間数をそのまま足す」形（すべて 1.0）であり、**改訂後も国内のみの旅程では
同じ金額になる**——値上げではなく、国際輸送の過少請求を直す変更である。

`location.region` は 4 サービス（booking / routing / handling / tracking）に複製された
地点マスタに配る（[ADR-014](014-location-replica-sync.md)）。

> **代用であることは変わらない。**「なぜその金額か」を読めるようにするため、
> **画面の内訳に区分と係数を出す**。粗い代用を黙って使うと、荷主に説明できない。

### 決定 8 の改訂: 出発地と目的地の国が異なる輸送は消費税を免除する（輸出免税）

**国際輸送の運賃は消費税が免除される。** IT11 は一律 10% で計算しており、
これはキャンペーンではなく**誤り**である。

- 判定は `location.country_code`（**既にある**。新しい列は要らない）
- 出発地と目的地の国コードが異なれば `TaxRate.exempt()`（0%）
- `invoice.tax_exempt`（BOOLEAN）に**判定の結果を保存する**——発行した請求書の税額は
  動かない（決定 4）ので、あとから国コードを引き直して再判定しない
- **請求書に税区分を出す**。「消費税 ¥0」だけでは、免税なのか計算漏れなのか読めない

**軽減税率は扱わない。** 対象になる貨物（食品）を判別する情報を持っておらず、
運賃には適用されない。

### 決定 1・8 の改訂に対する検査

| 改訂 | 破られたら何が起きるか | それが起きないことの検査 |
| :--- | :--- | :--- |
| 1 地域区分 | 国内の積み替え 1 回と太平洋横断が同額になり、荷主に説明できない。経理担当者は毎回調整欄で手直しし、料金計算が使われなくなる | `PortRegionTest`（`values()` から回して全区分が係数を持つ）・`ChargeableLegTest`（**重いほうを採る**——両端を入れ替えても同じ）・`TransportChargeTest#国内と遠洋で金額が違う`・`billing.spec.ts#デモ 8`・`LocationSeedReplicaTest`（4 サービスの種データが一致） |
| 8 輸出免税 | 国際輸送の荷主に、本来かからない 10% を請求し続ける | `BillingIdentifiersTest.Taxes#国が異なれば免税、同じ国なら課税`（**対で見る**）・`#国が分からなければ課税に倒す`・`CalculateChargeUseCaseTest#国内輸送の請求には消費税が付く`・`billing.spec.ts#デモ 9` |
